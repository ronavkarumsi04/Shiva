...
# IMPORTANT: shiva_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See shiva_bootstrap.py for full rationale.
try:
    import shiva_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when shiva_bootstrap isn't registered in the venv
    # yet — happens during partial ``shiva update`` where git-reset landed
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``shiva -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("SHIVA_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("SHIVA_SESSION_SOURCE", "")
...
from shiva_cli.timeouts import (
...
_loaded_env_paths = load_shiva_dotenv(shiva_home=_shiva_home, project_env=_project_env)
...
# (shiva_state._rows_to_conversation), so any code that mutates a loaded or
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Shiva-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.shiva/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[shiva-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `shiva --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from shiva_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``shiva chat -q``. In that mode,
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``shiva chat -Q``) always wins: those flows neutralize the rendering
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``SHIVA_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``SHIVA_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("SHIVA_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``SHIVA_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("SHIVA_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        the ``SHIVA_API_CALL_STALE_TIMEOUT`` env var. Reasoning-model floors
        ...
        if get_provider_stale_timeout(self.provider, self.model) is not None:
            return True
        return os.getenv("SHIVA_API_CALL_STALE_TIMEOUT") is not None
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        shiva-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See shiva-agent#21444 for symptom history."
        )
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Shiva against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "Shiva can't reach the model provider. You may be offline. "
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("SHIVA_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from shiva_cli import lifecycle as _lifecycle
    ...
    def _redact_message_content(content):
        ...
        Respects ``SHIVA_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.shiva/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Shiva-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects SHIVA_REDACT_SECRETS via
    ...
    def interrupt(
        ...
        *,
        ...
        # interrupt event rather than Shiva' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Shiva model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell Shiva-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``SHIVA_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            import os as _os
            env = _os.environ.get("SHIVA_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.shiva/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``SHIVA_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            import os as _os
            env = _os.environ.get("SHIVA_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another Shiva process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another Shiva process was writing to the state "
            ...
            if cause == "corrupt":
                return (
                    ...
                    "1. Run `shiva doctor --fix`\n"
                    "2. Salvage with: sqlite3 ~/.shiva/state.db \".recover\" "
                    "(then replace state.db)\n"
                    "3. Restore from a backup in ~/.shiva/backups/\n"
            ...
            return (
                ...
                "Check the state database health (`shiva doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (SHIVA_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (SHIVA_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset SHIVA_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("SHIVA_DEV_CREDITS"))
        ...
        if _dev:
            # SHIVA_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `shiva logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Shiva YOLO selected unrestricted
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``shiva auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.shiva/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``shiva setup``) updates
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from shiva_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from shiva_cli.auth import _resolve_zai_base_url
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            self._client_kwargs["default_headers"] = shiva_xai_default_headers()
        else:
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from shiva_cli.models import lmstudio_model_reasoning_options
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated shiva_logging._session_context (a
            ...
            try:
                from shiva_logging import set_session_context
            ...
            # SHIVA_SESSION_ID to the child id after an out-of-place
    ...
    def run_conversation(
        ...
        from shiva_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        ...
        try:
            # Serialize the full load -> run -> flush region across Shiva
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another Shiva process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other Shiva process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another Shiva process on "
                    ...
                    timeout_msg = (
                        "⏳ Another Shiva process kept this session busy too "
