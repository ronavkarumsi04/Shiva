...
_SHIVA_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (SHIVA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `shiva tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SHIVA_KANBAN_TASK env "
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in shiva_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Shiva toolsets (CLI + messaging platforms)
    #
    ...
    # the `shiva send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "shiva-cli": {
        ...
        "tools": _SHIVA_CORE_TOOLS,
        "includes": []
    },

    "shiva-cron": {
        # Mirrors shiva-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `shiva tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as shiva-cli; gated by `shiva tools`",
        "tools": _SHIVA_CORE_TOOLS,
        "includes": []
    },
    ...
    "shiva-email": {
        "description": "Email bot toolset - interact with Shiva via email (IMAP/SMTP)",
        "tools": _SHIVA_CORE_TOOLS,
        "includes": []
    },
    ...
    "shiva-sms": {
        "description": "SMS bot toolset - interact with Shiva via SMS (Twilio)",
        "tools": _SHIVA_CORE_TOOLS,
        "includes": []
    },
    ...
    "shiva-webhook": {
        ...
        "tools": _SHIVA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "shiva-gateway": {
        ...
        "tools": [],
        "includes": ["shiva-telegram", "shiva-discord", "shiva-whatsapp", "shiva-slack", "shiva-signal", "shiva-bluebubbles", "shiva-homeassistant", "shiva-email", "shiva-sms", "shiva-mattermost", "shiva-matrix", "shiva-dingtalk", "shiva-feishu", "shiva-wecom", "shiva-wecom-callback", "shiva-weixin", "shiva-qqbot", "shiva-webhook", "shiva-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``shiva-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_SHIVA_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``shiva-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (shiva-<name>).
        # Gives them _SHIVA_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("shiva-"):
            platform_name = name[len("shiva-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SHIVA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
