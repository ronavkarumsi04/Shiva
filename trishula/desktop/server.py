    ...
    def _make_handler(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):  # noqa: N802
                ...
                if url.path == "/api/hw/artifact":
                    # Generate a derived artifact from a prompt (re-builds plan).
                    # {kind: firmware_arduino|firmware_mp|netlist_csv|netlist_kicad|
                    #  scad|wiring_svg, prompt, answers}
                    try:
                        from trishula.engineering.planner import build_plan
                        kind = body.get("kind", "")
                        plan = build_plan(body.get("prompt", ""), body.get("answers") or {})
                        artifact = build_artifact(plan, kind)
                        if artifact is None:
                            return self._json({"error": f"unknown artifact {kind!r}"}, 400)
                        if kind == "wiring_svg":
                            return self._send(200, artifact.encode(),
                                              "image/svg+xml; charset=utf-8",
                                              {"Cache-Control": "no-store"})
                        filename = artifact_filename(kind, plan)
                        ctype = ("application/octet-stream" if kind.endswith("_scad")
                                 or kind.startswith("netlist") else "text/plain; charset=utf-8")
                        return self._send(
                            200, artifact.encode(), ctype,
...
def build_artifact(plan, kind: str) -> str | None:
    """Render a derived artefact from a built plan. Returns text or None."""
    if kind in ("firmware_arduino", "firmware_mp"):
        from trishula.engineering.firmware import generate
        return generate(plan, "arduino" if kind == "firmware_arduino" else "micropython").content
    if kind == "netlist_csv":
        from trishula.engineering.netlist import export_csv
        return export_csv(plan)
    if kind == "netlist_kicad":
        from trishula.engineering.netlist import export_kicad_netlist
        return export_kicad_netlist(plan)
    if kind == "scad":
        from trishula.engineering.cad import openscad_enclosure
        return openscad_enclosure(plan)
    if kind == "wiring_svg":
...
def artifact_filename(kind: str, plan) -> str:
    import re
    slug = re.sub(r"[^A-Za-z0-9]+", "_", plan.title).strip("_").lower() or "project"
    return {
        "firmware_arduino": f"{slug}.ino",
        "firmware_mp": f"{slug}.py",
