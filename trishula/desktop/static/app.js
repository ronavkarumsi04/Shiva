...
async function runHwPlan(prompt, a) {
  ...
  a.body.appendChild(drcBlock(plan));
  a.body.appendChild(costBlock(plan));
  a.body.appendChild(archBlock(plan));
  a.body.appendChild(wiringBlock(plan));
  a.body.appendChild(await wiringSchematicBlock(prompt, state.hwAnswers));
  a.body.appendChild(bomBlock(plan));
  a.body.appendChild(boardBlock(plan));
  a.body.appendChild(artifactsBlock(prompt, state.hwAnswers));
  ...
  btn.addEventListener("click", () => downloadText(slug(plan.title) + "-build-plan.md", md, "text/markdown"));
  bar.appendChild(btn);
...
function drcBlock(plan) {
  const items = plan.drc || [];
  ...
  const cls = errors.length ? "g-fail" : warns.length ? "g-warn" : "g-ok";
  b.innerHTML = `<div class="ehead"><span class="et ${cls}">${icon} design-rule check</span>
    <span class="es">${errors.length} errors · ${warns.length} warnings · ${items.filter(i=>i.severity==="info").length} notes</span></div>
    <div class="ebody">${items.map((d) => {
      const c = d.severity === "error" ? "g-fail" : d.severity === "warning" ? "g-warn" : "dim";
      const g = d.severity === "error" ? "✗" : d.severity === "warning" ? "⚠" : "•";
      return `<div class="gline"><span class="gi ${c}">${g}</span><span><b>${esc(d.rule)}</b> — ${esc(d.message)}${d.suggestion ? ` <span class="dim">→ ${esc(d.suggestion)}</span>` : ""}</span></div>`;
    }).join("")}</div>`;
  $(".ehead", b).addEventListener("click", () => b.classList.toggle("collapsed"));
  if (!errors.length) setTimeout(() => b.classList.add("collapsed"), 1500);
  return b;
}
function costBlock(plan) {
  const c = plan.cost; if (!c) return el("span");
  const b = el("div", "eng open");
  b.innerHTML = `<div class="ehead"><span class="et">$ cost estimate</span><span class="es">${esc(c.note || "")}</span></div>
    <div class="ebody no-pad"><table class="wtable">
      <tr><td>parts subtotal</td><td class="r mono">$${c.subtotal}</td></tr>
      <tr><td>with connectors/wire (≈15%)</td><td class="r mono">$${c.with_connectors}</td></tr>
      <tr><td><b>low-volume single unit (≈1.6×)</b></td><td class="r mono"><b>$${c.low_volume_unit_est}</b></td></tr>
    </table></div>`;
  return b;
}
async function wiringSchematicBlock(prompt, answers) {
  const b = el("div", "eng open");
  b.innerHTML = `<div class="ehead"><span class="et">⎆ point-to-point wiring</span>
    <span class="es">colour = bus · controller → modules</span></div>
    <div class="ebody dcanvas"><span class="dim">rendering…</span></div>`;
  $(".ehead", b).addEventListener("click", () => b.classList.toggle("collapsed"));
  try {
    const r = await fetch("/api/hw/artifact", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ kind: "wiring_svg", prompt, answers }),
    });
...
function artifactsBlock(prompt, answers) {
  ...
  b.innerHTML = `<div class="ehead"><span class="et">⬇ generate & download</span>
    <span class="es">firmware, netlist, enclosure — generated from this exact plan</span></div>
    <div class="ebody"><div class="art-grid"></div></div>`;
  const grid = $(".art-grid", b);
  arts.forEach(([kind, label, sub]) => {
    const cell = el("button", "art-cell");
    cell.innerHTML = `<div class="art-label">${esc(label)}</div><div class="art-sub dim">${esc(sub)}</div>`;
    cell.addEventListener("click", () => downloadArtifact(kind, prompt, answers));
    grid.appendChild(cell);
  });
  return b;
}
async function downloadArtifact(kind, prompt, answers) {
  const r = await fetch("/api/hw/artifact", {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ kind, prompt, answers }),
  });
