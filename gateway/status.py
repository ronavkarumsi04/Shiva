...
The PID file lives at ``{SHIVA_HOME}/gateway.pid``.  SHIVA_HOME defaults to
``~/.shiva`` but can be overridden via the environment variable.  This means
separate SHIVA_HOME directories naturally get separate PID files — a property
...
from shiva_constants import get_shiva_home, _get_platform_default_shiva_home
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_shiva_home() / "gateway-starts.log"
...
def _get_process_shiva_home() -> Path:
    """Return the process-level SHIVA_HOME, skipping context-local overrides.
    ...
    ``get_shiva_home()`` honors ``_SHIVA_HOME_OVERRIDE`` contextvar used for
...
def _canonical_shiva_home(path: Path | str) -> Path:
    """Return a stable absolute SHIVA_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_shiva_home(left: Path | str, right: Path | str) -> bool:
    """Compare SHIVA_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_shiva_home(left))) == os.path.normcase(
        str(_canonical_shiva_home(right))
    )


# Mirrors shiva_cli.profiles._PROFILE_ID_RE — duplicated here because gateway
# identity code must stay import-light (shiva_constants + stdlib only).
_PROFILE_LABEL_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")


def _profile_label_for_home(home: Path | str) -> Optional[str]:
    """Best-effort profile label for a SHIVA_HOME path.

    Returns the profile name for ``<root>/profiles/<name>`` layouts (both
    ``~/.shiva/profiles/coder`` and Docker ``/opt/data/profiles/coder``),
    ``"default"`` for the deployment's root home, and ``None`` when no label
    can be inferred.  Never raises — this feeds diagnostics only.
    """
    try:
        canonical = _canonical_shiva_home(home)
    except Exception:
        return None
    ...
    try:
        ...
        if _same_shiva_home(canonical, get_default_shiva_root()):
            return "default"
    except Exception:
        pass
    try:
        if _same_shiva_home(canonical, _get_platform_default_shiva_home()):
            return "default"
    except Exception:
        pass
    return None
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    SHIVA_HOME profile than the caller.  Prefers the explicit ``profile``
    field stamped by :func:`acquire_scoped_lock`; falls back to inferring the
    label from the persisted ``shiva_home`` for locks written before the
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting SHIVA_HOME."""
    home = _get_process_shiva_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("SHIVA_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "shiva" / _LOCKS_DIRNAME
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Shiva' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Shiva gateway lifecycle subcommand from a command line.
    ...
    test also matched ``shiva_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\shiva-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Shiva's
    ...
    for token in tokens:
        ...
        if basename in ("shiva-gateway", "shiva-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "shiva_cli.main" in joined
        or "shiva_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("shiva", "shiva.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `shiva gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Shiva-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Shiva gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a SHIVA_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.shiva`` or ``$SHIVA_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``shiva_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``SHIVA_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a SHIVA_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"shiva_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit SHIVA_HOME= on the argv. SHIVA_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "shiva_home=" in command_lc and f"shiva_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        # SHIVA_HOME-local.  Persist the owning gateway's process home so an
        # explicit cross-profile --replace can place its planned-takeover
        # marker where the target process will actually read it.
        "shiva_home": str(_canonical_shiva_home(_get_process_shiva_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    ``shiva_cli``; the helper itself is cached per process. A gateway
    keeps serving the module versions it imported at startup, so stamping
    the identity into ``gateway_state.json`` lets `shiva update` (and the
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `shiva status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``SHIVA_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``shiva_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    SHIVA_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different SHIVA_HOME dirs).
    """
    ...
    # is inferable; readers fall back to deriving it from shiva_home.
    profile = _profile_label_for_home(_get_process_shiva_home())
    if profile:
...
# enabled in the user's systemd (e.g. ``shiva.service`` + ``shiva-
# gateway.service``).
#
...
def _get_takeover_marker_path(shiva_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``shiva_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = shiva_home or _get_process_shiva_home()
    return _canonical_shiva_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-SHIVA_HOME --replace while
    ...
    our_home = _get_process_shiva_home()
    target_home = record.get("target_shiva_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_shiva_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_shiva_home")
        if replacer_home is not None and not _same_shiva_home(
    ...
    # ``shiva gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's SHIVA_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_shiva_home(
            target_home or _get_process_shiva_home()
        )
        ...
        record = {
            ...
            "target_shiva_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_shiva_home": str(
                _canonical_shiva_home(_get_process_shiva_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target SHIVA_HOME's gateway PID record, and the live
    ...
    pid_record_home = pid_record.get("shiva_home")
    if not isinstance(pid_record_home, str) or not _same_shiva_home(
