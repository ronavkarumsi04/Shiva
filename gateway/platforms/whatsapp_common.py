...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Shiva Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to SHIVA_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/shiva), this function
    mirrors the bridge source to a writable SHIVA_HOME location and returns that
    ...
    # Try SHIVA_HOME location first
    shiva_home = get_shiva_home()
    shiva_home_bridge = shiva_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to SHIVA_HOME if needed
    if shiva_home_bridge.exists():
        return shiva_home_bridge

    # Mirror the bridge source to SHIVA_HOME
    try:
        shiva_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            shiva_home_bridge,
            dirs_exist_ok=False,
