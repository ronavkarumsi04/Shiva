...
There are two ways to add a platform to the Shiva gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.shiva/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Shiva code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `shiva gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `shiva_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Shiva core.
...
And add it to the `shiva-gateway` composite:

```python
"shiva-gateway": {
    "includes": [..., "shiva-your-platform"]
}
```
...
## 12. Status Display (`shiva_cli/status.py`)
...
## 13. Gateway Setup Wizard (`shiva_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ shiva_cli/ toolsets.py \
  --include="*.py" -l | sort -u
