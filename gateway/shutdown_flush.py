...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<shiva_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active SHIVA_HOME."""
    from shiva_constants import get_shiva_home

    flush_dir = get_shiva_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<shiva_home>/pending_messages/``), so a runtime cap rotation no
