...
``<SHIVA_HOME>/state/gateway.lifecycle.json``:
...
def _process_shiva_home() -> Path:
    """SHIVA_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<SHIVA_HOME>/state/gateway.lifecycle.json``."""
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``shiva_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
