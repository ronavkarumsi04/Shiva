...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from shiva_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (shiva_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Shiva is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_shiva_home() / "sessions")
    ...
    # gateway behaves exactly as before — single SHIVA_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.shiva/config.yaml (primary user-facing config)
    3. ~/.shiva/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_shiva_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through shiva_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `shiva config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `shiva config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``shiva config set gateway.streaming.*``
