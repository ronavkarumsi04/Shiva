...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{SHIVA_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``SHIVA_HOME`` is a **durable** store — on Shiva
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``shiva gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting SHIVA_HOME."""
    base = home if home is not None else get_shiva_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable SHIVA_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (SHIVA_HOME is a
    durable Fly volume on Shiva Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    SHIVA_HOME volume, or an expired same-epoch orphan (#85433), is
