...
class PlatformEntry:
    ...
    # status displays (``shiva setup``, ``shiva status``, the dashboard
    ...
    # Env vars this platform needs (for ``shiva setup`` display).
    ...
    # built-ins).  Used by ``shiva gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved SHIVA_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `shiva`
        # invocation -- including plain `shiva chat`, which never touches any
        ...
        # `shiva setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``shiva setup``/``gateway status``, channel
