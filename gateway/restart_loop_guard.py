...
Defenses 1 and 2 (the ``_SHIVA_GATEWAY`` guard on ``shiva gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.shiva.gateway")``,
...
State lives in ``<SHIVA_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_shiva_home() / "gateway" / "restart_loop.json"
