...
If you only run one Shiva agent, you don't need this page — see
...
[Connecting Desktop to Many Shiva Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Shiva agents that should all
...
(`ai.shiva.gateway-<name>.plist`) or systemd user service
(`shiva-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
shiva profile create coder
shiva profile create personal-bot
shiva profile create research
...
```bash
shiva config set gateway.multiplex_profiles true
shiva gateway restart
```

Equivalently, in the default profile's `~/.shiva/config.yaml`:
...
You do **not** run `shiva gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `shiva gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.shiva/profiles/coder/.env`; the default listener key is rejected.
...
home). `shiva status` reports the multiplexer and the profiles it serves;
`shiva status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/shiva-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: shiva-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    shiva gateway "$action"
  else
    shiva -p "$profile" gateway "$action"
  fi
}
...
```bash
shiva-gateways start      # start every configured profile
shiva-gateways stop       # stop every configured profile
shiva-gateways restart    # restart all
shiva-gateways status     # status across all
shiva-gateways list       # delegates to `shiva gateway list`
```

:::tip
The `default` profile is targeted with `shiva gateway <action>` (no `-p`),
not `shiva -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `shiva -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.shiva.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/shiva-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.shiva.gateway.plist` /
`shiva-gateway.service`.
...
```bash
# Default profile
tail -f ~/.shiva/logs/gateway.log
tail -f ~/.shiva/logs/gateway.error.log

# Named profile
tail -f ~/.shiva/profiles/<name>/logs/gateway.log
tail -f ~/.shiva/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.shiva/logs/gateway.log ~/.shiva/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
shiva logs -f                  # follow default profile
shiva -p coder logs -f         # follow one profile
shiva logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
shiva profile list             # profiles + model + gateway state
shiva-gateways status          # full status across every profile
launchctl list | grep shiva    # macOS — PIDs and labels
systemctl --user list-units 'shiva-gateway-*'   # Linux — units
```
...
The default profile uses `~/.shiva/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
shiva config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.shiva/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=shiva --why="gateways running" \
  sleep infinity &
...
`shiva-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.shiva/.env ~/.shiva/profiles/*/.env
```

## Updating the code

`shiva update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `shiva gateway start` after a previous `shiva gateway stop`. The
...
```bash
ps -ef | grep "shiva_cli.*-p <profile>"
cat ~/.shiva/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.shiva.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.shiva.gateway-<profile>.plist

# Linux
systemctl --user restart shiva-gateway-<profile>.service
```

### Health check

```bash
shiva doctor                  # default profile
shiva -p <profile> doctor     # one profile
```
