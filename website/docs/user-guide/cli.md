...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Shiva Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Shiva Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `shiva setup --portal` — and you're ready to `shiva chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Shiva also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `shiva --tui` — see the [TUI](tui.md) guide.
:::
...
shiva chat --query-file prompt.txt
shiva chat --query-file - < prompt.txt

# With a specific model
shiva chat --model "anthropic/claude-sonnet-4"

# With a specific provider
shiva chat --provider nous        # Use Nous Portal
shiva chat --provider openrouter  # Force OpenRouter
...
shiva chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
shiva --continue             # Resume the most recent CLI session (-c)
shiva --resume <session_id>  # Resume a specific session by ID (-r)
shiva --resume latest        # Resume the most recent session (same as -c)
shiva --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
shiva chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
shiva -w                         # Interactive mode in worktree
shiva -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`shiva -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
shiva worktree list              # audit: age, size, verdict, reason per tree
shiva worktree prune             # remove safe trees + delete merged branches
shiva worktree prune --dry-run   # show the plan without changing anything
shiva worktree prune --trees-only     # leave local branches alone
shiva worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running shiva session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.shiva/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
The `shiva plugins` commands manage native Shiva plugins and portable Agent
...
shiva plugins disable <plugin-name>
shiva plugins update <plugin-name>
shiva plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Shiva currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Shiva CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Shiva CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `SHIVA_YOLO_MODE` is on (either `shiva --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Shiva to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.shiva/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Shiva supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Shiva replies |
...
```bash
shiva -s shiva-agent-dev,github-auth
shiva chat -s github-pr-workflow -s github-auth
```

Shiva loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.shiva/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.shiva/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Shiva..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Shiva also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Shiva — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Shiva is working, Shiva prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Shiva to the background — just like any terminal process. The shell prints a confirmation:

```
Shiva Agent has been suspended. Run `fg` to bring Shiva Agent back.
```
...
Resume this session with:
  shiva --resume 20260225_143052_a1b2c3
...
```bash
shiva --continue                          # Resume the most recent CLI session
shiva -c                                  # Short form
shiva -c "my project"                     # Resume a named session (latest in lineage)
shiva --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
shiva --resume "refactoring auth"         # Resume by title
shiva --resume latest                     # Resume the most recent session (same as -c)
shiva --resume latest --in ./my-project   # Latest session for ./my-project's workspace
shiva -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `shiva sessions rename <id> <title>` from the command line. Use `shiva sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Shiva's SQLite state database under `~/.shiva/state.db`. The database keeps:
...
Shiva immediately confirms the task and gives you back the prompt:
