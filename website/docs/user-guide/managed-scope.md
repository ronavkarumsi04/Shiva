...
`~/.shiva/config.yaml`, `~/.shiva/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/shiva`:

```text
/etc/shiva/
├── config.yaml     # managed config layer (wins over ~/.shiva/config.yaml)
└── .env            # managed env layer (wins over ~/.shiva/.env + shell)
```
...
The location can be relocated with the `SHIVA_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `SHIVA_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Shiva.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export SHIVA_MANAGED_DIR=/opt/org/shiva-policy
```

:::warning
A user who can set `SHIVA_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `shiva doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/shiva/config.yaml` (managed) | `/etc/shiva/.env` (managed) |
| 2 | `~/.shiva/config.yaml` (user) | `~/.shiva/.env` (user) |
...
```bash
shiva config        # shows a header naming the managed source + the pinned keys
shiva doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Shiva refuses and names the source:

```bash
$ shiva config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/shiva/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `shiva config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/shiva

# Pin some config values for every user on this machine
sudo tee /etc/shiva/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/shiva/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/shiva
sudo chmod 0644 /etc/shiva/config.yaml /etc/shiva/.env
```

Changes take effect on the next Shiva start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`shiva doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Shiva as `root`), managed scope is advisory.
