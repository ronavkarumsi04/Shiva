---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Shiva"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Shiva"
---
...
Import an OpenClaw setup (memories, skills) into Shiva.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `shiva skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Shiva Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Shiva`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Shiva Migration

Use this skill when a user wants to move their OpenClaw setup into Shiva Agent with minimal manual cleanup.
...
```bash
shiva claw migrate              # Full interactive migration
...
shiva claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `shiva setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_shiva.py` to:

- import `SOUL.md` into the Shiva home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Shiva memory entries
- merge OpenClaw command approval patterns into Shiva `command_allowlist`
- migrate Shiva-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Shiva working-directory configuration
- copy OpenClaw skills into `~/.shiva/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Shiva workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.shiva/tts/`
- archive non-secret docs that do not have a direct Shiva destination
...
- `scripts/openclaw_to_shiva.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.shiva/skills/migration/openclaw-migration/scripts/openclaw_to_shiva.py`

Do not guess a shorter path like `~/.shiva/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.shiva/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Shiva-compatible secrets, currently:
...
Shiva CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Shiva. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Shiva workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Shiva skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.shiva/skills/migration/openclaw-migration/scripts/openclaw_to_shiva.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.shiva/skills/migration/openclaw-migration/scripts/openclaw_to_shiva.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.shiva/skills/migration/openclaw-migration/scripts/openclaw_to_shiva.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.shiva/skills/migration/openclaw-migration/scripts/openclaw_to_shiva.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.shiva/skills/migration/openclaw-migration/scripts/openclaw_to_shiva.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.shiva/skills/migration/openclaw-migration/scripts/openclaw_to_shiva.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Shiva unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Shiva targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Shiva destination. Unsupported auth blobs must still be reported as skipped.
...
- Shiva persona state imported
- Shiva memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.shiva/skills/openclaw-imports/`
