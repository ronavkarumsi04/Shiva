---
title: "Honcho — Configure and troubleshoot Honcho memory for Shiva"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Shiva"
---
...
Configure and troubleshoot Honcho memory for Shiva.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `shiva skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Shiva

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Shiva profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/shiva#running-honcho-locally-with-shiva

### Verify

```bash
shiva honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Shiva profile's AI peer
...
Honcho models conversations as interactions between **peers**. Shiva creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Shiva instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Shiva run |
| `global` | Single session across all directories |

Manual override: `shiva honcho map my-project-name`
...
Each Shiva profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block shiva.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `shiva.coder` host block in `honcho.json`
...
```bash
shiva honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Shiva when Honcho memory is active.
...
Config file: `$SHIVA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `shiva honcho setup`. Ensure `memory.provider: honcho` is in `~/.shiva/config.yaml`.

### Memory not persisting across sessions
Check `shiva honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `shiva profile create <name> --clone`. For existing profiles: `shiva honcho sync`.
...
| Command | Description |
|---------|-------------|
| `shiva honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `shiva honcho status` | Show resolved config, connection test, peer info for active profile |
| `shiva honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `shiva honcho disable` | Disable Honcho for the active profile |
| `shiva honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `shiva honcho peers` | Show peer identities across all profiles |
| `shiva honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `shiva honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `shiva honcho sessions` | List known directory-to-session-name mappings |
| `shiva honcho map <name>` | Map current working directory to a Honcho session name |
| `shiva honcho identity` | Seed AI peer identity or show both peer representations |
| `shiva honcho sync` | Create host blocks for all Shiva profiles that don't have one yet |
| `shiva honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Shiva + Honcho |
| `shiva memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `shiva memory status` | Show active memory provider and config |
| `shiva memory off` | Disable external memory provider |
