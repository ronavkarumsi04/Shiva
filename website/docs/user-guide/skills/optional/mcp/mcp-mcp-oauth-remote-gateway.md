...
| | |
|---|---|
| Source | Optional — install with `shiva skills install official/mcp/mcp-oauth-remote-gateway` |
...
| Related skills | [`shiva-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-shiva-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter), [`fastmcp`](/docs/user-guide/skills/optional/mcp/mcp-fastmcp) |
...
# MCP OAuth on a Remote Shiva Gateway

## Overview

Shiva' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Shiva process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Shiva runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Shiva.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Shiva' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Shiva is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Shiva** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
laptop and fails to connect. The callback never reaches the Shiva process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the shiva user, an
empty or missing tokens directory (`$SHIVA_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When Shiva detects a remote session it prints two
...
Both require an interactive terminal to the Shiva host. The rest of this skill
is for when there is NO interactive TTY — Shiva running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Shiva Dashboard (try this BEFORE manual token surgery)

A remote Shiva gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `shiva dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'shiva dashboard'`). It exposes a connector/MCP console —
...
```bash
env | grep -iE "SHIVA_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`SHIVA_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$SHIVA_HOME/.env`. Those
...
Shiva' `ShivaTokenStorage` would have written, so on `/reload-mcp` Shiva finds
cached tokens and skips the browser flow entirely.
...
```bash
env | grep -iE "SHIVA|RAILWAY|CONTAINER"
echo "$DISPLAY $WAYLAND_DISPLAY $SSH_CLIENT"
```

No display + a remote indicator = remote gateway. `tools/mcp_oauth.py::_can_open_browser()`
uses these same env vars, so if Shiva' own auto-detect says "headless", the
built-in flow won't work.

### 2. Find SHIVA_HOME and the config path

```bash
SHIVA_HOME=$(python3 -c 'from shiva_constants import get_shiva_home; print(get_shiva_home())')
echo "config: $SHIVA_HOME/config.yaml"
echo "tokens: $SHIVA_HOME/mcp-tokens/"
```
...
```json
{
  "client_name": "Shiva Agent (manual OAuth)",
...
### 8. Write tokens in Shiva' exact schema

`tools/mcp_oauth.py::ShivaTokenStorage` expects two files under
`$SHIVA_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
body = json.dumps({
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "shiva-debug", "version": "1.0"},
    },
}).encode()
...
UA** — Cloudflare will 403 you even though Shiva (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Shiva sees `auth: oauth`, calls `ShivaTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Shiva process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Shiva needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Shiva does natively).
...
- **Don't claim Shiva doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Shiva. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Shiva' config.
