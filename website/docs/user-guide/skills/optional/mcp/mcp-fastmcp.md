...
| | |
|---|---|
| Source | Optional — install with `shiva skills install official/mcp/fastmcp` |
...
| Related skills | [`shiva-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-shiva-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Shiva or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Shiva. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.shiva/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.shiva/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Shiva integration, either:

- configure the server in `~/.shiva/config.yaml` using the `native-mcp` skill, or
...
### Shiva cannot see the deployed server

The server-building part may be correct while the Shiva config is not. Load the `native-mcp` skill and configure the server in `~/.shiva/config.yaml`, then restart Shiva.
