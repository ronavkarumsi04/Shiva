---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in Shiva"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in Shiva"
---
...
Set up Actual Computer (actual.inc) inference in Shiva.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `shiva skills install official/devops/actual-setup` |
| Path | `optional-skills/devops/actual-setup` |
| Version | `2.0.0` |
| Author | shl0ms + Shiva Agent |
| License | MIT |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Shiva inference
...
- User has an `ac_` key and wants Shiva routed through their Actual cluster.
...
- Shiva has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Shiva — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.shiva/.env`.
...
3. Select provider + model:
   ```bash
   shiva config set model.provider actual
   shiva config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   shiva chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Shiva at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.shiva/.env`, then:
   ```bash
   shiva config set model.provider actual
   shiva config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   shiva chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by Shiva since the first-class provider).**
   ...
   request still 400s this way on an old Shiva, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** Shiva' default toolset
   ...
   servers emit a bare `data: [DONE]` — Shiva reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Shiva the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
shiva chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
shiva chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
