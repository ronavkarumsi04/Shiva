...
| | |
|---|---|
| Source | Optional — install with `shiva skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Shiva Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Shiva terminal calls (desktop app flow only)
...
### Service Account (recommended for Shiva)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${SHIVA_HOME:-~/.shiva}/.env` (the skill will prompt for this on first load).
...
## Shiva Execution Pattern (desktop app flow)

Shiva terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/shiva-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/shiva-op.sock"
