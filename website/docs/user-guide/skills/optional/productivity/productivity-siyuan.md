...
| | |
|---|---|
| Source | Optional — install with `shiva skills install official/productivity/siyuan` |
...
3. Store it in `${SHIVA_HOME:-~/.shiva}/.env`:
   ```
...
```yaml
# In ~/.shiva/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
