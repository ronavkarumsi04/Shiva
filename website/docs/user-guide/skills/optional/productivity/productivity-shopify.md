...
| | |
|---|---|
| Source | Optional — install with `shiva skills install official/productivity/shopify` |
...
4. Save to `${SHIVA_HOME:-~/.shiva}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Shiva","productType":"Apparel","tags":["test"]}}'
```
