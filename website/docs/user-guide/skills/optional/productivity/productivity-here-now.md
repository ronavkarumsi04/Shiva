...
| | |
|---|---|
| Source | Optional — install with `shiva skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SHIVA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SHIVA_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${SHIVA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client shiva
```
...
```bash
PUBLISH="${SHIVA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client shiva
```
...
```bash
DRIVE="${SHIVA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `shiva`)    |
