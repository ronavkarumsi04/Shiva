...
| | |
|---|---|
| Source | Optional — install with `shiva skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Shiva Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${SHIVA_HOME:-~/.shiva}/.env`:
...
```bash
CANVAS="python $SHIVA_HOME/skills/productivity/canvas/scripts/canvas_api.py"
