...
| | |
|---|---|
| Source | Optional — install with `shiva skills install official/creative/kanban-video-orchestrator` |
...
short to a music video to an ASCII loop — in a Shiva Kanban pipeline that
...
3. **Generates** a setup script that creates Shiva profiles, project workspace, and the initial kanban task
...
For mapping role → which Shiva skills + toolsets it loads, see
...
3. Creates each Shiva profile via `shiva profile create --clone`
...
7. Fires the initial `shiva kanban create` task assigned to the director

Use `scripts/bootstrap_pipeline.py` to generate setup.sh from a brief +
team-design JSON. See **[references/kanban-setup.md](https://github.com/NousResearch/shiva-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/kanban-setup.md)**
...
```bash
shiva kanban watch --tenant <project-tenant>     # live events
shiva kanban list  --tenant <project-tenant>     # board snapshot
shiva dashboard                                   # visual board UI
```
...
playbook, see **[references/monitoring.md](https://github.com/NousResearch/shiva-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/monitoring.md)**.
...
task graphs. See **[references/examples.md](https://github.com/NousResearch/shiva-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/examples.md)**.
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `shiva kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${SHIVA_HOME:-~/.shiva}/.env` or the user's secret store.
