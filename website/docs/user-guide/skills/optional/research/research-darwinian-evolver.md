...
| | |
|---|---|
| Source | Optional — install with `shiva skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Shiva Agent |
| License | MIT |
...
import upstream classes into Shiva itself.
...
```bash
mkdir -p ~/.shiva/cache/darwinian-evolver && cd ~/.shiva/cache/darwinian-evolver
...
```bash
cd ~/.shiva/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.shiva/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.shiva/skills/research/darwinian-evolver
DE_DIR=~/.shiva/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Shiva core.
   Custom driver scripts under `~/.shiva/skills/...` are user-side and fine.
