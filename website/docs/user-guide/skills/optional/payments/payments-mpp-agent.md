...
| | |
|---|---|
| Source | Optional — install with `shiva skills install official/payments/mpp-agent` |
...
Wraps the Machine Payments Protocol (MPP, https://mpp.dev) clients so Shiva can pay for per-request API access against servers that respond with `HTTP 402 Payment Required`.
