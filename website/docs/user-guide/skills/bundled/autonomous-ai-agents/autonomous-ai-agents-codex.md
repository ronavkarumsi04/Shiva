...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`shiva-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-shiva-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Shiva terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Shiva itself, `model.provider: openai-codex` uses Shiva-managed Codex
OAuth from `~/.shiva/auth.json` after `shiva auth add openai-codex`. For the
...
## Shiva Gateway Caveat

When invoking the Codex CLI from a Shiva gateway/service context (for example,
