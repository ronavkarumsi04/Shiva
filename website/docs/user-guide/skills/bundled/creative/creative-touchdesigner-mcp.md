...
```
Shiva Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${SHIVA_HOME:-$HOME/.shiva}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Shiva config (if missing)
...
3. **Restart Shiva session** to pick up the new MCP server
