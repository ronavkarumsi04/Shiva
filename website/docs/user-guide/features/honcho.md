...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Shiva's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Shiva instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
shiva memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `shiva` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Shiva syncs them back at session init.
...
The `shiva honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `shiva memory setup honcho` (or run `shiva memory setup` and pick it from the list); the `shiva honcho` subcommand then appears on the next invocation.

```bash
shiva memory setup honcho    # Configure Honcho directly (works before activation)
shiva honcho status          # Connection status, config, and key settings
shiva honcho setup           # Redirects to `shiva memory setup` (post-activation alias)
shiva honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
shiva honcho peer            # Show or update peer names + dialectic reasoning level
shiva honcho mode            # Show or set recall mode (hybrid/context/tools)
shiva honcho tokens          # Show or set token budget for context and dialectic
shiva honcho identity        # Seed or show the AI peer's Honcho identity
shiva honcho sync            # Sync Honcho config to all existing profiles
shiva honcho peers           # Show peer identities across all profiles
shiva honcho sessions        # List known Honcho session mappings
shiva honcho map             # Map current directory to a Honcho session name
shiva honcho enable          # Enable Honcho for the active profile
shiva honcho disable         # Disable Honcho for the active profile
shiva honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `shiva honcho`

If you previously used the standalone `shiva honcho setup`:
...
No re-login or re-setup needed. Run `shiva memory setup` and select "honcho" — the wizard detects your existing config.
