...
When Shiva Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.shiva/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.shiva/config.yaml` (switchable per session
via `/personality`).
...
If you use Shiva' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.shiva/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Shiva Agent's deliverable
