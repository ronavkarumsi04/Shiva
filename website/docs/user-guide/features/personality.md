...
description: "Customize Shiva Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Shiva Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `SHIVA_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Shiva is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Shiva now seeds a default `SOUL.md` automatically in:

```text
~/.shiva/SOUL.md
```

More precisely, it uses the current instance's `SHIVA_HOME`, so if you run Shiva with a custom home directory, it will use:
...
- Shiva creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Shiva loads `SOUL.md` only from `SHIVA_HOME`
- Shiva does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Shiva falls back to a built-in default identity
...
If Shiva loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `SHIVA_HOME`, the personality belongs to the Shiva instance itself.

That also makes it easier to teach users:
- "Edit `~/.shiva/SOUL.md` to change Shiva' default personality."
...
- how Shiva should handle uncertainty, disagreement, or ambiguity
...
## What Shiva injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Shiva falls back to a built-in default identity ("You are Shiva Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Shiva ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Shiva, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Shiva its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.shiva/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.shiva/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-shiva)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Shiva speaks
- `display.skin` and `/skin` affect how Shiva looks in the terminal
