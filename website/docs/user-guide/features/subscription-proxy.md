...
chat completions — use your Shiva-managed provider subscription as their
...
| Use case | "Use Shiva as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Shiva stores
the refresh token in `~/.shiva/auth.json` — the same place all Shiva
...
Model:      Shiva-4-70B    # or Shiva-4.3-36B, Shiva-4-405B
```
...
interface in `shiva_cli/proxy/adapters/`.
...
If you see `not logged in`, run `shiva portal`. If you see
...
`shiva_cli/proxy/adapters/<provider>.py` and registering it in
