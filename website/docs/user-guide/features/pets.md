...
Shiva can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<SHIVA_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Shiva renders the sprite at full fidelity when your
...
shiva pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `shiva pets list [query] [--limit N]` |
| List installed pets | `shiva pets list --installed` |
| Install a pet | `shiva pets install <slug> [--select] [--force]` |
| Set the active pet | `shiva pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `shiva pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `shiva pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `shiva pets off` |
| Remove an installed pet | `shiva pets remove <slug>` |
| Diagnose setup | `shiva pets doctor` |

`shiva pets show` flags:
...
Beyond installing pre-made pets from the gallery, Shiva can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `shiva pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `shiva tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `SHIVA_PET_IMAGE_PROVIDER` env var (e.g. `SHIVA_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Shiva is
...
Run `shiva pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Shiva uses its own
  profile-scoped `<SHIVA_HOME>/pets/` instead — install through `shiva pets`.

## See also

- The [`shiva-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-shiva-agent.md)
