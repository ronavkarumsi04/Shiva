...
Type `@` followed by a reference to inject content directly into your message. Shiva expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Shiva env: `$SHIVA_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$SHIVA_HOME/skills/.hub/`
