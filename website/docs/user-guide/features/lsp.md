...
Shiva runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Shiva
...
1. Shiva captures a baseline of current diagnostics for the file.
...
…). Shiva auto-detects the binary on PATH or in
`<SHIVA_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Shiva at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<SHIVA_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`shiva lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Shiva installs both packages together when you
run `shiva lsp install typescript` or auto-install fires on first
use.

## CLI

```
shiva lsp status          # service state + per-server install status
shiva lsp list            # registry, optionally --installed-only
shiva lsp install <id>    # eagerly install one server
shiva lsp install-all     # try every server with a known recipe
shiva lsp restart         # tear down running clients
shiva lsp which <id>      # print resolved binary path
```

`shiva lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <SHIVA_HOME>/lsp/bin
...
When `install_strategy: auto`, Shiva installs binaries into
`<SHIVA_HOME>/lsp/bin/`. NPM packages land in
`<SHIVA_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Shiva-owned and is
...
**`shiva lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<SHIVA_HOME>/lsp/bin/`. Run
`shiva lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `shiva lsp status`**
...
When `shiva lsp status` shows a `Backend warnings` section, install
...
`~/.shiva/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.shiva/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `shiva lsp restart` to clear the set;
