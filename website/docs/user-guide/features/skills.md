...
All skills live in **`~/.shiva/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Shiva at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `shiva update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.shiva` profile):

```bash
curl -fsSL https://shiva-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
shiva profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
shiva skills opt-out            # stop future seeding — nothing on disk is touched
shiva skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
shiva skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `shiva update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `shiva skills opt-in`) to re-enable.

:::note Safe by default
`shiva skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Shiva installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Shiva to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.shiva/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
shiva chat --toolsets skills -q "What skills do you have?"
shiva chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Shiva-tool framing, no invented
commands).
...
/home/user/.shiva/cache/chart-q4-2025.png
...
When a missing value is encountered, Shiva asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `shiva setup` or `~/.shiva/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `shiva config migrate` prompts for unconfigured settings, and `shiva config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.shiva/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Shiva scans the
...
policy above), Shiva can run [NVIDIA SkillEvaluator](https://github.com/NVIDIA/SkillEvaluator)
...
If you maintain skills outside of Shiva — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Shiva to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.shiva/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.shiva/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Shiva process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Shiva ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.shiva/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Repos can carry their own skills, active only for sessions started inside that project — the same pattern other agent harnesses use for repo-local configuration. When you launch Shiva inside a git checkout, it looks for skills in:

```text
<project-root>/.shiva/skills/    # Shiva-native location
...
Skills are procedure documents the agent follows, so Shiva does **not** auto-load them from arbitrary cloned repos. The first time you run Shiva in a repo with project skills, the banner shows a notice:

```text
◆ 3 project skill(s) found in /home/you/myproject but not loaded — run `shiva skills trust` to enable them.
```

Trust the repo once (from inside it, or by passing the path):

```bash
shiva skills trust             # trust the current repo
shiva skills trust ~/myproject # or explicitly
shiva skills untrust           # revoke
```

Trusted roots are stored in `skills.trusted_project_dirs` in `~/.shiva/config.yaml`. Set `skills.project_discovery: false` to turn the feature off entirely (no scanning, no notices).

### Precedence

Project skills are the **highest-precedence tier**: `project → local (~/.shiva/skills/) → external_dirs`. A project skill named `deploy` overrides a same-named profile or bundled skill for sessions inside that repo — that's the point: vendored repo skills win on their home turf, without touching your global profile. Project skills are tagged `[project]` in the agent's skill index so provenance stays visible.

Like external dirs, project skill directories are treated as repo-owned: autonomous skill maintenance (the curator) never modifies them, and new agent-created skills always go to `~/.shiva/skills/`.

### Scan-time quarantine

Trust is a repo-level decision, but a repo's skill content changes with every `git pull`. To close that gap, every project skill is scanned with the same security scanner used for Skills Hub installs before it enters the index. A skill whose scan verdict is **dangerous** (prompt-injection directives, credential-exfiltration commands, hidden-text tricks) is quarantined: it does not appear in the skill index, `skills_list`, slash commands, and refuses to load by name with an explanatory error. Scans are content-hash cached under `~/.shiva/cache/project_skill_scans/` (never inside your repo) and re-run automatically when the skill's content changes.
...
Bundles live in **`~/.shiva/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `shiva bundles list`.
...
shiva bundles create research

# Overwrite an existing bundle
shiva bundles create backend-dev --skill ... --force

# Delete a bundle
shiva bundles delete backend-dev

# Re-scan ~/.shiva/skill-bundles/ and report changes
shiva bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.shiva/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.shiva/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.shiva/pending/skills/` and are
...
```bash
shiva skills browse                              # Browse all hub skills (official first)
shiva skills browse --source official            # Browse only official optional skills
shiva skills search kubernetes                   # Search all sources
shiva skills search react --source skills-sh     # Search the skills.sh directory
shiva skills search https://mintlify.com/docs --source well-known
shiva skills inspect openai/skills/k8s           # Preview before installing
shiva skills install openai/skills/k8s           # Install with security scan
shiva skills install official/security/1password
shiva skills install skills-sh/vercel-labs/json-render/json-render-react --force
shiva skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
shiva skills list --source hub                   # List hub-installed skills
shiva skills check                               # Check installed hub skills for upstream updates
shiva skills update                              # Reinstall hub skills with upstream changes when needed
shiva skills audit                               # Re-scan all hub skills for security
shiva skills uninstall k8s                       # Remove a hub skill
shiva skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
shiva skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
shiva skills publish skills/my-skill --to github --repo owner/repo
shiva skills snapshot export setup.json          # Export skill config
shiva skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Shiva. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `shiva skills search <query> --source skills-sh`. Shiva resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Shiva currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Shiva repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Shiva can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
shiva skills search https://mintlify.com/docs --source well-known
shiva skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
shiva skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Shiva can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
shiva skills install openai/skills/k8s
shiva skills tap add myorg/skills-repo
```
...
[Skills Hub](https://shiva-agent.nousresearch.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Shiva-side changes required.
...
- Shiva source id: `clawhub`

#### 6. LobeHub (`lobehub`)

Shiva can search and convert agent entries from LobeHub's public catalog into installable Shiva skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Shiva source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

Shiva integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Shiva' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Shiva source id: `browse-sh`
- Trust level: `community`

```bash
shiva skills search airbnb --source browse-sh
shiva skills inspect browse-sh/airbnb.com/search-listings-ddgioa
shiva skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Shiva also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Shiva source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Shiva does not enumerate or copy unrelated files from the host.

```bash
shiva skills install https://sharethis.chat/SKILL.md
shiva skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
shiva skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `shiva skills update` re-fetches from the same URL automatically when you want to refresh.
...
`shiva skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Shiva | Always trusted |
...
```bash
shiva skills check          # Report which installed hub skills changed upstream
shiva skills update         # Reinstall only the skills with updates available
shiva skills update react   # Update one specific installed hub skill
shiva skills update react --force   # Overwrite a skill you've edited locally
```

This uses the stored source identifier plus the current upstream bundle content hash to detect drift.

Skills you have edited locally (the on-disk content no longer matches the hash recorded at install time) are **skipped** by `shiva skills update` so your changes are never silently overwritten. Pass `--force` to replace them with the upstream version anyway.
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Shiva users add with `shiva skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.shiva.tags`, `version`, `author`, `platforms`, `metadata.shiva.config`).
...
Shiva discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Shiva user can subscribe and install:

```bash
shiva skills tap add my-org/shiva-skills
shiva skills search deploy
shiva skills install my-org/shiva-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.shiva/skills/.hub/taps.json`:
...
The `shiva skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `shiva skills tap list` shows the effective path per tap.
...
```bash
shiva skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.shiva/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`shiva skills reset`)

Shiva ships with a set of bundled skills in `skills/` inside the repo. On install and on every `shiva update`, a sync pass copies those into `~/.shiva/skills/` and records a manifest at `~/.shiva/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Shiva recomputes the hash of your local copy and compares it to the origin hash:
...
shiva skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `SHIVA_HOME`, so `shiva -p coder skills reset <name>` only affects that profile.
:::
