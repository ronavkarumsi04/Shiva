...
Shiva Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Shiva checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.shiva/config.yaml`:
...
| `timeout` | `300` | Seconds Shiva waits for an approval reply before timing out. |
...
| `destructive_slash_confirm` | `true` | When true, destructive session slash commands (`/clear`, `/new`, `/reset`, `/undo`) prompt before discarding conversation state. Three-option dialog (Approve Once / Always Approve / Cancel) routed through native yes/no buttons on Telegram, Discord, and Slack; text fallback elsewhere. Users who click **Always Approve** flip this key to `false`. The TUI also honors this setting for its `/clear`, `/new`, and `/reset` modal; `SHIVA_TUI_NO_CONFIRM=1` force-skips that modal regardless of the configured value. |
...
1. **CLI flag**: Start a session with `shiva --yolo` or `shiva chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `SHIVA_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `SHIVA_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Shiva shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Shiva refuses to run them **regardless** of:
...
Configure the timeout in `~/.shiva/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.shiva/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.shiva/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` shiva/gateway | Self-termination prevention |
...
The `SHIVA_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.shiva/config.yaml`:
...
:::tip
Use `shiva config edit` to review or remove patterns from your permanent allowlist.
:::

### Mining Approval History (`shiva approvals suggest`)
...
```bash
shiva approvals suggest            # dry run — prints a numbered proposal
shiva approvals suggest --apply 1,3  # merge picks into command_allowlist
shiva approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.shiva/state.db`) for
...
Before `write_file` or `patch` touches disk, Shiva checks the target path against a denylist and an optional sandbox. Blocked writes return an error to the agent immediately — **there is no approval prompt** and no way to override from the chat UI. The model may still claim the edit succeeded; when `display.file_mutation_verifier` is on (default), trust the [file-mutation verifier footer](./configuration.md#file-mutation-verifier) over the assistant's closing summary.

### Protected paths (always blocked)

These categories are always denied, even when `SHIVA_WRITE_SAFE_ROOT` is unset:

| Category | Examples |
|----------|----------|
| OS credential stores | `~/.ssh/` (keys, `authorized_keys`), `~/.aws/`, `~/.kube/`, `/etc/sudoers`, `~/.netrc` |
| Shiva credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under SHIVA_HOME (active profile and global root) |
| Project secret files | `.env`, `.env.local`, `.env.production`, `.envrc` anywhere on disk |

Sensitive paths inside the safe root are still blocked — pointing `SHIVA_WRITE_SAFE_ROOT` at `$HOME` does not allow writing `~/.ssh/id_rsa`.

Safe-root violations return `Write denied: '…' is outside SHIVA_WRITE_SAFE_ROOT (…)`. Credential-path blocks use `Write denied: '…' is a protected system/credential file.`
...
### SHIVA_WRITE_SAFE_ROOT (optional sandbox)
...
- Set automatically in the [official Docker image](https://github.com/NousResearch/shiva-agent) (`SHIVA_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.shiva/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.shiva/cron/jobs.json`, profile skills, or other Shiva state outside that prefix

To allow both a workspace and Shiva home:

```bash
export SHIVA_WRITE_SAFE_ROOT=/path/to/project:/home/you/.shiva
```

Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [SHIVA_WRITE_SAFE_ROOT](../reference/environment-variables.md#shiva_write_safe_root).

### Cron and other Shiva state

Do not ask the agent to `patch` `~/.shiva/cron/jobs.json` directly. Use the `cronjob` tool, [`shiva cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Shiva control files when write safety blocks direct edits.
...
When running the messaging gateway, Shiva controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.shiva/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.shiva/.env to allow open access,
...
For more flexible authorization, Shiva includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `shiva pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.shiva/config.yaml`:
...
shiva pairing approve telegram ABC12DEF

# Revoke a user's access
shiva pairing revoke telegram 123456789

# Clear all pending codes
shiva pairing clear-pending
```

:::tip Docker users: run pairing commands as the `shiva` user
The official Docker image runs the gateway as the unprivileged `shiva` user
...
```bash
docker exec -u shiva shiva-agent shiva pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/NousResearch/shiva-agent/issues/10270
:::

**Storage:** Pairing data is stored in `~/.shiva/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Shiva applies strict security hardening to every container.
...
Container resources are configurable in `~/.shiva/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.shiva/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `SHIVA_HOME`. Skills declare these in frontmatter:
...
When loaded, Shiva checks if these files exist in the active profile's `SHIVA_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Shiva infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Shiva infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Shiva integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Shiva under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.shiva/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.shiva/logs/` for unauthorized access attempts
10. **Keep updated** — run `shiva update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.shiva/.env`:
...
TERMINAL_SSH_USER=shiva
TERMINAL_SSH_KEY=~/.ssh/shiva_agent_key
```
...
Shiva ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `shiva_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `shiva doctor` for the full remediation.
- **`shiva doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Shiva installs these **lazily** on first use rather than eagerly under `shiva-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `shiva tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `shiva tools`.
