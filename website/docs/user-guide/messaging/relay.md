...
sidebar_position: 30
title: "Shiva Relay"
description: "Connect Shiva to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Shiva Relay (Connector)
...
Shiva Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.shiva/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.shiva/.env`:
...
`shiva gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.shiva/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
token. Re-login with `shiva auth add nous` (or `shiva setup`) and retry.
...
startup. Restart it (`shiva gateway restart`).
