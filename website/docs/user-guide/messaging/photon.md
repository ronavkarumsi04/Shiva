...
Connect Shiva to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Shiva — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Shiva runs it in a
...
shiva photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Shiva Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.shiva/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.shiva/.env`
...
(device token, dashboard project id) lives in `~/.shiva/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Shiva
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Shiva replies with a pairing code. Approve it with:

```bash
shiva pairing approve photon <CODE>
```

Use `shiva pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.shiva/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.shiva/.env`):
...
By default Shiva responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Shiva` and
`@Shiva agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Shiva will reply.
...
assigned iMessage line Shiva uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run shiva photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `shiva photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `shiva photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `shiva photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Shiva sends images, voice
  ...
  media.
- **Native polls are supported.** Shiva sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Shiva sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `shiva send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<shiva-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Shiva is set
...
| `PHOTON_MENTION_PATTERNS` | Shiva wake words  | JSON list / comma / newline regex patterns for group mentions |
