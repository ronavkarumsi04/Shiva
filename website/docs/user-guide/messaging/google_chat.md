...
sidebar_position: 12
title: "Google Chat"
description: "Set up Shiva Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Shiva Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Shiva
...
> Run `shiva gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Shiva can read (e.g.,
`~/.shiva/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a shiva restart)
...
- Principal: `shiva-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Shiva calls
...
- **App name**: whatever you want users to see ("Shiva" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/shiva-chat-events`.
...
`ADDED_TO_SPACE` event that Shiva uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Shiva

Add the Google Chat section to `~/.shiva/.env`:
...
Send "hola" in the test DM. The bot posts a "Shiva is thinking…" marker, then
...
`~/.shiva/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Shiva is thinking…").
...
Thread support: when a user replies inside a thread, Shiva detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Shiva session.
...
3. Download the JSON. Move it onto the host that runs Shiva.
4. Register the client with Shiva (run under the profile you want it scoped to):
...
shiva -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Shiva home (e.g.
`~/.shiva/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.shiva/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.shiva/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Shiva isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `shiva gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Shiva will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.shiva/google_chat_user_tokens/<sanitized_email>.json` (filesystem
