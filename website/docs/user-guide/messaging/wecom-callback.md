...
Connect Shiva to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Shiva supports two WeCom integration modes:
...
> Run `shiva gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Shiva decrypts the message, queues it for the agent
...
(Use `shiva gateway start` only after `shiva gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Shiva and the token the
...
in `~/.shiva/.env` values around `=` will also break signature checks. After
fixing, restart `shiva gateway run`.
...
Check `shiva gateway run` logs for the bound host/port. If the adapter bound to
