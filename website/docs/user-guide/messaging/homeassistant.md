...
description: Control your smart home with Shiva Agent via Home Assistant integration.
...
Shiva Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Shiva Agent"
5. Copy the token
...
Shiva Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.shiva/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Shiva Agent".
...
The adapter reads credentials from `~/.shiva/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Shiva profile
...
running Shiva — `curl -H "Authorization: Bearer <token>" <url>/api/` should
