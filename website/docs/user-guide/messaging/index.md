...
description: "Chat with Shiva from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Shiva from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Shiva](/guides/use-voice-mode-with-shiva).
...
```mermaid
flowchart TB
    subgraph Gateway["Shiva Gateway"]
...
For group chats, hooks, and automation flows, Shiva supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Shiva keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Shiva does not hide failures just because the text resembles a silence token.
...
```bash
shiva gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
shiva gateway              # Run in foreground
shiva gateway setup        # Configure messaging platforms interactively
shiva gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo shiva gateway install --system   # Linux only: install a boot-time system service
shiva gateway start        # Start the default service
shiva gateway stop         # Stop the default service
shiva gateway status       # Check default service status
shiva gateway status --system         # Linux only: inspect the system service explicitly
```
...
```bash
shiva gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Shiva sends heartbeats
...
| `/update` | Update Shiva Agent to the latest version |
...
with the `session_reset` section in `~/.shiva/config.yaml`:
...
Configure per-platform overrides in `~/.shiva/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.shiva/gateway-config.yaml`:
...
shiva pairing approve telegram XKGH5N7P

# Other pairing commands:
shiva pairing list          # View pending + approved users
shiva pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Shiva appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.shiva/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.shiva/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `SHIVA_HOME`:

- `~/.shiva/status_phrases.yaml` or any `*.yaml` in `~/.shiva/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to SHIVA_HOME
...
Off by default. When enabled, Shiva prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.shiva/config.yaml`:
...
```bash
SHIVA_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
shiva gateway install               # Install as user service
shiva gateway start                 # Start the service
shiva gateway stop                  # Stop the service
shiva gateway status                # Check status
journalctl --user -u shiva-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo shiva gateway install --system
sudo shiva gateway start --system
sudo shiva gateway status --system
journalctl -u shiva-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Shiva installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit shiva-gateway` (or `sudo systemctl edit shiva-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `shiva update`. When `shiva update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart shiva-gateway` command (it never blocks on an interactive password prompt).
...
```bash
shiva gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `shiva update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo shiva update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/shiva-gateway`:

```
shiva ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed shiva-gateway*, /usr/bin/systemctl --no-ask-password start shiva-gateway*, /usr/bin/systemctl --no-ask-password restart shiva-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Shiva will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Shiva installations on the same machine (with different `SHIVA_HOME` directories), each gets its own systemd service name. The default `~/.shiva` uses `shiva-gateway`; other installations use `shiva-gateway-<hash>`. The `shiva gateway` commands automatically target the correct service for your current `SHIVA_HOME`.
:::

### macOS (launchd)

```bash
shiva gateway install               # Install as launchd agent
shiva gateway start                 # Start the service
shiva gateway stop                  # Stop the service
shiva gateway status                # Check status
tail -f ~/.shiva/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.shiva.gateway.plist`. It includes three environment variables:
...
- **SHIVA_HOME** — scopes the gateway to your Shiva installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `shiva gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `SHIVA_HOME` directory gets its own launchd label. The default `~/.shiva` uses `ai.shiva.gateway`; other installations use `ai.shiva.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `shiva-cli` | Full access |
| Telegram | `shiva-telegram` | Full tools including terminal |
| Discord | `shiva-discord` | Full tools including terminal |
| WhatsApp | `shiva-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `shiva-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `shiva-slack` | Full tools including terminal |
| Google Chat | `shiva-google_chat` | Full tools including terminal |
| Signal | `shiva-signal` | Full tools including terminal |
| SMS | `shiva-sms` | Full tools including terminal |
| Email | `shiva-email` | Full tools including terminal |
| Home Assistant | `shiva-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `shiva-mattermost` | Full tools including terminal |
| Matrix | `shiva-matrix` | Full tools including terminal |
| DingTalk | `shiva-dingtalk` | Full tools including terminal |
| Feishu/Lark | `shiva-feishu` | Full tools including terminal |
| WeCom | `shiva-wecom` | Full tools including terminal |
| WeCom Callback | `shiva-wecom-callback` | Full tools including terminal |
| Weixin | `shiva-weixin` | Full tools including terminal |
| BlueBubbles | `shiva-bluebubbles` | Full tools including terminal |
| QQBot | `shiva-qqbot` | Full tools including terminal |
| Yuanbao | `shiva-yuanbao` | Full tools including terminal |
| Microsoft Teams | `shiva-teams` | Full tools including terminal |
| API Server | `shiva-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `shiva-webhook` | Full tools including terminal |
| Raft | `shiva-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.shiva/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
