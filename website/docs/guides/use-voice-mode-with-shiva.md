---
sidebar_position: 8
title: "Use Voice Mode with Shiva"
description: "A practical guide to setting up and using Shiva voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Shiva
...
- you want Shiva sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Shiva.
...
## Step 1: make sure normal Shiva works first
...
Shiva supports both local and cloud speech stacks.
...
### If you use `shiva setup`

If you choose NeuTTS in the setup wizard, Shiva checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Shiva transcribes and responds
...
- asking Shiva to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Shiva loop.

## Tuning CLI behavior

### Silence threshold

If Shiva starts/stops too aggressively, tune:
...
Shiva stays a normal chat bot, but can speak replies.
...
- you want Shiva to function like a portable research or ops assistant
...
Shiva joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Shiva detects speech boundaries
- transcripts are posted in the associated text channel
- Shiva responds in text and audio
...
1. get text Shiva working
2. run `shiva setup tts` to enable voice support
