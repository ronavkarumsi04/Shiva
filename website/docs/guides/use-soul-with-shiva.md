---
sidebar_position: 7
title: "Use SOUL.md with Shiva"
description: "How to use SOUL.md to shape Shiva Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Shiva

`SOUL.md` is the **primary identity** for your Shiva instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Shiva to feel like the same assistant every time you talk to it — or if you want to replace the Shiva persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Shiva should be
- what Shiva should avoid stylistically
- how Shiva should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Shiva is and how Shiva speaks
...
Shiva now uses only the global SOUL file for the current instance:

```text
~/.shiva/SOUL.md
```

If you run Shiva with a custom home directory, it becomes:
...
Shiva automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Shiva does not overwrite it
- if the file exists but is empty, Shiva adds nothing from it to the prompt

## How Shiva uses it

When Shiva starts a session, it reads `SOUL.md` from `SHIVA_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Shiva feels.
...
Shiva already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Shiva should sound.

# Avoid
What Shiva should not do.

# Defaults
How Shiva should behave when ambiguity appears.
```
...
Then restart Shiva or start a new session.
...
### I edited SOUL.md but Shiva still sounds the same

Check:
- you edited `~/.shiva/SOUL.md` or `$SHIVA_HOME/SOUL.md`
...
### Shiva is ignoring parts of my SOUL.md
