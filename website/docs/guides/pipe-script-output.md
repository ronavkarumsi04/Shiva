...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `shiva send`."
---

# Pipe Script Output to Messaging Platforms

`shiva send` is a small, scriptable CLI that pushes a message to any
messaging platform Shiva is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | shiva send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `shiva
...
shiva send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
shiva send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
shiva send --list

# Filter by platform
shiva send --list telegram
```
...
Any platform Shiva ships adapters for works as a target:
...
`shiva send` resolves the message body in this order:

1. **Positional argument** — `shiva send --to telegram "hi"`
2. **`--file PATH`** — `shiva send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | shiva send --to telegram`

When stdin is a TTY (no pipe), Shiva does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  shiva send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `shiva send` reuses your Shiva config, the same script works on
any host where Shiva is installed — no need to export bot tokens into
...
`shiva send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  shiva send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | shiva send \
...
```bash
./train.py --epochs 200 && \
  shiva send --to telegram "training done" || \
  shiva send --to telegram "training failed (exit $?)"
```
...
shiva send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(shiva send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `shiva send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `shiva send` calls
the platform's REST endpoint directly using credentials from
`~/.shiva/.env` and `~/.shiva/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `shiva gateway start` and retry.
...
# Machine-readable
shiva send --list --json
```

The listing is built from `~/.shiva/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`shiva gateway
...
| Approach | Multi-platform | Reuses Shiva creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `shiva send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`shiva send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `shiva send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `shiva send` shares with cron delivery.
