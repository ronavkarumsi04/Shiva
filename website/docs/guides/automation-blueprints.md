...
Copy-paste blueprints for common automation patterns. Each blueprint uses Shiva's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`shiva webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `shiva webhook subscribe`) |
...
```bash
shiva cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/shiva-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/shiva-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
shiva webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
shiva cron create "0 9 * * 1" \
  "Scan the NousResearch/shiva-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/shiva-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (shiva_cli/commands.py, shiva_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (shiva_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
shiva cron create "0 6 * * *" \
  "Run a dependency security audit on the shiva-agent project.

1. cd ~/.shiva/shiva-agent && source .venv/bin/activate
...
```bash
shiva webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
shiva webhook subscribe alert-triage \
...
```python title="~/.shiva/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Shiva-Monitor/1.0"})
...
shiva cron create "every 30m" \
  ...
  --script ~/.shiva/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
shiva cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Shiva Agent development." \
...
```bash
shiva webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
shiva webhook subscribe stripe-payments \
...
```bash
shiva cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the shiva-agent codebase.
