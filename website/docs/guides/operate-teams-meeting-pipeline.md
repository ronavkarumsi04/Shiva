...
```bash
shiva teams-pipeline token-health
shiva teams-pipeline token-health --force-refresh
```
...
```bash
shiva teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
shiva teams-pipeline maintain-subscriptions
shiva teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Shiva cron (recommended if you already run the Shiva gateway)

Shiva ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.shiva/scripts/`. First create the script:

```bash
mkdir -p ~/.shiva/scripts
cat > ~/.shiva/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec shiva teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.shiva/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
shiva cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/shiva-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Shiva Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=shiva
EnvironmentFile=/etc/shiva/env
ExecStart=/usr/local/bin/shiva teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/shiva-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Shiva Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now shiva-teams-pipeline-maintain.timer
systemctl list-timers shiva-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/shiva teams-pipeline maintain-subscriptions >> /var/log/shiva/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.shiva/.env` at the top of a wrapper script that crontab calls.
...
```bash
shiva teams-pipeline subscriptions   # should show expirationDateTime advanced
shiva teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
shiva teams-pipeline list
shiva teams-pipeline list --status failed
shiva teams-pipeline show <job-id>
```

### Replay a stored job

```bash
shiva teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
shiva teams-pipeline fetch --meeting-id <meeting-id>
shiva teams-pipeline fetch --join-web-url "<join-url>"
shiva teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `shiva teams-pipeline maintain-subscriptions --dry-run`
- inspect `shiva teams-pipeline list --status failed`
...
- run `shiva teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `shiva teams-pipeline run`
...
- [ ] `shiva teams-pipeline validate` returns an OK snapshot
- [ ] `shiva teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Shiva cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
