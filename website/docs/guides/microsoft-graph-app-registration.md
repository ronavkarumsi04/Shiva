...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.shiva/.env` at the end.
...
4. Fill in:
   - **Name:** `Shiva Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `shiva-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.shiva/.env`:
...
Shiva ships a Graph auth smoke-test. From your Shiva install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.shiva/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `shiva gateway restart`.
