...
This tutorial walks you through setting up a Telegram bot powered by Shiva Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Shiva Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.shiva/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Shiva itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Shiva Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_shiva_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Shiva Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Shiva Gateway...
...
# View live logs
journalctl --user -u shiva-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `shiva gateway install` to update the plist.
:::
...
```bash
shiva gateway stop && shiva gateway start
```
...
shiva pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.shiva/.env`:
...
Control how much detail the bot shows when using tools. In `~/.shiva/config.yaml`:
...
Customize how the bot communicates by editing `~/.shiva/SOUL.md`:

For a full guide, see [Use SOUL.md with Shiva](/guides/use-soul-with-shiva).
...
```bash
# From the CLI
shiva cron list          # View all scheduled jobs
shiva cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u shiva-gateway` (Linux) or `~/.shiva/logs/gateway.log` (macOS) |
| Cron job output | `~/.shiva/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.shiva/cron/jobs.json` |
| Pairing data | `~/.shiva/pairing/` |
| Session history | `~/.shiva/sessions/` |
