...
description: "Diagnose and fix common Shiva cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`shiva gateway` for foreground, or `shiva gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `shiva cron tick`.
...
```bash
date
shiva cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.shiva/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.shiva/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.shiva/.env` |
...
| `local` | Write access to `~/.shiva/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `shiva cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `shiva skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `shiva skills list`.
...
2. `~/.shiva/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `shiva cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Shiva config directory). Verify:
```bash
ls ~/.shiva/scripts/your-script.py   # Must exist
shiva cron edit <job_id> --script ~/.shiva/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `shiva skills install <skill-name>`.
...
Jobs are stored in `~/.shiva/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.shiva/cron/jobs.json
chmod 600 ~/.shiva/cron/jobs.json   # Your user should own it
```
...
```bash
shiva cron list                    # Show all jobs, states, next_run times
shiva cron run <job_id>            # Schedule for next tick (for testing)
shiva cron edit <job_id>           # Fix configuration issues
shiva logs                         # View recent Shiva logs
shiva skills list                  # Verify installed skills
```
...
1. Run the job with `shiva cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.shiva/logs/agent.log` for scheduler messages and `~/.shiva/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/shiva-agent](https://github.com/NousResearch/shiva-agent) with:
