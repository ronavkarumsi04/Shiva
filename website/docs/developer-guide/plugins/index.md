...
title: "Build a Shiva Plugin"
description: "Step-by-step guide to building a complete Shiva plugin with tools, hooks, data files, and skills"
---

# Build a Shiva Plugin

This guide walks through building a complete Shiva plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Shiva has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `SHIVA_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.shiva/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `shiva skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
shiva plugins install owner/repository --no-enable
shiva plugins list
shiva plugins enable <plugin-name>
```
...
Shiva validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Shiva.
...
Portable `streamable-http` entries are routed through Shiva' existing native
...
same full-trust posture as other installed Shiva plugins.
...
records it as Published. Shiva keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Shiva does not expose a
...
continue to work after a normal Shiva upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Shiva inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Shiva releases can therefore load a plugin whose manifest contains
...
Shiva enforces this contract with frozen external-plugin fixtures discovered
from an isolated `SHIVA_HOME`. Those tests load and invoke the plugin through
...
```bash
mkdir -p ~/.shiva/plugins/calculator
cd ~/.shiva/plugins/calculator
```

### Validate with Plugin Doctor

`shiva plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Shiva itself. It reports invalid hook names, callbacks that do
...
```bash
shiva plugins doctor . --ci
```

Doctor uses a temporary `SHIVA_HOME`, restores plugin registration state after
...
This tells Shiva: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `shiva_cli/plugin_capabilities.py` for the
...
`shiva_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`shiva_agent.plugins` entry point:

```toml
[project.entry-points."shiva_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."shiva_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Shiva reads these from installed metadata without importing your code, so
`shiva plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Shiva understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — Shiva validates them, and `shiva plugins install` / `shiva plugins doctor` surface missing ones with a `pip install` hint, but Shiva **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into Shiva' shared venv is a conflict and supply-chain
...
[#64165](https://github.com/NousResearch/shiva-agent/issues/64165) and
[#15220](https://github.com/NousResearch/shiva-agent/issues/15220). Plugin
...
4. **Accept `**kwargs`:** Shiva may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `shiva my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Shiva continues fine
...
Use plugin-relative config keys for user-visible behavior. Shiva resolves them
...
`<SHIVA_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `SHIVA_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
SHIVA_PLUGINS_DEBUG=1 shiva plugins list
```
...
The same logs are always written to `~/.shiva/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
shiva logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `shiva plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.shiva/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `shiva plugins update` / `remove` git-pull or delete it — your
...
# <shiva home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.shiva/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.shiva/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `shiva plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Shiva into installing arbitrary packages — only specs Shiva itself ships are eligible |
...
auditable in `~/.shiva/logs/agent.log`. Plugins load after built-in
...
does not ship with Shiva core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `shiva plugins list`
...
loader, the plugin is disabled and Shiva continues. The gate exists
...
You normally never edit this key by hand. `shiva plugins enable <name>`
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$SHIVA_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $SHIVA_HOME/hook_outputs
```
...
- **The system prompt is Shiva's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `shiva_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Shiva sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`shiva.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Shiva still loads on an older one.

### Register CLI commands

Plugins can add their own `shiva <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for shiva my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: shiva my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for shiva my-plugin."""
...
After registration, users can run `shiva my-plugin status`, `shiva my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `shiva name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `shiva chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `SHIVA_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `shiva <subcommand>` (e.g. `shiva kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "shiva kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Shiva from a hook.
...
Shiva has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.shiva/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `shiva gateway status` without SDK instantiation.
...
Shiva also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Shiva without any Python plugin. Declare them in `~/.shiva/config.yaml`:
...
Shiva connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.shiva/hooks/<name>/`:

```yaml
# ~/.shiva/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.shiva/hooks/long-task-alert/handler.py
...
shiva skills search my-workflow --source myorg/skills-repo
shiva skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install shiva-plugin-calculator
# Plugin auto-discovered on next shiva startup
```
...
```python
# Wrong — will break if Shiva passes extra context
def handler(args):
    ...
