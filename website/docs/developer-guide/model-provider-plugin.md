...
description: "How to build a model provider (inference backend) plugin for Shiva Agent"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that Shiva can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$SHIVA_HOME/plugins/model-providers/` with zero changes to the repo.
...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with Shiva
2. **User plugins** — `$SHIVA_HOME/plugins/model-providers/<name>/` — drop in any directory; no restart required for subsequent sessions
3. **Legacy single-file** — `<repo>/providers/<name>.py` — back-compat for out-of-tree editable installs

**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$SHIVA_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.
...
The only required file is `__init__.py`. `plugin.yaml` is used by `shiva plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
|---|---|---|
| Credential resolution | `shiva_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `shiva_cli/main.py` | Accepts `acme-inference` |
| `shiva model` picker | `shiva_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `shiva doctor` | `shiva_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `shiva setup` | `shiva_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `shiva_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
| `display_name` | str | Human label shown in `shiva model` picker |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.shiva/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. Shiva picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but Shiva' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
Point `SHIVA_HOME` at a temp directory so you don't pollute your real config:

```bash
export SHIVA_HOME=/tmp/shiva-plugin-test
mkdir -p $SHIVA_HOME/plugins/model-providers/my-provider
cat > $SHIVA_HOME/plugins/model-providers/my-provider/__init__.py <<'EOF'
from providers import register_provider
from providers.base import ProviderProfile
register_provider(ProviderProfile(
    name="my-provider",
...
- **Lowest precedence.** Entry-point plugins are discovered **before**
  ...
  bundled or `$SHIVA_HOME` profile of the same name always overrides a
...
See [Building a Shiva Plugin](/developer-guide/plugins#distribute-via-pip) for the full entry-points setup.
...
- [Building a Shiva Plugin](/developer-guide/plugins) — general plugin authoring
