...
sidebar_position: 1
title: "Architecture"
description: "Shiva Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Shiva Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ shiva_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ShivaCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── shiva_state.py           # SQLite session/state database with FTS5
├── shiva_constants.py       # SHIVA_HOME, profile-aware paths
...
│
├── shiva_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `shiva` subcommands (large file)
...
│   ├── skills_config.py      # shiva skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # shiva tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # shiva gateway start/stop
│
...
```text
User input → ShivaCLI.process_input()
...
Three discovery sources: `~/.shiva/plugins/` (user), `.shiva/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `shiva plugins` or `config.yaml`.
...
Exposes Shiva as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`shiva -p <name>`) gets its own SHIVA_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
