...
Shiva Agent saves conversation trajectories in ShareGPT-compatible JSONL format
...
It follows the Shiva function-calling prompt template with:
...
Trajectory saving is a `run_agent.py` / library-level switch — the `shiva` CLI
