...
Shiva Agent uses a SQLite database (`~/.shiva/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `shiva_state.py`


## Architecture Overview

```
~/.shiva/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `shiva_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `shiva_state.py` for the exact SQL.
...
Multiple shiva processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.shiva/state.db
...
Default path: `~/.shiva/state.db`

This is derived from `shiva_constants.get_shiva_home()` which resolves to
`~/.shiva/` by default, or the value of `SHIVA_HOME` environment variable.
