...
title: "Desktop Plugin SDK (@shiva/plugin-sdk)"
description: "Extend the native Shiva Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Shiva Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `ShivaPlugin`.
It imports one module — `@shiva/plugin-sdk` — and gets everything: the app's
...
`$SHIVA_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Shiva. This page is the **native
desktop app** (`shiva desktop`) SDK — the `@shiva/plugin-sdk` module and
`$SHIVA_HOME/desktop-plugins/`. The **web dashboard** (`shiva dashboard`) has
its own, unrelated plugin system on `window.__SHIVA_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Shiva Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$SHIVA_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$SHIVA_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `ShivaPlugin` contract, appear in **Settings → Plugins**,
...
Create `$SHIVA_HOME/desktop-plugins/hello/plugin.js` (that's `~/.shiva/...`
by default, or `~/.shiva/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.shiva/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@shiva/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Shiva' }),
      jsx('div', {
...
The only importable specifiers are `@shiva/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `ShivaPlugin`:

```ts
interface ShivaPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `shiva.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@shiva/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@shiva/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@shiva/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@shiva/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@shiva/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@shiva/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@shiva/plugin-sdk'
...
`window.shiva.send('get-price eth')` (or a declarative
`<button data-shiva-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@shiva/plugin-sdk'
...
for requests; `targetProfile` is the backend Shiva profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Shiva
...
ctx.os.notify({
  ...
  // Body click → focus Shiva + navigate. Same vocabulary as OS deep links:
  activate: 'shiva://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'shiva://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `shiva://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `shiva://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@shiva/plugin-sdk'
...
```javascript
import { queryClient } from '@shiva/plugin-sdk'
...
a package into `~/.shiva/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.shiva/plugins` is not reachable as a filesystem —
...
the `shiva://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular Shiva plugin and declare it in a
`manifest.json`:
...
shiva-agent codebase directly (`shiva_state`, `shiva_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.shiva/`)
...
(`shiva.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `ShivaPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@shiva/plugin-sdk` alias.
2. It's still lint-fenced to `@shiva/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@shiva/plugin-sdk`, `react`,
...
| Plugin contract | `ShivaPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `ShivaOpenTarget`, `Contribution` |
...
### Agents: the `shiva-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`shiva-desktop-plugins`** skill — it carries the same contract as this page in
...
`$SHIVA_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `shiva logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@shiva/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.shiva/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.shiva/logs/errors.log` for
