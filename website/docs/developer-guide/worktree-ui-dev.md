...
| Variable | Meaning |
|----------|---------|
| `SHIVA_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `SHIVA_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `SHIVA_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Shiva itself — they're private to these helpers. The variables Shiva *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `shiva --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_shiva_root)" || { echo "htui: not in a Shiva checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$SHIVA_MAIN_CHECKOUT/.venv/bin/python" -m shiva_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `SHIVA_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_shiva_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `SHIVA_TUI_DIR` are mutually exclusive
`SHIVA_TUI_DIR` points Shiva at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `shiva --tui --dev` exits with an error. Run `unset SHIVA_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_shiva_root)" || { echo "hgui: not in a Shiva checkout" >&2; return 1; }
  deps="${SHIVA_GUI_DEPS_CHECKOUT:-$SHIVA_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _shiva_link_deps "$desktop" "$deps/apps/desktop"
    _shiva_link_deps "$root" "$deps"
  else
  ...
  trap '_shiva_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    SHIVA_DESKTOP_SHIVA_ROOT="$root" \
    SHIVA_DESKTOP_PYTHON="$SHIVA_MAIN_CHECKOUT/.venv/bin/python" \
    SHIVA_DESKTOP_IGNORE_EXISTING=1 \
    SHIVA_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `SHIVA_DESKTOP_SHIVA_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `shiva`. |
| `SHIVA_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `SHIVA_DESKTOP_IGNORE_EXISTING` | Ignores any `shiva` on `PATH` so it can't shadow the worktree. |
| `SHIVA_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Shiva checkout.
_shiva_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/shiva_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_shiva_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'shiva_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `shiva --tui --dev` and the `SHIVA_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/NousResearch/shiva-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `SHIVA_*` variable Shiva reads
