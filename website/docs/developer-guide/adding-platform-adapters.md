...
This guide covers adding a new messaging platform to the Shiva gateway. A platform adapter connects Shiva to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.shiva/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core Shiva code. Your plugin is a directory with two files:

```
~/.shiva/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `shiva config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-shiva-config) below).
...
With `provides_tools` declared, Shiva imports only `tools.py` during plugin
...
`shiva tools enable my_platform --platform cli`, or by listing the toolset
...
| `shiva config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `shiva status` | Shows plugin platforms with `(plugin)` tag |
| `shiva gateway setup` | Plugin platforms appear in setup menu |
| `shiva tools` / `shiva skills` | Plugin platforms in per-platform config |
...
direct `shiva send --to ...` and cron `deliver=platform:...` by declaring send
...
def register(ctx):
    ctx.register_platform(
        ...
        # May be a regular function or async def. Shiva awaits any awaitable
...
Most users set up a platform by dropping env vars into `~/.shiva/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `shiva gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `shiva cron run` separate from `shiva gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `shiva config`

`shiva_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
version: 1.0.0
description: >
  My Platform gateway adapter for Shiva Agent.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #shiva)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the Shiva core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`shiva_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`shiva_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`shiva_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`shiva_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`shiva_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`shiva_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"shiva-newplat"` toolset definition with `_SHIVA_CORE_TOOLS`
2. **`toolsets.py`** — Add `"shiva-newplat"` to the `"shiva-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | shiva-newplat toolset |
