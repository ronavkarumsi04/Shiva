...
Plugins can launch and supervise fresh Shiva child sessions without importing
...
active agent turn fails closed with `No active Shiva parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Shiva can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Shiva's existing unsafe-tool block remains enforced.
