...
The ACP adapter wraps Shiva' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
shiva acp / shiva-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.shiva/.env
  -> configure stderr logging
  -> construct ShivaACPAgent
...
- `allow_once` -> Shiva `once`
- `allow_always` -> Shiva `always`
- reject options -> Shiva `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Shiva tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["shiva-acp"])
...
Instead it reuses Shiva' runtime resolver:

- `acp_adapter/auth.py`
- `shiva_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Shiva provider/credentials. It also always advertises a terminal setup auth method (`shiva-setup`, args `--setup`) so first-run ACP clients can open Shiva' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.shiva/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `shiva-acp` toolset definition
- `shiva_cli/main.py` — `shiva acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `shiva-acp` script
