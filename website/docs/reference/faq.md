...
description: "Frequently asked questions and solutions to common issues with Shiva Agent"
---
...
### What LLM providers work with Shiva?

Shiva Agent works with any OpenAI-compatible API. Supported providers include:
...
- **Anthropic** — Claude models (direct API, OAuth via `shiva auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `shiva model` or by editing `~/.shiva/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run Shiva in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Shiva inside WSL2
...
- let Shiva use the resulting MCP browser tools

This is more reliable than trying to force Shiva core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Shiva](../guides/use-mcp-with-shiva.md#wsl2-bridge-shiva-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Shiva Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.shiva/`.

### Can I use it offline / with local models?

Yes. Run `shiva model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Shiva minimum; set this to match your server's actual context window
```
...
Shiva persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Shiva — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Shiva auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `SHIVA_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Shiva Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Shiva Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Shiva programmatically:
...
#### `shiva: command not found` after installation
...
**Cause:** Shiva requires Python 3.11 or newer.
...
**Cause:** Shiva builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Shiva runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Shiva auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.shiva/config.yaml`:
...
#### The agent says "Shiva policy" or "Shiva guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden Shiva runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

Shiva enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. Shiva does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.

To isolate the source:

1. Run `/status` to confirm the active model and provider.
2. Check whether the refusal includes an actual Shiva tool error or approval prompt. If it is prose only, do not treat the model's attribution as runtime evidence.
3. Retry in a fresh session with another configured model or provider. A refusal that changes with the model is model/provider behavior, not a Shiva execution control.
4. If an explicit tool error appears, use its exact text when reporting the problem.

See [Security](/user-guide/security) for Shiva' documented execution controls and [Providers](/integrations/providers) for provider configuration.
...
**Solution:** Exit your session and use `shiva model` from your terminal to add new providers:

```bash
# Exit the Shiva chat session first (Ctrl+C or /quit)
...
After adding a new provider via `shiva model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `shiva model` (from terminal) |
| Enter/change API keys | `shiva model` (from terminal) |
...
# Or set directly
shiva config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.shiva/.env` for conflicting entries.
:::
...
# Set a valid model
shiva config set SHIVA_MODEL anthropic/claude-opus-4.7

# Or specify per-session
shiva chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `shiva chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Shiva detected the wrong context length for your model.
...
shiva chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Shiva may have the wrong context length for your model. Check what it detected:
...
**Cause:** Shiva detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Shiva never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `shiva chat`
...
cat ~/.shiva/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `shiva gateway setup`
- Check gateway logs: `cat ~/.shiva/logs/gateway.log | tail -50`
...
Configure in `~/.shiva/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.shiva/shiva-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `shiva gateway start` fails
...
tmux new -s shiva 'shiva gateway run'
# Reattach later: tmux attach -t shiva

# Option 3: Background via nohup
nohup shiva gateway run > ~/.shiva/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'shiva gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `shiva gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
shiva gateway install    # Re-snapshots your current PATH
shiva gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.shiva.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `shiva chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `shiva chat -t "terminal"`
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`shiva prompt-size`](/reference/cli-commands#shiva-prompt-size) to measure it, then trim: disable toolsets you don't use (`shiva tools`) and uninstall or disable skills you don't need (`shiva skills`).
...
cd ~/.shiva/shiva-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.shiva/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
shiva config show | grep -A 12 mcp_servers

# Restart Shiva or reload MCP after config changes
shiva chat
```

See also:
- [MCP (Model Context Protocol)](/user-guide/features/mcp)
- [Use MCP with Shiva](/guides/use-mcp-with-shiva)
...
:::warning
If an MCP server crashes mid-request, Shiva will report a timeout. Check the server's own logs (not just Shiva logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `shiva profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

This isolation is also the reason to never run two agents against the *same* profile or Shiva home: both write memory automatically and each loads the other's writes at session start, so their stored state degrades with every session. One agent per profile; for genuinely shared memory across agents, use an [external memory provider](/user-guide/features/memory-providers).

### What happens when I run `shiva update`?

`shiva update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `shiva update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.shiva/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Shiva can route subagents to a different model automatically. Set this in `~/.shiva/config.yaml`:
...
Now when you tell Shiva "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Shiva do this?

**Current limitation:** Shiva profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Shiva reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `shiva skills config` settings don't seem to take effect.

**Solution:** Use `shiva skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`shiva gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Shiva creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Shiva to another machine
...
1. Install Shiva Agent on the new machine:
   ```bash
   curl -fsSL https://shiva-agent.nousresearch.com/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   shiva backup
   ```
   This creates a zip of your entire `~/.shiva/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/shiva-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/shiva-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   shiva import ~/shiva-backup-<timestamp>.zip
   ```

4. On the new machine, run `shiva setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
shiva profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
shiva profile import ./work-backup.tar.gz work
```
...
### `shiva backup` vs `shiva profile export`

| Feature | `shiva backup` | `shiva profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.shiva` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='shiva-agent' ~/.shiva/ newmachine:~/.shiva/
```

:::tip
`shiva backup` produces a consistent snapshot even while Shiva is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Shiva installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Shiva-specific issue — it's a shell config permissions problem.
...
shiva config show | head -20

# Re-run model selection
shiva model

# Or test with a known-good model
shiva chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/shiva-agent/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Shiva version (`shiva --version`), and the full error message
