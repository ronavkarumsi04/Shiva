...
This page covers all commands related to [Shiva profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `shiva profile` without a subcommand shows help.
...
```bash
shiva profile use <name>
```

Sets `<name>` as the active profile. All subsequent `shiva` commands (without `-p`) will use this profile.
...
shiva profile use default
```

## `shiva profile create`

```bash
shiva profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `shiva profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `shiva update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.shiva`), use `shiva skills opt-out` / `shiva skills opt-in`. |
...
shiva profile create work2 --clone-from work

# Clone everything from a specific profile
shiva profile create work2-backup --clone-from work --clone-all
```

## `shiva profile describe`

```bash
shiva profile describe [<name>] [options]
```
...
shiva profile describe researcher

# Set it explicitly
shiva profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
shiva profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
shiva profile describe --all --auto
```

## `shiva profile delete`

```bash
shiva profile delete <name> [options]
```
...
```bash
shiva profile delete mybot
shiva profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.shiva`) cannot be deleted — use `shiva uninstall` to remove everything.
:::

## `shiva profile show`

```bash
shiva profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Shiva home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ shiva profile show work
Profile: work
Path:    ~/.shiva/profiles/work
...
```bash
shiva profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Shiva installation.
...
```bash
shiva profile alias work
# Creates/updates ~/.local/bin/work

shiva profile alias work --name mywork
# Creates ~/.local/bin/mywork

shiva profile alias work --remove
# Removes the wrapper script
```

## `shiva profile rename`

```bash
shiva profile rename <old-name> <new-name>
```
...
```bash
shiva profile rename mybot assistant
# ~/.shiva/profiles/mybot → ~/.shiva/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `shiva profile export`

```bash
shiva profile export <name> [options]
```
...
```bash
shiva profile export work
# Creates work.tar.gz in the current directory

shiva profile export work -o ./work-2026-03-29.tar.gz
```
...
## `shiva profile import`

```bash
shiva profile import <archive> [options]
```
...
```bash
shiva profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

shiva profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `shiva -p telemetry`). |
...
shiva profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
shiva profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
shiva profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
shiva profile install ./telemetry/
```

### `shiva profile update`

```bash
shiva profile update <name> [--force-config] [--yes]
```
...
```bash
shiva profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Shiva version, author, env var requirements, the source URL/path, and
...
`shiva profile list` also shows the distribution name and version in a
`Distribution` column, and `shiva profile show <name>` / `delete <name>`
...
shiva profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
shiva profile install https://github.com/your-org/internal-assistant.git
```
...
`shiva_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Shiva version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Shiva can clone from.
3. Tell recipients to run `shiva profile install <your-repo-url>`.
...
## `shiva -p` / `shiva --profile`

```bash
shiva -p <name> <command> [options]
shiva --profile <name> <command> [options]
```

Global flag to run any Shiva command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
shiva -p work chat -q "Check the server status"
shiva --profile dev gateway start
shiva -p personal skills list
shiva -p work config edit
```

## `shiva completion`

```bash
shiva completion <shell>
```
...
- `shiva profile <TAB>` — subcommands (list, use, create, etc.)
- `shiva profile use <TAB>` — profile names
- `shiva -p <TAB>` — profile names
