...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/shiva-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    shiva_meta = (fm.get("metadata") or {}).get("shiva") or {}
    tags = shiva_meta.get("tags") or []
    related = shiva_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `shiva skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Shiva Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Shiva ships with a large built-in skill library copied into `~/.shiva/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Shiva also syncs bundled skills on `shiva update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.shiva/skills/` tree, it is still shipped with Shiva; restore it with `shiva skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with shiva-agent — install via shiva skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with shiva-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "shiva skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "shiva skills install official/blockchain/solana",
        "shiva skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "shiva skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
