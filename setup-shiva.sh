...
# 5. Symlinks the 'shiva' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Shiva Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/shiva-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink shiva into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up shiva command..."

SHIVA_BIN="$SCRIPT_DIR/venv/bin/shiva"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SHIVA_BIN" "$COMMAND_LINK_DIR/shiva"
echo -e "${GREEN}✓${NC} Symlinked shiva → $COMMAND_LINK_DISPLAY_DIR/shiva"
...
# ============================================================================
# Seed bundled skills into ~/.shiva/skills/
# ============================================================================

SHIVA_SKILLS_DIR="${SHIVA_HOME:-$HOME/.shiva}/skills"
mkdir -p "$SHIVA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.shiva/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SHIVA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  shiva status        # Check configuration"
if is_termux; then
    echo "  shiva gateway       # Run gateway in foreground"
else
    echo "  shiva gateway install # Install gateway service (messaging + cron)"
fi
echo "  shiva cron list     # View scheduled jobs"
echo "  shiva doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m shiva_cli.main setup
fi
