"""Shared constants for Shiva Agent.
...
_UNSET = object()
_SHIVA_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_SHIVA_HOME_OVERRIDE", default=_UNSET
)
...
def set_shiva_home_override(path: str | Path | None) -> Token:
    """Set a context-local Shiva home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _SHIVA_HOME_OVERRIDE.set(value)


def reset_shiva_home_override(token: Token) -> None:
    """Restore the previous context-local Shiva home override."""
    _SHIVA_HOME_OVERRIDE.reset(token)


def get_shiva_home_override() -> str | None:
    """Return the active context-local Shiva home override, if any."""
    override = _SHIVA_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_shiva_home() -> Path:
    """Return the platform-native default Shiva home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "shiva"
    return Path.home() / ".shiva"


def _shiva_home_from_env() -> Path:
    """Resolve SHIVA_HOME from the process environment only.

    Reads the ``SHIVA_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_shiva_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_shiva_home`
    and :func:`get_process_shiva_home` so the two never drift.
    """
    val = os.environ.get("SHIVA_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_shiva_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``SHIVA_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_shiva_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[SHIVA_HOME fallback] SHIVA_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass SHIVA_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_shiva_home() -> Path:
    """Return the Shiva home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_shiva_home_override`) → ``SHIVA_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``SHIVA_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``SHIVA_HOME`` explicitly (see the systemd
    template in ``shiva_cli/gateway.py`` and the kanban dispatcher in
    ``shiva_cli/kanban_db.py``).  See https://github.com/NousResearch/shiva-agent/issues/18594.
    """
    override = get_shiva_home_override()
    if override:
        return Path(override)

    if not os.environ.get("SHIVA_HOME", "").strip():
        _warn_profile_fallback_once()

    return _shiva_home_from_env()


def shiva_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Shiva home/profile directory.
    ...
    """
    candidate = Path(path) if path is not None else get_shiva_home()
    resolved = candidate.expanduser().resolve(strict=False)
    return os.path.normcase(str(resolved))


def get_process_shiva_home() -> Path:
    """Return the Shiva home for the running process, ignoring task overrides.

    Unlike :func:`get_shiva_home`, this never follows the context-local
    override set by :func:`set_shiva_home_override`.  It resolves only the
    process ``SHIVA_HOME`` env var (falling back to the platform default),
...
# Process-level memo for get_default_shiva_root(). The function resolves
# SHIVA_HOME against the native home on every call (~80us of path
...
# Its result depends only on (SHIVA_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates SHIVA_HOME mid-process.
_default_shiva_root_memo: "tuple[str, str, Path] | None" = None


def get_default_shiva_root() -> Path:
    """Return the root Shiva directory for profile-level operations.

    In standard deployments this is the platform-native Shiva home
    (``~/.shiva`` on POSIX, ``%LOCALAPPDATA%\\shiva`` on native Windows).

    In Docker or custom deployments where ``SHIVA_HOME`` points outside
    ``~/.shiva`` (e.g. ``/opt/data``), returns ``SHIVA_HOME`` directly
    — that IS the root.

    In profile mode where ``SHIVA_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.shiva/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    global _default_shiva_root_memo
    native_home = _get_platform_default_shiva_home()
    env_home = os.environ.get("SHIVA_HOME", "")
    if _default_shiva_root_memo is not None:
        memo_native, memo_env, memo_result = _default_shiva_root_memo
    ...
    if not env_home:
        ...
        try:
            ...
            # SHIVA_HOME is under ~/.shiva (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — SHIVA_HOME itself is the root
                result = env_path
    _default_shiva_root_memo = (str(native_home), env_home, result)
    return result
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``SHIVA_OPTIONAL_SKILLS``.
    """
    override = os.getenv("SHIVA_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_shiva_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``SHIVA_OPTIONAL_MCPS``.
    """
    override = os.getenv("SHIVA_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_shiva_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``SHIVA_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<SHIVA_HOME>/skills`` last-resort
    """
    override = os.getenv("SHIVA_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_shiva_dir(
    ...
    *,
    ...
    """Resolve a Shiva subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to SHIVA_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to SHIVA_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Shiva home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local SHIVA_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_shiva_home()
...
def iter_shiva_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Shiva-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\shiva\\node``. POSIX installs use
    ``$SHIVA_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_shiva_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with shivaManagedNodePathEntries() in
...
_SHIVA_NODE_TARGET_MAJOR = int(os.environ.get("SHIVA_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/shiva_constants.py).
# Used by secure_parent_dir() to skip chmod on the install dir — chmodding it
# 0700 breaks shiva-user traversal in Docker (UID 10000). See #25821, #93050.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Shiva-managed Node trees live under ``$SHIVA_HOME/node`` (or a profile's
    ``SHIVA_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_shiva_node_executable``
    used to trust file presence alone, so ``shiva update`` would pick that
...
def shiva_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Shiva-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_shiva_node_dirs(home):
        for name in names:
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Shiva-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `shiva update` afterwards).",
        flush=True,
    )


def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%SHIVA_HOME%\\node`` on Windows.
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the shiva.exe quarantine);
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_SHIVA_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_SHIVA_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$SHIVA_HOME/node`` on POSIX.
    ...
    ``SHIVA_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_shiva_managed_node() -> str | None:
    """Install a Shiva-managed Node tree and return its npm path.
    ...
    Shiva never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$SHIVA_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_shiva_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_shiva_node_dirs():
...
def heal_shiva_managed_node() -> bool:
    """Redownload Shiva-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not shiva_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SHIVA_HOME": str(get_shiva_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_shiva_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_SHIVA_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_shiva_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _SHIVA_NODE_TARGET_MAJOR
    return False


def find_shiva_node_executable(command: str) -> str | None:
    """Return a Shiva-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_SHIVA_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_shiva_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Shiva-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Shiva-managed installs.

    This is for Shiva-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_shiva_node_executable(command)
    if managed:
        return managed
    if shiva_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_shiva_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Shiva-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_shiva_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``shiva update`` — leaving a **dangling symlink** that ``which`` still
...
def display_shiva_home() -> str:
    """Return a user-friendly display string for the current SHIVA_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.shiva``
        profile:  ``~/.shiva/profiles/coder``
        custom:   ``/opt/shiva-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.shiva``.  For code that needs a real ``Path``, use
    :func:`get_shiva_home` instead.
    """
    home = get_shiva_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``SHIVA_HOME`` or other path
    env vars resolve to an unexpected location.

    Also refuses to chmod the shiva-agent install tree (the directory this
    ...
    See https://github.com/NousResearch/shiva-agent/issues/25821 and
    https://github.com/NousResearch/shiva-agent/pull/93050.
    """
    parent = path.parent.resolve()
    # Refuse root and its direct children (/usr, /home, /var, /tmp, …).
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree root. chmodding it 0700 breaks shiva-user
    ...
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        # A credential file inside the install tree usually means SHIVA_HOME
        ...
        logging.getLogger(__name__).warning(
            ...
            "shiva-agent install directory (%s). Credential files are "
            "normally stored under the shiva home directory instead.",
            parent,
            _INSTALL_ROOT,
        )
        return
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{SHIVA_HOME}/home`` when the profile-home directory exists."""
    shiva_home = get_shiva_home_override() or (env or {}).get("SHIVA_HOME") or os.getenv("SHIVA_HOME")
    if not shiva_home:
        return None
    profile_home = os.path.join(shiva_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("SHIVA_REAL_HOME") or os.getenv("SHIVA_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Shiva profile HOME.

    ``SHIVA_HOME`` scopes Shiva state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={SHIVA_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{SHIVA_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{SHIVA_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Shiva' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["SHIVA_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Shiva itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/shiva-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under SHIVA_HOME.

    Replaces the ``get_shiva_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, shiva_logging.py, shiva_time.py, etc.).
    """
    return get_shiva_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under SHIVA_HOME."""
    return get_shiva_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under SHIVA_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_shiva_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._shiva_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``shiva_cli`` modules using three different Windows
    ...
    A few sites outside ``shiva_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``shiva_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Shiva itself. An ImportError
...
# `shiva_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Shiva.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("shiva_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `shiva update` is exactly
    ...
    return [
        ...
        "    shiva update",
        "If that also fails, reinstall: https://shiva-agent.nousresearch.com",
    ]
