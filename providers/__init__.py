...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with shiva-agent)
2. User plugins: ``$SHIVA_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``shiva_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$SHIVA_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$SHIVA_HOME/plugins/model-providers/`` if it exists."""
    try:
        from shiva_constants import get_shiva_home

        d = get_shiva_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple SHIVA_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_shiva_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``shiva_agent.plugins``
    ...
    * **Provider targets only.** The ``shiva_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$SHIVA_HOME``) keep their
    ...
    try:
        from shiva_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$SHIVA_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``shiva_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$SHIVA_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with shiva-agent.
    ...
    # 2. User plugins — under $SHIVA_HOME/plugins/model-providers/<name>/.
