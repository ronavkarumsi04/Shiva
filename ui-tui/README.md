...
React + Ink terminal UI for Shiva. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SHIVA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `shiva setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.shiva/.shiva_history` or under `SHIVA_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `shiva setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/shiva-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external shiva setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.shiva/tui-parent.log
    ...
    types/
      shiva-ink.d.ts            type declarations for @shiva/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ShivaCLI subprocess for slash commands
```
