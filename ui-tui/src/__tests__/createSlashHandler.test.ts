...
// DASHBOARD_TUI_MODE resolves once at module load from SHIVA_TUI_DASHBOARD,
...
describe('createSlashHandler', () => {
  ...
  it('routes /status to live session.status instead of slash worker', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ output: 'Shiva TUI Status' }))
    ...
    expect(ctx.gateway.gw.request).not.toHaveBeenCalled()
    await vi.waitFor(() => {
      expect(ctx.transcript.page).toHaveBeenCalledWith('Shiva TUI Status', 'Status')
    })
  })
  ...
  it('falls through to command.dispatch for skill commands, sending the body but showing the invocation', async () => {
    const skillMessage =
      '[IMPORTANT: The user has invoked the "shiva-agent-dev" skill, indicating they want you to follow its instructions.\n' +
          ...
          request: vi.fn((method: string) => {
            ...
            if (method === 'command.dispatch') {
              return Promise.resolve({
                ...
                name: 'shiva-agent-dev',
                display: '/shiva-agent-dev'
              })
            }
    ...
    const h = createSlashHandler(ctx)
    expect(h('/shiva-agent-dev')).toBe(true)
    await vi.waitFor(() => {
      expect(ctx.transcript.send).toHaveBeenCalledWith(skillMessage, true, '/shiva-agent-dev')
    })
  ...
  it('/save forwards to session.save RPC and reports the returned file', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ file: '/tmp/shiva_conversation_test.json' }))
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('conversation saved to: /tmp/shiva_conversation_test.json')
    })
  })
