...
const logDir = join(process.env.SHIVA_HOME?.trim() || join(homedir(), '.shiva'), 'logs')
...
// a real ~/.shiva (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('shiva-tui: parent lifecycle log unavailable\n')
    }
  }
}
