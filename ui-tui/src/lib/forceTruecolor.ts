...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SHIVA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Shiva on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SHIVA_TUI_TRUECOLOR=1.
