...
import { Box, Text } from '@shiva/ink'
...
/**
 * User widget apps — Shiva authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$SHIVA_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.shiva/plugins/`: files under SHIVA_HOME execute
...
const widgetsDir = () => join(process.env.SHIVA_HOME?.trim() || join(homedir(), '.shiva'), 'tui-widgets')
 ...
 *  every change, so a widget Shiva writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Shiva) ever writes must hot-load too — a 10s poll here read
