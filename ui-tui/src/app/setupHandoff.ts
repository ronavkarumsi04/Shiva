import type { RunExternalProcess } from '@shiva/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`shiva ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching shiva: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`shiva ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
