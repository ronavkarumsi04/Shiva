...
import { terminalBackgroundHex } from '@shiva/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $SHIVA_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see SHIVA_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['SHIVA_TUI_BACKGROUND', process.env.SHIVA_TUI_BACKGROUND ?? '(unset)'],
            ['SHIVA_TUI_THEME', process.env.SHIVA_TUI_THEME ?? '(unset)'],
