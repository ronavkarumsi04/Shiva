...
Edit with `shiva config edit` or `shiva config set section.key value`.
Full reference: https://shiva-agent.nousresearch.com/docs/user-guide/configuration
...
`shiva config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `shiva tools` (interactive) or `shiva tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_SHIVA_CORE_TOOLS` is the default bundle most platforms inherit).
