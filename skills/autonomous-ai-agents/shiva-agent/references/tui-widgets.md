...
Author widget apps for the Shiva TUI (`shiva --tui`): glanceable ambient
...
- The TUI must be in use (`shiva --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.shiva/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.shiva/logs/tui_gateway_crash.log` if a widget never appears.
