...
For occasional contributors and PR authors. Full developer docs: https://shiva-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.shiva/config.yaml` (settings), `~/.shiva/.env` (API keys) — both under `$SHIVA_HOME` when it is set.
...
`_SHIVA_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_shiva_home()` for paths,
never hardcode `~/.shiva`. For custom/local-only tools, write a plugin in
`~/.shiva/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `shiva_cli/commands.py`
...
- Tests auto-redirect `SHIVA_HOME` to temp dirs — never touch real `~/.shiva/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/shiva_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_shiva_home()` from `shiva_constants` for all paths (profile-safe)
