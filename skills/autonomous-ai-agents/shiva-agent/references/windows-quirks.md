...
Shiva runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`shiva config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Shiva's env scrubber dropping
...
Shiva-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Shiva tool and most Windows APIs. Prefer forward slashes in code
