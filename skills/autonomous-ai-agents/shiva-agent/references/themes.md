# Themes / Skins — Author a Shiva Color Theme

Author a Shiva **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`shiva_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.shiva/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Shiva itself.
...
- Write access to the Shiva home dir — `~/.shiva` by default, or `$SHIVA_HOME`
  / the active profile's dir. Skins live in `<shiva-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `shiva config set`).
...
3. `write_file` it to `<shiva-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `shiva config set display.skin default` (or they can `/skin default`).
...
```
shiva skin set <key> <hex>      # e.g. shiva skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `shiva skin set background "#08201f"` changes
only the background; `shiva skin use <name>` / `shiva skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.shiva`** when a profile is active — resolve the real home
  from `$SHIVA_HOME` first, falling back to `~/.shiva`.
...
- **Never hand-edit `config.yaml` to activate.** Use `shiva config set
...
- **You apply it, not the user.** `shiva config set display.skin <name>` is
...
- `read_file` the written `<shiva-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `shiva config get display.skin` and confirm it reports `<name>`.
