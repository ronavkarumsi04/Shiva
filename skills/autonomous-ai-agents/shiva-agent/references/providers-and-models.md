...
Set via `shiva model` (picker) or `shiva setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://shiva-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `shiva auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `shiva auth add openai-codex` |
| qwen-oauth | OAuth | `shiva auth add qwen-oauth` |
| minimax-oauth | OAuth | `shiva auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`shiva auth`).
Fallback chain when the primary fails: `shiva fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`shiva_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   shiva config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
