...
## Layer 1 — Is this thing a Shiva plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Shiva process**. Its LLM calls go through Shiva's already-configured provider. | Already uses Portal if user's Shiva is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Shiva. That LLM call is already authenticated
via Shiva's provider config. The plugin's own server auth (e.g.
...
What Shiva Agent has that *feels* like OAuth — `shiva login --provider nous`
opening a browser, user signs in, token lands in `~/.shiva/auth.json` — is a
**Shiva-specific browser flow**. Under the hood it produces a credential
Shiva uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Shiva's existing Portal credential out of `~/.shiva/auth.json`.
...
rides on the credential Shiva already holds.

Where this could live in Shiva:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Shiva itself.
...
1. First decide: Shiva plugin (runs inside Shiva) or separate app? If plugin,
   it already uses Portal via Shiva's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Shiva agent itself") is fully supported. Shiva treats agent
...
pitch in the Tool Gateway announcement is specifically about Shiva Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Shiva is configured to use Portal as its provider. It is
