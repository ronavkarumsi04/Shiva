...
Write plugins for the Shiva desktop app: statusbar items, layout panes,
...
system (`~/.shiva/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$SHIVA_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$SHIVA_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Shiva desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$SHIVA_HOME/desktop-plugins/` (usually
  `~/.shiva/desktop-plugins/`).

## How to Run

1. Create `$SHIVA_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.shiva/...` by default, or `~/.shiva/profiles/<profile>/...` under a
...
The ONLY import surface is `@shiva/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Shiva
  ...
  (`shiva://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.shiva/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@shiva/plugin-sdk`, `react`, and
