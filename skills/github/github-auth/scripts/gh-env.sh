#!/usr/bin/env bash
# GitHub environment detection helper for Shiva Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _shiva_env="${SHIVA_HOME:-$HOME/.shiva}/.env"; [ -f "$_shiva_env" ] && grep -q "^GITHUB_TOKEN=" "$_shiva_env" 2>/dev/null; then
