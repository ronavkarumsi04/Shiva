#!/usr/bin/env python3
"""Google Workspace API CLI for Shiva Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Shiva-facing JSON contract and falls back to the Python client
...
from _shiva_home import get_shiva_home
...
def _gws_binary() -> str | None:
    override = os.getenv("SHIVA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Shiva Agent")
