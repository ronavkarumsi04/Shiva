...
## 5. Shiva Configuration

### Config location

`$SHIVA_HOME/config.yaml` (defaults to `~/.shiva/config.yaml` when `SHIVA_HOME` is unset)
...
Restart the Shiva session for changes to take effect. The MCP connection is
