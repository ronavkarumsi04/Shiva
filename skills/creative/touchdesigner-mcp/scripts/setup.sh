...
SHIVA_HOME_DIR="${SHIVA_HOME:-$HOME/.shiva}"
SHIVA_CFG="${SHIVA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Shiva config has twozero_td MCP entry ──
if [[ ! -f "$SHIVA_CFG" ]]; then
    echo -e " ${FAIL} Shiva config not found at ${SHIVA_CFG}"
    manual_steps+=("Create ${SHIVA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$SHIVA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Shiva config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Shiva config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${SHIVA_CFG} manually"); }
    manual_steps+=("Restart Shiva session to pick up config change")
fi
