...
Write path is the CLI (``shiva cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_shiva_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`shiva cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `shiva cron notepad {job_id} delete "
