...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``shiva://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``shiva://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"shiva://blueprint/{quote(blueprint.key)}{qs}"
