...
Uses a file-based lock (~/.shiva/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `shiva update` reloads
# the module) fail with ModuleNotFoundError for shiva_time et al.
...
from shiva_time import now as _shiva_now
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `shiva fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `shiva cron pause {job_ref}` "
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running Shiva, pin it explicitly: "
                f"`shiva cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
    ...
    # checkout was updated underneath it (interrupted `shiva update`, manual
    ...
    if provider_reachable and re.search(
        ...
        if skew is not None:
            ...
            message += (
                ...
                "`shiva gateway restart` to fix it."
            )
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy shiva_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from shiva_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``shiva tools`` config for the ``cron`` platform.
    ...
    try:
        from shiva_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    """
    from shiva_constants import parse_reasoning_effort, resolve_reasoning_config
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``SHIVA_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("SHIVA_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid SHIVA_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    entry = {
        ...
        "at": _shiva_now().isoformat(),
    }
    with _running_lock:
        _forced_releases.append(entry)
        del _forced_releases[:-_FORCED_RELEASE_HISTORY]
    try:
        path = _get_shiva_home() / "cron" / "inflight_forced_releases.jsonl"
    ...
    *,
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_shiva_home())
...
# (SHIVA_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse SHIVA_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("SHIVA_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid SHIVA_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Resolves through _get_shiva_home() so profile-scoped paths work correctly.
def _usage_audit_path() -> Path:
    return _get_shiva_home() / "cron" / "usage_audit.jsonl"
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.shiva/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``shiva update`` / ``shiva gateway stop`` / systemd restart, or an
...
_shiva_home: Path | None = None


def _get_shiva_home() -> Path:
    """Resolve Shiva home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active SHIVA_HOME at call time
    ...
    """
    return _shiva_home or get_shiva_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    shiva_home = _get_shiva_home()
    lock_dir = shiva_home / "cron"
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from shiva_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    Runs ``shiva [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing
    ...
    scheduler's SHIVA_HOME) or a validated local profile name.  Returns None
    ...
    if shiva_bin:
        ...
        try:
            ...
            if _ilu.find_spec("shiva_cli") is not None:
                argv = [sys.executable, "-m", "shiva_cli.main"]
            else:
                return "bot-chat delivery failed: shiva CLI not resolvable"
        except Exception:
            return "bot-chat delivery failed: shiva CLI not resolvable"
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution in the child; a leftover SHIVA_HOME
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="shiva-cron-botchat-",
            delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _resolve_bot_chat_target(job: dict, profile_arg: str) -> Optional[dict]:
    ...
    ``profile_arg`` is ``""`` for the job's own profile (the SHIVA_HOME
    ...
    ``~/.shiva/profiles/`` tree, so same-named profiles on other gateways
    can never be targeted by accident.
    """
    if not profile_arg:
        # Own profile: chat subprocess inherits SHIVA_HOME, no name needed.
        return {"platform": BOT_CHAT_PLATFORM, "chat_id": "", "thread_id": None}
    try:
        from shiva_cli.profiles import normalize_profile_name, profile_exists
    ...
    for media_path, _is_voice in media_files:
        try:
            ...
            try:
                ...
                # SHIVA_CRON_MEDIA_SEND_TIMEOUT env override).
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    # at boot; a standalone process (manual `shiva cron run` from the CLI,
    ...
    for target in targets:
        ...
        if transport is not None:
            ...
            # (`shiva cron run`) has no live relay adapter, so surface the
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("SHIVA_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid SHIVA_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _get_media_send_timeout() -> int:
    ...
    SHIVA_CRON_MEDIA_SEND_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("SHIVA_CRON_MEDIA_SEND_TIMEOUT", "").strip()
    if env_value:
        try:
            timeout = int(float(env_value))
            if timeout > 0:
            ...
            logger.warning(
                "Invalid SHIVA_CRON_MEDIA_SEND_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _get_session_db_timeout() -> float:
    ...
    Mirrors the ``script_timeout_seconds`` resolution pattern: the
    SHIVA_CRON_SESSION_DB_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("SHIVA_CRON_SESSION_DB_TIMEOUT", "").strip()
    if env_value:
        try:
            return float(env_value)
        except (ValueError, TypeError):
            logger.warning(
                "Invalid SHIVA_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _run_job_script(
    ...
    Scripts must reside within SHIVA_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Shiva-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against SHIVA_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_shiva_home() / "scripts"
    ...
    # escape — scripts MUST reside within SHIVA_HOME/scripts/.
    try:
...
def _block_and_pause_job(
    ...
    now_iso = _shiva_now().strftime("%Y-%m-%d %H:%M:%S")
    doc = (
        f"# Cron Job: {job_name}\n\n"
        f"**Job ID:** {job_id}\n"
        f"**Run Time:** {now_iso}\n"
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("SHIVA_MODEL") or ""
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `shiva setup`), or pin a "
            "working provider via `shiva cron edit "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`shiva setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    if job_has_monitor(job):
        ...
        _mon_now = _shiva_now().strftime("%Y-%m-%d %H:%M:%S")
        if not _mon.ok:
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_shiva_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    _cron_session_id = f"cron_{job_id}_{_shiva_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # gateway message. Do not seed SHIVA_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads SHIVA_SESSION_PLATFORM
    #     and SHIVA_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via SHIVA_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     SHIVA_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read SHIVA_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the SHIVA_CRON_AUTO_DELIVER_* vars set
    # below, so clearing SHIVA_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # SHIVA_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ SHIVA_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "SHIVA_CRON_AUTO_DELIVER_PLATFORM",
        "SHIVA_CRON_AUTO_DELIVER_CHAT_ID",
        "SHIVA_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["SHIVA_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `shiva chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with SHIVA_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $SHIVA_KANBAN_TASK -- letting
        ...
        # load_shiva_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this SHIVA_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_shiva_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from shiva_cli.env_loader import (
            load_shiva_dotenv,
        ...
        if delivery_target:
            _VAR_MAP["SHIVA_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["SHIVA_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["SHIVA_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > SHIVA_MODEL env > config.yaml ``model:``
        # (string or ``{default: ...}``). The per-job value is intentionally
        # re-read from storage every tick so a ``shiva cron edit --model``
        ...
        try:
            ...
            _cfg_path = str(_get_shiva_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"SHIVA_MODEL={os.getenv('SHIVA_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`shiva cron edit {job_id} --model <name>` or set a "
                "default with `shiva model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from shiva_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("SHIVA_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject SHIVA_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running Shiva "
                        "pin it explicitly: "
                        f"`shiva cron edit {job_id} --provider <provider> "
        ...
        #
        # Bounded with its own timeout (separate from SHIVA_CRON_TIMEOUT,
        ...
        agent = AIAgent(
            ...
            # SHIVA_HOME. When a workdir is configured, also inject project
        ...
        # override via SHIVA_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from shiva_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
**Job ID:** {job_id}
**Run Time:** {_shiva_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_shiva_now().strftime('%b %d %H:%M')}"
            ...
            # shiva_state's session_lifecycle_statuses docstring — keep the
...
def run_one_job(
    ...
    *,
    ...
    profile_home = _get_shiva_home().resolve()
    with _running_lock:
    ...
    *,
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_shiva_home())
        )
        ...
        if delivery_error:
            ...
            # exact signature via `shiva cron incidents ack`.
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`shiva pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `shiva resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `shiva cron run` that claimed a job
        ...
        if not due_jobs:
            ...
            if verbose:
                logger.info("%s - No jobs due", _shiva_now().strftime('%H:%M:%S'))
            try:
        ...
        if verbose:
            logger.info("%s - %s job(s) due", _shiva_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set SHIVA_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("SHIVA_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid SHIVA_CRON_MAX_PARALLEL value; defaulting to unbounded")
