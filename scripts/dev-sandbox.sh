...
unprivileged `shiva` user, giving the layout most people have —
$SHIVA_HOME/shiva-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  SHIVA_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .shiva-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `shiva setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${SHIVA_DEV_SANDBOX_UPSTREAM:-https://github.com/NousResearch/shiva-agent.git}"
...
GIT_ROOT="${SHIVA_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${SHIVA_DEV_SANDBOX_DIR:-.shiva-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t shiva-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t shiva-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/shiva-agent.nousresearch.com"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/shiva-agent.nousresearch.com/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/shiva-agent.nousresearch.com/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://shiva-agent.nousresearch.com/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/shiva-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `shiva` launcher to ~/.local/bin and the
# checkout to $SHIVA_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/shiva-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q shiva-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t shiva-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Shiva sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/shiva-sandbox/next"
