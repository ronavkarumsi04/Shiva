...
# Python-driven tool session (e.g. Shiva terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/shiva-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/shiva-agent.git"
SHIVA_HOME="${SHIVA_HOME:-$HOME/.shiva}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${SHIVA_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$SHIVA_INSTALL_DIR"
...
#   code at /usr/local/lib/shiva-agent, command at /usr/local/bin/shiva,
#   data still at /root/.shiva (SHIVA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$SHIVA_HOME/.no-bundled-skills so future"
            echo "                   'shiva update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Shiva.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.shiva/shiva-agent"
            echo "                   default (root, Linux): /usr/local/lib/shiva-agent"
            echo "  --shiva-home PATH  Data directory (default: ~/.shiva, or \$SHIVA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Shiva installs the code under"
            echo "  /usr/local/lib/shiva-agent and links the command into"
            echo "  /usr/local/bin/shiva (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SHIVA_HOME"
            echo "  (default /root/.shiva).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SHIVA_HOME/shiva-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `shiva update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Shiva-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Shiva Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install shiva command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `shiva` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SHIVA_HOME/shiva-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SHIVA_HOME/shiva-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/shiva-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SHIVA_HOME/shiva-agent — then preserve it)
#
# Always no-op when the user set --dir or $SHIVA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SHIVA_HOME.
    if is_termux; then
        INSTALL_DIR="$SHIVA_HOME/shiva-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SHIVA_HOME/shiva-agent/.git" ]; then
            INSTALL_DIR="$SHIVA_HOME/shiva-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/shiva-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/shiva-agent"
        ...
        # /usr/local/bin/shiva wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/shiva"
        log_info "  Data:    $SHIVA_HOME (unchanged)"
...
# Point a Shiva-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $SHIVA_HOME/node/bin — which is NOT on
...
# Shiva's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Shiva-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$SHIVA_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$SHIVA_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$SHIVA_HOME/node/etc/npmrc"
}

get_shiva_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/shiva" ]; then
        echo "$link_dir/shiva"
    else
        echo "shiva"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://shiva-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Shiva owns its own uv at $SHIVA_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (shiva_cli/managed_uv.py) looks in the same
    # place, so install.sh and `shiva update` stay in sync.
    local _managed_uv="$SHIVA_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $SHIVA_HOME/bin ..."
    mkdir -p "$SHIVA_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/shiva-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/shiva-uv-installer.$$.sh")"
    ...
    # directly into $SHIVA_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$SHIVA_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Shiva-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Shiva-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Shiva-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Shiva-managed Node from a previous run over a too-old system one.
    if [ -x "$SHIVA_HOME/node/bin/node" ] && [ -x "$SHIVA_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$SHIVA_HOME/node/bin/node" --version)"; then
        export PATH="$SHIVA_HOME/node/bin:$PATH"
        log_success "Node.js $("$SHIVA_HOME/node/bin/node" --version) found (Shiva-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Shiva-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Shiva requires Node >=26) — installing Shiva-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.shiva/node/..."
    ...
    # Place into ~/.shiva/node/ and symlink binaries into the same bin dir
    # the shiva command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$SHIVA_HOME/node"
    mkdir -p "$SHIVA_HOME"
    mv "$extracted_dir" "$SHIVA_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$SHIVA_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$SHIVA_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$SHIVA_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$SHIVA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.shiva/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Shiva install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Shiva Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `shiva update` path
                # (#4735).
                ...
                local stash_name
                stash_name="shiva-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``shiva update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Shiva behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up shiva command..."

    if [ "$USE_VENV" = true ]; then
        SHIVA_BIN="$INSTALL_DIR/venv/bin/python"
        SHIVA_ENTRYPOINT="$INSTALL_DIR/shiva"
    else
        SHIVA_BIN="$(which shiva 2>/dev/null || echo "")"
        if [ -z "$SHIVA_BIN" ]; then
            log_warn "shiva not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$SHIVA_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$SHIVA_ENTRYPOINT" ]; }; then
        log_warn "Shiva launcher prerequisites not found"
    ...
    # Create a user-facing shim for the shiva command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $SHIVA_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$SHIVA_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/shiva"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/shiva" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SHIVA_BIN" "$SHIVA_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/shiva" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/shiva"
    log_success "Installed shiva launcher → $command_link_display_dir/shiva"

    # Also expose `shiva-agent`. The `shiva-agent` console script declared in
    ...
    rm -f "$command_link_dir/shiva-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/shiva-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SHIVA_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/shiva-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SHIVA_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/shiva-agent"
    log_success "Installed shiva-agent launcher → $command_link_display_dir/shiva-agent"

    # Also expose `shiva-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `shiva-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Shiva as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/shiva-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/shiva-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SHIVA_BIN" "$SHIVA_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/shiva-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/shiva-acp"
    log_success "Installed shiva-acp launcher → $command_link_display_dir/shiva-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "shiva command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v shiva' \
                >/dev/null 2>&1; then
        ...
        log_info "shiva not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Shiva Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Shiva Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Shiva Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so shiva works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.shiva directory structure (config at top level, code in subdir)
    mkdir -p "$SHIVA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.shiva/.env (top level, easy to find)
    if [ ! -f "$SHIVA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SHIVA_HOME/.env"
            log_success "Created ~/.shiva/.env from template"
        else
            touch "$SHIVA_HOME/.env"
            log_success "Created ~/.shiva/.env"
        fi
    else
        log_info "~/.shiva/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$SHIVA_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.shiva/config.yaml (top level, easy to find)
    if [ ! -f "$SHIVA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SHIVA_HOME/config.yaml"
            log_success "Created ~/.shiva/config.yaml from template"
        fi
    else
        log_info "~/.shiva/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in shiva_cli/default_soul.py — the
    ...
    if [ ! -f "$SHIVA_HOME/SOUL.md" ]; then
        cat > "$SHIVA_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.shiva/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.shiva/"

    # Seed bundled skills into ~/.shiva/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `shiva update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'shiva update'." \
            > "$SHIVA_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $SHIVA_HOME/.no-bundled-skills"
        log_info "  Future 'shiva update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.shiva/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.shiva/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SHIVA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$SHIVA_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.shiva/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Shiva Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Shiva will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Shiva Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `shiva update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `shiva tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$SHIVA_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Shiva' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$SHIVA_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'shiva tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `shiva tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `shiva computer-use install` on their own).
    ...
    # Same upstream installer `shiva computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Shiva install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Shiva'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'shiva tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: shiva computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'shiva setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m shiva_cli.main setup < /dev/tty
    else
        python -m shiva_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$SHIVA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Shiva to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SHIVA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'shiva whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                SHIVA_CMD="$(get_shiva_command_path)"
                $SHIVA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'shiva whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'shiva gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SHIVA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SHIVA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: shiva gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: shiva gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SHIVA_CMD gateway > "$SHIVA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.shiva/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: shiva gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: shiva gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.shiva-bootstrap-complete, which tells the Shiva
    ...
    local marker_path="$INSTALL_DIR/.shiva-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SHIVA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SHIVA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SHIVA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}shiva${NC}              Start chatting"
    echo -e "   ${GREEN}shiva setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}shiva config${NC}       View/edit configuration"
    echo -e "   ${GREEN}shiva config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}shiva gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}shiva update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'shiva' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'shiva' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'shiva' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$SHIVA_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$SHIVA_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$SHIVA_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$SHIVA_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Shiva` rename
...
# _purge_electron_build_cache() used by `shiva desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Shiva-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `shiva desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Shiva" ]; then
            app="$desktop_dir/release/linux-unpacked/Shiva"
        elif [ -x "$desktop_dir/release/linux-unpacked/shiva" ]; then
            app="$desktop_dir/release/linux-unpacked/shiva"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Shiva.app" \
            "$desktop_dir/release/mac/Shiva.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `shiva desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading SHIVA_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if SHIVA_HOME="$SHIVA_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from shiva_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `shiva update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Shiva-managed Node
            # provisioned during prerequisites/node-deps (at $SHIVA_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $SHIVA_HOME.
    # $SHIVA_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'shiva update' on this host install.
