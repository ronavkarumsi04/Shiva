#!/usr/bin/env python3
"""Live test harness for Shiva Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any shiva imports.
ORIGINAL_HOME = os.environ.get("SHIVA_HOME")
ORIGINAL_AUTH = Path.home() / ".shiva" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.shiva/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.shiva/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="shiva_ts_live_"))
    shiva_home = home_dir / ".shiva"
    shiva_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, shiva_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".shiva" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, shiva_home / ".env")
        ...
        load_shiva_dotenv(shiva_home=str(Path.home() / ".shiva"))
    ...
    (shiva_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return shiva_home
...
def reset_module_state():
    """Drop cached modules so the new SHIVA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "shiva_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["SHIVA_HOME"] = str(home)
...
def main():
    ...
    # Restore original SHIVA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SHIVA_HOME"] = ORIGINAL_HOME
    else:
