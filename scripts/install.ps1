# ============================================================================
# Shiva Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$ShivaHome = $(if ($env:SHIVA_HOME) { $env:SHIVA_HOME } else { "$env:LOCALAPPDATA\shiva" }),
    [string]$InstallDir = $(if ($env:SHIVA_HOME) { "$env:SHIVA_HOME\shiva-agent" } else { "$env:LOCALAPPDATA\shiva\shiva-agent" }),
    ...
    # builds apps/desktop into a launchable Shiva.exe.
    #
    # Why opt-in:
    #   * Shiva-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Shiva.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Shiva.exe
    ...
    #     `shiva desktop` already builds on demand.
...
# SHIVA_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[shiva] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'ShivaInstall.LongPath').Type) {
            Add-Type -Namespace 'ShivaInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [ShivaInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -ShivaHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('ShivaHome')) {
    $ShivaHome = ConvertTo-LongPath $ShivaHome
} else {
    $ShivaHome = ConvertTo-LongPath $(
        if ($env:SHIVA_HOME) { $env:SHIVA_HOME } else { "$env:LOCALAPPDATA\shiva" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:SHIVA_HOME) { "$env:SHIVA_HOME\shiva-agent" } else { "$env:LOCALAPPDATA\shiva\shiva-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: ShivaHome=$ShivaHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:NousResearch/shiva-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/shiva-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Shiva Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ShivaHome)) {
        New-Item -ItemType Directory -Force -Path $ShivaHome | Out-Null
    }
    $envFile = Join-Path $ShivaHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `shiva update`'s npx
    ...
    $prefixDir = Join-Path $ShivaHome "node"
...
function Install-Uv {
    # Shiva owns its own uv at $ShivaHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (shiva_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `shiva update` stay in sync.
    $managedUv = Join-Path $ShivaHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ShivaHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ShivaHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ShivaHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ShivaHome "bin"
        ...
        # (shiva_cli/managed_uv.py looks only at $ShivaHome\bin\uv.exe).
...
# Put the Shiva-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone shiva-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Shiva-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Shiva-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("shiva-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and SHIVA_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Shiva-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Shiva setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Shiva uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\shiva\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Shiva needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Shiva copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\shiva\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SHIVA_GIT_BASH_PATH`` (User scope) so Shiva can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Shiva-managed PortableGit install instead..."
    }

    # Download PortableGit into $ShivaHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ShivaHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Shiva features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Shiva needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SHIVA_GIT_BASH_PATH`` (User env scope) so Shiva can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ShivaHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ShivaHome\git\usr\bin\bash.exe
    $candidates += "$ShivaHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ShivaHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SHIVA_GIT_BASH_PATH", $candidate, "User")
            $env:SHIVA_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set SHIVA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Shiva may not find Git Bash."
    Write-Info "If needed, set SHIVA_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Shiva requires Node >=26)"
    }

    # Prefer a Shiva-managed Node from a previous run over a too-old system one.
    $managedNode = "$ShivaHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$ShivaHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$ShivaHome\node"
        Write-Success "Node.js $version found (Shiva-managed)"
        ...
        Update-ManagedNpm "$ShivaHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Shiva-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ShivaHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ShivaHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\shiva-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$ShivaHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$ShivaHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$ShivaHome\node") {
                    try {
                        Rename-Item "$ShivaHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Shiva-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$ShivaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ShivaHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\shiva-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `shiva
                    # update` path (#4735).
                    ...
                    $stashName = "shiva-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. shiva-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``shiva update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Shiva behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running shiva processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/shiva-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/shiva-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/shiva-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\shiva-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\shiva-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `shiva update` (see the notes at the shared clone-path
    ...
    # next `shiva update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Shiva-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running shiva process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running shiva processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Shiva_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (shiva.exe) plus its child tree.
            & taskkill /F /T /IM shiva.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM shiva.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m shiva_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not shiva.exe.
            ...
            # Shiva children re-exec through .shiva-runtime, so killing only
            ...
            # user (see shiva_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Shiva " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `shiva update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Shiva (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `shiva.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install shiva-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and shiva.exe broken with
    ...
    # here, not 30 seconds later when the user runs `shiva`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Shiva processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Shiva processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Shiva and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register shiva.exe in dist-info/RECORD but fail to
        ...
        # `shiva` command while shiva-agent.exe / shiva-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m shiva_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `shiva dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\shiva_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `shiva dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `shiva dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: shiva_cli/web_server.py"
        }
    }
...
function Install-ShivaCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the shiva launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring shiva.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "shiva.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the shiva command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # shiva_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("shiva", "shiva-acp")) {
    ...
    $requiredExe = Join-Path $Destination "shiva.exe"
    $requiredCmd = Join-Path $Destination "shiva.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the shiva command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up shiva command..."
    
    if ($NoVenv) {
        $shivaBin = "$InstallDir"
    } else {
        # $ShivaHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `shiva update`'s autostash
        ...
        # installer staged under shiva-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-ShivaCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $shivaBin = "$ShivaHome\bin"
        Install-ShivaCommandLaunchers -Root $InstallDir -Destination $shivaBin | Out-Null
    }
    ...
    #   shiva-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The shiva-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept shiva via $shivaBin)"
        }
    }
    
    if ($currentPath -notlike "*$shivaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$shivaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $shivaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SHIVA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\shiva instead
    # of the Unix default ~/.shiva
    $currentShivaHome = [Environment]::GetEnvironmentVariable("SHIVA_HOME", "User")
    if (-not $currentShivaHome -or $currentShivaHome -ne $ShivaHome) {
        [Environment]::SetEnvironmentVariable("SHIVA_HOME", $ShivaHome, "User")
        Write-Success "Set SHIVA_HOME=$ShivaHome"
    }
    $env:SHIVA_HOME = $ShivaHome
    
    # Update current session
    $env:Path = "$shivaBin;$env:Path"
    
    Write-Success "shiva command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.shiva-bootstrap-complete which tells the Shiva
    ...
    $markerPath = Join-Path $InstallDir ".shiva-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the SHIVA_HOME directory structure ($ShivaHome, default %LOCALAPPDATA%\shiva)
    New-Item -ItemType Directory -Force -Path "$ShivaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShivaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShivaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShivaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShivaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShivaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShivaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShivaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShivaHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ShivaHome\.env"
    ...
    # Create config.yaml
    $configPath = "$ShivaHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Shiva's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$ShivaHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in shiva_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $ShivaHome"
    
    # Seed bundled skills into $ShivaHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $ShivaHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $ShivaHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ShivaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $ShivaHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Shiva-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'shiva setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\shiva-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\shiva-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\shiva-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `shiva tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $ShivaHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Shiva' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Shiva' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'shiva tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `shiva
...
# paths still lazy-install via install_cua_driver() (shiva_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `shiva computer-use install` runs. Bounded
        ...
        # Shiva' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'shiva tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: shiva computer-use install"
            }
        } else {
...
# the final `electron` -> `Shiva` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Shiva.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_shiva_desktop command
    # resolves apps/desktop/release/win-unpacked/Shiva.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the Shiva-managed
    ...
    # The --dir mode produces an unpacked Shiva.exe in
    ...
    # are NOT trying to work around). The Shiva icon + product name are
    # stamped onto Shiva.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\shiva-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Shiva.exe",
        "$desktopDir\release\win-arm64-unpacked\Shiva.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Shiva.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Shiva icon + identity are stamped onto Shiva.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, shiva-agent#38216).
    ...
    #    Shiva.exe. We deliberately do NOT point them at `shiva desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Shiva.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Shiva.lnk')
        )
        ...
        # with the Shiva icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.shiva/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'shiva setup'."
        return
    }
    ...
    # Run shiva setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m shiva_cli.main setup
    } else {
        python -m shiva_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $shivaCmd = "$InstallDir\venv\Scripts\shiva.exe"
    if (-not (Test-Path $shivaCmd)) {
        $shivaCmd = "shiva"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ShivaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'shiva whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: shiva gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ShivaHome\logs\gateway.log"
            Start-Process -FilePath $shivaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ShivaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: shiva gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: shiva gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ShivaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ShivaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ShivaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ShivaHome\shiva-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   shiva              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   shiva setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   shiva config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   shiva config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   shiva gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   shiva update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Shiva repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Shiva-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Shiva to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://shiva-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
