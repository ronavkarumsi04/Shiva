...
# (%SHIVA_HOME%\shiva-setup.exe). That binary has no self-update path --
# copy_self_to_shiva_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `shiva update` refreshes
...
#     -InstallRoot <path>   repo checkout (SHIVA_HOME\shiva-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Shiva.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .shiva-update-in-progress in place (tests)
#
...
# .shiva-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim SHIVA_HOME\.shiva-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining SHIVA_UPDATE_STARTED_AT from the Desktop hand-off.
# shiva_cli/update_lock.py's ancestry rule lets our
# `shiva update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Shiva.exe pid.
try {
    Add-Type -Namespace ShivaHandoff -Name Win32 -MemberDefinition @'
...
$ShivaHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $ShivaHome ".shiva-update-in-progress"
$LogDir = Join-Path $ShivaHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $ShivaHome ".shiva-update-result.json"
$script:Ui = $null
$script:UiStage = "Shiva will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale shiva-update-ui-* leftovers from interrupted
    ...
    # update marker (.shiva-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "shiva-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-ShivaStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("shiva-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [ShivaHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `shiva debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"shiva debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [ShivaHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Shiva.exe before writing the replacement,
    ...
    # DO NOT spawn Shiva.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [ShivaHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [ShivaHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [ShivaHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Shiva process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [ShivaHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `shiva update` deliberately runs its build steps with stdout
# inherited (shiva_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:SHIVA_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:SHIVA_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:SHIVA_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:SHIVA_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `shiva update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (shiva_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:SHIVA_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:SHIVA_UPDATE_PROGRESS_LOG
}
...
public static class ShivaUpdateJob {
...
function Invoke-ShivaStep([string]$Exe, [string[]]$ShivaArgs, [string]$Tag) {
    ...
    # afterwards, where `shiva debug share` picks it up.
    #
    ...
    # .shiva-update-result.json, clearing .shiva-update-in-progress,
    ...
    # Shiva" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($ShivaArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [ShivaUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `shiva update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [ShivaUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
# the page can be QA'd in any browser; SHIVA_SELFTEST_FAIL=1 exercises the
# error state, SHIVA_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:SHIVA_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:SHIVA_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:SHIVA_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-ShivaStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Shiva install to
...
#            update log, the shape of every real `shiva update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:SHIVA_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:SHIVA_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:SHIVA_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:SHIVA_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "shiva-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "shiva-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "shiva-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "shiva-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "shiva-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "shiva-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "shiva-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "shiva-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `shiva update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-ShivaStep $powershell @(
    ...
    $flood = Invoke-ShivaStep $powershell @(
    ...
    $stall = Invoke-ShivaStep $powershell @(
    ...
    try {
        $logstall = Invoke-ShivaStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-ShivaStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-ShivaStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:SHIVA_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for Shiva to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Shiva window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Shiva fully and try again."
    ...
    Publish-UiProgress "Preparing Shiva files"
    $shim = Join-Path $InstallRoot "venv\Scripts\shiva.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Shiva install open (venv\Scripts\shiva.exe locked after 20s). Nothing was changed. Close other Shiva windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the shiva.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\shiva.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_shiva_exe must first rename the running shiva.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that shiva.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Shiva window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Shiva.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m shiva_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Shiva installer or `shiva doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "shiva_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Shiva windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m shiva_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed shiva predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-ShivaStep $pythonExe $updateArgs "update"
    Write-HandoffLog "shiva update exit code: $($res.Code)"

    if ($res.Code -ne 0 -and $res.Code -ne 2) {
        # One retry for the update-boundary class (fresh code on disk, stale
        # code in memory). Exit 2 ("close all Shiva windows") is not retryable.
    ...
    # `shiva update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "shiva update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-ShivaStep $pythonExe @("-m", "shiva_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `shiva desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `shiva debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Shiva is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Shiva was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Shiva."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Shiva"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Shiva to finish (it could not restart itself)."
