...
# Shiva WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: SHIVA_TINI_SHIM_TARGET overrides the `/init` path (and
# SHIVA_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${SHIVA_TINI_SHIM_TARGET:-/init}"
WRAPPER="${SHIVA_TINI_SHIM_WRAPPER:-/opt/shiva/docker/main-wrapper.sh}"
