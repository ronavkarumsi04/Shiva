#!/bin/sh
# shellcheck shell=sh
# /opt/shiva/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching SHIVA_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `shiva` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `shiva <args>` (subcommand passthrough)
#
# Drop to shiva via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${SHIVA_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${SHIVA_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export SHIVA_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset SHIVA_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid shiva "$@"; exec "$@"; }
...
# an arbitrary non-root, non-shiva UID, the bootstrap was skipped and the
# baked image dirs (owned by the shiva build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u shiva)" ]; then
    cat >&2 <<EOF
[shiva] ERROR: container started with --user $cur_uid (an arbitrary, non-shiva UID) — not supported.
...
# to the shiva user's home before dropping privileges so libraries that
...
_shiva_orig_cwd="${SHIVA_ORIG_CWD:-$PWD}"
...
# the user's command so `shiva chat` starts in the Docker -w
