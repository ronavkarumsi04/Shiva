...
def _xai_user_agent() -> str:
    try:
        ...
        return shiva_xai_user_agent()
    except Exception:
        return "shiva-agent/video_gen"
...
def _raise_if_blocked_local_input(ref: str) -> None:
    """Refuse to read a local media path that Shiva' read deny-list blocks.
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``shiva_cli/tools_config.py``) so the picker doesn't blindly
...
def _auth_required_response(prompt: str) -> Dict[str, Any]:
    return error_response(
        error=(
            "No xAI credentials found. Sign in via `shiva auth add xai-oauth` "
    ...
    *,
    ...
    try:
        ...
        storage_options = build_xai_storage_options(
            "video_gen",
            filename_prefix="shiva-xai-video",
            extension="mp4",
        )
