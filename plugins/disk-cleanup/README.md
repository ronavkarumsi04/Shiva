...
Auto-tracks and cleans up ephemeral files created during Shiva Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SHIVA_HOME` and `/tmp/shiva-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SHIVA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SHIVA_HOME` or `/tmp/shiva-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SHIVA_HOME/disk-cleanup/` is itself excluded
- `$SHIVA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
