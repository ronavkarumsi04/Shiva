"""disk_cleanup — ephemeral file cleanup for Shiva Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under SHIVA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SHIVA_HOME and /tmp/shiva-*
Never touches: ~/.shiva/logs/ or any system directory.
"""
...
try:
    ...
    def get_shiva_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("SHIVA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".shiva").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SHIVA_HOME/logs/``."""
    return get_shiva_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SHIVA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SHIVA_HOME or ``/tmp/shiva-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    shiva_home = get_shiva_home()
    try:
        path.resolve().relative_to(shiva_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/shiva-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("shiva-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "shiva-agent", "backups", "profiles", ".worktrees",
...
# Paths under $SHIVA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so SHIVA_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        shiva_home = get_shiva_home()
        for parent in ("cron", "cronjobs"):
            base = shiva_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SHIVA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under SHIVA_HOME, but never recurse into known
    # durable state trees.  Some installs place the Shiva checkout, venv,
    # and desktop build under SHIVA_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    shiva_home = get_shiva_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in shiva_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    shiva_home = get_shiva_home()
    try:
        rel = path.resolve().relative_to(shiva_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "shiva-agent",
        ...
        # Path isn't under SHIVA_HOME (e.g. /tmp/shiva-*) — fall through.
        pass
