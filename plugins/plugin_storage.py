...
``<shiva home>/plugins/<name>/``. That tree is the plugin *install* dir —
``shiva plugins remove`` deletes it and ``shiva plugins update`` git-pulls
...
``<shiva home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Shiva.
...
# Mirrors the plugin-name shape `shiva plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<shiva home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`shiva_constants.get_shiva_home` on every call. Don't cache the
    result across profile switches.
    """
    from shiva_constants import get_shiva_home

    root = get_shiva_home() / "plugin-data" / _validate_name(name)
