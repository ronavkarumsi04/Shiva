...
three virtual model IDs so the ``shiva tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `shiva tools` → Image "
                    "Generation → OpenAI to configure, or `shiva setup` "
                    "to add the key."
                ),
