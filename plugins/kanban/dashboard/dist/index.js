/**
 * Shiva Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.shiva/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SHIVA_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SHIVA_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "shiva-kanban-dot-triage",
    todo: "shiva-kanban-dot-todo",
    ready: "shiva-kanban-dot-ready",
    running: "shiva-kanban-dot-running",
    blocked: "shiva-kanban-dot-blocked",
    review: "shiva-kanban-dot-review",
    done: "shiva-kanban-dot-done",
    archived: "shiva-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-shiva-task";
  ...
  const DOCS_URL = "https://shiva-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://shiva-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "shiva.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="shiva-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])shiva-kanban-md-code\1/i.test(attrs)) {
      return ' class="shiva-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "shiva-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "shiva-kanban-md",
  ...
  // standard `drop` event and our `shiva-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("shiva-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("shiva-kanban-column--drop");
          if (target) target.classList.add("shiva-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("shiva-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("shiva-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("shiva-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__SHIVA_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "shiva-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "shiva-kanban-attention",
        "shiva-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "shiva-kanban-attention-bar" },
        h("span", { className: "shiva-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "shiva-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "shiva-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "shiva-kanban-attention-row",
                  "shiva-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "shiva-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "shiva-kanban-attention-row-id" }, task.id),
                h("span", { className: "shiva-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "shiva-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "shiva-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "shiva-kanban-diag-action-btn",
      action.suggested ? "shiva-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " shiva-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".shiva-kanban-drawer-comment-row input, .shiva-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "shiva-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("shiva-kanban-diag", sevClass) },
      h("div", { className: "shiva-kanban-diag-header" },
        h("span", { className: "shiva-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "shiva-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "shiva-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "shiva-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "shiva-kanban-diag-data-row" },
                  h("span", { className: "shiva-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "shiva-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "shiva-kanban-diag-data-row" },
                h("span", { className: "shiva-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "shiva-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "shiva-kanban-diag-reassign-row" },
            h("span", { className: "shiva-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "shiva-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "shiva-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "shiva-kanban-diag-msg",
              msg.ok ? "shiva-kanban-diag-msg--ok" : "shiva-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "shiva-kanban-section" },
      h("div", { className: "shiva-kanban-section-head-row" },
        h("span", { className: "shiva-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "shiva-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "shiva-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "shiva-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "shiva-kanban-docs-link",
      title: "Open Shiva Kanban docs in a new tab",
      "aria-label": "Shiva Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `shiva kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "shiva-kanban-msg-ok" : "shiva-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "shiva-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "shiva-kanban-boardswitcher" },
      h("div", { className: "shiva-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "shiva-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "shiva-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "shiva-kanban-dialog-title" },
        ...
        h("div", { className: "shiva-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Shiva profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "shiva-kanban-bulk" },
      h("span", { className: "shiva-kanban-bulk-count" },
      ...
      h("div", { className: "shiva-kanban-bulk-priority",
      ...
      h("div", { className: "shiva-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Shiva profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "shiva-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("shiva-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("shiva-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "shiva-kanban-trash",
        dragOver ? "shiva-kanban-trash--drop" : "",
        props.draggingTaskId ? "shiva-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "shiva-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "shiva-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".shiva-kanban-card")) return true;
      if (target.closest && target.closest(".shiva-kanban-column-add")) return true;
      if (target.closest && target.closest(".shiva-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("shiva-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("shiva-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".shiva-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "shiva-kanban-columns",
        isScrollable ? "shiva-kanban-columns--scrollable" : "",
        isPanning ? "shiva-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("shiva-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("shiva-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "shiva-kanban-column",
        dragOver ? "shiva-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "shiva-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "shiva-kanban-col-check",
        ...
        h("span", { className: cn("shiva-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "shiva-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "shiva-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "shiva-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "shiva-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "shiva-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "shiva-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "shiva-kanban-lane" },
                  h("div", { className: "shiva-kanban-lane-head" },
                    h("span", { className: "shiva-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "shiva-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "shiva-kanban-card--stale-red";
    if (age >= tier.amber) return "shiva-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".shiva-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "shiva-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "shiva-kanban-card",
        props.selected ? "shiva-kanban-card--selected" : "",
        props.failed ? "shiva-kanban-card--failed" : "",
        props.draggingSource ? "shiva-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "shiva-kanban-card-content" },
          h("div", { className: "shiva-kanban-card-row" },
            h("label", {
              className: "shiva-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "shiva-kanban-card-check",
            ...
            h("span", { className: "shiva-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or shiva kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "shiva-kanban-warning-badge",
                    "shiva-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "shiva-kanban-needs-assignee",
          ...
          h("div", { className: "shiva-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "shiva-kanban-card-row shiva-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "shiva-kanban-assignee",
                            title: `Assigned to Shiva profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "shiva-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "shiva-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "shiva-kanban-ago",
  ...
  // centered modal reusing the shiva-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "shiva-kanban-dialog shiva-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Shiva profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Shiva profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "shiva-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "shiva-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "shiva-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "shiva-kanban-drawer-close",
        ...
        data ? h("div", { className: "shiva-kanban-drawer-comment-foot" },
          h("div", {
            className: "shiva-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "shiva-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "shiva-kanban-section" },
      h("div", { className: "shiva-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "shiva-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "shiva-kanban-drawer-body" },
      h("div", { className: "shiva-kanban-drawer-title" },
        h("span", { className: cn("shiva-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "shiva-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "shiva-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "shiva-kanban-section" },
            h("div", { className: "shiva-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "shiva-kanban-section" },
            h("div", { className: "shiva-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "shiva-kanban-done-no-result shiva-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "shiva-kanban-section" },
            h("div", { className: "shiva-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "shiva-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "shiva-kanban-section" },
        h("div", { className: "shiva-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "shiva-kanban-comment" },
            h("div", { className: "shiva-kanban-comment-head" },
              h("span", { className: "shiva-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "shiva-kanban-diag-action-btn",
      ...
      h("div", { className: "shiva-kanban-section" },
        h("div", { className: "shiva-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "shiva-kanban-comment" },
                h("div", { className: "shiva-kanban-comment-head" },
                  h("span", { className: "shiva-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "shiva-kanban-comment-ago" },
      ...
      h("div", { className: "shiva-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "shiva-kanban-event",
              isDiag ? "shiva-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "shiva-kanban-event-header" },
                  h("span", { className: "shiva-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "shiva-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "shiva-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "shiva-kanban-event-header-plain" },
                  h("span", { className: "shiva-kanban-event-kind" }, e.kind),
                  h("span", { className: "shiva-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "shiva-kanban-event-phantom-row" },
                  h("span", { className: "shiva-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "shiva-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "shiva-kanban-event-payload" },
    ...
    return h("div", { className: "shiva-kanban-section" },
      h("div", { className: "shiva-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "shiva-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `shiva-kanban-run--${r.outcome || r.status || "ended"}`
          : "shiva-kanban-run--active";
        return h("div", { key: r.id, className: cn("shiva-kanban-run", outcomeClass) },
          h("div", { className: "shiva-kanban-run-head" },
            h("span", { className: "shiva-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "shiva-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "shiva-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "shiva-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "shiva-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "shiva-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "shiva-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "shiva-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "shiva-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "shiva-kanban-pre shiva-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "shiva-kanban-meta-row" },
      h("span", { className: "shiva-kanban-meta-label" }, props.label),
      h("span", { className: "shiva-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "shiva-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "shiva-kanban-meta-row" },
        h("span", { className: "shiva-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "shiva-kanban-meta-value shiva-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "shiva-kanban-meta-row" },
        h("span", { className: "shiva-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "shiva-kanban-meta-value shiva-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "shiva-kanban-meta-row" },
        h("span", { className: "shiva-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "shiva-kanban-meta-value shiva-kanban-editable",
    ...
    return h("div", { className: "shiva-kanban-meta-row" },
      h("span", { className: "shiva-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "shiva-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "shiva-kanban-section" },
      h("div", { className: "shiva-kanban-section-head-row" },
        h("span", { className: "shiva-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "shiva-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "shiva-kanban-section" },
      h("div", { className: "shiva-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "shiva-kanban-deps-row" },
        h("span", { className: "shiva-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "shiva-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "shiva-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "shiva-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "shiva-kanban-dep-chip-x",
      ...
      h("div", { className: "shiva-kanban-deps-row" },
        h("span", { className: "shiva-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "shiva-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "shiva-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "shiva-kanban-section" },
      ...
      h("div", { className: "shiva-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "shiva-kanban-home-sub shiva-kanban-home-sub--on"
              : "shiva-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__SHIVA_PLUGINS__ && typeof window.__SHIVA_PLUGINS__.register === "function") {
    window.__SHIVA_PLUGINS__.register("kanban", KanbanPage);
  }
})();
