...
Let the shiva agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where shiva runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `shiva meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `shiva meet` CLI |
...
| `cli.py` | `shiva meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$SHIVA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `shiva meet node {run,list,approve,remove,status,ping}` |
...
shiva meet install                                      # pip + Chromium
shiva meet setup                                        # preflight
shiva meet auth                                         # optional
shiva meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
shiva meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.shiva/.env
shiva meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
shiva meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
shiva meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.shiva/.env
shiva meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, shiva will **not** switch your system audio input automatically — the
...
shiva plugins enable google_meet
shiva meet node run --display-name my-mac --host 0.0.0.0 --port 18789
