...
```bash
shiva memory setup    # select "mem0"
```
...
Behavioral settings live in `$SHIVA_HOME/mem0.json` (set them via `shiva memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.shiva/.env`.
...
| `user_id` | `shiva-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Shiva session and call `mem0_search` — it connects to your server.
...
```bash
shiva memory setup mem0 --mode oss \
...
```bash
shiva memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$SHIVA_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.shiva/mem0_qdrant"}}
  }
}
```
...
```bash
shiva memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$SHIVA_HOME/mem0.json`).
