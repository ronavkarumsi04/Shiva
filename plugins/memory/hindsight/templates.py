...
the ``shiva`` integration, and applies a chosen manifest to the user's bank
...
from shiva_cli.urllib_security import open_credentialed_url
...
def fetch_shiva_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``shiva`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "shiva" in (e.get("integrations") or [])]
