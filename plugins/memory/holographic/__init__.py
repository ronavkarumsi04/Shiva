"""shiva-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $SHIVA_HOME/config.yaml (profile-scoped):
  plugins:
    shiva-memory-store:
      db_path: $SHIVA_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "shiva-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, shiva_home):
        """Write config to config.yaml under plugins.shiva-memory-store."""
        from pathlib import Path
        config_path = Path(shiva_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["shiva-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_shiva_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _shiva_home = str(get_shiva_home())
        _default_db = _shiva_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $SHIVA_HOME in user-supplied paths so config values like
        # "$SHIVA_HOME/memory_store.db" or "~/.shiva/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$SHIVA_HOME", _shiva_home)
            db_path = db_path.replace("${SHIVA_HOME}", _shiva_home)
