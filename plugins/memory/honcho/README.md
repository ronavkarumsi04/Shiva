...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/shiva>
...
```bash
shiva memory setup honcho   # configure Honcho directly (works on a fresh install)
shiva memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
shiva config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.shiva/.env
```

> `shiva honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `shiva memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `shiva model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SHIVA_HOME/honcho.json` | Profile-local (isolated Shiva instances) |
| 2 | `~/.shiva/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Shiva profile: `shiva` (default) or `shiva_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `shiva honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Shiva session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`shiva-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`shiva`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-shiva-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening shiva in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Shiva session ID (timestamp + hex). Every `shiva` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Shiva profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`shiva`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Shiva profile: `shiva` (default) or `shiva_<profile>` (e.g. `shiva -p coder` -> host key `shiva_coder`). Older `shiva.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `SHIVA_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `shiva-agent`) |
...
|---------|-------------|
| `shiva memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `shiva honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `shiva memory setup`) |
| `shiva honcho status` | Show resolved config for active profile |
| `shiva honcho enable` / `disable` | Toggle Honcho for active profile |
| `shiva honcho mode <mode>` | Change recall or observation mode |
| `shiva honcho peer --user <name>` | Update user peer name |
| `shiva honcho peer --ai <name>` | Update AI peer name |
| `shiva honcho tokens --context <N>` | Set context token budget |
| `shiva honcho tokens --dialectic <N>` | Set dialectic max chars |
| `shiva honcho map <name>` | Map current directory to a session name |
| `shiva honcho sync` | Create host blocks for all Shiva profiles |
