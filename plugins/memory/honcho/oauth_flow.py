...
``shiva://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Shiva OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "shiva-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``shiva-desktop`` / ``shiva-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Shiva.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local SHIVA_HOME override can't reach.
