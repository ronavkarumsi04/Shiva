...
- Hosted: API key from [app.supermemory.ai/integrations?connect=shiva](http://app.supermemory.ai/integrations?connect=shiva)
...
Before running `shiva memory setup`, add the local endpoint to
...
Then run `shiva memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$SHIVA_HOME/supermemory.json`
...
| `container_tag` | `shiva` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `shiva-{identity}` → `shiva-coder`). |
...
`https://api.supermemory.ai`. Shiva resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: shiva`, and document writes stamp
`metadata.sm_source: shiva`. This is a **functional routing key, not telemetry**:
it groups Shiva-written memories into a dedicated "Shiva" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Shiva profile:

```json
{
  "container_tag": "shiva-{identity}"
}
```

For a profile named `coder`, this resolves to `shiva-coder`. The default profile resolves to `shiva-default`. Without `{identity}`, all profiles share the same container.
