...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=shiva"
...
def _load_supermemory_config(shiva_home: str) -> dict:
    config = _default_config()
    config_path = Path(shiva_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, shiva_home: str) -> None:
    config_path = Path(shiva_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "shiva"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Shiva writes into the "Shiva" Space in the Supermemory
        ...
        merged = {"sm_source": "shiva", **(metadata or {})}
...
def _resolve_container_tag_for_setup(shiva_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(shiva_home)
...
def _probe_supermemory_connection(api_key: str, shiva_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(shiva_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(shiva_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `shiva memory setup`.
        # All other options are documented for $SHIVA_HOME/supermemory.json
    ...
    def save_config(self, values, shiva_home):
        ...
        _save_supermemory_config(sanitized, shiva_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, shiva_home)
    ...
    def post_setup(self, shiva_home: str, config: dict) -> None:
        ...
        from shiva_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(env_writes, shiva_home=shiva_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from shiva_constants import get_shiva_home
        self._shiva_home = kwargs.get("shiva_home") or str(get_shiva_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._shiva_home)
