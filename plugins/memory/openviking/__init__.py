...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Shiva peer ID in OpenViking (default: shiva)
...
_DEFAULT_AGENT = "shiva"
_OPENVIKING_USER_AGENT = f"openviking-memory-shiva/{_SHIVA_VERSION}"
_AGENT_PROMPT_LABEL = "Shiva peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "SHIVA_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Shiva slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Shiva to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Shiva refused the connection."
        ) from exc
...
def _load_shiva_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("SHIVA_HOME", "")).expanduser() if os.environ.get("SHIVA_HOME") else Path.home() / ".shiva"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Shiva did not start "
    ...
    try:
        ...
        # The Shiva Desktop backend can include the Shiva venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the Shiva venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the Shiva venv,
        # aborting `shiva update` with access-denied on .pyd files.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Shiva will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Shiva session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Shiva only", "write values only to Shiva .env"),
    ...
    _save_shiva_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Shiva .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See shiva-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_shiva_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Shiva peer ID in OpenViking, sent as the actor peer and "
    ...
    def save_config(self, values: Dict[str, Any], shiva_home: str) -> None:
    ...
    def post_setup(self, shiva_home: str, config: dict) -> None:
        ...
        from shiva_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        shiva_home_path = Path(shiva_home)
        env_path = shiva_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_shiva_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        shiva_home = str(kwargs.get("shiva_home") or "").strip()
        if not shiva_home:
            try:
                from shiva_constants import get_shiva_home
                shiva_home = str(get_shiva_home())
            except Exception:
                shiva_home = str(Path.home() / ".shiva")
        self._shiva_home = shiva_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Shiva will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.shiva/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts shiva (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Shiva will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._shiva_home:
            return None
        return Path(self._shiva_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._shiva_home:
            return None
        return Path(self._shiva_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_shiva_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Shiva' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Shiva canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See shiva-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/shiva/.overview.md).
