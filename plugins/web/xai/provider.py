...
``{title, url, description, position}`` rows every other Shiva web
provider produces.
...
prefers Shiva-managed xAI Grok OAuth (via ``shiva auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.shiva/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``shiva tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `shiva auth` to sign in with "
