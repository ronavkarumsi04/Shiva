...
def _needs_server_members_intent(
    ...
    """Return True when Shiva must request Discord's Server Members intent.
...
def _format_privileged_intents_guidance(*, needs_members: bool) -> str:
    ...
    lines.extend(
        [
            ...
            "Docs: https://shiva-agent.nousresearch.com/docs/user-guide/messaging/discord",
        ]
    )
    return "\n".join(lines)
...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("SHIVA_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning(
                "Ignoring invalid SHIVA_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.shiva/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("SHIVA_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("SHIVA_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            15.0,
            env_key="SHIVA_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold",
            2,
            env_key="SHIVA_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
        ...
        self._discord_recovery_store = DiscordRecoveryStore(get_shiva_home())
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Shiva gateway
    ...
    def _classify_connect_exception(self, error: Exception) -> tuple:
        ...
        if _is("PrivilegedIntentsRequired"):
            # Name the exact intents Shiva requested (#79430): Message
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``SHIVA_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("SHIVA_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_shiva_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Shiva response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Shiva work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Shiva/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:shiva|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Shiva manages."""
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "shiva_voice",
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Shiva pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Shiva session")
        ...
        @tree.command(name="status", description="Show Shiva session status")
        ...
        @tree.command(name="stop", description="Stop the running Shiva agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.shiva/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Shiva Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Shiva gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Shiva session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Shiva in the thread",
        ...
        # shiva_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from shiva_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from shiva_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from shiva_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Shiva session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Shiva: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Shiva has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Shiva"
        if len(content) > 80:
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    setattr(thread, "_shiva_auto_thread_initial_name", thread_name)
                except Exception:
                    pass
                return thread
            except Exception as direct_error:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Shiva: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Shiva semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Shiva handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Shiva to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **Shiva needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``shiva update --gateway``
        ...
        *,
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Shiva could not create a Discord thread for "
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_shiva_auto_thread_initial_name", None)
...
def _component_check_auth(
    ...
    the pairing store (``shiva pairing approve``).
    ...
    # so users approved via ``shiva pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``shiva update`` prompts.
...
# is not in this process (e.g. ``shiva cron`` running standalone) and no live
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "SHIVA_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
            "liveness_failure_threshold",
            "SHIVA_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ),
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `shiva setup` to install Discord support.",
        # Interactive setup wizard — replaces the central
        # shiva_cli/setup.py::_setup_discord function.  Same shape as Teams.
