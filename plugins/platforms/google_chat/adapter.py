...
# ``model_tools`` import time, so ``shiva status``, ``shiva chat``, etc.
...
# ``shiva_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Shiva is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a shiva MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "shiva-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SHIVA_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from shiva_constants import get_shiva_home as _get_shiva_home
            _shiva_home = _get_shiva_home()
        except (ModuleNotFoundError, ImportError):
            _shiva_home = _Path.home() / ".shiva"
        self._thread_count_store = _ThreadCountStore(
            _shiva_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Shiva is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SHIVA_HOME", str(_Path.home() / ".shiva"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SHIVA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a shiva MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Shiva is thinking…' marker message.
        ...
        "Shiva is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Shiva is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Shiva is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Shiva is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``shiva setup``.

    The setup wizard at ``shiva_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.shiva/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. shiva-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.shiva/.env")
    print_info("Restart the gateway: shiva gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``shiva cron`` running as a
    separate process from ``shiva gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Shiva plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `shiva setup` to install Google Chat support.",
        ...
        # the "Shiva is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Shiva is thinking…' marker message "
