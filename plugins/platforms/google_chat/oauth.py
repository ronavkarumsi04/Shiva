...
- Per-user tokens (keyed by sender email):
    ``${SHIVA_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${SHIVA_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${SHIVA_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${SHIVA_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${SHIVA_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's SHIVA_HOME helper so the token follows the user's
# profile (e.g. tests can override via SHIVA_HOME=/tmp/...).
try:
    from shiva_constants import display_shiva_home, get_shiva_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where shiva_constants isn't importable
...
def _shiva_home() -> Path:
    """Resolve SHIVA_HOME at call time (NOT module import).

    Tests and ``SHIVA_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.shiva/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _shiva_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _shiva_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _shiva_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _shiva_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _shiva_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `shiva setup` to install Google Chat support."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `shiva setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into SHIVA_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_shiva_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_shiva_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Shiva (native attachment delivery)"
    )
