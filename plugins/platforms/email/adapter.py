"""
Email platform adapter for the Shiva gateway.

Allows users to interact with Shiva by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "shiva-agent" "version" "{_shiva_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `shiva config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `shiva gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Shiva Agent")
        ...
        msg_id = f"<shiva-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# shiva_cli/gateway.py, and the _send_email dispatch in
