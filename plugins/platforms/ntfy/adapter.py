"""ntfy platform adapter (Shiva plugin).
...
``plugins/platforms/ntfy/``. The Shiva plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "shiva-in"              # subscribe topic (incoming)
          publish_topic: "shiva-out"     # optional — defaults to topic
...
_ECHO_TAG = "shiva-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``shiva cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Shiva dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `shiva gateway status` without
