"""
LINE Messaging API platform adapter for Shiva Agent.
...
to Shiva Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SHIVA_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            shiva_home = Path(get_shiva_home()).resolve()
        except Exception:
            shiva_home = Path.home().joinpath(".shiva").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``shiva status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``shiva status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``shiva setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.shiva/.env`` via ``shiva_cli.config``.
    """
    print()
    ...
    try:
        from shiva_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("shiva_cli.config not available; set LINE_* vars manually in ~/.shiva/.env")
        return
