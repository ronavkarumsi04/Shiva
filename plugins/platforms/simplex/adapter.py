"""SimpleX Chat platform adapter (Shiva plugin).
...
This adapter ships as a Shiva platform plugin under
``plugins/platforms/simplex/``. The Shiva plugin loader scans the
...
Optional environment variables:
    ...
    SHIVA_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``shiva setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Shiva — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("SHIVA_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``shiva setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.shiva/.env`` via
    ``shiva_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "shiva_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.shiva/.env"
        )
        return
