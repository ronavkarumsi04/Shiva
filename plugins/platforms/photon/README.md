...
This plugin connects Shiva Agent to iMessage (and other Spectrum
...
TypeScript-only, Shiva runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Shiva channel,
...
Run `shiva photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.shiva/.env` (the same place every other
...
Management metadata lives in `~/.shiva/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `shiva photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Shiva
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Shiva coalesces
...
- **Native polls are supported.** Shiva posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `shiva send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<shiva-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `shiva photon status`, a DM and a group roundtrip,
