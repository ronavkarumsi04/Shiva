...
Small Node helper that bridges Shiva Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Shiva is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Shiva-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Shiva can receive inbound messages
...
The Shiva plugin's `shiva photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Shiva is Python, so replies go through this sidecar
