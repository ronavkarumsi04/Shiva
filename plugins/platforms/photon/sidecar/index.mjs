// Shiva Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Shiva): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Shiva -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Shiva-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `shiva photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Shiva can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "shiva-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Shiva'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Shiva allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Shiva, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-shiva-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Shiva supplied them so a known-good
