"""Shell completion script generation for shiva CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_shiva_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Shiva Agent bash completion
# Add to ~/.bashrc:
#   eval "$(shiva completion bash)"

_shiva_profiles() {{
    local profiles_dir="$HOME/.shiva/profiles"
...
_shiva_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_shiva_profiles)" -- "$cur"))
        return
    fi
...
complete -F _shiva_completion shiva
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef shiva
# Shiva Agent zsh completion
# Add to ~/.zshrc:
...
_shiva_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.shiva/profiles" ]]; then
        profiles+=($HOME/.shiva/profiles/*(N/:t))
    fi
...
_shiva() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_shiva_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'shiva command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Shiva Agent fish completion",
        "# Add to your config:",
        "#   shiva completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __shiva_profiles",
        "    echo default",
        "    if test -d $HOME/.shiva/profiles",
        "        for d in $HOME/.shiva/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c shiva -f -s p -l profile"
        " -d 'Profile name' -xa '(__shiva_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__shiva_profiles)' -d 'Profile name'"
                )
