...
def validate_copilot_token(token: str) -> tuple[bool, str]:
    ...
    if token.startswith(_CLASSIC_PAT_PREFIX):
        return False, (
            ...
            "  → `copilot login` or `shiva model` to authenticate via OAuth\n"
...
# (``/api/model/options``, ``shiva tools``) probe Copilot auth several times
...
def _jwt_disk_path() -> Optional[Path]:
    ...
    try:
        ...
        return Path(get_shiva_home()) / _JWT_DISK_FILENAME
    except Exception:
        return None
