...
web_server - reached via the late-binding seam in :mod:`shiva_cli.web_deps`
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from shiva_cli.mcp_config import _replace_mcp_servers
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the SHIVA_HOME
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    process_home = str(get_shiva_home().expanduser().resolve(strict=False))

    def _read():
        with _profile_scope(profile):
            return _get_mcp_servers(), str(get_shiva_home().expanduser().resolve(strict=False))
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Shiva and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Shiva for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Shiva.</p>")
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `shiva mcp catalog` / `shiva mcp install` read.  ``profile`` scopes
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    from shiva_cli.config import validate_env_var_name_for_write
    ...
    # The -p subprocess rebinds SHIVA_HOME-derived paths in the child.
