...
late-binding seam in :mod:`shiva_cli.web_deps` (``late`` for callables,
...
from shiva_cli.web_deps import late, LateState
from shiva_cli.web_models import (
...
_log = logging.getLogger("shiva_cli.web_server")
...
_spawn_shiva_action = late("_spawn_shiva_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from shiva_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `shiva tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `shiva computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``shiva tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `shiva tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``shiva tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.shiva/.env`` via ``save_env_value`` —
    the same store ``shiva tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``shiva tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``shiva -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same SHIVA_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``shiva computer-use permissions grant`` as a background action.
