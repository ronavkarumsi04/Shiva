"""
Interactive setup wizard for Shiva Agent.
...
from shiva_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
from shiva_cli.nous_subscription import get_nous_subscription_features
from tools.tool_backend_helpers import managed_nous_tools_enabled
from shiva_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://shiva-agent.nousresearch.com/docs"
...
# display_shiva_home imported lazily at call sites (stale-module safety during shiva update)
...
from shiva_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from shiva_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Shiva Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Shiva using environment variables or config commands:")
    print_info("  shiva config set model.provider custom")
    print_info("  shiva config set model.base_url http://localhost:8080/v1")
    print_info("  shiva config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'shiva setup' in an interactive terminal to use the full wizard.")
    print()
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar(
    "shiva_setup_navigation", default=None
)
...
def is_noninteractive() -> bool:
    ...
    ``SHIVA_NONINTERACTIVE=1`` (see ``shiva_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("SHIVA_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``SHIVA_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from shiva_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'shiva setup')")


def _print_setup_summary(config: dict, shiva_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Shiva cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    shiva model            (pick any provider/model)")
        print_info("    shiva setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'shiva setup' to configure"))
    ...
    # Video generation — opt-in via `shiva tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from shiva_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'shiva setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'shiva setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'shiva tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'shiva setup terminal'"))
    ...
    # Spotify (OAuth via shiva auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'shiva setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {shiva_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('shiva setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('shiva setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('shiva setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('shiva setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('shiva setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('shiva config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('shiva config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('shiva config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('shiva', Colors.GREEN)}              Start chatting")
    print(f"   {color('shiva gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('shiva doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `shiva tools` and `shiva setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``shiva model``)
    ...
    provider added to ``shiva model`` is automatically available here.
    ...
    # Delegate to the shared shiva model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from shiva_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: shiva model")
    ...
    # on demand via `shiva auth add`, `shiva setup` vision, and
    # `shiva setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from shiva_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``shiva model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from shiva_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.shiva/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Shiva auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via shiva setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'shiva setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Shiva runs shell commands and code.")
    ...
    # under ~/.shiva/plugins/). Fail-soft: a broken plugin must not take the
    # setup wizard down.
    ...
    if selected_backend == "local":
        ...
        # configurable later via `shiva setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `shiva setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `shiva egress setup` then `shiva egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `shiva egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'shiva-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $SHIVA_HOME/bin is never on PATH, so a bare
            # which() misses the uv Shiva installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'shiva-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into SHIVA_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("SHIVA_MAX_ITERATIONS")
    ...
    print_info("  Run `shiva setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # SHIVA_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.shiva/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from shiva_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_shiva_home(Path(get_shiva_home()))
    except Exception:
        pass
...
def _profile_name_from_shiva_home(shiva_home) -> str | None:
    """Return the active profile name when SHIVA_HOME is a profile dir."""
    if shiva_home.parent.name == "profiles":
        return shiva_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Shiva delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Shiva to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from shiva_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://shiva-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://shiva-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  shiva config edit")
    print_info("   Open config in your editor:  shiva config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from shiva_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Shiva from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'shiva setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     shiva config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `shiva import` or `shiva setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `shiva setup tools` and `shiva tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Shiva profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``shiva_cli.auth`` — lists every supported
    ...
    """
    try:
        from shiva_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from shiva_cli.config
    ...
    if section_key == "model":
        ...
        from shiva_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_shiva.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_shiva migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_shiva", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Shiva.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Shiva to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Shiva at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Shiva at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Shiva at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Shiva at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Shiva equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Shiva config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Shiva.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Shiva's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(shiva_home: Path) -> bool:
    ...
    print_info("Shiva can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: shiva claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=shiva_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: shiva claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Shiva configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Shiva config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Shiva (use shiva claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``shiva setup --portal`` and ``shiva portal``. This is the
    ...
    Shiva session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``shiva setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``shiva model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``shiva portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Shiva Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `shiva portal` == quick setup's Nous step.
    try:
        from shiva_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `shiva portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `shiva portal`: %s", exc)
        print()
    ...
    print_info("  Run `shiva portal info` to inspect routing.")
    print_info("  Run `shiva` to start chatting.")


@contextmanager
def _setup_navigation_scope():
    """Install and reliably restore the setup menu navigation context."""
    from shiva_cli.curses_ui import (
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``shiva model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    Supports full, quick, and section-specific setup:
      shiva setup           — full or quick (auto-detected)
      shiva setup model     — just model/provider
      shiva setup tts       — just text-to-speech
      shiva setup terminal  — just terminal backend
      shiva setup gateway   — just messaging platforms
      shiva setup tools     — just tool configuration
      shiva setup telemetry — just local shared metrics
      shiva setup agent     — just agent settings
    """
    from shiva_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Shiva Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Shiva Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Shiva Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps(
                [("Quick Setup", lambda: _run_quick_setup(config, shiva_home))]
            )
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Shiva configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'shiva setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(shiva_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Shiva?",
            [
    ...
    print_info(f"Data folder:  {shiva_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'shiva config edit'")
    ...
    # Tune later with `shiva setup agent`.
    if not is_existing:
    ...
    def _gateway_step() -> None:
        ...
        from shiva_cli.gateway import ensure_gateway_service
    ...
    _print_setup_summary(config, shiva_home)


def _run_first_time_quick_setup(config: dict, shiva_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``shiva setup <section>``
    or switch providers with ``shiva model``.
    """
    from shiva_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'shiva setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `shiva import` from another machine).
        from shiva_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    shiva setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  shiva setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    print_info("  your terminal (and Shiva.app if you use Desktop), or run:")
    print_info("    open \"x-apple.systempreferences:com.apple.preference"
               ".security?Privacy_AllFiles\"")
    print_info("  The grant is permanent — it survives every Shiva update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``shiva-agent`` skill (the agent's operating manual for driving,
    configuring, and troubleshooting Shiva) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``shiva tools`` (which
    ...
    try:
        ...
        from shiva_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``shiva setup agent`` /
    ``shiva config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, shiva_home, is_existing: bool):
    ...
    print_info("essential `shiva-agent` skill is always kept so the agent")
    print_info("can help you drive and configure Shiva itself.")
    print()
    ...
    if path == 0:
        ...
        # `shiva update` runs don't re-inject them. Essential skills (the
        # `shiva-agent` operating manual) are still seeded by the sync.
        try:
        ...
        print_info("  Enable tools:        shiva tools")
        print_info("  Seed skills:         shiva skills opt-in --sync")
        print_info("  Add MCP servers:     shiva mcp add")
        print_info("  Enable plugins:      shiva plugins")
        print_info("  Tune agent settings: shiva setup agent")
        print()
        _print_setup_summary(config, shiva_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, shiva_home)


def _blank_slate_walkthrough(config: dict, shiva_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            set_bundled_skills_opt_out(True)
            # Essential skills (the `shiva-agent` operating manual) are
            # still seeded even for an opted-out profile.
            sync_skills(quiet=True)
            print_info("No skills seeded (except the essential `shiva-agent`")
            print_info("skill). A .no-bundled-skills marker keeps future")
            print_info("`shiva update` runs from re-injecting them. Opt back in any")
            print_info("time with `shiva skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `shiva tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `shiva plugins list` / `shiva plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `shiva plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `shiva mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `shiva mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   shiva tools")
    print_info("  Seed skills:         shiva skills opt-in --sync")
    print_info("  Add MCP servers:     shiva mcp add")
    print_info("  Tune agent settings: shiva setup agent")
    print()
...
def _run_quick_setup(config: dict, shiva_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'shiva setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Shiva to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'shiva setup gateway'.")
