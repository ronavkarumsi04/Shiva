"""``shiva worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``shiva_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    shiva worktree list             # audit: verdict + reason per tree
    shiva worktree prune            # reap safe trees + merged branches
    shiva worktree prune --dry-run  # show the plan, change nothing
    shiva worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `shiva worktree prune`."
        )
