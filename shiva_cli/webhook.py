"""shiva webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.shiva/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _shiva_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: shiva gateway run
"""
...
def webhook_command(args):
    """Entry point for 'shiva webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: shiva webhook {subscribe|list|remove|test}")
        print("Run 'shiva webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (shiva gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: shiva webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from shiva webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (shiva gateway run)")
