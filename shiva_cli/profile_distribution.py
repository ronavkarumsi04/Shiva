"""Profile distributions — shareable, packaged Shiva profiles via git.

A distribution is a Shiva profile published as a git repository (or
...
* ``shiva profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``shiva skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``shiva profile``, not a parallel tree):

    shiva profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "shiva-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            shiva_requires=str(data.get("shiva_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.shiva_requires:
            out["shiva_requires"] = self.shiva_requires
        if self.author:
...
def check_shiva_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Shiva {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``shiva profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Shiva profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Shiva distribution."
        )

    # Version check up-front so we fail fast
    check_shiva_requires(manifest.shiva_requires, shiva_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.shiva).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="shiva_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `shiva profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`shiva profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="shiva_dist_update_") as tmp:
        plan = plan_install(
