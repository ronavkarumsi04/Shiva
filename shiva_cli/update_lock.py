"""Cross-process mutual exclusion for in-flight Shiva updates.

Three different surfaces can start an update of the same install tree:

* ``shiva update`` from a terminal,
* the dashboard's Update button (``POST /api/shiva/update`` →
  ``_spawn_shiva_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``shiva-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``shiva update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <SHIVA_HOME>/.shiva-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``shiva update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Shiva is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``shiva-setup`` binary under ``~/.shiva`` is only refreshed by a full
  installer run (``copy_self_to_shiva_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Shiva is still running") on every
  GUI update forever, with no Shiva process actually running.
"""
...
MARKER_NAME = ".shiva-update-in-progress"

# Set by an orchestrating updater (the Tauri `shiva-setup --update` flow) to
# its own pid before spawning `shiva update` as a child stage. The parent
...
HANDOFF_PID_ENV = "SHIVA_UPDATE_HANDOFF_PID"
...
# to show "Shiva is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Shiva home (never the context-local profile override):
    the Rust updater resolves ``$SHIVA_HOME`` or the platform default, and the
    ...
    return get_process_shiva_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``shiva update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``shiva-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Shiva update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `shiva update` as a stage): we run under ITS claim
