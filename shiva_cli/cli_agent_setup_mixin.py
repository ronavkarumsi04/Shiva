"""Agent-construction and session-resume display methods for ``ShivaCLI``.

Extracted from ``cli.py`` as part of the god-file decomposition campaign
(``~/.shiva/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
Behavior-neutral: every method is lifted verbatim from ``ShivaCLI``. ``self.*``
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``ShivaCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("   Run 'shiva model' to choose a provider, or "
                      "'shiva setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: shiva setup")
            return False
        ...
        # `shiva chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `shiva auth add openai-codex`
        # without `shiva model`), fall back to the provider's first catalog
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        exact same flow as ``shiva model`` (which fronts Quick Setup / Nous
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'shiva model' or 'shiva setup' any time.")
            return False
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'shiva model' to retry.")
        return False
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (shiva sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (shiva sessions list).{_RST}")
                return False
        ...
        try:
            ...
            from shiva_constants import partial_update_hint
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "event":
                ...
                lines.append("  ◆ Shiva: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines() or [""]
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Shiva: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines() or [""]
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
