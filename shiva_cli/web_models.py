"""Pydantic request/response models for the Shiva dashboard web server.

Extracted verbatim from ``shiva_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from shiva_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``shiva model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `shiva import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`shiva -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the SHIVA_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under SHIVA_HOME.
    output: str = ""
