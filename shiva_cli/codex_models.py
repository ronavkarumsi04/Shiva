...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # in shiva_cli/models.py — only the openai-codex (OAuth) provider
    ...
    # Shiva uses. Removed here. If OpenAI re-enables them on Codex backend,
...
def _add_context_variants(model_ids: List[str]) -> List[str]:
    ...
    The suffix is Shiva-side only — it is stripped before the model id hits
...
def _read_cache_models(codex_home: Path) -> List[str]:
    ...
    if isinstance(entries, list):
        for item in entries:
            ...
            # public OpenAI API, while Shiva openai-codex talks to the same
