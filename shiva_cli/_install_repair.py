...
- ``shiva_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``shiva_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``shiva_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``shiva_cli.main`` imports
``managed_uv``, ``shiva_constants``, and friends only in its late path; the
...
:func:`shiva_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from shiva_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (shiva_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``shiva acp`` speaks JSON-RPC on stdout; an inherited-fd install child
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    # shiva_constants is stdlib-only, so the canonical layout helpers are safe
...
#: managed binary dir (the default Shiva root's ``bin``, next to uv.exe)
#: on the user PATH. Keep in lockstep with the launcher list in
#: scripts/install.ps1.
_WINDOWS_BIN_LAUNCHERS = ("shiva", "shiva-acp")


def _venv_is_relocatable(venv_dir: Path) -> bool:
    """True when the venv's pyvenv.cfg declares ``relocatable = true``.

    uv writes the flag; ``shiva_cli.managed_uv`` builds its replacement
...
def ensure_windows_bin_launchers(
    ...
    *,
    ...
    """Re-stage the Windows ``shiva`` launchers when they vanish.

    On Windows, ``shiva`` resolves through launchers derived from the venv
    ...
    the managed binary dir — the default Shiva root's ``bin``
    (``%LOCALAPPDATA%\\shiva\\bin``, next to the managed uv) — which lives
    OUTSIDE the git checkout so no git operation can ever touch it. It is
    a per-machine dir shared by every profile: ``get_shiva_home()`` would
    point inside ``profiles\\<name>`` under ``shiva -p``, so the anchor
    here is :func:`shiva_constants.get_default_shiva_root`.

    Earlier installer versions staged them at ``<checkout>\\bin`` instead —
    inside the git working tree — where ``shiva update``'s pre-update
    ...
    nothing restored them and ``shiva`` stopped resolving in every new
    ...
    - canonical managed binary dir: only when *root* is the managed clone
      (``root.parent == get_default_shiva_root()``), so source checkouts
    ...
    *windows* and *user_path_entries* are injectable for tests, same pattern
    as ``shiva_constants.venv_bin_dir``.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
    ...
    # Per-machine anchor: the DEFAULT Shiva root, not get_shiva_home() —
    # under ``shiva -p <name>`` that returns ``profiles\\<name>``, which
    ...
    try:
        home = Path(get_default_shiva_root())
    except Exception:
        return []
    ...
    # every shiva_cli.main process start (right after the profile
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print(
                "  ✓ Restored shiva launcher(s): " + ", ".join(restored),
                file=sys.stderr,
            )
    return restored
...
def _read_user_path_raw() -> tuple[list[str], int]:
    ...
    (same discipline as ``shiva_cli.uninstall``). Only called on Windows.
    """
    import winreg
...
def migrate_windows_bin_path(
    ...
    *,
    ...
    """One-time PATH migration to the ``SHIVA_HOME\\bin`` launcher layout.

    Runs from the ``shiva update`` tail (and mirrors what install.ps1's
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    from shiva_constants import get_default_shiva_root, venv_bin_dir
    ...
    if kept != entries:
        ...
        with contextlib.suppress(OSError, ValueError):
            print(
                f"  ✓ shiva launchers now resolve from {home_bin} "
...
def _quarantine_running_shiva_exe(
    ...
    """Rename live shiva*.exe shims aside so the installer can rewrite them.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    ``shiva acp`` speaks JSON-RPC on stdout.
    """
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = (
        _quarantine_running_shiva_exe(scripts_dir, failed_out=failed)
        if scripts_dir
        else []
    )
    if failed:
    ...
    try:
        ...
        # the quarantined shims renamed aside and `shiva` gone from PATH
...
def run_core_install(root: Path) -> None:
    ...
    - quarantine live ``shiva*.exe`` shims on Windows so they can be replaced
