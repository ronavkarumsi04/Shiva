...
The top-level command is ``shiva egress``.  Note that the inbound OAuth
reverse-proxy command (``shiva proxy``) lives elsewhere in
``shiva_cli/main.py`` — different direction, different purpose.
"""
...
# ---------------------------------------------------------------------------
# Argparse wiring — called from shiva_cli.main
# ---------------------------------------------------------------------------
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``shiva_cli.main`` as part of building the top-level
    ``shiva egress`` parser.
    """

    # dest='egress_command' — keeps this subparser tree disjoint from the
    # inbound OAuth ``shiva proxy`` subparser (which uses dest='proxy_command').
    ...
    setup.add_argument(
        ...
        help="Do not restart a running daemon after setup; you'll need to run "
             "`shiva egress restart` yourself for changes to take effect.",
    )
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if args.from_bitwarden:
        ...
        if not bw_cfg.get("enabled"):
            ...
            console.print(
                "  Run `shiva secrets bitwarden setup` first, or omit "
                "--from-bitwarden."
            )
            return 1
        try:
        ...
        # provider keys only in ~/.shiva/.env (loaded automatically when the
        # agent runs, but NOT exported into an interactive shell).  Fall back
        # to loading that file so `shiva egress setup` finds the same keys the
    ...
    # explicitly requested rotation.  This prevents re-running `shiva
    ...
    if rotate and existing:
        ...
        if _sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Shiva sandbox.  They will start "
    ...
    # `shiva egress reload` apply future ruleset changes without a
    ...
    # rotation), running `shiva egress setup` again WITHOUT
    ...
    if do_restart:
        try:
            ...
            console.print(
                "  Run [cyan]shiva egress start[/cyan] manually before "
        ...
        console.print(
            ...
            "changed.  Run [cyan]shiva egress restart[/cyan] (or "
    ...
    console.print(
        "  Start:   [cyan]shiva egress start[/cyan]\n"
        "  Restart: [cyan]shiva egress restart[/cyan]  (after any re-setup)\n"
        "  Reload:  [cyan]shiva egress reload[/cyan]   (apply ruleset edits "
        "in-place, no restart)\n"
        "  Status:  [cyan]shiva egress status[/cyan]\n"
        "  Stop:    [cyan]shiva egress stop[/cyan]\n"
        "  Disable: [cyan]shiva egress disable[/cyan]"
    )
    return 0
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print(
            "[yellow]proxy.enabled is false — run `shiva egress setup` "
            "first.[/yellow]"
        )
        return 1
    ...
    if credential_source == "bitwarden" and not refresh_bw:
        if bool(proxy_cfg.get("allow_env_fallback", False)):
            ...
            console.print(
                ...
                "back to env credentials with `shiva egress setup "
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            ...
            console.print(
                ...
                "`shiva egress setup --no-bitwarden` to switch back to "
        ...
        if not (bw_cfg or {}).get("project_id"):
            ...
            console.print(
                "  Run `shiva secrets bitwarden setup` to configure the "
                "project, or switch back via `shiva egress setup "
                "--no-bitwarden`."
            )
            return 1
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    ``shiva egress restart`` instead: the daemon reads real credentials
    ...
    console.print(
        ...
        "still need `shiva egress restart` — the daemon reads real "
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", "Next: run `shiva egress setup` to mint tokens and write proxy.yaml."])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", "Next: run `shiva egress start` before launching Docker sandboxes."])
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            ...
            "[cyan]shiva egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print(
            "[yellow](no config generated — run `shiva egress setup`)[/yellow]"
        )
        return 1
...
def _load_env_file_into_environ() -> int:
    """Backfill provider keys from ``~/.shiva/.env`` into ``os.environ``.

    ``shiva egress setup`` discovers providers by reading ``os.environ``, but
    many operators keep their keys ONLY in ``~/.shiva/.env`` (which the agent
