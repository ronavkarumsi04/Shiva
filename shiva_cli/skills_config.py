"""
Skills configuration for Shiva Agent.
`shiva skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.shiva/config.yaml under:
...
from shiva_cli.config import cfg_get, load_config, save_config
from shiva_cli.colors import Colors, color
from shiva_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``shiva-agent``) are silently dropped from the
