"""``shiva_cli/_scan_venv_blockers.py`` — Standalone venv-process scan for JSON consumption.

Invoked by the Desktop Electron app::

    venv\\Scripts\\python.exe -m shiva_cli._scan_venv_blockers
...
def _is_pausable_gateway(cmdline: str) -> bool:
    ...
    shiva_cli.main gateway run`` and the uv-side interpreter re-running the
    same argv). Reporting those as blockers dead-ends the Desktop update:
    the preflight aborts with ``venv-blocked`` *before* spawning
    ``shiva-setup``, so the CLI updater's own
    ``_pause_windows_gateways_for_update()`` — which exists precisely to
    stop these processes (and is always active: ``shiva-setup`` invokes
    ``shiva update --yes --gateway``) — never gets the chance to run.
...
def main() -> None:
    ...
    try:
        from shiva_cli.main import _detect_venv_python_processes  # noqa: PLC0415
