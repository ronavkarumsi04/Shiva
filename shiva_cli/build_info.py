"""
Baked-in build metadata for Shiva Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``shiva_cli/dump.py`` and ``shiva_cli/banner.py``).  That doesn't work inside
...
To make ``shiva dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$SHIVA_GIT_SHA``
arg into ``<project_root>/.shiva_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``SHIVA_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".shiva_build_sha"
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    rev-parse`` for source installs, the baked ``.shiva_build_sha`` for
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    Reads ``<project_root>/.shiva_build_sha`` if present.  The file is
    written by the Dockerfile's ``SHIVA_GIT_SHA`` build-arg and contains
