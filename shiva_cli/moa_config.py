...
MOA_MARKER_PREFIX = "__SHIVA_MOA_TURN_V1__"
...
def _coerce_float_or_none(value: Any) -> float | None:
    ...
    provider default applies', matching how a single-model Shiva agent
...
def resolve_moa_preset(config: Any, name: str | None = None) -> dict[str, Any]:
    ...
    if preset is None:
        ...
        raise MoAPresetNotFoundError(
            ...
            f"{available}. Run `shiva moa list`."
        )
...
def exact_moa_preset_name(config: Any, text: str) -> str | None:
    ...
    ``shiva_cli/model_switch.py``) to recognize a bare ``/model <preset>``
