...
Shiva is developed on Linux / macOS and tested natively on Windows too.
...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_SHIVA_GATEWAY_BREAKAWAY"
...
# Electron exits, so the gateway never gets respawned after a `shiva
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Shiva' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Shiva shells out to git from many non-interactive contexts — MCP catalog
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``shiva update`` on slow-WMI machines
