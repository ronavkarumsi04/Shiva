"""Durable aggregation and local export for Shiva shared metrics."""
...
from shiva_cli.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "shiva.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_shiva_home() / "telemetry" / "shared_metrics"
        ...
        return [
            {
                ...
                "resource": {
                    "shiva_version": row["shiva_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across Shiva processes.
            with write_txn(connection):
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "shiva_version": period_row["shiva_version"],
