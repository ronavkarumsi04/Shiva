"""Bounded product contract for the first Shiva shared-metrics slice."""
...
SCHEMA_KEY = "shiva.metrics.schema_version"
SCHEMA_VERSION = "shiva.metrics.event.v2"
MODEL_CALL_SCOPE = "shiva.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "shiva.task_run"
TOOL_CALL_SCOPE = "shiva.tool_call"
CLIENT_ACTIVE_MARK = "shiva.client.active"
TOOL_APPROVAL_MARK = "shiva.tool_approval"
SKILL_LIFECYCLE_MARK = "shiva.skill.lifecycle"
SKILL_LOAD_MARK = "shiva.skill.load"
SUBSCRIBER_NAME = "shiva.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "shiva.client.active"
LEGACY_MODEL_CALL_METRIC = "shiva.model_call.count"
MODEL_ROUTE_METRIC = "shiva.model_route.count"
TASK_STARTED_METRIC = "shiva.task_run.started"
TASK_FINISHED_METRIC = "shiva.task_run.finished"
TOOL_CALL_METRIC = "shiva.tool_call.count"
TOOL_APPROVAL_METRIC = "shiva.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "shiva.skill.lifecycle.count"
SKILL_LOAD_METRIC = "shiva.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Shiva installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(shiva_version or "").strip()
    ...
    return {
        ...
        "shiva_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("shiva_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Shiva logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("shiva.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from shiva_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Shiva terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Shiva registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "shiva-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Shiva tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Shiva."""
