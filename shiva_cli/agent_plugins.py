...
supported components into records consumed by Shiva' existing skill and MCP
...
def _translate_remote(config: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    The returned record targets Shiva' existing URL-based MCP runtime.
...
def has_enabled_agent_plugin_mcp(raw_config: Mapping[str, Any]) -> bool:
    ...
    Directory scanning belongs to :mod:`shiva_cli.plugins` so startup gating
    ...
    from shiva_cli.plugins import has_enabled_agent_plugin_mcp as _probe
