"""Shiva update pipeline — extracted from ``shiva_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``shiva_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``shiva_cli.main`` reference — so existing call sites and test monkeypatches
that target ``shiva_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``shiva_cli.main``.
...
Imports are one-way: ``shiva_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``shiva_cli.main`` reference.

    Lets callers keep patching ``shiva_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``shiva_cli.main`` -> ``shiva_cli.update_cmd`` stays one-way
    at import time.
    """
    from shiva_cli import main
...
#: ``_purge_stale_shiva_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "shiva_cli.shiva_logging",
    }
)


def _purge_stale_shiva_modules() -> None:
    """Evict every cached Shiva module after the checkout changed in-place.

    ``shiva update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from shiva_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``shiva_cli.gateway`` does
    ``from shiva_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Shiva
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Shiva module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Shiva modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``shiva update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``shiva update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``shiva_cli.config_defaults``,
    ``shiva_cli.config``, and ``shiva_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``shiva_cli._subprocess_compat`` and
    ``shiva_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "shiva_cli.config_defaults",
        "shiva_cli.config",
        "shiva_cli.config_migrations",
        "shiva_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from shiva_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from shiva_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``shiva update`` historically
    ...
    caller): scope config reads/writes via the context-local SHIVA_HOME
    ...
    try:
        ...
        from shiva_cli.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_shiva_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_shiva_home_override(entry)
            try:
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'shiva config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'shiva config migrate' to retry.")
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'shiva config migrate' manually to configure."
                )
                response = "n"
        ...
        if response in {"", "y", "yes", "auto"}:
            ...
            if (gateway_mode or assume_yes or response == "auto") and missing_env:
                print("  ℹ API keys require manual entry: shiva config migrate")
        else:
            print()
            print("Skipped. Run 'shiva config migrate' later to configure.")
    else:
    ...
    # context-local SHIVA_HOME override (never os.environ — other
    ...
    # it and warn loudly.
    try:
        from shiva_cli.backup import restore_cron_jobs_if_emptied
    ...
    try:
        from shiva_cli.backup import restore_cron_jobs_all_profiles
...
# Critical files that Shiva must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "shiva_cli/main.py",
    "shiva_cli/config.py",
    "shiva_cli/__init__.py",
    "shiva_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``shiva.exe`` has
    ...
    Skipping is safe because Shiva pins its editable finder to a *static*
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``shiva`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="shiva-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``shiva update`` can be driven by a
    ...
    """
    from shiva_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from shiva_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('shiva_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``shiva update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `shiva update`.
    ...
    print("  Preview now:  shiva curator run --dry-run")
    print("  Pause it:     shiva curator pause")
    print(
        "  Docs:         https://shiva-agent.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `shiva update`.
    ...
    try:
        ...
        return
    db_path = get_shiva_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `shiva sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    shiva sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  shiva sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``shiva update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``shiva update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: shiva curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from shiva_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    shiva dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``shiva_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.shiva-update-staging"
    backup = f"{dst}.shiva-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `shiva update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``shiva_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``shiva update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && shiva update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("SHIVA_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== shiva-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Shiva launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["shiva.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Shiva")
    print("  Desktop, a gateway, or another shiva REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `shiva` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``shiva.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.shiva-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".shiva-update-staging", ".shiva-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `shiva update`.")
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second Shiva process the updater's drain
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from shiva_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity

    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (shiva gateway stop), "
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update Shiva Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `shiva update "
            f"--branch {branch}`, or update against main with `shiva update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    zip_url = (
        f"https://github.com/NousResearch/shiva-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="shiva-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"shiva-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for shiva-agent itself
        ...
        # GitHub ZIPs extract to shiva-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"shiva-agent-{branch}")
        ...
        try:
            for item in entries:
                ...
                # Shiva.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `shiva update` to retry; if the agent won't start, "
            "reinstall from https://shiva-agent.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    from shiva_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if uv_bin:
        ...
        from shiva_cli.managed_uv import managed_python_env
    ...
    if not import_ok:
        ...
        print("  Re-run `shiva update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: shiva skills list-modified  "
    ...
    try:
        from shiva_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from shiva_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_shiva_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_shiva_home())
    ...
    try:
        from shiva_cli.update_receipt import finalize_update_receipt
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "shiva-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``shiva update --keep-stash`` (the desktop updater's mode): the
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Shiva behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes shiva completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Shiva couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Shiva couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Shiva behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Shiva couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Shiva couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/shiva-agent.git",
    "git@github.com:NousResearch/shiva-agent.git",
    "https://github.com/NousResearch/shiva-agent",
    "git@github.com:NousResearch/shiva-agent",
}

OFFICIAL_REPO_URL = "https://github.com/NousResearch/shiva-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_shiva_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_shiva_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Shiva repository.")
        print("  This means you may miss updates from NousResearch/shiva-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/shiva-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/shiva-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``shiva update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from shiva_constants import get_default_shiva_root

    default_home = get_default_shiva_root()
...
# ``fleet_restart_pending`` lives under SHIVA_HOME (not next to the venv).
...
def _fleet_restart_pending_marker_path() -> Path:
    """SHIVA_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    return get_shiva_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    try:
        from shiva_cli.build_info import get_code_identity
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `shiva update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `shiva update` or `shiva gateway restart`.",
            file=stream,
        )
...
def _restart_systemd_gateway_units_best_effort(failed: list) -> None:
    """Best-effort ``systemctl restart`` of every shiva-gateway/serve unit."""
...
def _run_pending_fleet_restart() -> bool:
    ...
    try:
        _m()._purge_stale_shiva_modules()
    except Exception:
        pass
    try:
        from shiva_cli.gateway import (
            find_gateway_pids,
            is_macos,
            is_windows,
    ...
    try:
        ...
        if is_windows():
            try:
                from shiva_cli import gateway_windows
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``shiva update``, finish a skipped restart.
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: shiva gateway restart")
    sys.exit(1)
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "shiva.exe"
    lines = ["✗ Another shiva.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Shiva Desktop, exit any open `shiva` REPLs, and")
    lines.append("  stop the gateway (`shiva gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `shiva update --force` if you've already")
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from shiva_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``shiva update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a Shiva Desktop backend child, an unrelated ``shiva`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``shiva tools``."""
    try:
        from shiva_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Shiva Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``shiva tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Shiva Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Shiva Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``shiva update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `shiva update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `shiva update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Shiva' editable-install extras or
    ...
    try:
        from shiva_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from shiva_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$SHIVA_HOME/bin/uv``, but on Termux the official
    ...
    """
    from shiva_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `shiva update` is exactly the step
...
def _npm_lockfile_changed(shiva_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `shiva
    ...
    try:
        ...
        cache_file = shiva_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(shiva_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    shiva update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `shiva update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `shiva update`.")
            failed = []
            if any(
    ...
    # Shiva profile using this checkout. Keep one per-checkout cache under the
    # shared Shiva root rather than rerunning npm once per named profile.
    shared_shiva_root = get_default_shiva_root()
    ...
    # `shiva update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_shiva_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `shiva update`.")
        return list(labels)
    ...
    nixos_env = with_shiva_node_path(_m()._nixos_build_env())
    ...
    # `shiva update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_shiva_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.shiva/logs/update.log`` only, never the terminal.

    During ``shiva update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``shiva update --check``: fetch and report without installing.
    ...
    from shiva_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from shiva_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``shiva update`` without requiring a reinstall.  The
    ...
    ``shiva`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/shiva.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v shiva'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/shiva, code at /usr/local/lib/shiva-agent).
    fhs_link = Path("/usr/local/bin/shiva")
    ...
    path_comment = (
        "# Shiva Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``shiva-acp`` launcher next to the ``shiva`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``shiva update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``shiva-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Shiva as not installed even when it is.

    The shim simply delegates to the sibling ``shiva`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``shiva`` / ``shiva-acp``
    launchers into the managed binary dir ``$ShivaHome\bin`` and puts THAT
    ...
    ``shiva_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``shiva-acp`` is already present next to the
    ``shiva`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        shiva_cmd = bin_dir / "shiva"
        acp_cmd = bin_dir / "shiva-acp"
        try:
            if not (shiva_cmd.is_file() or shiva_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of SHIVA_HOME under
      ``backups/`` (restorable via ``shiva import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from shiva_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from shiva_constants import
        # get_shiva_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from shiva_cli.config import get_shiva_home as _get_home
        ...
        try:
            from shiva_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from shiva_cli.update_receipt import record_step
    ...
    try:
        from shiva_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from shiva_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_shiva_home so the user sees ~/.shiva/...
    try:
        from shiva_constants import get_shiva_home, display_shiva_home

        home = get_shiva_home()
        try:
            display_path = f"{display_shiva_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  shiva import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``shiva update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``shiva update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run shiva from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.shiva-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".shiva-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The shiva.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m shiva_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``shiva
    ...
    for proc in proc_iter:
        ...
        # the `-m shiva_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``shiva_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``shiva.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``shiva`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `shiva` launch will complete it")
...
def _shiva_holder_subcommand(cmdline: str) -> str | None:
    """The actual Shiva SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``shiva_cli.main`` / ``shiva(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("shiva_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("shiva", "shiva.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``shiva dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other Shiva processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← Shiva backend (if the Desktop app is open, close it)",
        "dashboard": "  ← shiva dashboard (stop it: shiva dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _shiva_holder_subcommand(cmdline)
    ...
    lines.append(
        "  Close the Shiva desktop app / other Shiva terminals, then re-run:"
    )
    lines.append("    shiva update")
    lines.append("  (or use `shiva update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``shiva update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from shiva_cli._scan_venv_blockers import _is_pausable_gateway
...
def _ledger_manual_serve_holders(
    ...
    Shiva spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from shiva_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print(
            ...
            "restart them manually (shiva serve --host <ip> --port <port>)."
        )
    try:
        from shiva_cli.update_receipt import record_step
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning shiva-setup, and the update-in-progress marker
    ...
    the update with "Shiva is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Shiva backend (``shiva_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.shiva-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "shiva_cli.main" in argv_low and (
...
def _ledger_reapable_backend_pids(
    ...
    (``shiva_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Shiva ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``shiva.exe``) is open, NOTHING legitimate
    ...
    - Only Shiva backends (``shiva_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``shiva.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.shiva-runtime`` interpreter child alive and holding the install open
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``shiva serve`` / ``shiva dashboard`` argv.
    ...
    """
    if "shiva_cli.main" not in (cmdline or "").lower():
        return False
    return _shiva_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    shiva.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Shiva...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m shiva_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: shiva gateway run")
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    ``shiva gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from shiva_cli import gateway_windows
        from shiva_cli.gateway import find_gateway_pids
    except Exception as exc:
        raise RuntimeError(
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``shiva-gateway*.service``/``shiva-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("shiva-serve")`` alone would also accept the
        # unrelated ``shiva-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "shiva-gateway.service"
            or unit.startswith("shiva-gateway-")
            or unit == "shiva-serve.service"
            or unit.startswith("shiva-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``shiva-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``shiva-serve*`` units (#83438) don't, so sending them
    ...
    (``shiva-gateway-helper`` is fine — profile units are
    ``shiva-gateway-<profile>`` — but ``shiva-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "shiva-gateway" or svc_name.startswith("shiva-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from shiva_cli.gateway import is_macos
    ...
    print("    shiva gateway status")
    if any(not name.startswith("ai.shiva.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.shiva.") for name in ordered):
...
def _restart_launchd_gateway_after_update(
    *, supervision_verify: bool = True
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: shiva gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: shiva gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.shiva.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``shiva_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    that ``shiva-gateway-foobar.service`` was already restarted.  These are
    ...
    return name in {
        f"shiva-gateway-{profile}",
        f"ai.shiva.gateway-{profile}",
        f"gateway-{profile}",
    }
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``shiva_cli.gateway`` in the failing
...
def _recover_gateway_restart_after_abort(
    ...
    ``shiva update`` normally performs the fleet restart in the interpreter
    ...
    command = [
        sys.executable,
        "-m",
        "shiva_cli.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
    env["SHIVA_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_SHIVA_GATEWAY", "SHIVA_GATEWAY", "SHIVA_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``shiva_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``shiva update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``shiva-setup.exe``) executes
    ``$SHIVA_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_shiva_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``shiva update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``shiva update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `shiva desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "shiva_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Shiva-managed Node on PATH: when
    # `shiva update` runs inside the desktop updater chain (Desktop →
    # shiva-setup → shiva update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from shiva_constants import with_shiva_node_path

    build_env = with_shiva_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `shiva desktop` to retry)")
        tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
        if tail:
            print(tail)
        from shiva_constants import display_shiva_home as _dhh
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from shiva_cli.update_receipt import begin_update_receipt
    ...
    # running Shiva runtime, its supervisor, and its running code version —
    ...
    # On Windows, abort early if another shiva.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        scripts_dir = _m()._venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_shiva_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `shiva serve` backend) will keep .pyd
    ...
    # the shiva.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Shiva backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Shiva backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .shiva-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Shiva is still
        ...
        if _venv_holders:
            ...
            # `shiva serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Shiva backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_shiva_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Shiva backend process(es) "
    ...
    # process by design (a CLI `shiva update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by shiva.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Shiva windows/gateways and re-run: shiva update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Shiva gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in shiva_cli/config.py bricked
        # every user who ran ``shiva update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``shiva
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``shiva update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``shiva update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``shiva doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && shiva update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``shiva update`` can catch up even when git is already up to date.
        ...
        # the old bytecode (e.g. get_shiva_home added to shiva_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``shiva`` launch finishes the install
        ...
        # symbols in shiva_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `shiva` again "
        ...
        if not import_ok:
            ...
            print("    Run `shiva update` again — if it persists, reinstall:")
            print("    https://shiva-agent.nousresearch.com")
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if Shiva re-prompts for permissions you already "
                "granted (toggle shows ON), the stored grant is stale — run "
                "`tccutil reset ScreenCapture com.nousresearch.shiva` (repeat "
        ...
        try:
            from shiva_cli.macos_tcc_anchor import ensure_tcc_anchor
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_shiva_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.shiva/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit SHIVA_HOME so
        # it is not affected by sync_skills()'s module-level SHIVA_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SHIVA_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from shiva_cli.profiles import backfill_profile_envs
        ...
        # check `shiva curator status`. Self-stamps after printing so it
        ...
        # Self-heal the shiva-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Shiva on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("shiva-acp launcher self-heal failed: %s", e)

        # Migrate the Windows shiva launchers to the managed binary dir
        # (the default Shiva root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (shiva-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `shiva`
        ...
        # the managed clone under the default Shiva root).
        try:
            from shiva_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``shiva update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `shiva update`
                # must stay fast; `shiva computer-use install --upgrade`
        ...
        # When running as ``shiva update --gateway`` (spawned by the gateway's
        ...
        #
        # Purge stale cached Shiva modules FIRST: the import below pulls
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... shiva-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... shiva-gateway*)
            ...
            try:
                from shiva_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all shiva-gateway* units (default + profiles) plus
            # shiva-serve* units (the Desktop app's backend, #83438).
            if supports_systemd_services():
                try:
                    _ensure_user_systemd_env()
                except Exception:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: shiva gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # shiva-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `shiva update` restart it automatically, allow\n"
            ...
            # Restart EVERY ai.shiva.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: shiva gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: shiva -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_shiva_home() / ".update_exit_code"
                    try:
            ...
            try:
                from shiva_cli.update_receipt import record_gateway_restart
        ...
        # Warn if legacy Shiva gateway unit files are still installed.
        # When both shiva.service (from a pre-rename install) and the
        # current shiva-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `shiva update` surfaces the issue until the user migrates.
        try:
            from shiva_cli.gateway import (
                has_legacy_shiva_units,
                _find_legacy_shiva_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_shiva_units():
                print()
                print("⚠ Legacy Shiva gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_shiva_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (shiva.service) fight the current")
                print("  shiva-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    shiva gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes shiva-serve*,
        ...
        print("  shiva model              # Select provider and model")
        ...
        try:
            from shiva_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import shiva_cli.update_receipt as _ur
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``shiva update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from shiva_cli.main import main; main()" update --yes'
                    )
            try:
                from shiva_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``shiva_cli.gateway`` no longer imports,
