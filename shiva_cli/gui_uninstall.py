"""
Shiva Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``shiva desktop`` / ``shiva gui``)
     Built inside the agent checkout under ``$SHIVA_HOME/shiva-agent/``:
       ...
       - ``$SHIVA_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Shiva.app`` or ``~/Applications/Shiva.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Shiva`` (NSIS per-user)
...
the app name ("Shiva"), separate from ``$SHIVA_HOME``:
  - macOS:   ``~/Library/Application Support/Shiva``
  - Windows: ``%APPDATA%\\Shiva``
  - Linux:   ``$XDG_CONFIG_HOME/Shiva`` (default ``~/.config/Shiva``)
...
``shiva uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(shiva_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return shiva_home / "shiva-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Shiva"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Shiva"
...
def source_built_gui_artifacts(shiva_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``shiva desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``shiva-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(shiva_home)
    ...
    return [
        ...
        shiva_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Shiva" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Shiva.app"),
            home / "Applications" / "Shiva.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Shiva).
            local_base / "Programs" / "Shiva",
            # Older / alternate layout some builds used.
            local_base / "shiva-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Shiva")
    else:
        ...
        # ``uninstall_gui``.
        from shiva_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `shiva desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Shiva.desktop",
        ]
    return paths


def agent_is_installed(shiva_home: Path) -> bool:
    """Return True when a usable Python agent install exists under SHIVA_HOME.
    ...
    if (agent_root / "shiva_cli").is_dir():
        return True
...
def gui_is_installed(shiva_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(shiva_home):
        if p.exists():
            return True
...
def gui_install_summary(shiva_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = shiva_home if shiva_home is not None else get_shiva_home()
...
def uninstall_gui(shiva_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``shiva-agent/shiva_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$SHIVA_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Shiva disappears from the launcher.
        try:
            from shiva_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove shiva' or "
            "'sudo dnf remove shiva'). AppImage builds are a single file you "
