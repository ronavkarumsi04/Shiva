...
``shiva sessions import`` (and ``--resume @claude`` / ``--resume @codex``)
let a user pull a conversation they started in another agent CLI into
Shiva and continue it here.
...
role-alternation invariant Shiva enforces everywhere else:
...
def import_foreign_session(source: str, path, db=None) -> str:
    """Import one foreign session into the Shiva SessionDB.

    Returns the new Shiva session id.  The foreign file is only read.
...
def pick_foreign_session(
    ...
    if not sys.stdin.isatty():
        print(
            ...
            "  shiva sessions import --from claude|codex <path>"
        )
        return None
    try:
...
def run_sessions_import(args, db=None) -> Optional[str]:
    """`shiva sessions import` entry point. Returns new session id or None."""
    ...
    print(f"  Continue it with:  shiva --resume {session_id}")
    return session_id
