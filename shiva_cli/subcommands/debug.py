...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Shiva Agent. Use 'shiva debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    shiva debug share              Upload debug report (asks for confirmation)
    shiva debug share --yes        Skip confirmation (for scripts/CI)
    shiva debug share --lines 500  Include more log lines
    shiva debug share --expire 30  Keep paste for 30 days
    shiva debug share --local      Print report locally (no upload)
    shiva debug share --no-redact  Disable upload-time secret redaction
    shiva debug share --nous       Upload to Nous-internal storage (private)
    shiva debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'shiva debug share'",
    )
