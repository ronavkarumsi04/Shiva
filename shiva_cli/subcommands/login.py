...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``shiva login`` was removed in favor of ``shiva auth`` / ``shiva model``
    (the runtime handler in ``shiva_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``shiva login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``shiva --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``shiva login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``shiva model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `shiva auth` to manage credentials, "
            "`shiva model` to select a provider, or `shiva setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `shiva model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: shiva-cli)"
    )
