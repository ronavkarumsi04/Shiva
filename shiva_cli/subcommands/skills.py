"""``shiva skills`` subcommand parser.

Extracted from ``shiva_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_trust = skills_subparsers.add_parser(
        "trust",
        help="Trust a project so its repo-local skills (./.shiva/skills, ./.agents/skills) load",
    )
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.shiva/skills/.bundled_manifest) "
            "so future 'shiva update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `shiva update` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `shiva update` reports as user-modified and skips. "
            "Use `shiva skills diff <name>` to see changes and `shiva skills reset "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`shiva update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `shiva update`. Pass --sync to re-seed now."
        ),
    )
