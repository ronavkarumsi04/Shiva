"""``shiva update`` subcommand parser.
...
def build_update_parser(subparsers, *, cmd_update: Callable) -> None:
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Shiva Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        help=(
            ...
            "kind (git/docker/nix), every running Shiva service across all "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Force a FULL pre-update backup (quick state snapshot + SHIVA_HOME zip) for this run, regardless of updates.pre_update_backup",
    )
    update_parser.add_argument(
        "--yes",
        "-y",
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'shiva config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--keep-stash",
        action="store_true",
    ...
    update_parser.add_argument(
        ...
        help=(
            ...
            "If the local checkout is on a different branch, shiva will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another shiva.exe is detected. The concurrent process will likely cause WinError 32 warnings. Does NOT bypass the venv-process guard (see --force-venv).",
    )
    update_parser.add_argument(
        "--force-venv",
        action="store_true",
