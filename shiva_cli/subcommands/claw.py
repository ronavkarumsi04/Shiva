...
def build_claw_parser(subparsers, *, cmd_claw: Callable) -> None:
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Shiva",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Shiva",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.shiva/ (by default a "
        "single restore-point archive is written to ~/.shiva/backups/ "
        "before apply; restorable with 'shiva import').",
    )
    claw_migrate.add_argument(
