"""``shiva import-agent`` subcommand parser.

Follows the ``shiva claw`` pattern (see ``shiva_cli/subcommands/claw.py``):
parser building lives here, the handler is injected to avoid importing
``main``, and the import logic itself lives in ``shiva_cli/agent_import.py``.
"""
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent",
        help="Import a Claude Code or Codex CLI setup into Shiva",
        description=(
            "One-command import of another coding agent's setup into Shiva. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their Shiva equivalents. "
            "Always shows a preview before making changes. API keys and "
            "credentials are never imported — run 'shiva setup' for those."
        ),
    )
    parser.add_argument(
        "agent",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing Shiva items on name conflicts (default: skip)",
    )
    parser.add_argument(
