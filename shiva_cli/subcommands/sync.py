"""``shiva sync`` subcommand parser — Skill Sync.

Cloned from ``shiva_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    shiva sync status                 show gate/opt-in/head state
    shiva sync pull                   pull and materialize opted-in skills
    shiva sync push                   push opted-in skills
    shiva sync now                    reconcile: pull then push
    shiva sync enable <skill>         opt a skill into sync
    shiva sync disable <skill>        opt a skill out of sync
    shiva sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    shiva sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  shiva sync status            what is synced, and from where\n"
            "  shiva sync enable my-skill   include a skill in your sync\n"
            "  shiva sync now               pull, then push\n"
            "  shiva sync propose my-skill  share a skill with your team\n"
        ),
