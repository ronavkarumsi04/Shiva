"""``shiva peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Shiva gateway (any machine: homelab, Spark, Shiva
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    shiva peer dm spark "Message from 🤖 dixie (@dixie): disk status?"
    shiva peer dm spark/researcher "..."      # named profile (multiplexed peer)
...
``shiva -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
...
- Peer labels/URLs live in config.yaml (``bot_peers``); the peer's
  API_SERVER_KEY is a credential and lives in ``~/.shiva/.env`` as
  ``SHIVA_PEER_<NAME>_KEY``.
...
def _peer_key_env(name: str) -> str:
    return f"SHIVA_PEER_{name.upper().replace('-', '_')}_KEY"
    ...
    try:
        ...
        if exc.code == 400 and "title" in detail.lower():
            ...
            raise RuntimeError(
                ...
                f"(HTTP 400: {detail}). Update the peer's shiva-agent, or unhide the "
                f"session there: PATCH /api/sessions/<id> {{\"hidden\": false}}."
            ) from exc
        raise
    # Real api_server wraps the row: {"object": "shiva.session", "session": {...}}.
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (shiva peer dm <peer>[/<agent>] ...)")
...
def cmd_peer(args) -> int:
    ...
    if action in ("add", "set"):
        ...
        if key:
            ...
            print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.shiva/.env")
        else:
            print(
                f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
                f"  shiva peer add {name} --url {url} --key <key>\n"
                f"  (or add {_peer_key_env(name)}=<key> to ~/.shiva/.env)"
            )
        return 0
    ...
    if action in ("list", "ls", None):
        peers = _load_peers()
        if not peers:
            print("No peers registered. Add one: shiva peer add <name> --url http://host:port --key <API_SERVER_KEY>")
            return 0
    ...
    if action == "dm":
        ...
        if not isinstance(peer, dict) or not peer.get("url"):
            print(f"No peer named '{peer_name}'. Run: shiva peer list", file=sys.stderr)
            return 1
        key = _peer_secret(peer_name)
        if not key:
            print(
                f"No API key for peer '{peer_name}'. Set it: shiva peer add {peer_name} "
                f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.shiva/.env)",
                file=sys.stderr,
            )
            return 1
    ...
    print("Unknown peer action. See: shiva peer --help", file=sys.stderr)
    return 2
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer",
        help="Bot-to-bot DMs across machines (peer Shiva gateways)",
        description=(
            "Register other Shiva gateways as peers and message their agents. "
            "'shiva peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'shiva -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.shiva/.env."
        ),
        epilog=(
            "Examples:\n"
            "  shiva peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  shiva peer list\n"
            '  shiva peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  shiva peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  shiva peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.shiva/.env)")
