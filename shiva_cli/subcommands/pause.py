"""``shiva pause`` / ``shiva resume`` — the global emergency stop.

``shiva pause`` writes the ESTOP sentinel at ``$SHIVA_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``shiva resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Shiva paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `shiva resume` to lift the pause."
    )
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Shiva resumed — dispatch picks up on the next tick.")
    else:
        print(f"Shiva is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`shiva resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `shiva pause`",
