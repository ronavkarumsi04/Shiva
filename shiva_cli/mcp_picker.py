"""MCP picker — interactive `shiva mcp picker` (also the default `shiva mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``shiva mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `shiva plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from shiva_cli.cli_output import prompt_yes_no
from shiva_cli.curses_ui import curses_single_select
from shiva_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Shiva session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from shiva_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `shiva mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: shiva mcp install <name>    Picker: shiva mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Shiva is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Shiva — run `shiva update` "
...
def show_catalog() -> None:
    """`shiva mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`shiva mcp picker` (and default `shiva mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`shiva mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `shiva mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
