"""Slash command definitions and autocomplete for the Shiva CLI.
...
from shiva_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``shiva_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``shiva_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Shiva config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Shiva works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Shiva' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Shiva is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.shiva/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Shiva Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Shiva Agent version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`shiva_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/shiva`` subcommand mapping, and
    ...
    """
    try:
        from shiva_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
# Telegram allows up to 100 BotCommands. Shiva ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Shiva
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``shiva skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``shiva skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``shiva skills list`` and
...
# unrelated command landed. These claim their slots right after /shiva,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /shiva <command>).
...
# native slot, the alias spelling stays reachable via /shiva reset).
...
# ``/shiva <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/shiva" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /shiva topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /shiva moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /shiva debug on Slack.
#   - egress: Docker-only proxy status; reachable as /shiva egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /shiva init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /shiva version on
...
#   - diff: git working-tree diff; reached via /shiva diff on Slack so it
...
#     /shiva update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /shiva heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /shiva refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /shiva pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /shiva whoami on Slack.
...
#     /shiva platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_SHIVA_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "review", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/shiva <verb>`` subcommand.
    ...
    can still reach them via ``/shiva <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/shiva`` is always reserved as the first entry so the
    legacy ``/shiva <subcommand>`` form keeps working for anything that
    ...
    # Reserve /shiva as the catch-all top-level command.
    entries.append(("shiva", "Talk to Shiva or run a subcommand", "[subcommand] [args]"))
    seen.add("shiva")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_SHIVA_ONLY:
            # Intentionally Slack-via-/shiva only (see _SLACK_VIA_SHIVA_ONLY).
            return
    ...
    # everything except /shiva, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://shiva-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /shiva handler.

    Maps both canonical names and aliases so /shiva bg do stuff works
    the same as /shiva background do stuff.

    Plugin-registered slash commands are included so ``/shiva <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from shiva_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via shiva_cli.personality."""
        try:
