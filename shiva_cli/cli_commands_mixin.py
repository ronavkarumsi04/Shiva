...
``cli.py``'s ``ShivaCLI`` class. ``ShivaCLI`` inherits ``CLICommandsMixin`` so
...
from shiva_constants import display_shiva_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        Syntax:
            ...
            /rollback <N> --all       — classic full restore (may overwrite
                                        files you edited after Shiva did)
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: shiva --checkpoints")
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Shiva changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Shiva hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Shiva config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_shiva_home()}/state-snapshots/):\n")
    ...
    def _handle_export_command(self, command: str):
        ...
        """
        from shiva_cli.profiles import export_profile, get_active_profile_name
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `shiva profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        try:
            ...
            pass
        print(f"  Use it: shiva -p {imported}")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `shiva journey`).
        ...
        from cli import _cprint
        from shiva_cli.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from shiva_cli.clipboard import has_clipboard_image
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from shiva_cli.clipboard import (
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run shiva chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in shiva_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `shiva gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `shiva sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from shiva_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /sessions or `shiva sessions list` to see available sessions.")
            return
        ...
        # same directory as a startup `shiva -c`/`--resume`. The startup resume
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        commits, same as ``shiva -w``).

        ``prune`` is the same attended reclaim as ``shiva worktree prune``
        (shiva_cli/worktree_gc.py): never deletes tracked changes, unique
        ...
        if sub in {"prune", "gc", "clean"}:
            ...
            from shiva_cli import worktree_gc
        ...
        if sub in {"new", "add", "create"}:
            ...
            # same way `shiva -w` and session-resume cwd restore do.
            try:
                os.chdir(wt_info["path"])
            except OSError as e:
                print(f"  ⚠ Created worktree but could not enter it: {e}")
            os.environ["TERMINAL_CWD"] = wt_info["path"]
            # Register for the same keep-if-unpushed cleanup as `shiva -w`.
    ...
    def _handle_branch_command(self, cmd_original: str) -> None:
        ...
        try:
            self._session_db.create_session(
                ...
                source=os.environ.get("SHIVA_SESSION_SOURCE", "cli"),
                model=self.model,
                model_config={
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        if len(parts) > 1:
            ...
            if not name:
                ...
                try:
                    from shiva_cli.config import cfg_get, read_raw_config
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from shiva_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_hatch_command(self, cmd: str):
        ...
        from shiva_cli.pets import _set_active
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from shiva_cli.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from shiva_cli.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to shiva_cli.curator so the CLI and the `shiva curator`
        ...
        try:
            from shiva_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to shiva_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            if out is not None:
                print(out)
                return
        from shiva_cli.skills_hub import handle_skills_slash
    ...
    def _handle_init_command(self, cmd: str):
        ...
        """
        from shiva_cli.init_command import build_init_prompt_for_cwd
    ...
    def _handle_background_command(self, cmd: str):
        ...
        def run_background():
            ...
            try:
                ...
                if response:
                    try:
                        from shiva_cli.skin_engine import get_active_skin
                        _skin = get_active_skin()
                        label = _skin.get_branding("response_label", "⚕ Shiva")
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``shiva bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: shiva bundles create "
        ...
        _cprint(
            ...
            f"Manage with `shiva bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Shiva-managed isolated debug "
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `shiva cron`.
        """
        from cli import _DIM, _RST, _cprint
        from shiva_cli.heartbeat import parse_interval, format_interval
        ...
        if interval is None:
            ...
            return
        if interval < 0:
            from shiva_cli.heartbeat import MIN_INTERVAL_SECONDS
        ...
        _cprint(
            ...
            f"lives only while this Shiva process runs — use `shiva cron` for "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Shiva keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_loop_command(self, cmd: str) -> None:
        ...
        from shiva_cli.loops import dispatch_loop_command
        ...
        if result.get("created"):
            try:
                from shiva_cli.loops import goal_blocks_loop_tick
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from shiva_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_shiva_home()}/skins/\n")
            return
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="shiva_prompt_")
        try:
    ...
    def _handle_focus_command(self, cmd_original: str) -> None:
        ...
        from shiva_cli.colors import Colors as _Colors
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from shiva_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from shiva_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from shiva_cli.focus_view import format_hidden_line
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Shiva is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Shiva is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while Shiva is busy; /stop still cancels it."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from shiva_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Shiva Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``shiva update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Shiva Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Shiva Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Shiva Agent",
            detail="This will exit the current session and run `shiva update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Shiva' hotword listener.
