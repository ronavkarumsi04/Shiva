...
Add, remove, or reorder entries here — both `shiva setup` and
`shiva` provider-selection will pick up the change automatically.
"""
...
from shiva_cli.urllib_security import open_credentialed_url, url_origin
...
_SHIVA_USER_AGENT = f"shiva-cli/{_SHIVA_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from shiva_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `shiva model`
    flow without maintaining a separate static list.
    """
    from shiva_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# $SHIVA_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Shiva-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $SHIVA_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # "opencode/latest"; we send honest Shiva attribution and don't
    ...
    # endpoint expects (see shiva_cli/model_setup_flows.py).
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Shiva that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Shiva that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_shiva_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$SHIVA_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  shiva model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `shiva model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `shiva model` TUI
...
#
# Some vendors expose several Shiva provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `shiva model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``shiva model``, the
...
# In-repo fallback for the model Shiva silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from shiva_cli.model_catalog import get_default_model_from_cache
...
# ``shiva model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in shiva_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``shiva auth add openai-codex`` without
    ``shiva model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``shiva model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    shiva-agent is tool-calling-first — every provider path assumes the model
...
# copy, `shiva -p`, a cron job, or a freshly booted gateway answers
...
def _reasoning_caps_disk_path() -> Path:
    from shiva_constants import get_shiva_home
    return get_shiva_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _SHIVA_USER_AGENT}
    try:
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from shiva_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — shiva-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Shiva lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from shiva_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from shiva_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return shiva-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; shiva's picker expects
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    env_base = None
    try:
        from shiva_cli.auth import _nous_inference_env_override
    ...
    creds_base = ""
    try:
        from shiva_cli.auth import resolve_nous_runtime_credentials
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from shiva_cli.providers import custom_provider_slug
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``shiva model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from shiva_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _SHIVA_USER_AGENT}
...
def _resolve_static_model_alias(
    ...
    try:
        from shiva_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Shiva' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``shiva_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Shiva should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``shiva auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.shiva/.env``.
    ...
    """
    try:
        from shiva_cli.auth import resolve_api_key_provider_credentials
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from shiva_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Shiva release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        from shiva_cli.codex_models import get_codex_model_ids
        ...
        # a Shiva release). Falls back to the hardcoded catalog if no token
        # or the endpoint is unreachable.
        access_token = None
        try:
            from shiva_cli.auth import resolve_codex_runtime_credentials
    ...
    if normalized == "nous":
        ...
        try:
            from shiva_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Shiva release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``shiva model`` shows.
            from shiva_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $SHIVA_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from shiva_constants import get_shiva_home
    return get_shiva_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$SHIVA_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `shiva config set
...
def copilot_default_headers(*, is_agent_turn: bool = True) -> dict[str, str]:
    ...
    """
    try:
        from shiva_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=is_agent_turn)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "ShivaAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _SHIVA_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Shiva' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Shiva' native Anthropic adapter. The live catalog may
...
def opencode_zen_free_headers() -> dict:
    ...
    """
    try:
        from shiva_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
        "HTTP-Referer": "https://shiva-agent.nousresearch.com",
        "X-Title": "Shiva Agent",
        "User-Agent": f"ShivaAgent/{_v}",
    }
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _SHIVA_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"shiva-agent/{_SHIVA_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=shiva"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from shiva_constants import get_shiva_home
    return get_shiva_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `shiva moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Shiva will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Shiva will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Shiva
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Shiva cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models Shiva actually ships.  (#46850)
            #
