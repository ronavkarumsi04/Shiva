"""Offline, non-destructive recovery for a damaged Shiva session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``shiva_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``shiva_state._backup_db_file``.
    """
    from shiva_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Shiva process using this profile and retry. "
                "This includes the interactive `shiva` CLI session this "
                ...
                "Shiva process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `shiva` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from shiva_cli.session_lost_and_found import (
