"""
shiva fallback — manage the fallback provider chain.
...
Subcommands:
  shiva fallback [list]   Show the current fallback chain (default when no subcommand)
  shiva fallback add      Pick provider + model via the same picker as `shiva model`,
                           then append the selection to the chain
  shiva fallback remove   Pick an entry to delete from the chain
  shiva fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.shiva/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from shiva_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `shiva model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  shiva fallback add")
        print()
        return
    ...
    print("  Docs: https://shiva-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `shiva model`, then append the selection to the chain."""
    from shiva_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `shiva model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `shiva fallback list` to view, or `shiva fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``shiva fallback [subcommand]``."""
