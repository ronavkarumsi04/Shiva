"""
Shiva Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.shiva/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Shiva PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing shiva-agent or shiva PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Shiva Agent" comment and following line
                if '# Shiva Agent' in line or '# shiva-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('shiva' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing shiva
                if 'shiva' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Shiva-owned file, and
...
def remove_wrapper_script():
    """Remove the shiva wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "shiva",
        Path.home() / ".local" / "bin" / "shiva-acp",
        Path.home() / ".local" / "bin" / "shiva-agent",
        Path("/usr/local/bin/shiva"),
        Path("/usr/local/bin/shiva-acp"),
        Path("/usr/local/bin/shiva-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains shiva_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'shiva_cli' in content or 'shiva-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(shiva_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``shiva`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Shiva home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (shiva_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``shiva gateway run`` processes
    ...
    try:
        from shiva_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from shiva_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SHIVA_HOME`` and ``SHIVA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\shiva\git\cmd``, ``%LOCALAPPDATA%\shiva\git\bin``,
#      ``%LOCALAPPDATA%\shiva\git\usr\bin``, ``%LOCALAPPDATA%\shiva\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\shiva\git\`` and Node to
#      ``%LOCALAPPDATA%\shiva\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``shiva dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\shiva\gateway-service\`` and sometimes
...
def _shiva_path_markers(shiva_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify Shiva-owned User-PATH entries.

    ``include_managed_bin`` adds the managed binary dir (``<root>\\bin``,
    holding the shiva launchers and the managed uv) — only wanted when
    ...
    """
    root = str(shiva_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare shiva-agent install dir.
    markers = [root + "\\shiva-agent", root + "\\git", root + "\\node", root + "\\venv"]
    if include_managed_bin:
        markers.append(root + "\\bin")
    # Also match if SHIVA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "shiva".  We don't want to catch
    # unrelated entries like "cshiva-foo" or "ephermeral", so we look for
    # backslash-shiva as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(shiva_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Shiva-owned entries from User-scope PATH in the registry.
    ...
    ``include_managed_bin`` adds ``<shiva_home>\\bin`` (the managed binary
    dir holding the shiva launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _shiva_path_markers(shiva_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_shiva_env_vars_windows() -> list[str]:
    """Delete SHIVA_HOME and SHIVA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SHIVA_HOME", "SHIVA_GIT_BASH_PATH"):
                try:
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``shiva`` launchers install.ps1 staged in the managed
    binary dir (the default Shiva root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``shiva`` in a new terminal resolves to a launcher whose target is
    ...
    ``_install_repair._quarantine_running_shiva_exe`` relies on), so
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        from shiva_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from shiva_constants import get_default_shiva_root

        bin_dir = get_default_shiva_root() / "bin"
    except Exception as e:
...
def _is_default_shiva_home(shiva_home: Path) -> bool:
    """Return True when ``shiva_home`` points at the default (non-profile) root."""
    try:
        from shiva_constants import get_default_shiva_root
        return shiva_home.resolve() == get_default_shiva_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SHIVA_HOME directory.

    We shell out to ``shiva -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SHIVA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m shiva_cli.main` so we don't depend on a `shiva`
    #    wrapper that may be half-removed mid-uninstall.
    shiva_invocation = [_sys.executable, "-m", "shiva_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                shiva_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's SHIVA_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``shiva uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$SHIVA_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from shiva_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(shiva_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Shiva Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Shiva Chat GUI installation was found.")
        print(f"  Checked: {shiva_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Shiva agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(shiva_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Shiva agent at {shiva_home / 'shiva-agent'}")
        print(f"  • Your config, sessions, and secrets under {shiva_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Shiva agent is still installed. Run 'shiva' to use the CLI,")
    print("or 'shiva uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.shiva/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.shiva/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie SHIVA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_shiva_home(shiva_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.shiva data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Shiva Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {shiva_home / 'config.yaml'}")
    print(f"  Secrets: {shiva_home / '.env'}")
    print(f"  Data:    {shiva_home / 'cron/'}, {shiva_home / 'sessions/'}, {shiva_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SHIVA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Shiva data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Shiva code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, shiva_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Shiva PATH entries from shell configs / Windows User PATH")
    print("  • Shiva wrapper scripts and Shiva-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Shiva config/data: {shiva_home}")
        if _is_default_shiva_home(shiva_home):
        ...
        print(f"  • Keep Shiva config/data: {shiva_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``shiva`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$SHIVA_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in shiva_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\shiva\git\cmd, not %LOCALAPPDATA%.
        # The managed binary dir (shiva\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_shiva_home(shiva_home)
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(shiva_home))),
        ...
        if removed_path_entries:
            ...
            log_info("No Shiva-owned PATH entries in User environment")

        log_info("Removing SHIVA_HOME / SHIVA_GIT_BASH_PATH User env vars...")
        removed_env = remove_shiva_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Shiva-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing shiva command...")
    ...
    #     would dangle — `shiva` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows shiva launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows shiva launchers found")

    # 3b. Remove node/npm/npx symlinks the installer left in ~/.local/bin
    #     (only when they still point into this Shiva home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Shiva-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(shiva_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Shiva-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(shiva_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE SHIVA_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from shiva_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(shiva_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.shiva/, just remove the shiva-agent subdir
            if shiva_home in project_root.parents or project_root.parent == shiva_home:
    ...
    #     under SHIVA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(shiva_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(shiva_home)
    ...
    # 5. Optionally remove ~/.shiva/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SHIVA_HOME dirs live under
        ...
        try:
            if shiva_home.exists():
                shutil.rmtree(shiva_home)
                log_success(f"Removed {shiva_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {shiva_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {shiva_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://shiva-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://shiva-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Shiva Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m shiva_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import shiva_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``shiva_constants`` + ``shiva_cli.colors``
    (and lazily ``shiva_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m shiva_cli.uninstall")
    parser.add_argument(
        "--mode",
