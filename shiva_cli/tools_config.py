"""
Unified tool configuration for Shiva Agent.

`shiva tools` and `shiva setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.shiva/config.yaml under
...
from shiva_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from shiva_cli.nous_account import format_nous_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``shiva tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`shiva_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from shiva_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _SHIVA_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `shiva tools` → Video Generation, which walks
...
# `shiva tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `shiva tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from shiva_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `shiva tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``shiva tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from shiva_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from shiva_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``shiva tools`` checklist actually offers
    for ``platform``.
    ...
    so ``shiva tools`` never claims to add or remove a toolset the user was
...
from shiva_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Shiva routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set SHIVA_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "SHIVA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "SHIVA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "SHIVA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "SHIVA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "SHIVA_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `shiva model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("SHIVA_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``SHIVA_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Shiva telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Shiva' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $SHIVA_HOME/bin is never on PATH, so a bare which()
    # misses the uv Shiva installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``shiva update`` and
      by ``shiva computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``shiva update`` sets
    ...
    a further ~600s wait on Windows). ``shiva computer-use install
    ...
    ``shiva update`` already prints a contextual line before its update
    ...
    ``shiva update`` and must not inherit the explicit install command's
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `shiva update` calls this
    ...
    override = os.environ.get("SHIVA_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    SHIVA_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails Shiva' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Shiva' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `shiva update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Shiva cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("SHIVA_CUA_DRIVER_CMD", "").strip():
            _print_info(
        ...
        if is_windows and require_confirmed_update:
            ...
            _print_info(
                ...
                "shiva computer-use install --upgrade"
            )
            return False
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `shiva computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `shiva update` (require_confirmed_update=True) keeps the
    ...
    # `shiva computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: shiva computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: shiva computer-use doctor")
            return False
    if ok and before:
        try:
...
# Optional refreshes launched by ``shiva update`` are quiet and unattended.
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    # Unattended refreshes (installer_timeout set by `shiva update`) fail
    ...
    def _reap_after_timeout(proc):
        ...
        ceiling stops bounding anything and ``shiva update`` hangs past its
    ...
    try:
        # When not verbose (e.g. `shiva update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `shiva update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.shiva/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Shiva process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the Shiva-managed ``$SHIVA_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Shiva installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # (PATH -> Homebrew/Shiva-managed node -> npx) rather than a bare
        # shutil.which — Shiva-managed-Node-only setups resolve agent-browser
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/shiva-agent:latest"
            )
            return

        # browser_cmd was already resolved above (same PATH -> Homebrew ->
        # Shiva-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Shiva-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.shiva/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.shiva/config.yaml")
        ...
        # Run the full `shiva auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.shiva/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from shiva_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: shiva auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `shiva auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: shiva auth spotify")
        except Exception as exc:
        ...
        try:
            from shiva_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: shiva plugins enable observability/langfuse")
        _print_info("    Restart Shiva for tracing to take effect.")
        _print_info("    Verify: shiva plugins list")
        ...
        try:
            ...
            _print_info("    Run later: shiva auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `shiva auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: shiva auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: shiva auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``shiva tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``shiva tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: shiva tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``shiva-discord``, which contains those
...
#:
#: Saving ``shiva tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[shiva-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``shiva tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"shiva-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (shiva-acp, shiva-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``shiva-discord``) is an opt-in to the platform's native default-off
    ...
    # explicit config like ``[shiva-cli, a2a]`` must survive filtering just
    ...
    # "shiva-cli" (which include all _SHIVA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[shiva-cli, spotify]`` after enabling Spotify via ``shiva
        ...
        # (e.g. "shiva-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``shiva tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `shiva tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"shiva-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("shiva-")}
    ...
    # shipped as a bundled plugin but user must opt in via `shiva tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `shiva tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by shiva tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `shiva config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `shiva` instead of
    # `shiva-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `shiva update`/`shiva doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `shiva tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (shiva-cli, shiva-telegram, etc.)
    ...
    # Opening `shiva tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `shiva tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``shiva tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``shiva tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from shiva_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``shiva tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from shiva_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (shiva_cli/web_server.py
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `shiva model`).
    if managed_feature:
        from shiva_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``shiva model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``shiva model`` aux-task
    ...
    """
    try:
        from shiva_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `shiva model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `shiva model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `shiva tools` and `shiva setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Shiva Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://shiva-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print(color(f"  Tool configuration saved to {display_shiva_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'shiva' or gateway restart.", Colors.DIM))
    print()
