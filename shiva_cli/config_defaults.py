"""Default configuration data for Shiva Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from shiva_cli/config.py. Must not import from shiva_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["shiva-cli"],
    # SQLite journal mode used by every Shiva database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Shiva server processes.
    ...
    "session": {
        # Per-terminal `shiva -c`: each CLI session drops a breadcrumb file
        # under $SHIVA_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by shiva_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `shiva chat --run-budget N`.
        ...
        # an interactive `shiva gateway restart` must never block for hours
        ...
        # the Shiva-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Shiva
        ...
        # identity slot (SOUL.md). Empty by default. The SHIVA_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Shiva adds a coding operating brief
        ...
        # `shiva status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``SHIVA_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # bot_relay from a short-lived `shiva -p <bot> chat -Q` recipient
        ...
        #             SHIVA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force SHIVA_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Shiva auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Shiva sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Shiva
        ...
        # bundled Shiva image, which drops to the `shiva` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `shiva tools` picker's
    ...
    "browser": {
        ...
        # browser tools) runs on a Shiva-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Shiva' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.shiva/browser-profile/) so copied
        ...
        # approval does it run `shiva browser close-profile` (terminates the
        ...
        "camofox": {
            # When true, Shiva sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Shiva should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``shiva chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.shiva/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: shiva sweeps the checkpoint base at startup
        ...
        # ``shiva checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .shiva.md, or .cursorrules before Shiva applies
    ...
    # Single-query (``shiva -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Shiva truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Shiva' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); shiva =
                                      # Shiva' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Shiva' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Shiva does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``shiva kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``shiva kanban decompose`` and the kanban
        ...
        # ``shiva profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `shiva model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `shiva` (and `shiva chat`) launches by default:
        ...
        # REPL and `--tui` (or SHIVA_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `shiva --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `shiva -c` muscle memory.  Default off so existing
        # users aren't surprised.  SHIVA_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `shiva --tui` drops a one-time hint
        ...
        # model — see shiva_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to SHIVA_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/shiva`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `shiva pets`. Disabled until
        ...
        "pet": {
            ...
            # get_shiva_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see shiva_cli/web_server.py — an event-loop stall
        ...
        # SHIVA_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (SHIVA_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``SHIVA_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``SHIVA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``SHIVA_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``SHIVA_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``SHIVA_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/shiva``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Shiva appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.shiva/cache/piper-voices/
    ...
    # "Hey Shiva" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey shiva",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_shiva" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.shiva/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.shiva/cache/delegation/
    ...
    # response. If not, Shiva feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Shiva auto-pauses the goal and
    ...
    "moa": {
        ...
        # <shiva_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.shiva/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.shiva/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``shiva skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``shiva skills trust`` / ``shiva skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${SHIVA_SKILL_DIR} and ${SHIVA_SESSION_ID} in SKILL.md
        ...
        # (`shiva skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.shiva/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.shiva/.curator_backups/blobs/. Enables `shiva curator ledger`
        # and single-mutation `shiva curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `shiva curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `shiva curator run --consolidate` overrides
        ...
        # normally restored on every `shiva update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `shiva curator purge` deletes
        ...
        # skipped), snapshot ~/.shiva/skills/ into
        # ~/.shiva/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `shiva curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for shiva-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Shiva Agent*" header.
    ...
    # dangerous command. -q runs export SHIVA_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; SHIVA_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Shiva' built-in platform hint for a single messaging platform
    ...
    # ``shiva plugins enable|disable`` and intentionally omitted here so an
    ...
    # stored approval from ~/.shiva/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SHIVA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .shiva config) always require human
        ...
        # longer trigger the startup banner. Add via `shiva doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``shiva_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Shiva to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $SHIVA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via SHIVA_CRON_MAX_PARALLEL env var.
        ...
        # SHIVA_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # SHIVA_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via SHIVA_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `shiva -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `shiva` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `shiva kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     shiva-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Shiva tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.shiva/logs/.
    ...
    # update model picker lists without shipping a shiva-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://shiva-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `shiva model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Shiva provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal SHIVA_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.shiva/sessions/sessions.json entirely.
        ...
        # the SHIVA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.shiva.gateway` that defenses 1-2 don't
        ...
        # `shiva update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``SHIVA_GATEWAY_MAX_STARTS`` / ``SHIVA_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.shiva/.env,
        ...
        # behavior: files must live under the Shiva cache, under
        ...
        # user. Bridged to SHIVA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Shiva
        # cache (~/.shiva/cache/{documents,images,audio,video,screenshots})
        ...
        # to SHIVA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # SHIVA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to SHIVA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.shiva/state.db.
    ...
    "sessions": {
        ...
        # the default of ``shiva sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.shiva/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `shiva sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `shiva update` prints a one-line notice with
        ...
        # ~/.shiva/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to SHIVA_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # SHIVA_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `shiva sessions export`. Checked
    ...
    # ``shiva doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `shiva doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``shiva update`` behaviour.
    "updates": {
        ...
        #     DBs) into <SHIVA_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``shiva backup``-style zip
        #     of SHIVA_HOME into <SHIVA_HOME>/backups/, restorable with
        #     ``shiva import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `shiva update` does with uncommitted local changes to the
        ...
        # When `shiva update` finds the source checkout parked on a feature
        ...
        #                         merge. `shiva update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<SHIVA_HOME>/lsp/bin/`` on first use.
    ...
    # `shiva tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.shiva/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.shiva/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.shiva/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Shiva may use this encrypted
            ...
            # ~/.shiva/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.shiva/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Shiva disables it for our users unless they
        # explicitly opt in here. When false (default), Shiva sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Shiva also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Shiva computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Shiva passes the
    ...
    #
    # Configure with `shiva egress setup`.  Disabled by default — the rest of
    # Shiva works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.shiva/bin/ on
    ...
    # Shiva Desktop (Electron app) launch options. These only affect
    # `shiva desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the SHIVA_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the SHIVA_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Shiva mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'shiva model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Shiva auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Shiva' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Shiva, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @shiva:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SHIVA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #shiva)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: shiva-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'shiva-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Shiva API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Shiva API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Shiva API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SHIVA_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to SHIVA_TOOL_PROGRESS_MODE for backward
    ...
    # SHIVA_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "SHIVA_PREFILL_MESSAGES_FILE": {
