...
1. **Update receipt** — a machine-readable JSON record of what one
   ``shiva update`` run discovered, did, skipped (and why), written to
   ``<SHIVA_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``shiva_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``shiva update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``shiva update`` run."""
...
def _receipt_dir() -> Path:
    ...
    return get_shiva_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``shiva update`` has many early-termination paths (Windows
