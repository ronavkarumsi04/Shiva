...
the tool registry or launching Shiva (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``shiva-cli`` to the non-existent ``shiva``. ``resolve_toolset('shiva')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``shiva-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``shiva`` instead of ``shiva-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `shiva tools` to reconfigure."
        )
    return warnings
