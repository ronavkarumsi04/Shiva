"""Dependency-light venv recovery that runs BEFORE shiva_cli.main's imports.

The ``shiva`` console entry point is ``shiva_cli.main:main``.  Importing
``shiva_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``shiva_cli.env_loader``, ``yaml`` via ``shiva_cli.config``, ...).  In
...
a corrupted venv.  ``shiva_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``shiva_cli.main`` to become
...
#
# ``shiva update`` renames the live ``shiva*.exe`` shims aside
# (``shiva.exe.old.<unix-ms>``) so uv can write replacements. Putting them BACK
# is the safety-critical direction: losing that rename leaves the install with
# no ``shiva`` on PATH, and the command that would repair it IS ``shiva
# update`` (#75584).
#
...
def restore_quarantined_shims(
    ...
    *,
    ...
    Messages go to stderr by default -- the startup sweep runs on EVERY shiva
    invocation, and ``shiva acp`` speaks JSON-RPC on stdout.
    """
    if stream is None:
        stream = sys.stderr
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Shiva already vendors
    (``~/.shiva/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".shiva" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``shiva acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``shiva_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``shiva_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``shiva update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `shiva update`: CLI and Desktop retries preserve
...
# every `shiva` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``shiva update`` (or the self-lock preflight, #83569) left the
    ...
    ``shiva_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``shiva acp`` included).
    ...
    """
    try:
        from shiva_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; shiva will keep working from "
