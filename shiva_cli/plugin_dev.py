"""Runtime-backed validation behind ``shiva plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary SHIVA_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="shiva-plugin-doctor-")
    stack = ExitStack()
    ...
    stack.enter_context(
        patch.dict(
            ...
            {
                "SHIVA_HOME": str(home),
                "SHIVA_BUNDLED_PLUGINS": str(bundled),
                "SHIVA_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    modules_before = {
        ...
        if name == "shiva_plugins" or name.startswith("shiva_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Shiva discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "shiva_plugins" or name.startswith("shiva_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_shiva_home() / "plugins"
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".shiva" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from shiva_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Shiva supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Shiva never auto-installs plugin dependencies; "
    ...
    if schema:
        from shiva_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Shiva' real scanner and registration path."""
    try:
