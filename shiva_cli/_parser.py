"""
Top-level argparse construction for the shiva CLI.
...
# argparse runs (it sets ``SHIVA_HOME`` and strips itself from ``sys.argv``),
...
# tests/shiva_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``shiva --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``shiva_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    shiva                        Start interactive chat
    shiva chat -q "Hello"        Single query mode
    shiva --tui                  Launch the modern TUI (or set display.interface: tui)
    shiva --cli                  Force the classic REPL (overrides display.interface: tui)
    shiva -c                     Resume the most recent session
    shiva -c "my project"        Resume a session by name (latest in lineage)
    shiva --resume <session_id>  Resume a specific session by ID
    shiva --resume latest        Resume the most recent session (same as -c)
    shiva --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    shiva setup                  Run setup wizard
    shiva logout                 Clear stored authentication
    shiva auth add <provider>    Add a pooled credential
    shiva auth list              List pooled credentials
    shiva auth remove <p> <t>    Remove pooled credential by index, id, or label
    shiva auth reset <provider>  Clear exhaustion status for a provider
    shiva model                  Select default model
    shiva fallback [list]        Show fallback provider chain
    shiva fallback add           Add a fallback provider (same picker as `shiva model`)
    shiva fallback remove        Remove a fallback provider from the chain
    shiva config                 View configuration
    shiva config edit            Edit config in $EDITOR
    shiva config set model gpt-4 Set a config value
    shiva gateway                Run messaging gateway
    shiva -s shiva-agent-dev,github-auth
    shiva -w                     Start in isolated git worktree
    shiva gateway install        Install gateway background service
    shiva sessions list          List past sessions
    shiva sessions browse        Interactive session picker
    shiva sessions rename ID T   Rename/title a session
    shiva logs                   View agent.log (last 50 lines)
    shiva logs -f                Follow agent.log in real time
    shiva logs errors            View errors.log
    shiva logs --since 1h        Lines from the last hour
    shiva debug share             Upload debug report for support
    shiva console                Open the safe Shiva command console
    shiva update                 Update to latest version
    shiva dashboard              Start web UI dashboard (port 9119)
    shiva dashboard --stop       Stop running dashboard processes
    shiva dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="shiva",
        description="Shiva Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `shiva chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via SHIVA_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `shiva setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to SHIVA_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.shiva/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Shiva Agent",
    )
    ...
    # top-level parser: when the user writes `shiva -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/shiva_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also SHIVA_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.shiva/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Shiva itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
