"""``shiva slack ...`` CLI subcommands.

Today only ``shiva slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ shiva slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Shiva. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Shiva deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Shiva on Slack's older Assistant messaging
    ...
    """
    from shiva_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Shiva in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Shiva in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Shiva agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``shiva_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$SHIVA_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Shiva")
    ...
    """
    name = getattr(args, "name", None) or "Shiva"
    description = getattr(args, "description", None) or "Your Shiva agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "shiva slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"shiva slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "shiva slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "shiva slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Shiva app\n"
