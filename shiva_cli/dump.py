"""
Dump command for shiva CLI.

Outputs a compact, plain-text summary of the user's Shiva setup
...
from shiva_cli.config import get_shiva_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.shiva/.env.
    ...
    exports. ``shiva debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.shiva_build_sha`` by the Dockerfile's
    ``SHIVA_GIT_SHA`` build-arg (see ``shiva_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``shiva dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from shiva_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(shiva_home: Path) -> int:
...
def _cron_summary(shiva_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = shiva_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"shiva_home:      {display_shiva_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.shiva/.env: a managed
        ...
        # A credential added via `shiva auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `shiva auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["shiva-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(shiva_home)}")
    lines.append(f"  skills:             {_count_skills(shiva_home)}")
