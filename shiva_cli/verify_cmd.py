"""``shiva verify`` — detect a project's run recipe and smoke-test it.
...
``.shiva/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``shiva verify`` satisfies
