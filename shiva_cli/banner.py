...
Pure display functions with no ShivaCLI state dependency.
"""
import json
import logging
import os
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `shiva ... > file`) raises
...
from shiva_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

SHIVA_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# (e.g. nix-built shiva — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/shiva-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/shiva-agent"
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/nousresearch/shiva-agent/"
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "shiva-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `shiva update`,
...
def check_for_updates() -> Optional[int]:
    """Check whether a Shiva update is available.

    Two paths: if ``SHIVA_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = shiva_home / ".update_check"
    embedded_rev = os.environ.get("SHIVA_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # SHIVA_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/shiva/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from shiva_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $SHIVA_HOME/shiva-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = shiva_home / "shiva-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Shiva git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$SHIVA_HOME/shiva-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``shiva_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from shiva_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/shiva-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Shiva checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/shiva-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Shiva Agent v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from shiva_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
    ...
    # UPDATE_AVAILABLE_NO_COUNT: nix-built shiva; we know an update
...
def _banner_snapshot_path() -> Path:
    return get_shiva_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_shiva_home() / ".env"):
            try:
                st = p.stat()
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else SHIVA_CADUCEUS
    except Exception:
        _bskin = None
        _hero = SHIVA_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or shiva setup[/]"
            )
        else:
    ...
    if not _has_native_mcp:
        try:
            from shiva_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from shiva_cli.codex_runtime_switch import get_current_runtime
        from shiva_cli.config import load_config as _load_cfg
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else SHIVA_AGENT_LOGO
        console.print(_logo)
        console.print()
