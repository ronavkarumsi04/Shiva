"""CLI handlers for ``shiva secrets onepassword ...``.
...
the CLI through OS package managers and signed installers, so Shiva expects
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        Panel.fit(
            ...
            "Shiva resolves [cyan]op://vault/item/field[/cyan] references through your\n"
    ...
    console.print(
        "  Map credentials:  [cyan]shiva secrets onepassword set OPENAI_API_KEY "
        "\"op://Private/OpenAI/api key\"[/cyan]\n"
        "  Preview:          [cyan]shiva secrets onepassword sync[/cyan]\n"
        "  Status:           [cyan]shiva secrets onepassword status[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]shiva secrets onepassword setup[/cyan] to enable.")
        return 0
    if binary and not token_set:
        ...
        if who:
            ...
            console.print(
                ...
                "Shiva will warn and skip 1Password on next startup.[/yellow]"
            )
    if not references:
        console.print(
            "\n  [yellow]No references mapped yet.[/yellow]  Add one: "
            "[cyan]shiva secrets onepassword set ENV_VAR \"op://…\"[/cyan]"
        )
    return 0
...
def cmd_set(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "[cyan]shiva secrets onepassword setup[/cyan] to turn it on.[/yellow]"
        )
    return 0
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "run `shiva secrets onepassword setup` to turn it on.[/yellow]"
        )
    return 0
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "`shiva secrets onepassword setup` first.[/yellow]"
        )
        return 1
    ...
    if not references:
        console.print(
            ...
            "`shiva secrets onepassword set ENV_VAR \"op://…\"`.[/yellow]"
        )
        return 0
    ...
    console.print(
        ...
        "[cyan]shiva[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] to export "
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "next Shiva invocation.\n"
        "  Your reference mappings are left in config.yaml — remove them with "
        "[cyan]shiva secrets onepassword remove ENV_VAR[/cyan] if you no longer "
        "need them."
    )
    return 0
