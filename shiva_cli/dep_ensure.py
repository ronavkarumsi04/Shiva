...
from shiva_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import shiva_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $SHIVA_HOME/node is
...
def _has_shiva_agent_browser() -> bool:
    ...
    # install.sh installs globally into $SHIVA_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-ShivaHome", str(get_shiva_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = shiva_subprocess_env(inherit_credentials=False)
