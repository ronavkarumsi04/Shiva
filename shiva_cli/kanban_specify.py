...
Used by ``shiva kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``shiva_cli/goals.py`` — same aux
...
SHIVA_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("SHIVA_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Shiva Agent board.
...
def _profile_author() -> str:
    """Mirror of ``shiva_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("SHIVA_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=SHIVA_KANBAN_SPECIFY_MAX_TOKENS,
