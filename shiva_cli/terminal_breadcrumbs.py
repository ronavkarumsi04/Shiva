"""Per-terminal session breadcrumbs for ``shiva -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$SHIVA_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``shiva -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_shiva_home() / "terminal-sessions"
