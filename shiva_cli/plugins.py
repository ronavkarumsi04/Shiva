...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with shiva-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.shiva/plugins/<name>/``
3. **Project plugins** – ``./.shiva/plugins/<name>/`` (opt-in via
   ``SHIVA_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``shiva_agent.plugins``
   entry-point group.
...
from shiva_cli.config import cfg_get, load_config_readonly
from shiva_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from shiva_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from shiva_cli.plugin_capabilities import (
...
# ---------------------------------------------------------------------------
#
# Set ``SHIVA_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.shiva/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("SHIVA_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When SHIVA_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Shiva loggers.
    """
    ...
    logger.debug(
        "SHIVA_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Shiva' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by shiva_cli.kanban_db when a task
    ...
    # separate `shiva -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `shiva kanban dispatch`),
    ...
    # ``shiva kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: shiva_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
ENTRY_POINTS_GROUP = "shiva_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "shiva_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``shiva_agent.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- shiva-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- shiva-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- shiva-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``shiva:<event>``.
SHIVA_EVENT_NAMESPACE = "shiva"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Shiva home."""
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "shiva", "depends",
}

# Highest manifest schema version this Shiva understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Shiva "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `shiva plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    #              in ~/.shiva/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``shiva plugins list``. For a flat plugin at
    ...
    # ``shiva_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Shiva never
    ...
    # (``shiva plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Shiva paths, traversal syntax, and the security-
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_shiva_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            raise
        from shiva_cli import config as config_mod
        ...
        # threads. Other Shiva config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from shiva_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Return the active Shiva profile name (e.g. ``"default"``).

        Derived from ``SHIVA_HOME`` via
        :func:`shiva_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.shiva/profiles/<name>``, or ``"custom"`` when
        ``SHIVA_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Shiva core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``shiva honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``shiva <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`shiva_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from shiva_cli.dashboard_auth import DashboardAuthProvider
        from shiva_cli.dashboard_auth.registry import (
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        sources run during ``load_shiva_dotenv()`` startup — after
        ``~/.shiva/.env`` loads, before Shiva reads credentials — when
        ...
        NOTE ON TIMING: ``load_shiva_dotenv()`` usually runs at import
        *before* plugin discovery.  After discovery completes, the plugin
        ...
        + ``load_shiva_dotenv``) so the first process sees them (#64177).
    ...
    def register_slack_action_handler(
        ...
        Shiva' Slack adapter wires registered handlers into its
    ...
    def register_telegram_handler(self, factory: Callable) -> None:
        ...
        Shiva' Telegram adapter invokes registered factories at ``connect()``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``shiva model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: shiva_cli.main imports plugins indirectly
        from shiva_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``shiva:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``shiva:``
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, SHIVA_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{SHIVA_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``shiva:<event>``
    ...
    def register_skill(
        ...
        ``~/.shiva/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or shiva_home_key()
        ...
        # by resolved shiva home). The ledger is therefore keyed per manager
        # — i.e. per (shiva_home, plugin_id) — and every release/restore
        ...
        # contributed, so `shiva plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if force:
                ...
                self.unload()
            if env_var_enabled("SHIVA_SAFE_MODE"):
                logger.info("SHIVA_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_shiva_dotenv() already ran at import time. Re-pull so the
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from shiva_cli.env_loader import load_shiva_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Shiva plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Shiva core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Shiva core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Shiva Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # Built-in backends auto-load — they ship with shiva and must
            ...
            # all ~20 of them added several seconds to every `shiva`
            # invocation — including plain `shiva chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Shiva ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `shiva plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from shiva_cli.approval_transport import RegisteredApprovalTransport
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_shiva_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_shiva_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.shiva/plugins/)
        user_dir = get_shiva_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.shiva/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("SHIVA_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".shiva" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set SHIVA_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("SHIVA_SAFE_MODE"):
            return False
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from shiva_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from shiva_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Shiva process (e.g. a pip
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        ...
        declarations from the ``shiva_agent.plugin_capabilities`` group.
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``shiva plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`shiva plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``shiva tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `shiva plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Shiva validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Shiva does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `shiva plugins list` would
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from shiva_cli.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_shiva_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        """Import a directory-based plugin as ``shiva_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``shiva_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
        ...
        # from a different Shiva home (profile switch reusing a slug
        # like "shiva-lcm") or from an earlier force=True reload in the
            ...
            try:
                if use_timeout:
                    ...
                    thread = threading.Thread(
                        target=_runner,
                        name=f"shiva-hook-{callback_name}"[:40],
                        daemon=True,
                    )
                    thread.start()
...
# Keyed cache: resolved Shiva home -> PluginManager. Shiva supports
# multiple profiles via different SHIVA_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_shiva_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# shiva_constants.set_shiva_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_shiva_home() / "plugins"`` and some
    plugins (notably context engines such as shiva-lcm) capture that home
    at registration time for profile-scoped storage. A long-lived process
    can temporarily switch Shiva home (env var *or* the context-local
    ``set_shiva_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Shiva home instead of
    being one process-wide singleton.
    """
    try:
        return get_shiva_home().expanduser().resolve()
    except Exception:
        return get_shiva_home().expanduser()
...
def _clear_plugin_submodules(manager: Optional[PluginManager]) -> None:
    ...
    ``shiva_plugins.<slug>`` and registers that top-level module in
    ...
    *relative* import (``from . import foo``, ``from .sub import bar``)
    ...
    ``shiva_plugins.<slug>.<submodule>``. When we swap in a fresh manager
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Shiva profile/home.
    ...
    profile switch — via ``SHIVA_HOME`` or the context-local
    ``set_shiva_home_override()`` — gets its own manager with its own
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=shiva_home_key(current_home))
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_shiva_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from shiva_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="shiva-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``shiva:<event>``) to subscriber callbacks in
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``shiva tools`` TUI so plugin-provided toolsets appear
