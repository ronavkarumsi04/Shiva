...
manual BotFather token copy-paste. Shiva talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.shiva-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "ShivaSetupBot"

DEFAULT_BOT_NAME = "Shiva Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``shiva_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``shiva_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"shiva_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Shiva Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Shiva Telegram onboarding service.")
