...
Profile-aware location: ``$SHIVA_HOME/logs/dashboard-auth.log``.
...
from ``shiva_constants`` or other shiva_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$SHIVA_HOME/logs/dashboard-auth.log``.

    Uses ``shiva_constants.get_shiva_home()`` (a leaf module — no import
    ...
    return get_shiva_home() / "logs" / "dashboard-auth.log"
