...
Three cookies in play:
  ...
  - shiva_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/shiva) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/shiva`` covers
...
setter wrote ``__Secure-shiva_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "shiva_session_at"
SESSION_RT_COOKIE = "shiva_session_rt"
...
SESSION_PROVIDER_COOKIE = "shiva_session_provider"
PKCE_COOKIE = "shiva_session_pkce"
...
SSO_ATTEMPT_COOKIE = "shiva_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /shiva`` we want ``Path=/shiva`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (shiva #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/shiva``)
