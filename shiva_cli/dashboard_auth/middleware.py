...
from shiva_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /shiva``, the
    ``login_url`` is prefixed (``/shiva/login?next=...``) so the
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["shiva.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``shiva_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``shiva_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from shiva_cli.dashboard_auth.cookies import clear_session_cookies
        from shiva_cli.dashboard_auth.prefix import prefix_from_request
