"""Unified provider-credential lifecycle across every store Shiva reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.shiva/.env``                     — the canonical secret store
    2. ``~/.shiva/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.shiva/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `shiva auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``shiva auth add``.
    """
    from shiva_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from shiva_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from shiva_cli.config import load_env, remove_env_value
