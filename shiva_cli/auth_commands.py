...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_shiva_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:shiva_pkce",
    ...
    if provider == "nous":
        ...
        # <shiva-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `shiva --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``shiva auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``shiva auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`shiva auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Shiva reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `shiva auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `shiva auth` is called bare."""
