"""Safe Shiva Console command engine.

This module backs ``shiva console`` and is intentionally narrower than the
full Shiva CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["ShivaConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'shiva doctor'")
        and last.startswith("Run 'shiva setup'")
    ):
        return text.rstrip()
...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from shiva_cli._subprocess_compat import split_command_line
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="shiva", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `shiva "):
        return ""
    return summary
...
# dashboard opens a fresh ShivaConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("shiva_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["ShivaConsoleEngine", list[str]], str]:
    def handler(_engine: ShivaConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    engine: "ShivaConsoleEngine",
    *,
    root: str,
    paths: Iterable[Sequence[str]],
    handler_factory: Callable[[Sequence[str]], Callable[["ShivaConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `shiva {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `shiva {usage}`?",
        )


class ShivaConsoleEngine:
    """Curated line-command executor for Shiva Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Shiva Console does not run shell syntax. Use one supported "
                    "Shiva command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Shiva component status.", _status)
        self.register(("version",), "version", "Show Shiva version information.", _version)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Shiva logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Shiva configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Shiva Console."""

        extracted = {
            "dump": (
                "shiva_cli.subcommands.dump",
                "build_dump_parser",
                "cmd_dump",
                [()],
                set(),
            ...
            "prompt-size": (
                "shiva_cli.subcommands.prompt_size",
            ...
            "insights": (
                "shiva_cli.subcommands.insights",
            ...
            "security": (
                "shiva_cli.subcommands.security",
            ...
            "import": (
                "shiva_cli.subcommands.import_cmd",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Shiva configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "shiva_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("shiva_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "shiva_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("shiva_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("shiva_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Shiva Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Shiva Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`shiva {first}` is not available in Shiva Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Shiva Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Shiva Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Shiva Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Shiva Console.",
            ("skills", "publish"): "`skills publish` is not available in Shiva Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Shiva Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Shiva Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Shiva Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Shiva Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Shiva Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Shiva Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Shiva Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Shiva Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Shiva Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Shiva Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Shiva Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Shiva Console.")
...
def _version(_engine: ShivaConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "version")
    from shiva_cli._startup_fast import print_fast_version_info

    return _capture_output(lambda: print_fast_version_info(check_updates=True))


def _status(_engine: ShivaConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: ShivaConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: ShivaConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Shiva Console.")
    ...
    from shiva_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: ShivaConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: ShivaConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: ShivaConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from shiva_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: ShivaConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from shiva_cli.config import get_config_path

    return str(get_config_path())


def _config_set(_engine: ShivaConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
...
def _config_migrate(_engine: ShivaConsoleEngine, args: list[str]) -> str:
...
def _sessions_export(_engine: ShivaConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: ShivaConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: ShivaConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: ShivaConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from shiva_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: ShivaConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="shiva_cli.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: ShivaConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from shiva_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: ShivaConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from shiva_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: ShivaConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from shiva console")
...
def _cron_resume(_engine: ShivaConsoleEngine, args: list[str]) -> str:
    ...
    from cron.jobs import AmbiguousJobReference, _shiva_now, rearm_oneshot, resume_job

    try:
        if ns.at or ns.run_now:
            job = rearm_oneshot(ns.job, _shiva_now().isoformat() if ns.run_now else ns.at)
        else:
...
def _cron_run(_engine: ShivaConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``shiva console`` REPL."""
    ...
    engine = ShivaConsoleEngine()
    if interactive:
        print("Shiva Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("shiva> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
