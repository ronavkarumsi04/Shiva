...
configured for "cli" in `shiva tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (SHIVA_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `shiva chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"shiva -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "shiva -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"shiva -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "shiva -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "shiva -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to SHIVA_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("SHIVA_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "shiva -z: --provider requires --model (or SHIVA_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"shiva -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("shiva -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``shiva -z`` / oneshot mode.

    Oneshot bypasses ``ShivaCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when shiva is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from shiva_cli.config import load_config
    from shiva_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("SHIVA_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from shiva_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("SHIVA_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # ShivaCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    from shiva_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                SHIVA_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via SHIVA_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via SHIVA_YOLO_MODE=1
