...
this. Capabilities govern the *host API surfaces* Shiva hands out (which
...
until the user re-consents (``shiva plugins update`` surfaces the diff).
...
def parse_declared_capabilities(raw: Any, plugin_name: str = "?") -> List[str]:
    ...
    for a newer Shiva may declare ids this build doesn't know; they can
