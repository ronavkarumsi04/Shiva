...
on Japanese locales, etc.).  Shiva's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``shiva_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Shiva is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SHIVA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SHIVA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Shiva-managed Git install directories so
    ...
    # already-running shells don't see the change — which means shiva
    ...
    # install without Shiva-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Shiva's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Shiva's
    ``shiva config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Shiva (User env var in Windows Settings, or
    export in a PowerShell profile) and Shiva picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Shiva-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\shiva\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``shiva`` from right after install) retain their old PATH.

    Any subprocess Shiva spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Shiva-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "shiva", "git", "cmd"),
        os.path.join(local_appdata, "shiva", "git", "bin"),
        os.path.join(local_appdata, "shiva", "git", "usr", "bin"),
        # Shiva venv Scripts directory — host of the shiva.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes shiva, but harmless to include.
        os.path.join(local_appdata, "shiva", "shiva-agent", "venv", "Scripts"),
