...
This module is imported by ``shiva_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no shiva_cli.config,
...
``shiva_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(shiva_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect SHIVA_HOME?"""
    active_profile = os.path.join(shiva_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    shiva_home = os.environ.get("SHIVA_HOME", "").strip()
    if shiva_home:
        return shiva_home
    return os.path.join(os.path.expanduser("~"), ".shiva")
...
def container_mode_may_be_active() -> bool:
    ...
    shiva_home = os.environ.get("SHIVA_HOME", "").strip()
    if shiva_home:
        if os.path.exists(os.path.join(shiva_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(shiva_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(shiva_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".shiva")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect SHIVA_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``shiva --version`` output (also used by /version).
    ...
    ``shiva version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        from shiva_cli.banner import format_banner_version_label

        print(format_banner_version_label())
    except Exception:
        from shiva_cli import __release_date__, __version__

        print(f"Shiva Agent v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from shiva_cli.config import detect_install_method
    ...
    try:
        from shiva_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``shiva --version`` before the heavy import wall.
    ...
    container. Termux keeps the SHIVA_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("SHIVA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
