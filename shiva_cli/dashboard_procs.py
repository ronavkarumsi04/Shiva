"""Dashboard process-hygiene helpers — extracted from ``shiva_cli/main.py``.
...
``_detect_concurrent_shiva_instances``) are lifted verbatim. References to
helpers that STAY in ``shiva_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``shiva_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``shiva_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``shiva_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``shiva dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``shiva update`` replaces files on disk, the running
    ...
    ``shiva-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Shiva Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``shiva serve`` as
    ...
    environment variable ``SHIVA_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`shiva serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "shiva.*dashboard"`) keeps us consistent
            # with `shiva_cli.gateway._scan_gateway_pids` and avoids the
    ...
    # miss profiled launches — `shiva --profile p serve --host <ip>` contains
    # neither "shiva serve" nor "shiva_cli.main serve". Every serve/
    ...
    # truncated view.
    try:
        from shiva_cli.process_identity import ledger_entries
...
def _shiva_home_for_pid(pid: int) -> str | None:
    """Best-effort ``SHIVA_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("SHIVA_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"SHIVA_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Shiva Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``shiva update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], shiva_home: str | None = None
) -> str:
    """Stable owner key: ``SHIVA_HOME`` when known, else ``--profile`` / ``-p``.

    ``SHIVA_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.shiva`` roots) keep a resolved ``home:`` key so
    ...
    if shiva_home:
        try:
            home_path = Path(shiva_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(shiva_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(shiva_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    *,
    ...
    """Select which killed manual backends to respawn after ``shiva update``.

    Each candidate is ``(pid, argv, shiva_home)``.  *own_home* is the
    updating install's home; it defaults to this process's
    ``get_shiva_home()`` and exists as a parameter so tests can pin it.

    Rules (#78821, #94030):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``SHIVA_DESKTOP_CHILD_PID``) owns their
       lifecycle.  These are also the PPID-1 orphans that previously
       multiplied across updates because ``--port 0`` always binds a
       fresh free port.
    2. Never replay a backend from a **foreign** ``SHIVA_HOME``.  The
    ...
    4. Cap at most one managed backend per profile / ``SHIVA_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``shiva update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, shiva_home in candidates:
        if not argv:
            continue
        if _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and shiva_home and _normalized_home_for_compare(shiva_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, shiva_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``shiva dashboard`` / ``shiva serve`` processes.

    Called at the end of ``shiva update`` (default ``reason``) and also
    from ``shiva dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``shiva update`` path), a detected
    ``shiva-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``shiva-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``shiva update``'s systemd
    fleet-restart loop, which restarts ``shiva-serve*`` units before this
    ...
    # When the Shiva Desktop Electron app spawns this dashboard as a
    # backend child, it sets SHIVA_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] = set()
    raw_pid = os.environ.get("SHIVA_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `shiva update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot SHIVA_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _shiva_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. shiva update's
    ...
    #    remote backend (shiva serve) under Restart=on-failure never comes
...
def _detect_concurrent_shiva_instances(
    ...
    The Shiva Desktop Electron app spawns ``shiva.EXE`` as a backend child,
    so during ``shiva update`` the user-invoked process and the desktop's
    ...
    shims (``shiva.exe`` / ``shiva-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``shiva update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``shiva.exe``) is a separate process from the Python
    ...
    for shim in _m()._shiva_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated shiva.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``shiva update``
    ...
    #      shiva.exe sitting *under* a non-Shiva parent (e.g. a Shiva
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        shiva serve --host 127.0.0.1 --port 0
        shiva serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "shiva" not in cmd and "shiva_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (SHIVA_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("SHIVA_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``shiva serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``shiva serve`` on a Mac
...
def _shiva_home_dir() -> Path:
    """Resolved Shiva home (SHIVA_HOME override or ~/.shiva)."""
    override = os.environ.get("SHIVA_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "shivaPath", "shivaHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated SHIVA_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{shiva_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _shiva_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local shiva serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    ...
    """Kill leftover Desktop-local ``shiva serve`` backends with no parent.
    ...
    running under SHIVA_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never SHIVA_DESKTOP_CHILD_PID entries
    ...
    - never a candidate younger than ``_REAP_MIN_AGE_SECONDS`` (or whose age
      ...
      only after the backend reports SHIVA_BACKEND_READY, so during
    ...
    for pid, cmd in scanned:
        ...
        # SHIVA_BACKEND_READY, so a sibling spawned seconds ago is not yet
