"""`shiva checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.shiva/checkpoints/``.  Actions:

    shiva checkpoints               # same as `status`
    shiva checkpoints status        # total size, project count, breakdown
    shiva checkpoints list          # per-project checkpoint counts + workdir
    shiva checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    shiva checkpoints clear [-f]    # nuke the entire base (asks first)
    shiva checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    shiva checkpoints
    shiva checkpoints prune --retention-days 3 --max-size-mb 200
    shiva checkpoints clear -f
...
from shiva_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: shiva checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``shiva checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `shiva checkpoints` → status
