"""shiva claw — OpenClaw migration commands.

Usage:
    shiva claw migrate              # Preview then migrate (always shows preview first)
    shiva claw migrate --dry-run    # Preview only, no changes
    shiva claw migrate --yes        # Skip confirmation prompt
    shiva claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    shiva claw migrate --no-backup  # Skip pre-migration snapshot
    shiva claw cleanup              # Archive leftover OpenClaw directories
    shiva claw cleanup --dry-run    # Preview what would be archived
"""
...
from shiva_cli.config import get_shiva_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Shiva may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Shiva gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Shiva gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'shiva gateway stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_shiva.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_shiva", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: shiva claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Shiva")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'shiva claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Shiva migration."""
    ...
    # migrate-shiva posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: shiva claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {shiva_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.shiva/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: shiva claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Shiva home ─────────
    # Delegates to shiva_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `shiva import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from shiva_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(shiva_home=shiva_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: shiva import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Shiva home."
            )
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: shiva claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: shiva claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  shiva claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  shiva claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  shiva config set OPENROUTER_API_KEY sk-or-v1-...")
