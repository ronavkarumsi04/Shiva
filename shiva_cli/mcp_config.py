"""
MCP Server Management CLI — ``shiva mcp`` subcommand.

Implements ``shiva mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.shiva/config.yaml under the ``mcp_servers`` key.
"""
...
from shiva_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_shiva_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from shiva_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.shiva/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``shiva mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import ShivaTokenStorage
        return ShivaTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # shiva_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  shiva mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  shiva mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  shiva mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_shiva_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: shiva mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_shiva_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `shiva mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  shiva mcp add <name> --url <endpoint>')
        _info('  shiva mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``shiva mcp login`` and
    ``shiva mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `shiva mcp remove` + `shiva mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `shiva mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Shiva desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `shiva mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``shiva mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``shiva mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: shiva mcp reauth <name>   |   shiva mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'shiva mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``shiva mcp`` subcommands."""
    ...
    if action == "picker":
        from shiva_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from shiva_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from shiva_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `shiva plugin`.
        from shiva_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("shiva mcp                                    Open the catalog picker (default)")
        _info("shiva mcp catalog                            List Nous-approved MCPs")
        _info("shiva mcp install <name>                     Install a catalog MCP")
        _info("shiva mcp serve                              Run as MCP server")
        _info("shiva mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("shiva mcp add <name> --command <cmd>         Add a stdio server")
        _info("shiva mcp add <name> --preset <preset>       Add from a known preset")
        _info("shiva mcp remove <name>                      Remove a server")
        _info("shiva mcp list                               List configured servers")
        _info("shiva mcp test <name>                        Test connection")
        _info("shiva mcp configure <name>                   Toggle tools")
        _info("shiva mcp login <name>                       Re-authenticate OAuth")
        _info("shiva mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
