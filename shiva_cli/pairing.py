...
Usage:
    shiva pairing list              # Show all pending + approved users
    shiva pairing approve <platform> <request-id|code>  # Approve a pairing request
    shiva pairing revoke <platform> <user_id> # Revoke user access
    shiva pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle shiva pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: shiva pairing {list|approve|revoke|clear-pending}")
        print("Run 'shiva pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: shiva pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.shiva/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'shiva pairing list' to see pending requests.\n")
