...
_TASK_NAME_DEFAULT = "Shiva_Gateway"
_TASK_DESCRIPTION = "Shiva Agent Gateway - Messaging Platform Integration"
...
def _preserve_shiva_home_path(path: str | Path) -> str:
    """Render Shiva-owned paths under the configured SHIVA_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\shiva`` as a symlink/junction to
    ...
    path lives under SHIVA_HOME.
    """
    candidate = Path(path)
    try:
        from shiva_cli.config import get_shiva_home

        home = Path(get_shiva_home())
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Shiva profile."""
    from shiva_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "shiva_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("SHIVA_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("SHIVA_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("SHIVA_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["SHIVA_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["SHIVA_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["SHIVA_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("SHIVA_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("SHIVA_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("SHIVA_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Shiva_Gateway``
    Named profile X: ``Shiva_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during shiva_cli boot.
    from shiva_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\shiva\\gateway-service\\<task_name>.cmd``
    (or ``<SHIVA_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Shiva installs stay self-contained).
    """
    _assert_windows()
    from shiva_cli.config import get_shiva_home

    script_dir = Path(get_shiva_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``SHIVA_HOME`` whenever it
    ...
    source checkout only if ``SHIVA_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports SHIVA_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m shiva_cli.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``shiva gateway run``.
    ...
    lines.append(f'set "SHIVA_HOME={shiva_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "SHIVA_GATEWAY_DETACHED=1"')
    python_exe_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports shiva_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_shiva_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_shiva_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_shiva_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [python_exe_path, "-m", "shiva_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_shiva_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_shiva_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('SHIVA_HOME')}) = {_quote_vbs_string(shiva_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('SHIVA_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_shiva_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_shiva_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    shiva_home = str(Path(get_shiva_home()))
    profile_arg = _profile_arg(shiva_home)

    content = _build_gateway_cmd_script(python_path, working_dir, shiva_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, shiva_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Shiva builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_shiva_home_path(get_python_path())
    )
    project_root = _preserve_shiva_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    shiva_home = str(Path(get_shiva_home()))
    profile_arg = _profile_arg(shiva_home)

    argv = [python_exe, "-m", "shiva_cli.main"]
    if profile_arg:
    ...
    env_overlay = {
        ...
        "SHIVA_GATEWAY_DETACHED": "1",
        "VIRTUAL_ENV": _preserve_shiva_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_shiva_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (SHIVA_HOME, VIRTUAL_ENV,
    PYTHONPATH) so the respawn doesn't depend on the watcher's transient
    working directory.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m shiva_cli.main
    ...
    from shiva_cli.config import get_shiva_home
    from shiva_cli.gateway import PROJECT_ROOT
    ...
    try:
        shiva_home = str(Path(get_shiva_home()).resolve())
    except Exception:
        shiva_home = ""
    ...
    if shiva_home:
        env_overlay["SHIVA_HOME"] = shiva_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m shiva_cli.main gateway run`` directly — NOT
    ...
    log_dir = Path(get_shiva_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("SHIVA_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("SHIVA_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from shiva_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `shiva -p <profile> gateway install` must be safe.
    ...
    from shiva_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"shiva {profile_arg} gateway start" if profile_arg else "shiva gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: shiva gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Shiva gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: shiva gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: shiva gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from shiva_cli.config import get_shiva_home
        print(f"    type {Path(get_shiva_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_shiva_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from shiva_cli.config import get_shiva_home

    shiva_home = Path(get_shiva_home())
    print()
    print("Next steps:")
    print("  shiva gateway status                      # Check status")
    print(f"  type {shiva_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Shiva gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: shiva gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: shiva gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: shiva gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `shiva gateway status`."
        )
