"""``shiva logs`` — view and filter Shiva log files.
...
Usage examples::
    ...
    shiva logs                    # last 50 lines of agent.log
    shiva logs -f                 # follow agent.log in real time
    shiva logs errors             # last 50 lines of errors.log
    shiva logs gateway -n 100    # last 100 lines of gateway.log
    shiva logs gui -f            # follow gui.log (dashboard/pty/ws)
    shiva logs desktop -f        # follow desktop.log (Electron app boot/backend)
    shiva logs --level WARNING    # only WARNING+ lines
    shiva logs --session abc123   # filter by session ID substring
    shiva logs --component tools  # only tool-related lines
    shiva logs --since 1h         # lines from the last hour
    shiva logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_shiva_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Shiva runs — try 'shiva chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_shiva_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_shiva_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_shiva_home()}/logs/")
        return

    print(f"Log files in {display_shiva_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'shiva chat' to generate logs)")
