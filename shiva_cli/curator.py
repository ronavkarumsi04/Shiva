"""CLI subcommand: `shiva curator <subcommand>`.
...
def _print_unmanaged_summary() -> None:
    ...
    print(
        ...
        "`shiva curator adopt <name>` hands one over"
    )
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `shiva curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`shiva curator status` and run `shiva curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_pin(args) -> int:
    ...
    if not skill_usage.set_pinned(args.skill, True):
        print(
            ...
            "`shiva curator list-unmanaged` shows which skills the curator tracks."
        )
        return 1
    if not skill_usage.is_curator_managed(args.skill):
        ...
        print(
            ...
            f"`shiva curator adopt {args.skill}` to put it under curator "
            "management)"
        )
        return 0
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `shiva curator adopt <name>`, "
          "or all with `shiva curator adopt --all-unmanaged`")
    return 0
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`shiva curator unpin {args.skill}`"
        )
        return 1
    tok = skill_ledger.set_ledger_actor("user")
    try:
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.shiva/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_ledger(args) -> int:
    ...
    print(
        "\nRoll back a single mutation with `shiva curator rollback <id>`; "
        "whole-tree snapshots remain available via `shiva curator rollback --list`."
    )
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if entry_id:
        ...
        if entry is None:
            print(
                ...
                "See `shiva curator ledger` for entry ids, or use "
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`shiva curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.shiva/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.shiva/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.shiva/skills/ from a curator snapshot, or a single "
             "mutation by ledger entry id (see `shiva curator ledger`)",
    )
    p_rollback.add_argument(
        "entry_id", nargs="?", default=None,
        help="Ledger entry id for single-mutation rollback (from "
             "`shiva curator ledger`). Omit for whole-tree snapshot rollback.",
    )
    p_rollback.add_argument(
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by shiva_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="shiva curator")
    register_cli(parser)
