"""Helpers for loading Shiva .env files consistently across entrypoints."""
...
# load_shiva_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Paths we've already emitted a UTF-32 refuse-to-mangle warning for.
# load_shiva_dotenv can call _sanitize_env_file_if_needed multiple times
# for the same file (user env + project env + hot-reload); once per path
# is enough.
_WARNED_UTF32_PATHS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_shiva_dotenv().
# Used by setup / `shiva model` flows to label detected credentials so
...
# SHIVA_HOME paths we've already pulled external secrets for during this
# process.  ``load_shiva_dotenv()`` is called at module-import time from
# several hot modules (cli.py, shiva_cli/main.py, run_agent.py,
...
def _known_shiva_env_keys() -> set[str]:
    """Return the combined set of known Shiva env-var keys.
    ...
    ``load_shiva_dotenv()`` calls.
    """
    from shiva_cli.config import _EXTRA_ENV_KEYS
    from shiva_cli.config_defaults import OPTIONAL_ENV_VARS

    return set(OPTIONAL_ENV_VARS.keys()) | set(_EXTRA_ENV_KEYS)


# Behavioral routing keys a parent Shiva process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Shiva keys absent from ``.env``.
    ...
    parent Shiva process injects and that silently change *which provider
    ...
    ``tests/shiva_cli/test_dump_env_visibility.py``), and a startup scrub
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_shiva_dotenv()`` call.  Returns
...
def get_secret_source_values(
    shiva_home: str | os.PathLike,
) -> dict[str, str]:
    """Return the external-secret value snapshot for ``shiva_home``."""
    home_key = str(Path(shiva_home).resolve())
...
def hydrate_profile_secret_sources(
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(shiva_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_shiva_dotenv()'s .op.env bootstrap: the 1Password
...
def reset_secret_source_cache() -> None:
    """Forget which SHIVA_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `shiva setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``shiva_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from shiva_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_shiva_dotenv(
    *,
    shiva_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load Shiva environment files with user config taking precedence.

    Behavior:
    - `~/.shiva/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(shiva_home or os.getenv("SHIVA_HOME", Path.home() / ".shiva"))
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Shiva keys
    ...
    #   EnvironmentFile=-/path/to/.shiva/.op.env
    ...
    # 2. A fresh ``shiva update`` retry just completed a deferred dependency
    ...
    from shiva_cli import _early_recovery
    ...
    # TERMINAL_ENV=docker left in ~/.shiva/.env (e.g. written by an older
    # `shiva setup` before the user switched terminal.backend in config.yaml)
    ...
    # cron standalone runs) call load_shiva_dotenv() repeatedly and used to
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``shiva_cli.config.apply_terminal_config_to_env`` — the
    ...
    Scoped to the process SHIVA_HOME: the shared bridge reads the
    process-global config, so re-applying it for a *different* profile's
    ``load_shiva_dotenv(shiva_home=...)`` call would bridge the wrong
    ...
    """
    try:
        if Path(home_path).resolve() != _process_shiva_home().resolve():
            return
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of SHIVA_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Shiva reads
    ...
    owns the once-per-SHIVA_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_shiva_dotenv()`` runs at import
    time from several hot modules (cli.py, shiva_cli/main.py,
    ...
    if not cfg:
        ...
        # on its next load_shiva_dotenv() call instead of never.
        return
    ...
    # a generic dict entry must not force crypto load on every shiva launch.
    ...
    # so the 3-5 import-time load_shiva_dotenv() calls per startup don't
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `shiva model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `shiva` startup, so populating the
    # shared cache here lets main.py's early bridge and shiva_logging reuse
    ...
    if home_path == _process_shiva_home():
        try:
...
def _process_shiva_home() -> Path:
    """The SHIVA_HOME the shared config cache is keyed to."""
    try:
