...
A pack is a single YAML file (``shiva-pack.yaml``) that pins a set of
...
* Capability consent is NEVER bulk-granted: after each plugin installs,
  ...
  as a normal ``shiva plugins install`` (#64228).
    ...
    for entry in pack.plugins:
        ...
        try:
            if index_entries is None:
                from shiva_cli.plugin_index import load_index

                index_entries, _src = load_index()
            from shiva_cli.plugin_index import resolve_name
...
def render_pack_review(console, pack: PluginPack, resolved: List[ResolvedPackPlugin]) -> None:
    ...
    if pack.skills:
        ...
        console.print(
            "[dim]Install them manually, e.g. `shiva skills install <id>`.[/dim]"
        )
    console.print(
...
def install_pack_plugins(
    ...
    *,
    ...
    single install (:func:`shiva_cli.plugins_cmd._run_capability_consent`).
...
def export_pack(*, enabled_only: bool = False, pack_name: str = "my-shiva-pack") -> tuple[str, List[str]]:
    ...
    doc: dict[str, Any] = {
        "name": pack_name,
        "description": "Exported by `shiva plugins pack export`.",
        "version": "1.0.0",
        "plugins": entries,
    }
    if config:
...
def cmd_pack_show(source: str) -> None:
    """``shiva plugins pack show <path-or-url>`` — dry-run review."""
    ...
    console.print("\n[dim]Dry run only. Install with `shiva plugins pack install ...`.[/dim]")


def cmd_pack_install(source: str, *, force: bool = False) -> None:
    """``shiva plugins pack install <path-or-url>``.
...
def cmd_pack_export(*, enabled_only: bool = False, name: str = "my-shiva-pack") -> None:
    """``shiva plugins pack export [--enabled-only]`` — pack YAML on stdout."""
    from rich.console import Console
...
def pack_command(args) -> None:
    """Dispatch ``shiva plugins pack <action>``."""
    ...
    if action == "install":
        ...
        cmd_pack_export(
            ...
            name=getattr(args, "name", None) or "my-shiva-pack",
        )
    elif action == "show":
        ...
        Console().print(
            "[red]Error:[/red] Usage: shiva plugins pack {install|export|show}"
        )
        sys.exit(1)
