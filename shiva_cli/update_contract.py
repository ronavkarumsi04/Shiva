...
``shiva update`` mutation (CLI apply, CLI --check, dashboard update
endpoint). The decision layers:

1. **Baked provenance marker** (``/etc/shiva/image-provenance.json``,
   written by the image build — see :mod:`shiva_cli.image_provenance`):
