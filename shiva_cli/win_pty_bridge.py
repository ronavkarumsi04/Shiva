"""Windows ConPTY bridge for the `shiva dashboard` chat tab.

Drop-in counterpart to ``shiva_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``shiva_cli.web_server`` consumes: ``spawn``, ``read``,
