...
Design notes / invariants (same contract as ``shiva_cli/goals.py``):
...
- This module has zero hard dependency on ``cli.ShivaCLI``, the gateway
...
def _get_session_db() -> Optional[Any]:
    ...
    """
    try:
        from shiva_cli.goals import _get_session_db as _goals_db
...
def save_loop(session_id: str, state: LoopState) -> None:
    ...
    db = _get_session_db()
    if db is None:
        from shiva_cli.goals import _warn_dropped_write
