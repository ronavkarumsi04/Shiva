"""
Status command for shiva CLI.

Shows the status of all Shiva Agent components.
"""
...
from shiva_cli.auth import AuthError, resolve_provider
from shiva_cli.colors import Colors, color
from shiva_cli.config import get_env_path, get_env_value, get_shiva_home, load_config
from shiva_cli.models import provider_label
from shiva_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from shiva_cli.nous_subscription import get_nous_subscription_features
from shiva_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``shiva config``'s
...
def _estop_status_line():
    """One-line pause banner for `shiva status`, or None when not paused.

    Cheap: a single stat on $SHIVA_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `shiva resume` to lift)"


def show_status(args):
    """Show status of all Shiva Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Shiva Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `shiva status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: shiva portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: shiva model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: shiva auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: shiva auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: shiva model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from shiva_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'shiva-agent[vercel]')"
        ...
        # provider's doctor rows (fail-soft — never break `shiva status`).
        try:
    ...
    try:
        from shiva_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   shiva gateway")
    ...
    jobs_file = get_shiva_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_shiva_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'shiva doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'shiva setup' to configure", Colors.DIM))
    print()
