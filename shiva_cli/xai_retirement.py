...
Pure logic: walks a Shiva config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `shiva doctor` and a future `shiva migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Shiva config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Shiva config and return retirement issues.
...
def apply_migration(
    ...
    from shiva_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `shiva migrate xai` reports nothing to migrate
