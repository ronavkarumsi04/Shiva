"""
Single source of truth for provider identity in Shiva Agent.
...
2. **Shiva overlays** — transport type, auth patterns, aggregator flags,
...
# -- Shiva overlay ----------------------------------------------------------
# Shiva-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ShivaOverlay:
    """Shiva-specific provider metadata layered on top of models.dev."""
...
SHIVA_OVERLAYS: Dict[str, ShivaOverlay] = {
    "moa": ShivaOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": ShivaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": ShivaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": ShivaOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": ShivaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": ShivaOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="SHIVA_QWEN_BASE_URL",
    ),
    "lmstudio": ShivaOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ShivaOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ShivaOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ShivaOverlay(
    ...
    "kimi-for-coding": ShivaOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": ShivaOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": ShivaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": ShivaOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ShivaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ShivaOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": ShivaOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": ShivaOverlay(
        transport="openai_chat",
    ...
    "opencode": ShivaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ShivaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": ShivaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": ShivaOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": ShivaOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "shiva", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Shiva overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Shiva overlay
    ...
    overlay = SHIVA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + shiva extra
    ...
    if overlay is not None:
        # Shiva-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and SHIVA_OVERLAYS, so
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Shiva overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in SHIVA_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Shiva provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from shiva_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
