"""Install and remove the Linux desktop entry (``shiva.desktop``).

``shiva desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "shiva.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``shiva.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``shiva desktop``.

    Prefer the real ``shiva`` executable (argv[0] or PATH). When Shiva
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from shiva_cli.relaunch import resolve_shiva_bin
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `shiva` script with
            ...
            # sys.executable is the interpreter actually running Shiva (the
            # venv one), so prefix it explicitly.
            argv = [str(Path(sys.executable).resolve()), str(resolved), "desktop"]
        else:
            argv = [str(resolved), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "shiva_cli.main", "desktop"]
    return " ".join(_quote_exec_arg(a) for a in argv)


def _needs_interpreter(bin_path: Path) -> bool:
    """Whether ``bin_path`` is a Python script that must run under
    ``sys.executable`` to see Shiva' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Shiva\n"
        "GenericName=Shiva Desktop\n"
        "Comment=Launch Shiva Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Shiva\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Shiva desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "shiva"
