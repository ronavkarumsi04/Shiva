...
# xAI's public API is OpenAI-compatible for the endpoints Shiva commonly
# uses. The Responses endpoint is included because Shiva' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Shiva-managed OAuth credentials."""

    auth_hint = "shiva auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`shiva auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`shiva auth reset xai-oauth` or re-authenticate with "
                    "`shiva auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `shiva auth add xai-oauth --type oauth`."
            )
