"""CLI handlers for the ``shiva proxy`` subcommand."""
...
from shiva_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "shiva proxy requires aiohttp. Run `shiva setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"shiva auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Shiva proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Shiva proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: shiva proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``shiva proxy <subcommand>``."""
    ...
    print(
        "shiva proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  shiva proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
