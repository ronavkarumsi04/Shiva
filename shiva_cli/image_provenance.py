"""Image-authored deployment provenance for immutable Shiva runtimes.

The published image bakes ``/etc/shiva/image-provenance.json`` outside both
``$SHIVA_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/shiva/image-provenance.json")
