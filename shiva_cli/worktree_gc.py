"""On-demand worktree + branch reclaim (``shiva worktree`` / ``/worktree prune``).

The startup pruner in ``cli._prune_stale_worktrees`` is deliberately
conservative and silent: it runs before the banner on every ``shiva -w``
...
- **Orphaned local branches** beyond the two auto-generated prefixes the
  startup pass deletes (``shiva/shiva-*``, ``pr-*``): salvage lanes, port
...
- untracked-only dirt is ARCHIVED to ``~/.shiva/archive/worktree-prune/``
...
def _archive_untracked(tree: Path, untracked: List[str]) -> Optional[Path]:
    ...
    dest = (
        Path.home() / ".shiva" / "archive" / "worktree-prune"
        / f"{tree.name}-{stamp}"
    )
    try:
...
def audit_worktrees(repo_root: str, *, with_sizes: bool = True) -> List[TreeRecord]:
    ...
    for entry in sorted(worktrees_dir.iterdir()):
        ...
        if lock_state == "live":
            rec("keep", "in use by a running shiva session")
            continue
...
def audit_branches(repo_root: str) -> List[BranchRecord]:
    ...
    Generalizes the startup pass's prefix list (``shiva/shiva-*``/``pr-*``)
    ...
    if workers > 1:
        try:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=workers, thread_name_prefix="shiva-branch-gc"
            ) as pool:
