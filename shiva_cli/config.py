"""
Configuration management for Shiva Agent.

Config files are stored in ~/.shiva/ for easy access:
- ~/.shiva/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.shiva/.env         - API keys and secrets

This module provides:
- shiva config          - Show current configuration
- shiva config edit     - Open config in editor
- shiva config get      - Print a resolved configuration value
- shiva config set      - Set a specific value
- shiva config unset    - Remove a user configuration value
- shiva config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``shiva config set`` (which rewrites
    ...
    ``config.yaml`` in place: shiva never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.shiva/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``shiva logs``
    ...
    if fallback == "last-known-good":
        ...
        msg = (
            ...
            f"Fix the YAML (shiva config edit) and retry."
        )
    else:
        msg = (
            f"Failed to parse {config_path}: {exc}. "
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  shiva config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Shiva makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Shiva itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``shiva update``, the TUI build.
...
#   on every plugin install / ``shiva update``.
...
# * ``SHIVA_HOME`` / ``SHIVA_PROFILE`` / ``SHIVA_CONFIG`` /
#   ``SHIVA_ENV`` — Shiva runtime location flags. Writing these into
...
#
# IMPORTANT: ``SHIVA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (SHIVA_LANGFUSE_PUBLIC_KEY,
# SHIVA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Shiva runtime location — never via dashboard env writer.
    # NOT a SHIVA_* blanket: integration credentials (SHIVA_GEMINI_*,
    # SHIVA_LANGFUSE_*, SHIVA_SPOTIFY_*, ...) ARE allowed.
    "SHIVA_HOME", "SHIVA_PROFILE", "SHIVA_CONFIG", "SHIVA_ENV",
    "SHIVA_CONFIG_PATH", "SHIVA_ENV_PATH",
    ...
    "SHIVA_COPILOT_ACP_COMMAND", "SHIVA_COPILOT_ACP_ARGS",
    # Shiva security policy / approval-routing context. These remain available
    ...
    "SHIVA_YOLO_MODE", "SHIVA_ACCEPT_HOOKS", "SHIVA_REDACT_SECRETS",
    "SHIVA_INTERACTIVE", "SHIVA_EXEC_ASK", "SHIVA_GATEWAY_SESSION",
    "SHIVA_CRON_SESSION", "SHIVA_SINGLE_QUERY_SESSION",
    "SHIVA_SESSION_KEY", "SHIVA_SESSION_PLATFORM",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Shiva runtime location "
            "and security policy (SHIVA_HOME, SHIVA_YOLO_MODE, ...) "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # SHIVA_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `shiva plugins enable
    # observability/langfuse` or `shiva tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "SHIVA_LANGFUSE_ENV",
    "SHIVA_LANGFUSE_RELEASE",
    "SHIVA_LANGFUSE_SAMPLE_RATE",
    "SHIVA_LANGFUSE_MAX_CHARS",
    "SHIVA_LANGFUSE_CAPTURE",
    "SHIVA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "SHIVA_ACP_AUTH_METHOD",
    "SHIVA_ACP_AUTO_APPROVE",
    "SHIVA_COPILOT_ACP_COMMAND",
    "SHIVA_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from shiva_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set SHIVA_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SHIVA_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_shiva_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # SHIVA_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if Shiva is running in package-manager-managed mode.

    Two signals: the SHIVA_MANAGED env var (set by the systemd service),
    or a .managed marker file in SHIVA_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Shiva through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``shiva_cli/`` — i.e. the git checkout for source
    installs, ``/opt/shiva`` inside the published image. It is a property of
    the running interpreter, NOT of ``$SHIVA_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Shiva was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$SHIVA_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. SHIVA_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.shiva``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$SHIVA_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.shiva:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$SHIVA_HOME``, a
    ...
    ``docker`` and ``shiva update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/shiva-agent`` image bakes a ``docker``
        stamp into ``/opt/shiva`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $SHIVA_HOME.
    try:
    ...
    #    host install whose shared $SHIVA_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``shiva update``.
    try:
        method = (
            (get_shiva_home() / ".install_method")
    ...
    # detect Nix installs that don't set SHIVA_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``shiva_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$SHIVA_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/shiva`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``shiva update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# shiva_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Shiva, it doesn't update
...
✗ ``shiva update`` doesn't apply inside the Docker container.

Shiva Agent runs as a published image (nousresearch/shiva-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/shiva-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate shiva-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/shiva-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/shiva-agent/tags
  • Your config and session history live under ``$SHIVA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``shiva update`` inside Docker.
...
def format_managed_message(action: str = "modify this Shiva installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Shiva installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Shiva."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SHIVA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, shiva_bin
    or None if container mode is not active, we're already inside the
    container, or SHIVA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SHIVA_DEV") == "1":
        return None
    ...
    container_mode_file = get_shiva_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "shiva-agent")
    exec_user = info.get("exec_user", "shiva")
    shiva_bin = info.get("shiva_bin", "/data/current-package/bin/shiva")
...
# Re-export from shiva_constants — canonical definition lives there.
from shiva_constants import get_shiva_home, get_process_shiva_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_shiva_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_shiva_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_shiva_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the SHIVA_UID / SHIVA_GID env vars set by Docker deployments.

    Docker containers running Shiva commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level SHIVA_HOME once, but
    subdirectories created at runtime by ``ensure_shiva_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("SHIVA_UID", "").strip()
    gid_str = os.environ.get("SHIVA_GID", "").strip()
    try:
...
def _chown_to_shiva_uid(path) -> None:
    """Chown ``path`` to ``SHIVA_UID:SHIVA_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_shiva_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_shiva_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the shiva group can
    share state with the gateway service.

    The mode can be overridden via the SHIVA_HOME_MODE environment variable
    (e.g. SHIVA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SHIVA_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``SHIVA_UID``/``SHIVA_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SHIVA_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_shiva_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Shiva runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SHIVA_CONTAINER") or os.environ.get("SHIVA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SHIVA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SHIVA_HOME, upgrading legacy empty templates.
...
# ensure_shiva_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_SHIVA_HOME_ENSURED: set = set()


def ensure_shiva_home():
    """Ensure ~/.shiva directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``SHIVA_HOME`` we only re-run
    ...
    ``get_shiva_home()`` and therefore re-run for the new path.
    """
    home = get_shiva_home()
    key = str(home)

    if key in _SHIVA_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``shiva profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old SHIVA_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_shiva_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _SHIVA_HOME_ENSURED.add(key)


def _ensure_shiva_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SHIVA_HOME {home} does not exist."
        )
...
from shiva_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `shiva config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.shiva.config`` entries, then checks
    ...
    try:
        ...
        # should never break `shiva update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Shiva' own config writer has historically emitted it
    ...
    # Shiva (``_save_discovered_models_to_config``), not hand-curated.
    # Older Shiva versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Shiva versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``shiva_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (shiva_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (shiva_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (shiva_cli/setup.py)
    "known_plugin_toolsets", # written/read by shiva_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (shiva_cli/nous_subscription.py, #92647)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'shiva setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Shiva won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, shiva send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'shiva doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "shiva update / shiva -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from shiva_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ shiva config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in shiva_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from shiva_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: shiva config set <key> <value>")
    ...
    # `shiva update`, but only the version bump is persisted.
    ...
    # metadata.shiva.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.shiva/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_shiva_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``ShivaCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``shiva config set``
    ...
    backends) — while display paths (``shiva status``/``dump``) read ``name``
...
#   - survives the ``str() → int()`` round-trip through the SHIVA_MAX_ITERATIONS
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _shiva_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in shiva_cli.personality.
from shiva_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`shiva_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from shiva_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from shiva_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``SHIVA_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`shiva_cli.personality` (single owner).
    """
    from shiva_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.shiva/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_shiva_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.shiva/config.yaml.
    ...
    (which change ``SHIVA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Shiva host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/shiva/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_shiva_dotenv()
...
#   openai-codex (OAuth — shiva auth) — OpenAI Codex
#   nous         (OAuth — shiva auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.shiva/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Shiva writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.shiva/.env.
    ...
    interactive menu render (`shiva tools`, `shiva setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `shiva tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.shiva/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.shiva/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``SHIVA_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"SHIVA_CUSTOM_{slug}_API_KEY" if slug else "SHIVA_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.shiva/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.shiva/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Shiva — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Shiva vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.shiva/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.shiva/.env`` over ``os.environ``.

    Used for Shiva-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Shiva Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from shiva_cli.auth import get_anthropic_key
    ...
    # Warn on stale SHIVA_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("SHIVA_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale SHIVA_MAX_ITERATIONS={_env_ghost} "
                f"(run 'shiva doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  shiva config edit     # Edit config file", Colors.DIM))
    print(color("  shiva config set <key> <value>", Colors.DIM))
    print(color("  shiva setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``SHIVA_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("SHIVA_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `shiva cron list`, then pin the "
        "intended values with `shiva cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (shiva_cli/plugins_cmd.py, shiva_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `shiva config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``shiva config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``shiva config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``shiva config set --force``.
    """
    if is_managed():
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   shiva config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   shiva config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``shiva config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``shiva config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    shiva config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    shiva config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `shiva config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_shiva_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `shiva config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `shiva config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: shiva config get <key> [--json]")
            print()
            print("Examples:")
            print("  shiva config get model")
            print("  shiva config get terminal.backend")
            print("  shiva config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: shiva config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  shiva config set model anthropic/claude-sonnet-4")
            print("  shiva config set terminal.backend docker")
            print("  shiva config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: shiva config unset <key>")
            print()
            print("Examples:")
            print("  shiva config unset model")
            print("  shiva config unset terminal.backend")
            print("  shiva config unset OPENROUTER_API_KEY")
            sys.exit(1)
        try:
        ...
        if missing_config:
            ...
            print("    Run 'shiva config migrate' to add them")
        ...
        print("  shiva config           Show current configuration")
        print("  shiva config edit      Open config in editor")
        print("  shiva config get <key>          Print a resolved config value")
        print("  shiva config set <key> <value>   Set a config value")
        print("  shiva config unset <key>        Remove a config value")
        print("  shiva config check     Check for missing/outdated config")
        print("  shiva config migrate   Update config with new options")
        print("  shiva config path      Show config file path")
        print("  shiva config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``shiva config`` UI so users
