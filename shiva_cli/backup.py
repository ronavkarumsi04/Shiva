"""
Backup and import commands for shiva CLI.

`shiva backup` creates a zip archive of the entire ~/.shiva/ directory
(excluding the shiva-agent repo and transient files).

`shiva import` restores from a backup zip, overlaying onto the current
SHIVA_HOME root.
"""
...
from shiva_constants import get_default_shiva_root, get_shiva_home, display_shiva_home
from utils import (
    _preserve_file_mode,
...
from shiva_cli.sizefmt import format_bytes as _format_size
...
# Where ``shiva backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``shiva-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/shiva-agent/``
...
# SHIVA_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "shiva-agent",     # the codebase repo — re-clone instead
    ...
    "browser-profile",
    # Python dependency trees (plugin / MCP-server venvs under SHIVA_HOME) —
...
# File names that ``shiva import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE SHIVA_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Shiva backup slot."""
...
@contextmanager
def _backup_operation_lock(shiva_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = shiva_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Shiva backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside SHIVA_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to shiva root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``shiva-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/shiva-agent/`` — must be preserved.
        if part == "shiva-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside SHIVA_HOME.
...
# to the user as a hung `shiva update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # shiva_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Shiva home directory."""
    shiva_root = get_default_shiva_root()

    if not shiva_root.is_dir():
        print(f"Error: Shiva home directory not found at {shiva_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(shiva_root):
            _run_backup_locked(args, shiva_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, shiva_root: Path) -> None:
    ...
    try:
        if args.output:
            ...
            if out_path.is_dir():
                ...
                out_path = out_path / f"shiva-backup-{stamp}.zip"
        else:
            stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
            out_path = Path.home() / f"shiva-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_shiva_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(shiva_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(shiva_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``shiva-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "shiva-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(shiva_root)
    ...
    # outside SHIVA_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_shiva_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: shiva import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Shiva backup.
    ...
    # Look for telltale files that a shiva home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Shiva backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.shiva/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a shiva dir name
        if prefix in {".shiva", "shiva"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``shiva import``, which is the
    ...
    so without the chown a ``sudo shiva import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``shiva import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Shiva' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_shiva_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (shiva_root / "config.yaml").exists()
        has_env = (shiva_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Shiva configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under SHIVA_HOME.
            ...
            target = shiva_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(shiva_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_shiva_home()}."
            )
        ...
        profiles_dir = shiva_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # shiva_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: shiva profile list  (after installing shiva)")

        # Guidance
        print()
        if not (shiva_root / "shiva-agent").is_dir():
            print("Note: The shiva-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: shiva update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  shiva -p {pname} gateway install")
        ...
        try:
            from shiva_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your Shiva configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and shiva backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SHIVA_HOME).
...
# — `shiva update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $SHIVA_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside SHIVA_HOME and the entry is silently
...
def _quick_snapshot_root(shiva_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: shiva snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(shiva_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside SHIVA_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``shiva update``.
    ...
    Args:
        ...
        shiva_home: Override for the Shiva home directory (tests).
...
def restore_cron_jobs_all_profiles(
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_cron_jobs_if_emptied(snap_id, shiva_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(shiva_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for shiva backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_shiva_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, shiva_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(shiva_root):
            return _write_full_zip_backup_locked(out_path, shiva_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, shiva_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``shiva_root`` to ``out_path``.
...
def _pre_update_backup_dir(shiva_home: Optional[Path] = None) -> Path:
    home = shiva_home or get_shiva_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SHIVA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SHIVA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``shiva update``) should continue even if the backup fails.
    """
    shiva_root = shiva_home or get_default_shiva_root()
    if not shiva_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(shiva_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, shiva_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `shiva claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of SHIVA_HOME under ``backups/`` before a
    ``shiva claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``shiva import <archive>``.  Writes to
    ``<SHIVA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `shiva import` and the
