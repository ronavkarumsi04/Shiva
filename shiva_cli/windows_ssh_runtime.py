...
def _root() -> Path:
    ...
    # helper's token, including custom SHIVA_HOME layouts.
    return get_default_shiva_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, shiva_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(shiva_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the shiva exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs shiva_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "shiva_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "shiva_cli.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, shiva_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, shiva_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves shiva_cli's own location and prepends its parent, because the
    ...
    # the shiva_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the shiva_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    configured_path = str(payload["shivaPath"])
    if not os.path.isabs(configured_path):
        raise ValueError("Shiva path must be absolute")
    shiva_path = os.path.abspath(configured_path)
    ...
    venv_dir = os.path.dirname(shiva_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("Shiva Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('shiva_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_shiva(shiva_path: str) -> dict[str, Any]:
    path = os.path.abspath(shiva_path)
    if not os.path.isabs(shiva_path) or not os.path.isfile(path):
        raise ValueError("Shiva path is not an executable file")
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "shivaHome": str(get_default_shiva_root()), "python": sys.executable}
