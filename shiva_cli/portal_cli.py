"""``shiva portal`` — the human-readable entry point for Nous Portal.

Running ``shiva portal`` with no subcommand performs the one-shot Portal
...
``shiva auth add nous --type oauth`` (which still works), is identical to
``shiva setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``shiva auth`` or ``shiva tools``. It's the onboarding + discovery
...
DOCS_URL = "https://shiva-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `shiva model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `shiva portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `shiva auth add nous --type
    oauth`. It reuses the exact wiring behind `shiva setup --portal` (which in
    ...
    """
    from shiva_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `shiva portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `shiva portal` is the
        # human-readable alias for `shiva auth add nous --type oauth` /
        # `shiva setup --portal`.
    ...
    print("Run `shiva portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `shiva portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `shiva portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `shiva auth add "
            "nous --type oauth`, identical to `shiva setup --portal`). "
