"""Implementation of the ``shiva bundles`` CLI subcommand.

Mirrors the structure of ``shiva_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `shiva bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  shiva bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``shiva bundles`` argparse tree.

    Called from ``shiva_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `shiva bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``shiva bundles <subcommand>`` to the right handler."""
