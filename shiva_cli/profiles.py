"""
Profile management for multiple isolated Shiva instances.

Each profile is a fully independent SHIVA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.shiva/profiles/<name>/`` by default.

The "default" profile is ``~/.shiva`` itself — backward compatible,
zero migration needed.

Usage::

    shiva profile create coder          # fresh profile + bundled skills
    shiva profile create coder --clone  # also copy config, .env, SOUL.md, skills
    shiva profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    shiva -p coder chat                 # or via flag
    shiva profile use coder             # set as sticky default
    shiva profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See shiva_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `shiva profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `shiva
...
# `shiva skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Shiva infrastructure directories that only the default profile
         (``~/.shiva``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_shiva_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.shiva) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "shiva-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when SHIVA_HOME equals the
...
# into the export. The set below identifies the *known Shiva profile
# artifacts* at the root of SHIVA_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``shiva_constants``.
...
_RESERVED_NAMES = frozenset({
    "shiva", "default", "test", "tmp", "root", "sudo",
})

# Shiva subcommands that cannot be used as profile names/aliases
_SHIVA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the shiva root, NOT to the current SHIVA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SHIVA_HOME points outside
    ``~/.shiva``, profiles live under ``SHIVA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_shiva_home() / "profiles"


def _get_default_shiva_home() -> Path:
    """Return the default (pre-profile) SHIVA_HOME path.

    In standard deployments this is ``~/.shiva``.
    In Docker/custom deployments where SHIVA_HOME is outside ``~/.shiva``
    (e.g. ``/opt/data``), returns SHIVA_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_shiva_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Shiva installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SHIVA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_shiva_home()
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Shiva home
    (:func:`shiva_constants.get_shiva_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, shiva subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _SHIVA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a shiva subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            shiva_exe = shutil.which("shiva") or "shiva"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(shiva_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Shiva. If we
    ...
    try:
        ...
        # not be migrated. The next `shiva doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``shiva -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``shiva profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``shiva -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``shiva profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``SHIVA_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Shiva config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_shiva_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, shiva_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own SHIVA_HOME. When
    ...
    The returned ``shiva_home`` is the path to pass to
    ``set_shiva_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_shiva_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``shiva update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.shiva)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.shiva/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `shiva -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Shiva profile.\n"
    ...
    if not soul_path.exists():
        try:
            from shiva_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `shiva update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`shiva profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Shiva (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `shiva profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `shiva -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SHIVA_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``shiva profile create
    ...
    only the essential skills (e.g. ``shiva-agent``), reporting
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SHIVA_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_shiva_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Shiva *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Shiva backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``SHIVA_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `shiva -p <canon> profile
    ...
    shiva_markers = ("shiva_cli.main", "shiva-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('shiva')` resolution) spawns the
    ...
    # interpreter, not "shiva", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _SHIVA_CONSOLE_SCRIPT_NAMES = frozenset({"shiva", "shiva-agent", "shiva-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Shiva process: either an entrypoint marker in argv, a
            # resolved executable named `shiva`, or a python interpreter
            # directly exec'ing a `shiva`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_shiva = (
                any(marker in joined for marker in shiva_markers)
                or exe_name == "shiva"
                or exe_name.startswith("shiva")
            )
            if not is_shiva and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: shiva, shiva-agent,
                # shiva-acp) rather than a bare `startswith("shiva")` --
                ...
                # "shiva-notes.py" or "shiva-unrelated-tool" as the shim,
                ...
                is_shiva = script_stem in _SHIVA_CONSOLE_SCRIPT_NAMES
            if not is_shiva:
                continue
            ...
            # ...or by SHIVA_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("SHIVA_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.shiva).\n"
            "To remove everything, use: shiva uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_shiva_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``SHIVA_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from shiva_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SHIVA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SHIVA_HOME")
    try:
        os.environ["SHIVA_HOME"] = str(profile_dir)
        from shiva_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SHIVA_HOME"] = old_home
        elif "SHIVA_HOME" in os.environ:
            del os.environ["SHIVA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.shiva/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: shiva profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SHIVA_HOME.

    Returns ``"default"`` if SHIVA_HOME is not set or points to ``~/.shiva``.
    Returns the profile name if SHIVA_HOME points into ``~/.shiva/profiles/<name>``.
    Returns ``"custom"`` if SHIVA_HOME is set to an unrecognized path.
    """
    from shiva_constants import get_shiva_home
    shiva_home = get_shiva_home()
    resolved = shiva_home.resolve()

    default_resolved = _get_default_shiva_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      SHIVA_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Shiva file the
      user may have lying alongside SHIVA_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``shiva sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``SHIVA_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.shiva itself — its parent is ~/ and its
        # directory name is ".shiva", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: shiva profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.shiva itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.shiva). "
            "Specify a different name: shiva profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="shiva_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"shiva_{old_name}"
    legacy_old_host = f"shiva.{old_name}"
    new_host = f"shiva_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_shiva_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("shiva_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SHIVA_HOME path string.

    Called early in the CLI entry point, before any shiva modules
    are imported, to set the SHIVA_HOME environment variable.

    When SHIVA_HOME is already set, the configured spelling IS the
