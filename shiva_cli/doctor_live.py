"""``shiva doctor --live`` — opt-in bounded real-call tool-backend probes.
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from shiva_cli.doctor import SHIVA_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (SHIVA_HOME / "node" / "bin",
                          SHIVA_HOME / "node",
                          SHIVA_HOME / "node_modules" / ".bin"):
    ...
    # shiva_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``shiva mcp test``.
    """
