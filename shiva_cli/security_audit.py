"""On-demand supply-chain audit for Shiva Agent installs.

Scans three surfaces a Shiva user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Shiva venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.shiva/plugins``
...
def _discover_plugins(shiva_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.shiva/plugins``.
...
def _discover_components(
    *,
    ...
    home = shiva_home or Path(get_shiva_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `shiva security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, shiva_home=home
    )
