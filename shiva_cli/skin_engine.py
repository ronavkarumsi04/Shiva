"""Shiva skin/theme engine — the theme SDK for every surface.

A data-driven skin system that lets users (and Shiva itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.shiva/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.shiva/skins/ therefore
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Shiva Agent"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " ⚕ Shiva "       # Response box header label
...
.. code-block:: python
    ...
    from shiva_cli.skin_engine import get_active_skin, list_skins, set_active_skin

    skin = get_active_skin()
    print(skin.colors["banner_title"])    # "#FFD700"
    print(skin.get_branding("agent_name"))  # "Shiva Agent"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.shiva/skins/
...
Drop a YAML file in ``~/.shiva/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces SHIVA_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces SHIVA_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Shiva — gold and kawaii",
        # Dark-authored. Values match the TUI's DARK_THEME so the classic CLI
        # and the TUI render the same Shiva gold.
        "colors": {
        ...
        "branding": {
            "agent_name": "Shiva Agent",
            "welcome": "Welcome to Shiva Agent! Type your message or /help for commands.",
            "goodbye": "Goodbye! ⚕",
            "response_label": " ⚕ Shiva ",
            "prompt_symbol": "❯",
