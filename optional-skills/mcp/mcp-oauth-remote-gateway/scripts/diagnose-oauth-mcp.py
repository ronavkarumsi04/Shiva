...
Usage:
  ...
  <server_name> matches the files in $SHIVA_HOME/mcp-tokens/<server>.json etc.
...
def _shiva_home():
    # Prefer Shiva' own resolver (profile-safe); fall back to env then ~/.shiva.
    try:
        from shiva_constants import get_shiva_home
        return str(get_shiva_home())
    except Exception:
        return os.environ.get("SHIVA_HOME") or os.path.expanduser("~/.shiva")


def _tokens_dir():
    return os.path.join(_shiva_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "shiva-diag", "version": "1.0"}},
        },
        headers={
