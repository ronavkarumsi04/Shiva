...
Usage (via cron with --no-agent):
    ...
    shiva cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SHIVA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Shiva-Watcher/1.0"})
