...
Usage (via cron with --no-agent):
    ...
    shiva cron create shiva-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$SHIVA_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name shiva-issues --repo NousResearch/shiva-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Shiva .env file
(``${SHIVA_HOME:-~/.shiva}/.env``) to avoid the 60 req/hr
