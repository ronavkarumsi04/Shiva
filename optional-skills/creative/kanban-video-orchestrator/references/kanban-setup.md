...
the actual `setup.sh` that creates the project workspace, configures Shiva
...
2. **Create profiles** — `shiva profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.shiva/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `shiva kanban create` assigned to the director
...
```bash
shiva profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.shiva/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.shiva/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Shiva
...
Each profile gets a `SOUL.md` at `~/.shiva/profiles/<name>/SOUL.md` that
...
```bash
shiva kanban create "Direct production of <video title>" \
    --assignee director \
...
the Shiva `.env` (`${SHIVA_HOME:-$HOME/.shiva}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _shiva_env="${SHIVA_HOME:-$HOME/.shiva}/.env"
    if grep -q "^${var}=" "$_shiva_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_shiva_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_shiva_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY shiva ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY shiva OPENROUTER_API_KEY || exit 1
# ...
```
