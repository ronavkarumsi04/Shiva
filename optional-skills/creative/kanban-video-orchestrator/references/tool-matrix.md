...
Maps each role archetype to the Shiva skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public shiva-agent
...
## Shiva skills relevant to video production

### Visual / rendering skills (`shiva-agent/skills/creative/`)
...
### Design / pre-production skills (`shiva-agent/skills/creative/`)
...
### Audio / media skills (`shiva-agent/skills/creative/` + `skills/media/`)
...
These are **not** Shiva skills but external CLIs / APIs that profiles invoke.
...
## Built-in Shiva tools for media review

These are native Shiva tools — not invoked via terminal but through their own
...
key is present in `${SHIVA_HOME:-~/.shiva}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Shiva profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${SHIVA_HOME:-~/.shiva}/.env` → environment variable.
...
When a new Shiva-public video skill ships:
