...
### 1. Start order: editor first, Shiva second

Shiva probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Shiva session.
...
restart the editor, restart the Shiva session.
...
server fails to bind (Output Log) or Shiva' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.shiva/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Shiva

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Shiva' connection
lives in `~/.shiva/config.yaml` via `shiva mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Shiva — just don't
mistake it for the Shiva setup step.
...
explicitly warns against overlapping calls. Shiva executes same-turn tool
...
### 9. Timeouts: Shiva gives up before Unreal does

Shiva' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Shiva runs elsewhere (SSH backend,
