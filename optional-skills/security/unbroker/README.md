...
The design is **Shiva-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
shiva skills install official/security/unbroker
```

Then start a new Shiva session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Shiva home directory (`0600`, optionally
