...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SHIVA_HOME", os.path.expanduser("~/.shiva")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SHIVA_HOME", Path.home() / ".shiva")) / "skills" / "red-teaming" / "godmode"
...
SHIVA_HOME = Path(os.getenv("SHIVA_HOME", Path.home() / ".shiva"))
CONFIG_PATH = SHIVA_HOME / "config.yaml"
PREFILL_PATH = SHIVA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Shiva models — already uncensored, just needs clean prompt
    "shiva": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "shiva" in model_lower or "nous" in model_lower:
        return "shiva"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Shiva config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.shiva/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Shiva for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Shiva for changes to take effect.")
