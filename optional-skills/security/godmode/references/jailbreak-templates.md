...
## 5. GODMODE FAST — Shiva 4 405B (Zero Refusal)

**Model:** `nousresearch/shiva-4-405b`
**Strategy:** Shiva models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Shiva Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.shiva/config.yaml`:
