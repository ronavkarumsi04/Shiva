#!/usr/bin/env python3
"""Canvas LMS API CLI for Shiva Agent.
...
def _check_config():
    ...
    if missing:
        shiva_env = os.path.join(
            os.environ.get("SHIVA_HOME", os.path.expanduser("~/.shiva")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {shiva_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Shiva Agent"
    )
