"""Shared utility functions for shiva-agent."""
...
def atomic_replace(tmp_path: Union[str, Path], target: Union[str, Path]) -> str:
    ...
    ``~/.shiva/`` to a git-tracked profile package or dotfiles repo
    (GitHub #16743).
...
def warn_if_credential_file_broadly_readable(
    ...
    *,
    ...
    Secret-bearing files that users create by hand (or that older Shiva
...
def atomic_roundtrip_yaml_save(
    ...
    * Non-dict values (lists, scalars) are overwritten wholesale — list
    ...
    Shares the fail-closed contract ``shiva_cli.config.atomic_config_write``
    ...
    ``shiva_cli.config`` itself imports from this module.
    """
    from ruamel.yaml import YAML
    from ruamel.yaml.comments import CommentedMap
