...
storage discipline as the other indexes). On a populated database, run
    ...
    shiva sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.shiva/config.yaml` to disable. Override
the .so location with `SHIVA_FTS5_CJK_SO`.
