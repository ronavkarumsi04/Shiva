...
Shiva includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Shiva and does not require a Shiva observability plugin. Shiva remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Shiva execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `shiva-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Shiva `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `SHIVA_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `SHIVA_NEMO_RELAY_ATOF_*` and `SHIVA_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Shiva does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Shiva requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Shiva installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Shiva process so configured interceptors can
...
Collection remains off unless Shiva policy enables it:
...
opt-in. Set `SHIVA_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Shiva does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Shiva
...
retain the same `session_id` and add `shiva.session.segment` plus
`shiva.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Shiva
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Shiva process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Shiva logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Shiva process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Shiva
process and share the same policy boundary.

Run profiles in separate Shiva processes when they require different trust
...
Shiva core owns one Relay host and one isolated Relay session scope per Shiva
session. Core lifecycle producers use
`shiva_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Shiva plugin registration. Shared-metrics marks must
...
```text
Shiva turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Shiva shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Shiva sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Shiva used for the logical call, such as
...
New calls use `shiva.model_route.count`. The previous
`shiva.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `shiva.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Shiva
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Shiva processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `shiva.task_run`, parented to
the owning Shiva session. The start counter contains only bounded execution
...
provider attempts for the same Shiva API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Shiva tool-call
...
cancellations. Active task ownership follows the task ID if Shiva rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`shiva.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Shiva
...
tool or plugin names. Shiva does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `shiva.tool_approval` marks and
...
Successful skill mutations emit `shiva.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `shiva.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Shiva derives reuse and patch-generation
...
```text
$SHIVA_HOME/telemetry/shared_metrics/metrics.sqlite3
$SHIVA_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Shiva version,
...
schema field name, its current scope is one `SHIVA_HOME`, so it is more
...
Deleting `$SHIVA_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `SHIVA_HOME`. To reset it, stop Shiva
processes and remove `$SHIVA_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Shiva CLI turn against the deterministic local model server:
