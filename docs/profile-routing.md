...
`shiva config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.shiva/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `SHIVA_HOME`, secret scope, and
