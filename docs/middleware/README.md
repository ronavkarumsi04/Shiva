...
Shiva middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Shiva'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Shiva calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Shiva retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Shiva guardrails,
...
- `telemetry_schema_version`: currently `shiva.observer.v1`
- `middleware_schema_version`: currently `shiva.middleware.v1`
...
Shiva stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Shiva runs
them as a nested chain in registration order. Middleware failures are fail-open:
Shiva logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Shiva applies middleware in this order:
...
For each tool call, Shiva applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Shiva pre-execution path against the effective arguments:
...
For isolated local testing, use one `SHIVA_HOME` for plugin enablement and the
agent run:

```bash
export SHIVA_HOME=/tmp/shiva-middleware-test
mkdir -p "$SHIVA_HOME"
shiva plugins enable <plugin-name>
shiva chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run shiva plugins enable <plugin-name>
uv run shiva chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["shiva_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/shiva-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Shiva expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Shiva treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Shiva preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Shiva does not convert downstream
