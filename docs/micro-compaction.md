...
has to be thrown away or summarized. Shiva has always done this in one batch:
...
Shiva folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Shiva's normal session flush is
...
With `micro_compact` unset or `false` Shiva behaves exactly as it always has:
...
Defaults to `$SHIVA_HOME/logs/agent.log`. It reports passes, outcome mix, net
