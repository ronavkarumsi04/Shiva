...
This document is the formal interface between the **Shiva gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `shiva gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`shiva gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `shiva` CLI.
