...
The canonical transcript store is SQLite via `SessionDB` (from `shiva_state`). The
...
Counts consecutive restarts via a JSON file (`{SHIVA_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `shiva update`, `shiva gateway restart`,
or `/restart`.
