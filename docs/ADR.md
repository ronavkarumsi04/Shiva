...
## 2026-07-13: Scope plugin manager state by Shiva home/profile (keyed cache)

Status: Accepted

Context:
Shiva supports multiple profiles via different Shiva home directories.
Homes are switched two ways in a running process: the `SHIVA_HOME`
environment variable (single-profile CLI/gateway processes), and the
context-local `set_shiva_home_override()` (`shiva_constants.py`), which
...
`get_shiva_home() / "plugins"`, and context-engine plugins (e.g.
`shiva-lcm`) capture profile-scoped state — such as the LCM database
path — at registration time. A single-slot cache meant:

1. Switching homes via `set_shiva_home_override()` was invisible to a
   naive "did `SHIVA_HOME` change" check, so the singleton silently kept
...
2. Even when a fresh `PluginManager` *was* created for a new home, plugin
   modules are imported into `sys.modules` as `shiva_plugins.<slug>` by
   ...
   `shiva_plugins.<slug>.<submodule>`, and Python's import machinery
...
- Replace the single-slot singleton with a cache keyed on the *resolved*
  Shiva home path (`_plugin_managers_by_home: Dict[Path, PluginManager]`).
  `get_plugin_manager()` resolves the current home via `get_shiva_home()`
  (which itself already consults `get_shiva_home_override()` before
...
- Per-profile LCM instances (and any other context-engine plugin) use
  ...
  through `SHIVA_HOME` or `set_shiva_home_override()`.
...
- Regression coverage exercises the real production path
  (`set_shiva_home_override()`) rather than only the env-var path, and
