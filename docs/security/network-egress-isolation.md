...
When running Shiva inside Docker, the default `network_mode: host` gives the
...
The Shiva [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   SHIVA_UID=$(id -u) SHIVA_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=shiva,shiva-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://shiva-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Shiva trust model and vulnerability reporting
