# Shiva Agent - Development Guide

Instructions for AI coding assistants and developers working on the shiva-agent codebase.

**Never give up on the right solution.**

## What Shiva Is

Shiva is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Shiva ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`shiva tools`, `shiva setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `SHIVA_HOME`. Mocks hide integration bugs.
...
- **New `SHIVA_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `shiva tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.shiva/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `shiva <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `shiva webhook`, `shiva cron`, `shiva tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.shiva/plugins/` or a pip package, discovered at runtime.
...
one behind a plain URL + token, or Shiva Cloud. Only the first two are spawned
by us and carry `SHIVA_DESKTOP=1`. Every env-keyed GUI gate is therefore a
...
- **The toolset is the surface gate.** Keep the tools off `_SHIVA_CORE_TOOLS`
...
- **Ask which identity you actually mean.** `SHIVA_DESKTOP=1` legitimately
  ...
  and the embedded terminal pane (`shiva --tui` against that same backend) is
...
`$HOME/.shiva/shiva-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _SHIVA_CORE_TOOLS list
├── cli.py                # ShivaCLI class — interactive CLI orchestrator (~11k LOC)
├── shiva_state.py       # SessionDB — SQLite session store (FTS5 search)
├── shiva_constants.py   # get_shiva_home(), display_shiva_home() — profile-aware paths
├── shiva_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── shiva_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── shiva-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `shiva --tui`
...
**User config:** `~/.shiva/config.yaml` (settings), `~/.shiva/.env` (API keys only).
**Logs:** `~/.shiva/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_shiva_home()`.
Browse with `shiva logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Shiva: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`shiva_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `ShivaCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.shiva/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`shiva_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/shiva` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `shiva_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `ShivaCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `shiva --tui` or `SHIVA_TUI=1`.
...
npm run dev       # watch mode (rebuilds shiva-ink + tsx --watch)
npm start         # production
npm run build     # full build (shiva-ink + tsc)
...
### TUI in the Dashboard (`shiva dashboard` → `/chat`)

The dashboard embeds the real `shiva --tui` — **not** a rewrite.  See `shiva_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `shiva_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `shiva --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `shiva --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@shiva/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `shiva serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `SHIVA_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `shiva` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `shiva --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Shiva built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Shiva core.
Use the plugin route instead: create `~/.shiva/plugins/<name>/plugin.yaml`
and `~/.shiva/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Shiva tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_SHIVA_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_SHIVA_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_shiva_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `SHIVA_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_shiva_home()` for the base directory — never `Path.home() / ".shiva"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `shiva_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `shiva_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `shiva tools`, `shiva setup`, most CLI subcommands | `shiva_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`shiva_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
shiva_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.shiva/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Shiva gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `shiva_cli/skin_engine.py`:
...
Users create `~/.shiva/skins/<name>.yaml`:
...
Shiva has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.shiva/plugins/` and pip-installed entry points.

### General plugins (`shiva_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.shiva/plugins/`, `./.shiva/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `shiva` at startup so
  `shiva <pluginname> <subcmd>` works with no change to `main.py`
...
bundled, `$SHIVA_HOME/plugins/`, `./.shiva/plugins/` (opt-in via
`SHIVA_ENABLE_PROJECT_PLUGINS`), and `shiva_agent.memory_providers` entry
...
`post_setup(shiva_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `shiva <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `shiva --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `shiva_cli/main.py`, etc.).
...
into `~/.shiva/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `shiva memory setup` / `post_setup()` without
...
`~/.shiva/plugins/` (or via pip entry points). They register through
...
2. User: `$SHIVA_HOME/plugins/model-providers/<name>/`
...
[`shiva-example-plugins`](https://github.com/NousResearch/shiva-example-plugins)
companion repo, not in this tree.

### Bot Mode (`apps/desktop/src/plugins/shiva-bots/`)

The desktop "Bots" experience ships bundled in-tree. Each bot is a Shiva
...
`ui_meta['shiva-bots'].chat` and verified it per click; five hardening
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `shiva skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.shiva.tags`, `metadata.shiva.category`,
`metadata.shiva.related_skills`, `metadata.shiva.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.shiva.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Shiva tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Shiva Agent" is the secondary collaborator. If the
   contributor's commit shows "Shiva Agent" as author (because they
   used Shiva to draft the skill), replace it with their actual name
...
lives in the `shiva-agent-dev` skill at
...
`"messaging"`); `_SHIVA_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `shiva tools` (the curses UI) or the
...
Shiva returns a delegation id immediately and the result re-enters the
...
go to `~/.shiva/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `shiva_cli/curator.py` wires `shiva curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.shiva/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `shiva cron <verb>`
...
- File lock at `~/.shiva/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `shiva kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `shiva_cli/kanban.py` wires `shiva kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`shiva-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `SHIVA_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
- **Plan** (`shiva_cli/update_inventory.py`, `shiva update --plan`):
...
- **Snapshot** (`shiva_cli/backup.py`): pre-update quick snapshot for
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE
  (every `shiva-gateway*` unit / `ai.shiva.gateway*` LaunchAgent),
...
- **Report**: every run writes a machine-readable receipt to
  `~/.shiva/logs/update_receipts/` (`latest.json` pointer; steps,
...
`shiva serve` (control plane, desktop-spawned child) dies with the app
— by design. The messaging gateway (`gateway run`) SURVIVES the app: the
serve backend's `/api/gateway/*` endpoints spawn it detached
(`_spawn_shiva_action` — `start_new_session` / `DETACHED_PROCESS`), so
...
Shiva-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `SHIVA_BACKGROUND_NOTIFICATIONS` env var):
...
Shiva supports **profiles** — multiple fully isolated instances, each with its own
`SHIVA_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `shiva_cli/main.py` sets
`SHIVA_HOME` before any module imports. All `get_shiva_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_shiva_home()` for all SHIVA_HOME paths.** Import from `shiva_constants`.
   NEVER hardcode `~/.shiva` or `Path.home() / ".shiva"` in code that reads/writes state.
   ```python
   # GOOD
   from shiva_constants import get_shiva_home
   config_path = get_shiva_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".shiva" / "config.yaml"
   ```

2. **Use `display_shiva_home()` for user-facing messages.** Import from `shiva_constants`.
   This returns `~/.shiva` for default or `~/.shiva/profiles/<name>` for profiles.
   ```python
   # GOOD
   from shiva_constants import display_shiva_home
   print(f"Config saved to {display_shiva_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.shiva/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_shiva_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_shiva_home()`,
   not `Path.home() / ".shiva"`.

4. **Tests that mock `Path.home()` must also set `SHIVA_HOME`** — since code now uses
   `get_shiva_home()` (reads env var), not `Path.home() / ".shiva"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"SHIVA_HOME": str(tmp_path / ".shiva")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not SHIVA_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".shiva" / "profiles"`, NOT `get_shiva_home() / "profiles"`.
   This is intentional — it lets `shiva -p coder profile list` see all profiles regardless
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line`
  (gateway run), `shiva_cli.update_cmd._shiva_holder_subcommand`
  (top-level subcommand of any Shiva argv). Never hand-roll token scans.
...
### DO NOT hardcode `~/.shiva` paths
Use `get_shiva_home()` from `shiva_constants` for code paths. Use `display_shiva_home()`
for user-facing print/log messages. Hardcoding `~/.shiva` breaks profiles — each profile
has its own `SHIVA_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### All CLI menu-pickers MUST use curses.
Interactive menus must use `shiva_cli/curses_ui.py`. See `shiva_cli/tools_config.py` for an example.
...
canary ledger, shiva#85796 / gateway-gateway#210). Violating any of them
...
with actual imports (not mocks) against a temp `SHIVA_HOME`.

### Tests must not write to `~/.shiva/`
The `_isolate_shiva_home` autouse fixture in `tests/conftest.py` redirects `SHIVA_HOME` to a temp dir. Never hardcode `~/.shiva/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_shiva_home()` resolve within the temp dir.
Use the pattern from `tests/shiva_cli/test_profiles.py`:
```python
@pytest.fixture
...
subprocess (`--file-retries`, default 1; `SHIVA_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.shiva/` | Your real config+auth.json                  | Temp dir per test                         |
...
Shiva supports Linux, macOS and native Windows, and plenty of its behaviour
...
`tests/shiva_cli/test_venv_holder_windows_live.py` on a real
