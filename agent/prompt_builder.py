...
_SHIVA_MD_NAMES = (".shiva.md", "SHIVA.md")


def _find_shiva_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.shiva.md`` or ``SHIVA.md``.
    ...
    # could pick up a .shiva.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _SHIVA_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Shiva Agent, an intelligent AI assistant created by Nous Research. "
...
SHIVA_AGENT_HELP_GUIDANCE = (
    "You run on Shiva Agent (by Nous Research). When the user needs help with "
    "Shiva itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://shiva-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `shiva-agent` skill with skill_view(name='shiva-agent') "
...
SHIVA_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on Shiva Agent (by Nous Research). When the user needs help with "
    "Shiva itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://shiva-agent.nousresearch.com/docs is the "
    "authoritative reference and always holds the latest, most up-to-date "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.shiva/kanban.db`. Your task id is in "
    "`$SHIVA_KANBAN_TASK`; your workspace is `$SHIVA_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $SHIVA_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $SHIVA_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${SHIVA_KANBAN_BRANCH:-wt/$SHIVA_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$SHIVA_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`shiva profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `shiva kanban <verb>` for board operations. Use "
...
#
# The shiva-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to shiva-agent's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Shiva "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Shiva floating over another application, so the
    user is rarely asking about Shiva — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Shiva "
        ...
        "rather than anything inside Shiva. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Shiva terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Shiva desktop app — a graphical chat "
        ...
        "Widgets can talk back: window.shiva.send(\"prompt\") — or a "
        "data-shiva-send=\"prompt\" attribute on any clickable element — sends "
    ...
    "webui": (
        "You are in the Shiva WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Shiva itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Shiva runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Shiva itself is running. The host OS, home, and cwd "
                f"of the Shiva process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Shiva "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Shiva
    ...
    extra = (os.getenv("SHIVA_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_shiva_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("SHIVA_PLATFORM") or os.environ.get("SHIVA_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.shiva/skills/`` directory.  External dirs
    ...
    # SHIVA_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_shiva_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.shiva/skills, ./.agents/skills at
        ...
        if _home_token is not None:
            reset_shiva_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Shiva Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `shiva-agent` skill "
            "first. It has the actual commands (e.g. `shiva config set …`, `shiva tools`, "
            "`shiva setup`) so you don't have to guess or invent workarounds.\n"
...
def load_soul_md(
    ...
    """Load SOUL.md from SHIVA_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the SHIVA_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from shiva_cli.config import ensure_shiva_home
        ensure_shiva_home()
    except Exception as e:
        logger.debug("Could not ensure SHIVA_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_shiva_home()
...
def _load_shiva_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.shiva.md / SHIVA.md — walk to git root."""
    shiva_md_path = _find_shiva_md(cwd_path)
    if not shiva_md_path:
        return ""
    try:
        content = shiva_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = shiva_md_path.name
        try:
            rel = str(shiva_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".shiva.md", context_length=context_length,
            read_path=str(shiva_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", shiva_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .shiva.md / SHIVA.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from SHIVA_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Shiva install/source
    ...
    # explicitly configured cwd is honored verbatim — the Shiva tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Shiva in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Shiva install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_shiva_md(cwd_path, context_length)
    ...
    # SOUL.md from SHIVA_HOME only — skip when already loaded as identity
    if not skip_soul:
