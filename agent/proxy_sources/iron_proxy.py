...
* The ``iron-proxy`` binary is auto-installed into ``<shiva_home>/bin/iron-proxy``
  on first use.  Shiva pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<shiva_home>/proxy/ca.{crt,key}`` is generated on
  first ``shiva egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<shiva_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``shiva egress start``), pidfile
  at ``<shiva_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<shiva_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``shiva egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<shiva_home>/proxy/management.token`` (0600), and injected into the
...
_MGMT_API_KEY_ENV = "SHIVA_IRON_PROXY_MGMT_KEY"
...
# credential (Shiva' auth.py keys Google on both GEMINI_API_KEY and
...
def _shiva_bin_dir() -> Path:
    from shiva_constants import get_shiva_home

    return get_shiva_home() / "bin"
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.shiva/proxy/`` just to
    check whether a pidfile exists.
    """
    from shiva_constants import get_shiva_home

    return get_shiva_home() / "proxy"
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<shiva_home>/bin/iron-proxy``  (our managed copy — preferred)
    ...
    managed = _shiva_bin_dir() / _platform_binary_name()
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``shiva proxy install`` surface lets them
    propagate so the wizard can show a clear error.
    """

    bin_dir = _shiva_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="shiva-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "shiva-agent"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``shiva egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="shiva-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
...
def mint_proxy_token(prefix: str = "shiva-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<shiva_home>/proxy/management.token`` with 0600 perms.
    The daemon receives it via the ``SHIVA_IRON_PROXY_MGMT_KEY`` env var
    (named in the generated config's ``management.api_key_env``);
    ``shiva egress reload`` reads the same file to authenticate.
    """
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="shiva-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `shiva egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `shiva egress setup` and use "
            "`shiva egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `shiva egress setup`, "
            "then `shiva egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``shiva egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `shiva egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<shiva_home>/proxy/proxy.yaml``.

    Uses ``yaml.safe_dump`` so we never emit Python tags.
    """

    try:
        import yaml  # PyYAML is already a Shiva dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``shiva egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``shiva egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_SHIVA_IRON_PROXY_NONCE_ENV = "SHIVA_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_SHIVA_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `shiva egress install`."
        )
    ...
    env[_SHIVA_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # close-on-success path, every restart leaked one fd in the Shiva
    # process.
    #
    ...
    # ``shiva egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``shiva egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `shiva egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`shiva egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `shiva secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
