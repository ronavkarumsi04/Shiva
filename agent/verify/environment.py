...
The manifest lives at ``<project>/.shiva/environment.json`` and is the
...
MANIFEST_VERSION = 1
_MANIFEST_RELPATH = Path(".shiva") / "environment.json"
...
def load_manifest(root: Path) -> Recipe | None:
    ...
    fresh detection instead of breaking ``shiva verify``.
    """
