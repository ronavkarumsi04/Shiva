...
row taxonomy is inferred from the atlas shape so Shiva can render both legacy
...
# rows (1536x1872), while older Shiva/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Shiva' activity state names. They are not always identical to the
...
# Legacy Shiva/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Shiva activity names -> accepted row-name aliases in descending
