...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the SHIVA_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Shiva runtime / deployment
    "SHIVA_HOME", "SHIVA_PROFILE", "SHIVA_GATEWAY_LOCK_DIR",
    "SHIVA_MAX_ITERATIONS", "SHIVA_MAX_TOKENS", "SHIVA_API_TIMEOUT",
    "SHIVA_REDACT_SECRETS", "SHIVA_NOUS_TIMEOUT_SECONDS",
    "_SHIVA_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "SHIVA_KANBAN_DB", "SHIVA_KANBAN_WORKSPACES_ROOT", "SHIVA_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "SHIVA_KANBAN_",
    "SHIVA_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Shiva writes
    ...
    ``shiva_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Shiva parser: save_env_value
    ...
    from shiva_cli.config import _parse_env_value
...
def build_profile_secret_scope(shiva_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(shiva_home)
    secrets = load_env_file(home / ".env")

    try:
        from shiva_cli.env_loader import get_secret_source_values
