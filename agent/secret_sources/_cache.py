...
cache problem must never block Shiva startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Shiva home used for cache paths.

    ``home_path`` is whatever ``load_shiva_dotenv()`` already resolved;
    falling back to ``$SHIVA_HOME`` / ``~/.shiva`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from shiva_constants import get_shiva_home

        home_path = get_shiva_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<shiva_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if SHIVA_HOME is ever made traversable.
            try:
