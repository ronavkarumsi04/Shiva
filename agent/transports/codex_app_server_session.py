...
Owns one Codex thread per Shiva session. Drives `turn/start`, consumes
...
# Shiva' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_SHIVA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Shiva/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Shiva session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _SHIVA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("SHIVA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="shiva",
            client_title="Shiva Agent",
            client_version=_get_shiva_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Shiva' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Shiva' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own shiva-tools callback we
            # auto-accept — the user already approved Shiva' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Shiva
        approval-mode/timeout logic. The Shiva-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Shiva approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Shiva approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Shiva returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Shiva approval mode/timeout resolution
    ...
    # "prompt expired"; the Shiva-side messaging already distinguishes them.
    return "decline"
...
def _get_shiva_version() -> str:
    """Best-effort Shiva version string for codex's userAgent line."""
    try:
