...
projection into Shiva' display, approval bridging, transcript projection into
...
"codex_app_server"`. Shiva' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Shiva secret — gateway
        ...
        spawn_env = shiva_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("SHIVA_KANBAN_TASK"):
            kanban_db = spawn_env.get("SHIVA_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "SHIVA_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("SHIVA_HOME", os.path.expanduser("~/.shiva")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "shiva",
        client_title: str = "Shiva Agent",
