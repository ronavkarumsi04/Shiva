...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Shiva with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.shiva/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Shiva
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``shiva tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``shiva setup`` for
