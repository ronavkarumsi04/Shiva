...
Every Shiva request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Shiva Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=shiva-agent",
        "client=shiva-client-v<__version__>",
    ]

The version is sourced live from ``shiva_cli.__version__`` so it auto-aligns
...
``shiva_cli.__version__`` is the canonical source of truth.
"""
...
def _shiva_version() -> str:
    """Return the current Shiva release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``shiva_cli`` cannot be imported (should
...
def shiva_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=shiva-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=shiva-client-v{_shiva_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Shiva session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Shiva conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.shiva/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Shiva
    ...
    """
    tags = ["product=shiva-agent", shiva_client_tag()]
