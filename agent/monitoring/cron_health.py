...
def build_cron_health_snapshot() -> CronHealthSnapshot:
    metrics: list[GatewayMetric] = []
    for name, reader in (
        ("shiva.cron.scheduler.heartbeat_age_seconds", get_ticker_heartbeat_age),
        ("shiva.cron.scheduler.last_success_age_seconds", get_ticker_success_age),
    ):
        try:
            value = reader()
    ...
    try:
        ...
        metrics.append(GatewayMetric("shiva.cron.jobs.enabled", len(enabled), {}))
        metrics.append(
            GatewayMetric(
    ...
    try:
        metrics.append(
            GatewayMetric("shiva.cron.jobs.running", len(get_running_job_ids()), {})
        )
    except Exception:
