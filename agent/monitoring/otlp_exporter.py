...
configured under ``monitoring.export.otlp``. Lets an operator stream Shiva
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install shiva-agent[otlp]``), imported lazily so the
...
def _span_attrs(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kind = ev.get("event")
    attrs: Dict[str, Any] = {"shiva.event": kind or "unknown"}
    keep_by_kind = {
...
def export_batch(provider, batch: List[Dict[str, Any]]) -> int:
    ...
    tracer = provider.get_tracer("shiva.monitoring")
    n = 0
    for ev in batch:
        try:
            name = f"shiva.{ev.get('event', 'event')}"
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'shiva-agent[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
