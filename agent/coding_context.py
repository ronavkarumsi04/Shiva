"""Coding-context awareness — base Shiva, every interactive surface.

When the user runs Shiva inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Shiva shifts into a **coding posture**. This module is the
...
from shiva_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("shiva.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "shiva", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _SHIVA_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``SHIVA_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Shiva should operate in its coding posture right now."""
