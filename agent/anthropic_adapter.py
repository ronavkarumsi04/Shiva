"""Anthropic Messages API adapter for Shiva Agent.

Translates between Shiva's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in shiva_cli/runtime_provider.py.
    """
...
# Shiva effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to shiva's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to shiva's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + ShivaAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://shiva-agent.nousresearch.com")
        headers.setdefault("X-Title", "Shiva Agent")
        headers.setdefault("User-Agent", f"ShivaAgent/{_SHIVA_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Shiva loads that into the process env from
    # ``~/.shiva/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Shiva notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Shiva historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``shiva models``) never mutates ``~/.shiva/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Shiva)
      ...
      5. Anthropic credential_pool OAuth entry (~/.shiva/auth.json)
    ...
    # 1. Shiva-managed OAuth/setup token env var
    ...
    # 5. Shiva credential_pool OAuth entry.
...
# ── Shiva-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.shiva/.anthropic_oauth.json (our own file).
...
def _get_shiva_oauth_file() -> Path:
    return get_shiva_home() / ".anthropic_oauth.json"
...
def run_shiva_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Shiva-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Shiva with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from shiva_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_shiva_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Shiva-managed OAuth credentials from ~/.shiva/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Shiva OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See shiva-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See shiva-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See shiva-agent#13848 (Kimi) and
    shiva-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Shiva Agent", "Claude Code")
                text = text.replace("Shiva agent", "Claude Code")
                text = text.replace("shiva-agent", "claude-code")
        ...
        #      a) bare Shiva-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Shiva surfaces in its CLI. We
