...
``auth.json``.  They intentionally have no dependency on ``shiva_cli.auth`` so
...
# Sources Shiva owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Shiva-owned OAuth/device-code state)
