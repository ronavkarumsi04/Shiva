...
appropriate.  All installs go to a Shiva-owned bin staging dir,
``<SHIVA_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``shiva lsp
  status``.
...
# ``<SHIVA_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `shiva lsp status` reports the host's presence.
...
def shiva_lsp_bin_dir() -> Path:
    """Return the Shiva-owned bin staging dir for LSP servers."""
    from shiva_constants import get_shiva_home

    p = get_shiva_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(shiva_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $SHIVA_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Shiva installed and
    ...
    if npm is None:
        ...
        return None
    staging = shiva_lsp_bin_dir().parent  # <SHIVA_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a shiva-owned target dir.
    ...
    console script.
    """
    pip_target = shiva_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = shiva_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``shiva lsp status`` CLI to give users a quick
