...
``shiva pause`` writes a sentinel file at ``$SHIVA_HOME/ESTOP``;
``shiva resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Shiva is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.shiva/ESTOP``.
...
def _shiva_home() -> Path:
    """Resolve the active SHIVA_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active SHIVA_HOME."""
    return _shiva_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on SHIVA_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Shiva is paused ({reason}). New work is on hold; "
            "run `shiva resume` to pick things back up."
        )
    return (
        "⏸️ Shiva is paused. New work is on hold; "
        "run `shiva resume` to pick things back up."
    )
