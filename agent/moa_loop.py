...
MoA-enabled; the normal Shiva agent loop still owns tool calling and turn
...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from shiva_cli.moa_config import coerce_privacy_filter
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Shiva boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model Shiva agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Shiva agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Shiva agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
