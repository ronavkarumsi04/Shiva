"""Upload a Shiva session transcript to Hugging Face as an agent trace.

Shiva stores sessions in its own SQLite store (``shiva_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``shiva trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Shiva' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "shiva-traces"
_SHIVA_VERSION = "shiva-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Shiva OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Shiva' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Shiva conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.shiva/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `shiva trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/shiva-traces`` dataset. Returns a
