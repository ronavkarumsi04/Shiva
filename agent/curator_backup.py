...
A pre-run snapshot of ``~/.shiva/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.shiva/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.shiva/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_shiva_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.shiva/cron/jobs.json``)."""
    return get_shiva_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.shiva/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.shiva/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.shiva/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.shiva/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `shiva curator rollback --list` to see available snapshots)",
            None,
        )
