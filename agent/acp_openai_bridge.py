"""OpenAI-shape bridge shared by Shiva' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Shiva as
...
the agent's *own* tool notifications. Shiva' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Shiva' perspective).
...
of its own (Copilot) forwards everything Shiva offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Shiva'
agent-level tools, because re-offering the overlapping ones makes Shiva re-run
...
class StreamChunks(list):
    ...
    Shiva reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``shiva_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Shiva'
