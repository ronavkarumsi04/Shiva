...
def _shiva_home_path() -> Path:
    """Resolve the active SHIVA_HOME (profile-aware) without circular imports."""
    try:
        from shiva_constants import get_shiva_home  # local import to avoid cycles
        return get_shiva_home()
    except Exception:
        return Path(os.path.expanduser("~/.shiva"))


def _shiva_root_path() -> Path:
    """Resolve the Shiva root dir (always the parent of any profile, never per-profile)."""
    try:
        from shiva_constants import get_default_shiva_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    shiva_home = _shiva_home_path()
    shiva_root = _shiva_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(shiva_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(shiva_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(shiva_home / "cache" / "bws_cache.enc.json"),
            str(shiva_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved SHIVA_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("SHIVA_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    shiva_dirs = []
    for base in (_shiva_home_path(), _shiva_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in shiva_dirs:
                shiva_dirs.append(real)
        except Exception:
            continue

    for base_real in shiva_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside SHIVA_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Shiva path.

    Three categories are blocked:

      * Internal Shiva cache files under ``SHIVA_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under SHIVA_HOME and the global Shiva
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.shiva/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active SHIVA_HOME (profile-aware) AND the global
    # Shiva root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (SHIVA_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    shiva_dirs: list[Path] = []
    for base in (_shiva_home_path(), _shiva_root_path()):
        try:
            real = base.resolve()
            if real not in shiva_dirs:
    ...
    for hd in shiva_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Shiva cache file "
    ...
    for hd in shiva_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Shiva credential store "
    ...
    for hd in shiva_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Shiva MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Shiva MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
    ...
    for hd in shiva_dirs:
        ...
        if resolved == browser_profile:
            return (
                f"Access denied: {path} is the Shiva real-profile browser "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is inside the Shiva real-profile browser "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Shiva read (see
...
#
# Shiva profiles are separate SHIVA_HOME dirs under
...
#
# Reference: May 2026 incident where a shiva-security profile session
# edited skills under both ``~/.shiva/profiles/shiva-security/skills/``
# AND ``~/.shiva/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under SHIVA_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from SHIVA_HOME.
    ...
    """
    try:
        home_real = _shiva_home_path().resolve()
        root_real = _shiva_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Shiva scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Shiva entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Shiva profile {info['target_profile']!r}, but the "
...
#
#   <SHIVA_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.shiva/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.shiva/…`` segment and warns
# regardless of which Shiva profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.shiva/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.shiva`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.shiva/…`` and returns the
    index where the inner Shiva-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .shiva / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".shiva":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Shiva state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.shiva``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.shiva`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Shiva resolver to
    succeed, so it works correctly even when called from contexts where
    SHIVA_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Shiva process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real SHIVA_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.shiva/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.shiva/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host SHIVA_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Shiva state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Shiva "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real SHIVA_HOME. Use the "
