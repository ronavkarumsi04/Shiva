...
Plugins built on Shiva Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Shiva
...
:class:`~shiva_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Shiva' main config
...
handles every provider, fallback chain, and per-task override Shiva
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`shiva_cli.plugins.PluginContext`
