...
Data resolution order:
  ...
  2. Disk cache (~/.shiva/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Shiva ↔ models.dev
# ---------------------------------------------------------------------------

# Shiva provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Shiva' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Shiva ids (built lazily; many-to-one,
# e.g. both "meta" and "meta-ai" may map to the same models.dev id).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_shiva_ids(mdev_id: str) -> List[str]:
    """Return the Shiva provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for shiva_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(shiva_id)
...
def _get_cache_path() -> Path:
    ...
    return get_shiva_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from shiva_constants import get_shiva_home
    return get_shiva_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``shiva config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Shiva provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Shiva id.
    for shiva_id in _models_dev_to_shiva_ids(provider_key):
        if shiva_id != provider_key:
            candidates.append(shiva_id)
...
def _get_provider_models(
    ...
    """Resolve a Shiva provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``shiva model``), which is an interactive user-facing flow
    where a fresh catalog is worth a short network wait.
    """
    from shiva_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Shiva agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Shiva provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Shiva ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Shiva or models.dev provider ID.  Tries exact match then
