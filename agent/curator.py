...
def _state_file() -> Path:
    return get_shiva_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.shiva/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `shiva update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``shiva curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``shiva update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``shiva curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `shiva curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Shiva' background skill CURATOR. This is an "
    ...
    "into ~/.shiva/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: shiva-config-*, shiva-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.shiva/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.shiva/logs/curator/``)
    ...
    authored skill data in ``~/.shiva/skills/``.

    ``ensure_shiva_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_shiva_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every shiva install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: shiva curator status
        keep an umbrella stable: shiva curator pin document-tools
    ...
    lines.append("full report: shiva curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: shiva curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.shiva/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.shiva/skills/.archive/` for "
            "safety and can be restored via `shiva curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.shiva/skills/.archive/`. "
            "Restore any via `shiva curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `shiva curator restore <name>`")
    lines.append("- All archives live under `~/.shiva/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``shiva curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `shiva curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `shiva update` only if "
        ...
        # recorded in state so `shiva curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``shiva model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `shiva model` → auxiliary
