...
# Shiva' own machine-authored openers. A compaction handoff or a resumed
...
def _auto_title_enabled() -> bool:
    ...
    try:
        ...
        # from agent code paths where a module-level shiva_cli import risks
...
def auto_title_session(
    ...
    post-``shiva update`` stale-module window, where this function's lazy
    ...
    try:
        ...
        logger.warning(
            ...
            "restart the running Shiva process): %s",
            e,
        )
...
def _is_real_user_turn(message: Any) -> bool:
    ...
    Shiva persists a lot of machinery under ``role="user"`` — compaction
