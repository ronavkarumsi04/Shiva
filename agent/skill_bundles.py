...
Storage
-------
Bundles live in ``~/.shiva/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SHIVA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``shiva bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under SHIVA_HOME.

    Honors ``SHIVA_BUNDLES_DIR`` for tests; falls back to
    ``<SHIVA_HOME>/skill-bundles``.
    """
    override = os.environ.get("SHIVA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_shiva_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `shiva bundles` CLI subcommand.
# ---------------------------------------------------------------------------
