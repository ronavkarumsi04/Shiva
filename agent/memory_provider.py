...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `shiva backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - shiva_home (str): The active SHIVA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.shiva``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "shiva").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'shiva memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], shiva_home: str) -> None:
        ...
        Called by 'shiva memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``shiva_home`` is the active SHIVA_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE SHIVA_HOME.

        ``shiva backup`` only walks SHIVA_HOME, so any provider state kept
        ...
        of the archive, and ``shiva import`` restores them to their original
