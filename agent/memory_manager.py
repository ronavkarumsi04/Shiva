...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_SHIVA_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Shiva expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile SHIVA_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped SHIVA_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``shiva_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_shiva_home()`` themselves.
        """
        if "shiva_home" not in kwargs:
            from shiva_constants import get_shiva_home
            kwargs["shiva_home"] = str(get_shiva_home())
        for provider in self._providers:
            try:
