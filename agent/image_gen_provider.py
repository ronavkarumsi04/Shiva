...
as ``kind: backend``) or ``~/.shiva/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``shiva tools``
...
def _images_cache_dir() -> Path:
    """Return ``$SHIVA_HOME/cache/images/``, creating parents as needed."""
    from shiva_constants import get_shiva_home

    path = get_shiva_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$SHIVA_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$SHIVA_HOME/cache/images/``.
