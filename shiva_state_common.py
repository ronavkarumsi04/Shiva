...
Extracted verbatim from shiva_state.py so the SessionDB mixin modules
(shiva_state_search / shiva_state_schema / shiva_state_portability) can
reference them without importing shiva_state (which would be a cycle).
shiva_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``shiva sessions optimize-storage``. A legacy DB sits at
...
# `shiva sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `shiva db optimize`. This is
...
#
# Several independent Shiva processes routinely share one state.db (gateway
# service, the Desktop app's `shiva serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `shiva_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not shiva_state)
# because the search/schema mixins cannot import shiva_state (cycle).
#
