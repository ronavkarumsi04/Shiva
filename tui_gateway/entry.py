...
# from shadowing Shiva's own top-level modules.  ``shiva_bootstrap`` lives at
...
import shiva_bootstrap

shiva_bootstrap.harden_import_path()
...
# shared owner in shiva_cli.mcp_startup — the wait/in-flight/join helpers
...
# shiva_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `SHIVA_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("SHIVA_TUI_SIDECAR_URL")
...
# tested; override via ``SHIVA_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("SHIVA_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``SHIVA_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``shiva --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``shiva_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from shiva_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # → shiva_cli.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's SHIVA_HOME first), so a launch profile with no
    ...
    try:
        from shiva_cli.mcp_startup import (
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``shiva --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``shiva dashboard`` spawn theirs via
    ``shiva_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``shiva_cli.mcp_startup`` thread used by the
    ...
    try:
        from shiva_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def _has_configured_mcp_servers() -> bool:
    ...
    from shiva_cli.mcp_startup import _has_configured_mcp_servers as configured
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's SHIVA_HOME override — the shared owner in
    ``shiva_cli.mcp_startup`` captures the caller's context-local override
...
def main():
    ...
    # Live-apply skins Shiva activates mid-conversation.
    ...
    try:
        from shiva_cli.model_switch import prewarm_picker_cache_async
