...
def _default_registry_path() -> Path:
    return get_shiva_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_shiva_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_shiva_home = expected_shiva_home if expected_shiva_home is not None else str(get_shiva_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["SHIVA_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("shiva_home") or "")
        if got_home and got_home != self.expected_shiva_home:
            raise RuntimeError(f"compute host SHIVA_HOME mismatch: {got_home} != {self.expected_shiva_home}")
