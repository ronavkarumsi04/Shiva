...
//!
//! Mirrors `shiva_constants.get_shiva_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\shiva
//!   macOS:   ~/.shiva
//!   Linux:   ~/.shiva  (override via $SHIVA_HOME)
//!
//! NOTE (macOS): Python's get_shiva_home(), scripts/install.sh, and the
//! Electron desktop's resolveShivaHome() ALL use ~/.shiva on macOS — there
...
/// Returns the canonical Shiva home directory, respecting $SHIVA_HOME if set.
pub fn shiva_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("SHIVA_HOME") {
    ...
    {
        // %LOCALAPPDATA%\shiva — matches scripts/install.ps1's $ShivaHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("shiva");
        }
    }

    // macOS + Linux + fallback: ~/.shiva (matches Python get_shiva_home(),
    // install.sh, and the Electron desktop's resolveShivaHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".shiva");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    shiva_home().join("bootstrap-cache")
}
...
/// SHIVA_HOME so it survives repo checkout deletion (unlike anything under
/// shiva-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\shiva\shiva-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_shiva`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under SHIVA_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves SHIVA_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    shiva_home().join(".shiva-update-in-progress")
}
...
pub fn copy_self_to_shiva_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to SHIVA_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_SHIVA_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".shiva-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under SHIVA_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[shiva-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("SHIVA_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_shiva_home() -> String {
    shiva_home().to_string_lossy().into_owned()
}
