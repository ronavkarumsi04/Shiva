...
export const $shivaHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.shiva/logs/bootstrap-installer.log')
    $shivaHome.set('~/.shiva')
  ...
  try {
    const [logPath, shivaHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_shiva_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $shivaHome.set(shivaHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Shiva" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Shiva-Setup.exe --update);
...
export async function launchShivaDesktop(): Promise<void> {
  ...
  await invoke('launch_shiva_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Shiva repository' },
