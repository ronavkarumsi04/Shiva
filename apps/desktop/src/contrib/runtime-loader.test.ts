...
import type { ShivaReadDirResult } from '@/global'
import type * as ShivaModule from '@/shiva'
...
// getStatus would supply the connected backend's shiva_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ shiva_home: '/remote/box/.shiva' }))

vi.mock('@/shiva', async importActual => ({
  ...(await importActual<typeof ShivaModule>()),
  getStatus: () => getStatus()
}))

const desktopPluginsRoot = vi.fn<() => Promise<string>>()
const agentPluginsRoot = vi.fn<() => Promise<string>>()
const readDir = vi.fn<(path: string) => Promise<ShivaReadDirResult>>()
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { shivaDesktop: unknown }).shivaDesktop = {
    agentPluginsRoot,
    desktopPluginsRoot,
...
describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend shiva_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.shiva/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.shiva/plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.shiva/desktop-plugins')
    expect(readDir).toHaveBeenCalledWith('/local/.shiva/plugins')
    // The remote backend's shiva_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.shiva/desktop-plugins')
  })
  ...
  it('treats a package without a Desktop half as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.shiva/plugins') {
        return { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.shiva/plugins/my-feature' }] }
      }

      if (dir === '/local/.shiva/plugins/my-feature') {
        return {
          entries: [{ isDirectory: false, name: 'plugin.yaml', path: '/local/.shiva/plugins/my-feature/plugin.yaml' }]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.shiva/plugins/my-feature')
    expect(readDir).not.toHaveBeenCalledWith('/local/.shiva/plugins/my-feature/desktop')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    agentPluginsRoot.mockResolvedValue('')
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.shiva/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.shiva/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.shiva/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.shiva/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('still scans the standalone root when agentPluginsRoot is absent (older shell)', async () => {
    delete (window.shivaDesktop as unknown as { agentPluginsRoot?: unknown }).agentPluginsRoot
    desktopPluginsRoot.mockResolvedValue('/local/.shiva/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.shiva/plugins') {
        return { entries: [{ isDirectory: true, name: 'uni', path: '/local/.shiva/plugins/uni' }] }
      }

      if (dir === '/local/.shiva/plugins/uni') {
        return {
          entries: [{ isDirectory: true, name: 'desktop', path: '/local/.shiva/plugins/uni/desktop' }]
        }
      }

      if (dir === '/local/.shiva/plugins/uni/desktop') {
        return {
            ...
            ? [
                {
                  isDirectory: false,
                  name: 'plugin.js',
                  path: '/local/.shiva/plugins/uni/desktop/plugin.js'
                }
              ]
            : []
        }
    ...
    try {
      ...
      // ~/.shiva/plugins stays installed-but-inert until the user toggles it.
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches both Electron-resolved local roots, never the backend shiva_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.shiva/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.shiva/plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.shiva/desktop-plugins')
    expect(watchDirectory).toHaveBeenCalledWith('/local/.shiva/plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.shiva/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.shiva/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.shivaDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.shiva/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.shiva/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.shiva/desktop-plugins/huge/plugin.js'
      })
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'shiva-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "shiva-bots", name: "Bot Mode", register() {} }',
        'shiva-bots',
        { file: '/local/.shiva/desktop-plugins/shiva-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['shiva-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['shiva-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.shiva/desktop-plugins/shiva-bots/plugin.js'
      })
