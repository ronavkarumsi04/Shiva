/**
 * Live skin sync from the Shiva backend.
 *
 * The backend resolves the active skin (built-in or `$SHIVA_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Shiva
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { ShivaSkin } from '@shiva/shared/skin'
 ...
 */
export function ingestBackendSkin(skin: ShivaSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as ShivaSkin)
