...
const SKIN_KEY = 'shiva-desktop-theme-v2'
const MODE_KEY = 'shiva-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'shiva-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'shiva-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'shiva-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.shivaTheme = skinName
  root.dataset.shivaMode = rendered
  ...
  window.shivaDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('shiva-boot-background', chromeBg)
    window.localStorage.setItem('shiva-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.shivaThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.shivaDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Shiva authoring/activating a skin from a
