...
import type { ShivaReviewFile, ShivaReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Shiva' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'shiva.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'shiva.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'shiva.desktop.reviewTreeMode'
const SELECTED_KEY = 'shiva.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ShivaReviewFile[]>([])
...
export const $reviewShipInfo = atom<ShivaReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['shivaDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ShivaReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly ShivaReviewFile[], path: string): ShivaReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.shivaDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.shivaDesktop?.openExternal?.(url)
    }
