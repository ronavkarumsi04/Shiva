 ...
 * composer with the reply scrolling above it, so Shiva can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.shivaDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.shivaDesktop?.hud
...
export function resetHudLayout(): void {
  void window.shivaDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.shivaDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.shivaDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
