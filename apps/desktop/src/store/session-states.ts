...
import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@shiva/shared'
...
const TILES_KEY = 'shiva.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'shiva.desktop.sessionTiles.v1'
...
export const $focusedStoredSessionId = computed(
  ...
  (groupId, tree, selected, workspaceMode) => {
    ...
    // it to the sidebar group, whose active pane ('shiva-bots:pane') is not a
...
// Dev hook for automation (mirrors __SHIVA_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__SHIVA_SESSION_TILES__ = {
