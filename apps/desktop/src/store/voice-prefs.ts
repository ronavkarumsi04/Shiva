...
import { getShivaConfigRecord, saveShivaConfig } from '@/shiva'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getShivaConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveShivaConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
