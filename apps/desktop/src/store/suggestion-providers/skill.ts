 ...
 *  Working in `~/www/shiva-agent`, the draft says "shiva-agent" constantly
 *  — a "Use skill: shiva-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`shiva-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/shiva-agent-dev`, `plugin:skill`). */
