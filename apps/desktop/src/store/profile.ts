import { LOCAL_CONNECTION_ID } from '@shiva/shared'
import { atom, batch, computed } from 'nanostores'

import type { ShivaConnection } from '@/global'
import { getProfiles, shivaApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/shiva'
...
import type { ProfileInfo } from '@/types/shiva'
...
// /api/profiles/active `current`). "default" is the root ~/.shiva. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'shiva.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'shiva.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await shivaApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new SHIVA_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.shivaDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<ShivaConnection | null> {
  const getConnection = window.shivaDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<ShivaConnection | null> {
  const getConnectionFor = window.shivaDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'shiva.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.shivaDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.shivaDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.shiva) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.shivaDesktop?.touchBackend?.(target).catch(() => undefined)
}
