...
import type { ShivaActiveWork } from '@/global'
import { $sessions } from '@/store/session'
import { $workingSessionIds } from '@/store/session-states'

const $activeWork = computed([$workingSessionIds, $sessions], (workingIds, sessions): ShivaActiveWork => {
...
if (typeof window !== 'undefined') {
  ...
  $activeWork.subscribe(work => {
    ...
    lastSent = next
    window.shivaDesktop?.setActiveWork?.(work)
  })
}
