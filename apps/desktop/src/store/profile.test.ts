...
const { getProfiles } = await import('@/shiva')
...
const remoteConn = (over: Partial<ShivaConnection> = {}): ShivaConnection =>
  ({ baseUrl: 'https://shiva-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ShivaConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { shivaDesktop: { getConnection } })
