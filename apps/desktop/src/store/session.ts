import type { ConnectionState } from '@shiva/shared'
...
import type { SessionInfo, UsageStats } from '@/types/shiva'
...
const COMPOSER_MODEL_KEY = 'shiva.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'shiva.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'shiva.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'shiva.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'shiva.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'shiva.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'shiva.desktop.lastRoute'
...
function workspaceCwdKey(connection: ShivaConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.shivaDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.shivaDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<ShivaConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'shiva.desktop.sessionOwnerHints.v1'
...
export const setConnection = (next: Updater<ShivaConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useShivaConfig.
