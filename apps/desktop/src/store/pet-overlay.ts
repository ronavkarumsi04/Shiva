 ...
 * app's bounds and stays visible while Shiva is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'shiva.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'shiva.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.shivaDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.shivaDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.shivaDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.shivaDesktop?.petOverlay?.close()
}
