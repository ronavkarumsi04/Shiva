...
vi.mock('@/shiva', () => ({
  getShivaConfigRecord: vi.fn(async () => ({})),
  saveShivaConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye shiva', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye shiva')
  })
