...
import { getShivaConfigRecord, type ShivaConfigRecord, saveShivaConfig } from '@/shiva'
...
export interface I18nConfigClient {
  getConfig: () => Promise<ShivaConfigRecord>
  saveConfig: (config: ShivaConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.shivaDesktop?.api) {
      return Promise.resolve({})
    }

    return getShivaConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.shivaDesktop?.api) {
...
export function getConfigDisplayLanguage(config: ShivaConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ShivaConfigRecord, locale: Locale): ShivaConfigRecord {
