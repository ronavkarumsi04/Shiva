...
import { capabilityScoped, shivaApi, type ProfileScope, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getProfiles(): Promise<ProfilesResponse> {
  return shivaApi<ProfilesResponse>({
    path: '/api/profiles',
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return shivaApi<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string, scope?: ProfileScope): Promise<{ ok: boolean; path: string }> {
  ...
  return shivaApi<{ ok: boolean; path: string }>({
    ...profileOwnerScoped(scope),
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return shivaApi<ProfileSetupCommand>({
...
export function exportProfileArchive(
  ...
  return shivaApi<{ archive: string; ok: boolean }>({
...
export function importProfileArchive(
  ...
  return shivaApi<{ desktop: null | ProfileDesktopOverlay; name: string; ok: boolean; path: string }>({
