import { JsonRpcGatewayClient } from '@shiva/shared'

import type { ShivaApiRequest } from '@/global'
...
// "Timed out connecting to Shiva backend" that hangs the UI (#48504).
//
...
export class ShivaGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Shiva gateway connection closed',
      connectErrorMessage: 'Could not connect to Shiva gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Shiva gateway is not connected',
...
// process to route them to that host (shiva:api's registry branch). Null
...
export function shivaApi<T>(request: ShivaApiRequest): Promise<T> {
  return window.shivaDesktop.api<T>({ ...connectionScoped(), ...request })
}
