...
// Bot Mode title or `shiva profile rename` display_name — must update what
...
test('mentionNameForms: slugged + collapsed, reserved tokens dropped', () => {
  ...
  // A bot renamed "Shiva" or "everyone" cannot hijack reserved tags.
  assert.equal(mentionNameForms('Shiva').length, 0)
...
test('botMentionTag: renamed slug wins, profile handle is the fallback', () => {
  ...
  assert.equal(botMentionTag({ name: 'default' }), 'shiva')
  // display_name (shiva profile rename) drives the tag too.
...
test('parseGroupChatMentions: display_name and ui_meta title forms address a member', () => {
  ...
  const members = [
    ...
    { name: 'ops', title: '', ui_meta: { 'shiva-bots': { title: 'Night Watch' } } }
  ]
