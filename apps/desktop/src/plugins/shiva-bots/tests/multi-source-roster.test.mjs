...
function runtime() {
  ...
  const code = source
    .replace(/^import \* as sdk from '@shiva\/plugin-sdk'\r?\n/m, '')
...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "Shiva". Regression:
  ...
  assert.equal(name(active, undefined), 'Shiva')
})
...
test('botHandle: precomputed multi-source handle wins; default stays shiva', () => {
  ...
  assert.equal(botHandle('default'), 'shiva')
})
...
test('displayName: local default stays Shiva; remote default uses the device label', () => {
...
test('resolveRosterMentions: @shiva in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'shiva' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @shiva and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })
