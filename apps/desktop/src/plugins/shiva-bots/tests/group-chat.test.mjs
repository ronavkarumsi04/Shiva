...
function load(turnScript, { busyUntilResumeCall, clarifyUntilResumeCall, approvalUntilResumeCall, conflictOnce = false, deferredTimers = false } = {}) {
      ...
      request: async (method, params) => {
        ...
        if (method === 'profiles.configure') {
          if (conflictOnce && !injectedConflict) {
            ...
            sharedUiMeta['shiva-bots-groups'] = {
              version: 2,
              rooms: {
                Shared: {
                  revision: 1,
            ...
            uiMetaRevisions['shiva-bots-groups'] = 1
            return {
              applied: {
                ui_meta: false,
                ui_meta_conflicts: { 'shiva-bots-groups': { expected: 0, actual: 1 } },
                ui_meta_revisions: { 'shiva-bots-groups': 1 }
              }
            }
          }
  ...
  const source = pluginSource
    .replace(/^import\s+\*\s+as\s+sdk\s+from '@shiva\/plugin-sdk'\r?\n/m, '')
    .replace(/^import\s+\{[\s\S]*?\}\s+from '@shiva\/plugin-sdk'\r?\n/m, '')
...
test('group room messages and members mirror through bounded gateway profile metadata', async () => {
  ...
  const envelope = configure.params.ui_meta['shiva-bots-groups']
...
test('an explicit final-room disband may clear the gateway room mirror', async () => {
  ...
  const configure = gc.requests.filter(call => call.method === 'profiles.configure').at(-1)
  assert.deepEqual(Object.keys(configure.params.ui_meta['shiva-bots-groups'].rooms), [])
  assert.ok(configure.params.ui_meta['shiva-bots-groups'].deleted['name:Research'] > 0)
})
...
test('sync worker retries a gateway CAS conflict and publishes the merged room', async () => {
  ...
  for (let i = 0; i < 20 && (gc.uiMetaRevisions['shiva-bots-groups'] || 0) < 2; i++) {
    await new Promise(resolve => setImmediate(resolve))
  }

  const stored = gc.sharedUiMeta['shiva-bots-groups']
  assert.equal(gc.uiMetaRevisions['shiva-bots-groups'], 2)
  assert.equal(
...
test('fan-out class: a room write is mirrored to every reachable default-profile gateway', async () => {
  ...
  gc.host.requestProfile = async (route, method) => {
    ...
    if (method === 'profiles.configure') {
      return { applied: { ui_meta: true, ui_meta_revisions: { 'shiva-bots-groups': 1 } } }
    }
    return {}
  }
...
test('default profile speaks as Shiva in room transcripts, not @default', () => {
  const gc = load(() => '(pass)')
  const line = gc.formatGroupChatLine({ from: { kind: 'member', name: 'default' }, text: 'hello room' }, 'builder')
  assert.equal(line, 'Shiva: hello room')
  ...
  assert.equal(you, 'Shiva (you): hi')
  const plain = gc.formatGroupChatLine({ from: { kind: 'member', name: 'builder' }, text: 'yo' }, 'research')
  assert.equal(plain, 'builder: yo')
})

test('speaker labels honor friendly identity: Bot Mode title, then display_name, never a stale Shiva', () => {
  const gc = load(() => '(pass)')

  // A renamed default (core display_name via `shiva profile rename`) must
  // read as its new name — the community report was "Lucy" still showing
  // "Shiva is thinking…" in group rooms.
  ...
  // Untitled rows keep today's behavior: default → Shiva, others verbatim.
  gc.$botMeta.set({})
  assert.equal(gc.groupSpeakerLabel('default'), 'Shiva')
...
test('turn prompt addresses the default profile as @shiva', () => {
  const gc = load(() => '(pass)')
  ...
  assert.match(prompt, /You are @shiva,/)
  ...
  assert.match(peerView, /group chat with @shiva/)
})

test('mention routing: @shiva resolves to the default member', () => {
  const gc = load(() => '(pass)')
  const members = [{ name: 'default', title: '' }, { name: 'builder', title: '' }]
  const parsed = gc.parseGroupChatMentions('@shiva take a look', members)
...
test('source contract: workspace speaker labels use displayName with a click-to-reveal handle', () => {
  // Speaker labels come from the roster displayName (default → Shiva)…
...
test('group room preview renders the bot HANDLE, not the raw profile name', () => {
  ...
  // @shiva, so users concluded mention routing was broken.
