...
function load({ cacheKeyConnection = 'local' } = {}) {
  ...
  queryClient.setQueryData(['shiva-bots', 'roster', cacheKeyConnection], ROSTER)
    ...
    host: {
      ...
      requestProfile: async (route, method, params) => {
        ...
        if (method === 'profiles.list') {
          return {
            profiles: [{ name: 'default', ui_meta: { 'shiva-bots': { chat: 'remote-pin' } } }]
          }
        }
...
test('the roster cache entry lives under the connection-suffixed key, invisible to the bare-key read', () => {
  ...
  assert.equal(queryClient.getQueryData(['shiva-bots', 'roster']), undefined)
  assert.equal(queryClient.getQueriesData({ queryKey: ['shiva-bots', 'roster'] }).length, 1)
})
...
test('the middleware identifies a remote @name-device mention without delivering (identification-only)', async () => {
  ...
  assert.doesNotMatch(result.text, /shiva -p '?default/)
