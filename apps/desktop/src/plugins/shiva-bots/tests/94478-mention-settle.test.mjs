 ...
 *
 * Mechanism (traced on 6ce7ab8-era main, apps/desktop/src/plugins/shiva-bots/plugin.js):
 *
...
test('#94478: detects an unanswered member mention in the post-user tail', () => {
  // Room state from the issue: user asks, member radar replies citing @shiva.
  const log = [
    { id: 1, from: { kind: 'user' }, text: 'kick this off' },
    { id: 2, from: { kind: 'member', name: 'radar' }, text: 'done — @shiva please review' }
  ]

  const mentioned = mentionedSinceLastUser(log, text => (text.includes('@shiva') ? ['shiva'] : []))
  ...
  // continuation round can be driven for exactly [shiva].
  assert.ok(mentioned.has('shiva'), 'cited member must be detected as pending')
})
...
test('#94478: does not flag a handoff the cited member already answered', () => {
  const log = [
    { id: 1, from: { kind: 'user' }, text: 'kick this off' },
    { id: 2, from: { kind: 'member', name: 'radar' }, text: 'done — @shiva please review' },
    { id: 3, from: { kind: 'member', name: 'shiva' }, text: 'reviewed, all good' }
  ]

  // shiva has a member entry AFTER the mentioning one — handoff resolved.
  // The fix's detector must not re-drive an answered mention (that would loop).
  const mentionIdx = log.findIndex(e => e.id === 2)
  const answer = [...log].reverse().find(e => e.from.kind === 'member' && e.from.name === 'shiva')
