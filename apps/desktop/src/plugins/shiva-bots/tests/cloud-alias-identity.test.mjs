...
// "Shiva" or a hostname label — when Cloud is the only active source.
...
test('Cloud-only mode: the sole active-gateway default renders as the alias, not Shiva', () => {
...
test('alias never leaks to same-named defaults on other connections', () => {
  ...
  // Local default while the ACTIVE gateway is local: untouched "Shiva".
  assert.equal(ctx.__displayName({ name: 'default' }, null), 'Shiva')
})
