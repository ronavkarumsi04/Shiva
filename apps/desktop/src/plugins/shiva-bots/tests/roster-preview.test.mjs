...
test('render: BotRow previews the pinned canonical chat, not an unrelated latest session', () => {
  // shiva-agent#88200: the row opens the pinned chat on click, so the
...
test('previewKind: the primary profile surfaces as @shiva, never @default', () => {
  ...
  assert.equal(fromBotOf("Message from agent 'default': deploy is green"), 'shiva')
})
