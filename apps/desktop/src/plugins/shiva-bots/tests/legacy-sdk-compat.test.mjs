...
function sdkNamedImports(source) {
  const match = source.match(/import\s+\{([\s\S]*?)\}\s+from '@shiva\/plugin-sdk'/)
...
function writeLegacySdk(root) {
  const packageRoot = join(root, 'node_modules', '@shiva', 'plugin-sdk')
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: '@shiva/plugin-sdk', type: 'module', exports: './index.js' }, null, 2)}\n`
  )
...
test('legacy SDK without optional capability exports still links Bot Mode', async t => {
  const root = mkdtempSync(join(tmpdir(), 'shiva-bot-mode-legacy-sdk-'))
  ...
  assert.equal(loaded.default.id, 'shiva-bots')
