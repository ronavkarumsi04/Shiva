...
function loadProvide({ roster, active = 'default', focused = active, meta = {} } = {}) {
  ...
  const context = {
    ...
    queryClient: {
      ...
      // key-ignoring mock.
      getQueryData: key =>
        key.length === 3 && key[0] === 'shiva-bots' && key[1] === 'roster'
          ? roster
          : undefined,
      getQueriesData: () => (Array.isArray(roster?.profiles) ? [[['shiva-bots', 'roster', 'local'], roster]] : []),
...
test('offers roster handles for a matching prefix, excluding the active profile', () => {
  ...
  assert.ok(inserts.includes('@shiva'), 'default surfaces as @shiva')
