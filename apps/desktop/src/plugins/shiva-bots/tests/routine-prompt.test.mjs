...
test('integration: a different active profile retains the delegated routine wrapper', () => {
  ...
  assert.match(prompt, /shiva -p 'research' chat/)
...
test('security: delegated routine arguments remain literal shell values', () => {
  ...
  const command = prompt.slice(prompt.indexOf('shiva '), prompt.lastIndexOf('\n\nIf the command'))
  const script = `shiva() { printf '%s\\037' "$@"; }\n${command}`
...
test('security: upgrade pauses persisted delegated routines before they can execute', async () => {
  const titleSentinel = `/tmp/shiva-bot-mode-title-${process.pid}`
  const taskSentinel = `/tmp/shiva-bot-mode-task-${process.pid}`
  ...
  const oldPrompt =
    ...
    `shiva -p research chat -c "Routine: ${title}" -q ${JSON.stringify(`[Scheduled routine] ${instruction}`)}\n\n` +
  ...
  const command = recreated.slice(recreated.indexOf('shiva '), recreated.lastIndexOf('\n\nIf the command'))
  const script = `shiva() { printf '%s\\037' "$@"; }\n${command}`
...
test('regression: Create Cronjob passes the active profile to routinePrompt', () => {
  ...
  assert.doesNotMatch(__routines.routinePrompt('ops', 'Check', instruction, 'ops'), /shiva -p/)
})
