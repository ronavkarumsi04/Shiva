/**
 * Shiva Bot Mode — a "one chat per agent" roster for the Shiva desktop.
 *
 * Left pane "Bots": one row per Shiva profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Shiva cron jobs) scoped to the
 ...
 * bot-initiated sends use `shiva -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import * as sdk from '@shiva/plugin-sdk'
import {
  atom,
  Button,
  Checkbox,
...
const BOT_ATTENTION_HINTS = {
  ...
  missing_config: 'Provider not configured — run shiva model',
...
/** Mirrors host.paneVisibility('shiva-bots:pane') — wired in register(). */
...
const GROUP_CHAT_SYNC_META_KEY = 'shiva-bots-groups'
 ...
 *  clients (Shiva Go, headless backends) see rooms regardless of which
...
// to (local, remote URL, SSH, Shiva Cloud, docker) must be able to find
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['shiva-bots']?.title || profile?.display_name || ''),
...
async function saveBotMeta(owner, patch) {
  ...
  try {
    ...
    const request = route ? requestForBot(bot, 'profiles.configure', { name, ui_meta: { 'shiva-bots': rest } }) :
      host.request('profiles.configure', { name, ui_meta: { 'shiva-bots': rest } })
...
// handoffs (`shiva -p <bot> chat --in ~ -c "Bot Chat" --create-if-missing`)
  ...
  for (const bot of roster) {
    ...
    if (image && typeof image === 'string' && image.startsWith('data:')) {
      ...
      Promise.resolve(request)
        .then(() => queryClient.invalidateQueries({ queryKey: ['shiva-bots', 'roster'] }))
...
function mergeServerMeta(roster, fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['shiva-bots']
...
/** Permanently delete a bot's Shiva profile, then remove plugin-local state
 ...
 * directory (shiva-agent#52279). That is the "can't delete a bot" error. */
...
if (typeof document !== 'undefined' && !document.getElementById('shiva-bots-roster-css')) {
  const style = document.createElement('style')
  style.id = 'shiva-bots-roster-css'
  style.textContent =
    '.shiva-bots-roster [data-radix-scroll-area-viewport] > div {' +
    ' display: block !important; width: 100%; min-width: 0; }' +
    '.shiva-scroll-cap > [data-radix-scroll-area-viewport] { max-height: inherit; }' +
    '@keyframes shiva-bots-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }' +
    '.shiva-bots-pulse { animation: shiva-bots-pulse 1.2s ease-in-out infinite; }'
...
// and the row falls back to the "run shiva mcp / Settings" hint. profile is
...
function McpSetupButton({ profile, entry, onDone, ensureProfile }) {
  ...
  const beginOAuth = async () => {
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.shivaDesktop && window.shivaDesktop.openExternal) {
        window.shivaDesktop.openExternal(authUrl)
      } else {
      ...
      tab === 'generate'
        ? imagen
          ...
          : jsx('div', {
              ...
              children:
                imagen === false
                  ? 'No image model available. If you just enabled one (or updated Shiva), restart the gateway: Ctrl+K → "Restart gateway".'
...
function PetTab({ image, onImage }) {
  ...
  if (!pets.length) {
    return jsx('div', {
      ...
      children: 'No pets in the petdex gallery. Run `shiva pets` to explore.'
    })
  }
 ...
 *  protocol into the system prompt itself (shiva-agent bot_mode_probe).
...
function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (shiva-agent #86875) also expose the union
 ...
 *  callable alias is 'shiva' — the mention middleware resolves it back to
...
function botHandle(name, bot) {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'shiva' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`shiva profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Shiva"
 *  can never hijack the primary profile's @shiva alias. */
function mentionNameForms(value) {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'shiva'].includes(form)
  )
}
...
function botFriendlyNames(bot) {
  ...
  return [bot?.ui_meta?.['shiva-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
function newBotChat(bot) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({ kind: 'error', message: 'Update Shiva Desktop to open another Bot chat.' })
...
function resolveRosterMentions(text, roster, active = {}) {
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'shiva') {
      token = byForm.has('shiva') ? 'shiva' : token
    }
...
// raw Cloud hostname, or to generic 'Shiva' in Cloud-only mode.
//
...
// grandfather, or recover. Stored-id pins (ui_meta['shiva-bots'].chat) were
...
async function openStoredBotChat(owner, storedId, summary) {
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This Shiva Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (shiva-agent#89617) — clicking Retry
  ...
  // until an app restart (shiva-agent#93604). A resume is cheap and
...
async function prepareBotSource(bot) {
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error('Update Shiva Desktop to chat with agents on other connections.')
  }
...
async function ensureBotMetadata(bot) {
  ...
  const server = row?.ui_meta?.['shiva-bots']
...
function displayName(bot, meta) {
  ...
  // "Shiva". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `shiva profile rename
  ...
  // never generic "Shiva" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Shiva (the agent it is)
...
function durableGroupChatMembers(bots) {
  return (bots || []).map(bot => {
    ...
    const title = String(botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['shiva-bots']?.title || bot.title || '').trim()
...
// turn in its OWN persistent per-group Shiva session and is fed only the
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Shiva"
 *  (community report, Aug 21 2026: renamed default still read "Shiva is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Shiva (matching displayName and the
 *  @shiva handle) so the main agent never loses its name in rooms. */
function groupSpeakerLabel(name) {
  ...
  // Core profile display_name (`shiva profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Shiva' : trimmed
}
...
function messagingProtocolSection(name, roster) {
  ...
  return [
    ...
    '',
    '```',
    'shiva -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from \uD83E\uDD16 ' + handle + ' (@' + handle + '): your message"',
    '',
    ...
    '',
    'The roster grows over time — run `shiva profile list` for the LIVE',
...
function previewKind(preview) {
  ...
  if (match) {
    ...
    // profile reads @shiva, never @default (#89484).
 ...
 *  (shiva-agent#90268). Older gateways omit worker_session — always false. */
...
function AdvancedProfileConfig({ bot, state, setState }) {
  ...
  if (unsupported) {
    return jsx('div', {
      className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
      children: 'Full configuration needs a newer gateway (restart it after updating Shiva).'
    })
  }
  ...
  // (shiva-agent#87317): Skills (installed list + one-click hub installs +
...
// https://shiva-agent.nousresearch.com/docs/skills?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'shiva-skill-pick', ...} to us (shiva-agent#86243). We validate
...
const HUB_ORIGIN = 'https://shiva-agent.nousresearch.com'
...
function HubSkillsSection({ forProfile, onInstalled }) {
  ...
  useEffect(() => {
    ...
    const onMessage = event => {
      ...
      if (!data || data.type !== 'shiva-skill-pick' || !data.name) {
        return
      }
  ...
  const ensureAgentCreated = () => {
    ...
    const flight = singleFlight(flightRef, async () => {
      ...
      if (remoteTarget) {
        ...
        try {
          void requestForTarget('profiles.configure', { name: slug, ui_meta: { 'shiva-bots': look } }).catch(() => undefined)
                    ...
                    advTab === 'general'
                      ...
                      : capsFailed
                        ? jsx('div', {
                            className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
                            children:
                              'Capability catalog needs a newer gateway (restart it after updating Shiva).'
                          })
                        : !caps
                          ? jsx('div', {
                              className: 'flex justify-center py-4',
...
// shiva -p <bot> chat delegation wrapper so the run reaches that bot's
...
function routinePrompt(bot, title, instruction, activeProfile) {
  ...
  return (
    ...
    `shiva -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Shiva-native schedule string; Advanced exposes it raw.
const FREQUENCIES = [
...
/** Compose the Shiva schedule string from picker state. */
...
function GroupChatWorkspace({ group, members, onBack, visible = true }) {
  ...
  const renderEntry = (entry, index) => {
                  ...
                  // primary "default" profile renders as Shiva, remote dupes
...
function BotsHomeView() {
  ...
  const gateway = bot.connectionLabel || (bot.connectionId === 'local' ? 'This device' : 'Shiva gateway')
...
function GroupRow({ active, group, members, needsYou, onOpen, onDisband }) {
  ...
  // and the group prompt: the primary profile is @shiva, not @default.
...
function BotsPane() {
  ...
  const activityOf = bot => {
    const created = botRosterMeta(bot, allMeta)?.created || bot.ui_meta?.['shiva-bots']?.created || 0
        ...
        : error && !roster.length
          ? jsxs('div', {
              ...
              children: [
                jsx('div', {
                  children: gatewayUp
                    ? `Roster unavailable: ${error instanceof Error ? error.message : 'gateway error'}. If your gateway predates profiles.list, update Shiva and restart the gateway.`
          ...
          : roster.length === 0
            ...
            : allBotsHidden && !hiddenExpanded
              ...
              : rosterRows.length === 0 && matchingHiddenBots.length === 0
                ...
                : jsx(ScrollArea, {
                    className: 'shiva-bots-roster min-h-0 flex-1',
                    children: jsx('div', {
        ...
        description: deleting
          ? jsxs('span', {
              children: [
                ...
                ' and its associated Shiva profile at ',
...
export default {
  ...
  register(ctx) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @shiva.
    ctx.register({
    ...
    if (!document.getElementById('shiva-bots-keyframes')) {
      const style = document.createElement('style')
      style.id = 'shiva-bots-keyframes'
      style.textContent = '@keyframes shiva-bots-bob { from { transform: translateY(0); } to { transform: translateY(-3px); } }'
      document.head.appendChild(style)
    }
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.shivaDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.shivaDesktop?.connections?.onChanged?.(payload => {
