...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `ShivaPlugin` and
...
`$SHIVA_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`shiva-desktop-plugins` skill.
