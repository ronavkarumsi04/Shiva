...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Shiva Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Shiva docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Shiva Agent Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Shiva docs', url: 'https://example.com/docs' }
    ])
  })
})
  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://shiva-agent.nousresearch.com/docs' },
    ...
    expect(view.title).toBe('Failed to open shiva-agent.nousresearch.com/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://shiva-agent.nousresearch.com/docs' },
        result: { success: true, url: 'https://shiva-agent.nousresearch.com/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened shiva-agent.nousresearch.com/docs')
  })
})
