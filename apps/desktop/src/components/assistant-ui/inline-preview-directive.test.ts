...
describe('frameSizeFromMessage', () => {
  const msg = (over: Record<string, unknown> = {}) => ({
    type: 'shiva-inline-preview-size',
    token: 'tok',
    height: 500,
    width: 300,
    ...over
...
describe('intentFromMessage', () => {
  const msg = (over: Record<string, unknown> = {}) => ({
    type: 'shiva-inline-preview-intent',
    token: 'tok',
  ...
  it('rejects wrong token, wrong type, empty, and hostile shapes', () => {
    ...
    expect(intentFromMessage(msg({ type: 'shiva-inline-preview-size' }), 'tok')).toBeNull()
...
describe('withInlineChrome intent wiring', () => {
  it('injects shiva.send and the data-shiva-send click bridge', () => {
    const framed = withInlineChrome('<html><body><h1>w</h1></body></html>', 'tok', '')

    expect(framed).toContain('window.shiva={send:send}')
    expect(framed).toContain('data-shiva-send')
    expect(framed).toContain('shiva-inline-preview-intent')
  })
})
