...
describe('agent message detection', () => {
  it('matches the Bot Mode delivery prefix with sender and body', () => {
    const m = AGENT_MESSAGE_RE.exec('Message from 🤖 Shiva: hello there')

    expect(m?.[1]?.trim()).toBe('Shiva')
  ...
  it('does not match prose that merely contains the phrase', () => {
    expect(AGENT_MESSAGE_RE.test('I got a Message from 🤖 Shiva: earlier')).toBe(false)
