...
function sessionsBotsTree() {
  ...
  registerPane('shiva-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'shiva-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('shiva-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('shiva-bots:pane')
    expect($hiddenStripTabs.get()).toContain('shiva-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('shiva-bots:pane')

    expect(setStripTabHidden('shiva-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('shiva-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('shiva-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('shiva-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('shiva.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('shiva-bots:pane')
    ...
    revealTreePane('shiva-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('shiva-bots:pane')
    expect(window.localStorage.getItem('shiva.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'shiva-bots:pane', title: 'shiva-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'shiva-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
