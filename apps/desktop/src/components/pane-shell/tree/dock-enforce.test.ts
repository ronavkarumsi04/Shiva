...
const TREE_KEY = 'shiva.desktop.layoutTree.v2'
const USER_PLACED_KEY = 'shiva.desktop.userPlacedPanes.v1'
const LEGACY_HEAL_KEY = 'shiva.desktop.paneDockHeals.v1'
  ...
  children: [
    {
      ...
      children: [
        ...
        { type: 'group', id: 'g-bots', panes: ['shiva-bots:pane'], active: 'shiva-bots:pane' }
      ]
    },
...
describe('enforced dock (stacked Bots pane → sessions-zone tab, every boot)', () => {
  ...
  it('re-homes a stacked bots pane into the sessions tab strip, keeping sessions active', async () => {
    ...
    const group = model.findGroupOfPane(tree.$layoutTree.get()!, 'shiva-bots:pane')!

    expect(group.panes).toEqual(['sessions', 'shiva-bots:pane'])
    ...
    expect(JSON.stringify(persisted)).toContain('"panes":["sessions","shiva-bots:pane"]')
  })

  it('re-homes even a USER-PLACED pane — the owner invariant beats the drag record', async () => {
    window.localStorage.setItem(USER_PLACED_KEY, JSON.stringify(['shiva-bots:pane']))
  ...
  it('re-homes even when the retired heal token was already burned, and clears the stale ledger', async () => {
    window.localStorage.setItem(LEGACY_HEAL_KEY, JSON.stringify(['shiva-bots:pane:sessions-tab-v1']))
  ...
  it('is idempotent within a boot and does not fight an intra-session drag', async () => {
    ...
    expect(model.findGroupOfPane(tree.$layoutTree.get()!, 'shiva-bots:pane')!.panes).toContain('sessions')
    ...
    expect(group.panes).toEqual(['shiva-bots:pane'])
  })
  ...
  it('re-homes again on the NEXT boot after a drag persisted the stacked shape', async () => {
    ...
    const group = second.model.findGroupOfPane(second.tree.$layoutTree.get()!, 'shiva-bots:pane')!
      ...
      children: [
        {
          type: 'group',
          id: 'g-left',
          panes: ['sessions', 'shiva-bots:pane'],
          active: 'shiva-bots:pane',
          headerHidden: true
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
  ...
  it('re-homes an edge-enforced pane stranded in the sessions tab strip', async () => {
      ...
      children: [
        {
          type: 'group',
          id: 'g-sessions',
          panes: ['sessions', 'shiva-bots:pane', 'shiva-bots:routines'],
          active: 'shiva-bots:pane'
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
    ...
    const botsGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'shiva-bots:pane')!
    const routinesGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'shiva-bots:routines')!

    expect(botsGroup.panes).toEqual(['sessions', 'shiva-bots:pane'])
    expect(botsGroup.active).toBe('shiva-bots:pane')
    expect(routinesGroup.panes).toEqual(['shiva-bots:routines'])
    ...
    const dockedRoutinesTree = {
      ...
      children: [
        ...
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' },
        {
          type: 'group',
          id: 'g-routines',
          panes: ['shiva-bots:routines'],
          active: 'shiva-bots:routines'
        }
      ]
    }
