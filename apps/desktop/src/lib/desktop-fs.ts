import { shivaApi } from '@/api/client'
import type {
  ShivaConnection,
  ShivaReadDirResult,
  ShivaReadFileTextResult,
  ShivaSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: ShivaConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: ShivaConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<ShivaReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<ShivaReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<ShivaReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<ShivaReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.shivaDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: ShivaSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
