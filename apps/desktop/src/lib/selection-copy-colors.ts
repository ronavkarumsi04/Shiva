...
// Shiva is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.shivaMode
 ...
 * the only carriers of Shiva' palette — are dropped so the paste target's
