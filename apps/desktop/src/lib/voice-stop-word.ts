...
//
// When someone is in a hands-free "Hey Shiva" voice chat, the natural way to
...
// phrase (optionally addressed to Shiva), so a real turn that merely contains
...
// Optional address prefixes so "shiva stop" / "ok stop" / "hey shiva, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey shiva', 'hey shiva,', 'shiva', 'shiva,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("shiva") is not a stop command on its own.
      continue
    }
 ...
 * to Shiva). Returns false for anything that merely contains "stop" as part of
