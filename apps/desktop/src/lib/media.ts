...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.shivaDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.shivaDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.shivaDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.shivaDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaGatewayStreamUrl(path: string): string {
  ...
  if (isRemoteGateway()) {
    ...
    return `shiva-media://remote/${file}${scope ? `?${scope}` : ''}`
  }
...
export function mediaStreamUrl(path: string): string {
  return `shiva-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.shiva/cache, etc.); /api/media is intentionally
...
export async function downloadGatewayMediaFile(
  ...
  if (!window.shivaDesktop?.saveGatewayFile) {
    throw new Error('Desktop file download bridge is unavailable')
  }

  return window.shivaDesktop.saveGatewayFile({
