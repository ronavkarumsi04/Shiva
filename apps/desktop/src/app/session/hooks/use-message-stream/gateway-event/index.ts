import { registryBackendScopeKey } from '@shiva/shared'
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshShivaConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshShivaConfig — two REST calls (config + defaults) per event, per
