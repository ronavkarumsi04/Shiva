...
import { getShivaConfig } from '@/shiva'
...
import { useShivaConfig } from './use-shiva-config'

vi.mock('@/shiva', () => ({
  getShivaConfig: vi.fn(),
  getShivaConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'shiva.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getShivaConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getShivaConfig>>)

describe('useShivaConfig refreshShivaConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Shiva' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useShivaConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshShivaConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useShivaConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getShivaConfig>>>()
    vi.mocked(getShivaConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useShivaConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshShivaConfig(true)
    })
    expect(getShivaConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getShivaConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getShivaConfig>>>()
    vi.mocked(getShivaConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useShivaConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshShivaConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getShivaConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getShivaConfig>>>()
    vi.mocked(getShivaConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshShivaConfig(true)
      refreshC = result.current.refreshShivaConfig(true)
    })
