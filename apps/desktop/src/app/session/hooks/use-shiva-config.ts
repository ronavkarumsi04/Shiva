...
import { getShivaConfig, getShivaConfigDefaults } from '@/shiva'
...
interface ShivaConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useShivaConfig({ activeSessionIdRef }: ShivaConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshShivaConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getShivaConfig(), getShivaConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Shiva' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshShivaConfig, sttEnabled, voiceMaxRecordingSeconds }
}
