...
import type { ShivaReadDirEntry, ShivaReadDirResult } from '@/global'
...
export type ProjectTreeEntry = ShivaReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: ShivaReadDirEntry) {
...
async function filterIgnored(entries: ShivaReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ShivaReadDirResult> {
  if (!window.shivaDesktop) {
