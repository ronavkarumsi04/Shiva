import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@shiva/shared'
import { useEffect, useRef } from 'react'

import { shouldApplyPostBootProgressError } from '@/components/boot-failure-reauth'
import type { ShivaConnection } from '@/global'
import { ShivaGateway } from '@/shiva'
...
import type { RpcEvent } from '@/types/shiva'
...
export function primaryRuntimeConnectionId(connection: Pick<ShivaConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.shivaDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ShivaGateway | null) => void
  refreshShivaConfig: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ShivaConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Shiva...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Shiva…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Shiva backend'
        )
        ...
        // as an opaque "Could not connect to Shiva gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshShivaConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshShivaConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startShiva re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new ShivaGateway()
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting Shiva…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshShivaConfig(),
          ...
          // retry it), not brick boot behind the "Shiva couldn't start"
