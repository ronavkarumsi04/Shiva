...
import { saveShivaConfig } from '@/shiva'
import { useI18n } from '@/i18n'
import { notifyError } from '@/store/notifications'
import type { ShivaConfigRecord } from '@/types/shiva'

import { setShivaConfigCache, useShivaConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: ShivaConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = useShivaConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<ShivaConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setShivaConfigCache, so
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="shiva-terminal-font-families"
          ...
          <datalist id="shiva-terminal-font-families">
