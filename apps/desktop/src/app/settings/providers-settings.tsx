...
import { disconnectOAuthProvider, listOAuthProviders } from '@/shiva'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/shiva'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.shivaDesktop?.terminal)
...
//      in shiva_cli/provider_catalog.py) — the SAME provider identity
//      `shiva model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Shiva can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Shiva can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Shiva never deletes creds another tool owns behind a silent API call.
