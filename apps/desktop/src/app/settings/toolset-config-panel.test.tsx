...
import type { ToolsetConfig } from '@/types/shiva'
...
const getShivaConfigSchema = vi.fn()
...
vi.mock('@/shiva', () => ({
  ...
  getShivaConfigSchema: () => getShivaConfigSchema(),
...
beforeEach(() => {
  ...
  getShivaConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getShivaConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveShivaConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveShivaConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
