import type { GatewayWsUrlResult } from '@shiva/shared'
import type { TranslucencyState } from '@shiva/shared/translucency'
...
declare global {
  interface Window {
    shivaDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ShivaConnection>
      ...
      // Resume this session in the user's own terminal emulator (`shiva --tui
      ...
      // Shiva Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new SHIVA_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ShivaApiRequest) => Promise<T>
      notify: (payload: ShivaNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<ShivaReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<ShivaReadFileTextResult>
      selectPaths: (options?: ShivaSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ShivaPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ShivaPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<ShivaPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: ShivaActiveWork) => void
      setTitleBarTheme?: (payload: ShivaTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ShivaReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<SHIVA_HOME>/desktop-plugins),
      ...
      /** LOCAL `<SHIVA_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<SHIVA_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<ShivaGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<ShivaGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<ShivaGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<ShivaRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: ShivaReviewScope, baseRef?: null | string) => Promise<ShivaReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: ShivaReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<ShivaReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<ShivaRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<ShivaPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: ShivaTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ShivaTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ShivaWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: ShivaPreviewFileChanged) => void) => () => void
...
export interface ShivaTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ShivaTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `shiva update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  shivaRoot?: string
...
export interface ShivaConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface ShivaTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface ShivaActiveWork {
  count: number
  titles: string[]
}

export interface ShivaWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Shiva Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Shiva Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Shiva Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the shiva gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Shiva Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface ShivaApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface ShivaPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ShivaReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ShivaPreviewWatch {
  id: string
  path: string
}
...
export interface ShivaGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface ShivaGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface ShivaGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface ShivaRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface ShivaRepoStatus {
  branch: null | string
...
// turn began.
export type ShivaReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface ShivaReviewFile {
  path: string
  added: number
  removed: number
...
export interface ShivaReviewList {
  files: ShivaReviewFile[]
...
export interface ShivaReviewPr {
  url: string
  state: string
  number: number
}
...
export interface ShivaBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface ShivaRepoPullRequests {
  ghReady: boolean
  prs: ShivaBranchPullRequest[]
}
...
export interface ShivaPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface ShivaReviewShipInfo {
  ghReady: boolean
  pr: ShivaReviewPr | null
}

export interface ShivaReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ShivaReadDirResult {
  entries: ShivaReadDirEntry[]
  error?: string
}

export interface ShivaPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ShivaSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
