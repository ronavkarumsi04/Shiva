...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'shiva:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.shivaDesktop?.saveImageFromUrl) {
        if (await window.shivaDesktop.saveImageFromUrl(src)) {
