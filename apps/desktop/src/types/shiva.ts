...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Shiva doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in shiva_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `shiva model` uses — instead of
...
export type ShivaConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `shiva model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Shiva simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (shiva.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors shiva_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Shiva, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `shiva tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Shiva); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** shiva-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** shiva-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/shiva/update/check` — the backend's own update state.
