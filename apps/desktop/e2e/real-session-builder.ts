 ...
 * AIAgent loop, using the E2E mock provider configured in `shivaHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(shivaHome: string) {
  ...
  static async start(shivaHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(shivaHome)
