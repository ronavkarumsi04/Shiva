...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.shivaHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.shivaHome)
  const builder = await RealSessionBuilder.start(sandbox.shivaHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.shivaHome, mock.url)
  fs.appendFileSync(path.join(sandbox.shivaHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.shivaHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
