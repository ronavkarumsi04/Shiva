/**
 * Shared E2E fixtures for the Shiva desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → shiva serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `shiva desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `shiva-e2e-${prefix}-${Math.random()}`))
  const shivaHome = path.join(root, 'shiva-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(shivaHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(shivaHome, 'config.yaml')
 ...
 */
export function writeEnvFile(shivaHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(shivaHome, '.env')
 ...
 */
function writeEmptyConfig(shivaHome: string): void {
  const configPath = path.join(shivaHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - SHIVA_HOME → sandbox shiva-home (isolated config/sessions)
 *  - SHIVA_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - SHIVA_DESKTOP_IGNORE_EXISTING=1 → don't pick up `shiva` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - SHIVA_DESKTOP_SHIVA_ROOT → REPO_ROOT (dev checkout resolution)
 *  - SHIVA_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    SHIVA_HOME: sandbox.shivaHome,
    SHIVA_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    SHIVA_DESKTOP_IGNORE_EXISTING: '1',
    SHIVA_DESKTOP_SHIVA_ROOT: REPO_ROOT,
    SHIVA_DESKTOP_APP_NAME: `ShivaE2E-${Date.now()}`,
    ...
    SHIVA_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated SHIVA_HOME + userData
 * @param env      - the process environment (already has SHIVA_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.shivaHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via SHIVA_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`shiva serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.shivaHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { SHIVA_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Shiva backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Shiva.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Shiva.app', 'Contents', 'MacOS', 'Shiva')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'shiva')
}
 ...
 * progress without spawning a real Shiva backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * SHIVA_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    SHIVA_DESKTOP_BOOT_FAKE: '1',
    SHIVA_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + shiva-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).SHIVA_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).SHIVA_DESKTOP_SHIVA
  delete (env as Record<string, string | undefined>).SHIVA_DESKTOP_SHIVA_ROOT
