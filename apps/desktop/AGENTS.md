...
How to build Shiva Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Shiva surface can also
...
- A **runtime home change** (switching the underlying `SHIVA_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Shiva — do not assume
