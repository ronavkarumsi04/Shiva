 ...
 *     `X-Shiva-Session-Token` header; WS uses `?token=`.
...
// shiva_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - shiva_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - shiva_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / shiva #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-shiva_session_at', '__Secure-shiva_session_at', 'shiva_session_at']
const RT_COOKIE_VARIANTS = ['__Host-shiva_session_rt', '__Secure-shiva_session_rt', 'shiva_session_rt']

// The Nous portal (NAS) does NOT use Shiva gateway session cookies — it is a
...
// Keep this aligned with shiva_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['shiva', 'test', 'tmp', 'root', 'sudo'])
...
/** Which Shiva profile the remote SSH dashboard should actually run as.
...
// Shiva Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteShivaPath = String(entry.remoteShivaPath || '').trim()

  if (remoteShivaPath) {
    out.remoteShivaPath = remoteShivaPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Shiva installation. Preserve an explicit mapping
  // when it is a valid Shiva profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_shiva_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `SHIVA_HOME` scopes it.
 *
 ...
 * `shiva:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://shiva.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `shiva:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a shiva session ACCESS-token cookie
 ...
 * the Shiva gateway session cookies, so cloud sign-in / discovery liveness
