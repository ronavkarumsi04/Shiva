// Unit tests for the pure Windows `shiva` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvShivaCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `shiva` shim shadows
//      the real shiva.cmd/shiva.exe.
...
//   3. resolveVenvShivaCommand() — must probe the venv python via
//      canImportShivaCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvShiva: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvShiva: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvShiva: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvShiva: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvShiva: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvShiva: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvShivaCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportShivaCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    shivaHome: '/fake/shiva-home',
...
test('resolveVenvShivaCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvShivaCommand('/root/venv/Scripts/shiva.exe', [], deps), null)
})

test('resolveVenvShivaCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvShivaCommand('/root/venv/Scripts/shiva.cmd', [], deps), null)
})

test('resolveVenvShivaCommand: returns null when the basename is not shiva/shiva.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvShivaCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvShivaCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvShivaCommand('/root/venv/bin/shiva.exe', [], deps), null)
})

test('resolveVenvShivaCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvShivaCommand('/root/venv/Scripts/shiva.exe', [], deps), null)
})

test('resolveVenvShivaCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportShivaCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvShivaCommand('/root/venv/Scripts/shiva.exe', ['serve'], deps)
...
test('resolveVenvShivaCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvShivaCommand('/root/venv/Scripts/shiva.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'shiva_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvShivaCommand: is case-insensitive on shiva.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvShivaCommand('/root/venv/Scripts/SHIVA.EXE', [], deps))
  assert.ok(resolveVenvShivaCommand('/root/venv/SCRIPTS/shiva.exe', [], deps))
})
