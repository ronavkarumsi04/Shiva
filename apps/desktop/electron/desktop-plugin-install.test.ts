...
describe('resolvePluginGitUrl', () => {
  it('maps owner/repo shorthand to github git url', () => {
    expect(resolvePluginGitUrl('NousResearch/shiva-example-plugins')).toEqual({
      gitUrl: 'https://github.com/NousResearch/shiva-example-plugins.git',
      subdir: null
    })
  })
...
describe('resolveSubdirWithin', () => {
  it('rejects path traversal', () => {
    const root = mkdtemp('shiva-plugin-root-')
...
describe('findDesktopEntry', () => {
  it('finds root plugin.js', () => {
    const root = mkdtemp('shiva-plugin-detect-')
...
describe('detectPluginComponents', () => {
  ...
  it('detects agent-only layout', async () => {
    const root = mkdtemp('shiva-plugin-agent-')
    roots.push(root)
  ...
  it('detects dual layout', async () => {
    const root = mkdtemp('shiva-plugin-dual-')
    roots.push(root)
    fs.mkdirSync(path.join(root, 'desktop'), { recursive: true })
