...
import { buildDesktopBackendEnv, shivaManagedNodePathEntries, normalizeShivaHomeRoot } from './backend-env'
import {
...
import {
  ...
  resolveVenvShivaCommand
} from './windows-shiva-path'
import {
...
const USER_DATA_OVERRIDE = process.env.SHIVA_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.SHIVA_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.SHIVA_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with SHIVA_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[shiva] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[shiva] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. SHIVA_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[shiva] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[shiva] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via SHIVA_DESKTOP_PASSWORD_STORE, bridged by the
// `shiva desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[shiva] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[shiva] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `shiva desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Shiva sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[shiva] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[shiva] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[shiva] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[shiva] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[shiva] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('shiva:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Shiva burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[shiva] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[shiva] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[shiva] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[shiva] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// SHIVA_HOME — the user-facing root for everything Shiva-related. Mirrors
// scripts/install.ps1's $ShivaHome and scripts/install.sh's $SHIVA_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\shiva (matches install.ps1)
//   macOS / Linux: ~/.shiva (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.shiva directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\shiva yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// SHIVA_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// SHIVA_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.shiva / %LOCALAPPDATA%\shiva.
function resolveShivaHome() {
  if (process.env.SHIVA_HOME) {
    return normalizeShivaHomeRoot(process.env.SHIVA_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'shiva-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a SHIVA_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\shiva and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('SHIVA_HOME')

    if (fromRegistry) {
      return normalizeShivaHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'shiva')
    const legacy = path.join(app.getPath('home'), '.shiva')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.shiva setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.shiva')
}

const SHIVA_HOME = resolveShivaHome()

function pathWithShivaManagedNode(...entries) {
  const managed = shivaManagedNodePathEntries(SHIVA_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_SHIVA_ROOT — the canonical mutable Shiva install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_SHIVA_ROOT = path.join(SHIVA_HOME, 'shiva-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_SHIVA_ROOT, 'venv')
...
// boots, going straight to `resolveShivaBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_SHIVA_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_SHIVA_ROOT, '.shiva-bootstrap-complete')
...
// active-profile.json records which Shiva profile the desktop launches its
// local backend as. When set, startShiva() passes `shiva --profile <name>
// dashboard …`, which deterministically pins SHIVA_HOME (see
// _apply_profile_override in shiva_cli/main.py) and bypasses the sticky
// ~/.shiva/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors shiva_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// shivaDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under SHIVA_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by shiva_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(SHIVA_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (shiva_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.SHIVA_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.SHIVA_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.SHIVA_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.SHIVA_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.SHIVA_DESKTOP_APP_NAME || 'Shiva'
...
// 'shiva:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.shiva) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.shiva')
}

// Seed the native About panel with the live Shiva version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `shiva update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveShivaVersion(),
...
// startShiva(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.SHIVA_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.SHIVA_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
//   * WSL2 IPC stall  = the renderer's `shiva:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.SHIVA_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startShiva() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning shiva serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Shiva backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  if (shivaLog.length > 300) {
    shivaLog.splice(0, shivaLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-shiva-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('shiva:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the shiva:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('shiva:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_SHIVA_ROOT
  })
}
...
// The Tauri updater writes SHIVA_HOME/.shiva-update-in-progress for the whole
...
// (`force_kill_other_shiva`, taskkill /IM shiva.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(SHIVA_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Shiva will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(SHIVA_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Shiva update did not finish',
        `${result.message}\n\nDetails: ${path.join(SHIVA_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `shiva` — must not shadow `shiva.cmd`/`shiva.exe`.
...
function unwrapWindowsVenvShivaCommand(command, backendArgs) {
  return resolveVenvShivaCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `shiva serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `shiva` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'shiva_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `shiva --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, SHIVA_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveShivaBackend step 4); execFileSync of a .cmd without
...
function isShivaSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'shiva_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.SHIVA_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Shiva deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the shiva-import
...
// findGitBash — locate bash.exe on Windows. Resolves SHIVA_GIT_BASH_PATH
...
// normal SHIVA_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\shiva\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'shiva', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'shiva', 'git', 'bin', 'git.exe'))
  }
...
function recentShivaLog() {
  return shivaLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's shiva root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_SHIVA_ROOT.
// SHIVA_DESKTOP_SHIVA_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.SHIVA_DESKTOP_SHIVA_ROOT && path.resolve(process.env.SHIVA_DESKTOP_SHIVA_ROOT),
    !IS_PACKAGED && isShivaSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isShivaSourceRoot(ACTIVE_SHIVA_ROOT) ? ACTIVE_SHIVA_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_SHIVA_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('shiva:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'shiva-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `shiva update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(SHIVA_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `shiva update` can write
// fresh entry points. On Windows this is the file the running backend
// `shiva.exe` holds open; on POSIX it's never mandatory-locked.
function venvShivaShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'shiva.exe')
    : path.join(updateRoot, 'venv', 'bin', 'shiva')
}
...
// only signals the direct child, so on Windows a backend `shiva.exe` that
// spawned its own grandchildren (a `shiva` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Shiva backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Shiva backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.shivaBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Shiva backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.shivaBackendIdentity
...
// `shivaProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `shiva.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a shiva
// REPL, a pty terminal, the gateway) keep `shiva.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const shivaProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\shiva.exe, but the
  // backend is `python.exe -m shiva_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (shivaProcess && Number.isInteger(shivaProcess.pid)) {
    initialPids.push(shivaProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(shivaProcess, {
    forceKillProcessTree,
    stopAllPoolBackends
  })

  const shim = venvShivaShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentShivaProcess = backendConnectionState.getProcess()

        if (currentShivaProcess && Number.isInteger(currentShivaProcess.pid)) {
          stragglers.push(currentShivaProcess.pid)
        }
...
// `shiva update`). Instead we spawn the staged Shiva-Setup binary with
// --update and quit, so it can run `shiva update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `shiva update` → relaunch),
      ...
      // in: the SHIVA_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `shiva update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `shiva desktop`, never the Tauri installer that self-copies
      // shiva-setup.exe into SHIVA_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `shiva` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `shiva update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `shiva update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, shivaRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(SHIVA_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Shiva — this window will close and the updater will open. Don’t reopen Shiva yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(SHIVA_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // shiva.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running shiva, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Shiva install open ' +
        '(a second Shiva window or a terminal running shiva?). Close it and retry.'
    ...
    // Shiva processes running from this venv.  The updater normally refuses
    ...
    // `shiva update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `shiva update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          SHIVA_HOME,
          SHIVA_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithShivaManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `shiva update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(SHIVA_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Shiva update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(SHIVA_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Shiva will keep running — try again, or run \`shiva update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvShiva = path.join(venvBin, IS_WINDOWS ? 'shiva.exe' : 'shiva')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Shiva launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the shiva
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.shiva-bootstrap-complete')),
      hasVenvShiva: fileExists(venvShiva),
...
// Python-level pre-update snapshot inside `shiva update`.
function preflightStateDb(shivaHome, rememberLog) {
  const stateDbPath = path.join(shivaHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(shivaHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(shivaHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(shivaHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `shiva update`, swaps/relaunches the app bundle, and writes
// .shiva-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'shiva update', percent: null })

    return { ok: true, manual: true, command: 'shiva update', shivaRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      SHIVA_HOME,
      SHIVA_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithShivaManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Shiva — this window will close. Don’t reopen Shiva yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_SHIVA_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isShivaSourceRoot(ACTIVE_SHIVA_ROOT) &&
    fileExists(venvPython) &&
    canImportShivaCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_SHIVA_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `shiva
...
function resolveWebDist() {
  const override = process.env.SHIVA_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set SHIVA_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: shiva desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: shiva desktop --force-build`
  )
...
function resolveShivaCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Shiva.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveShivaCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_SHIVA_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Shiva at ${ACTIVE_SHIVA_ROOT}`,
    command,
    args: ['-m', 'shiva_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      shivaHome: SHIVA_HOME,
      pythonPathEntries: [ACTIVE_SHIVA_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_SHIVA_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveShivaBackend(backendArgs) {
  // 1. Explicit override -- SHIVA_DESKTOP_SHIVA_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.SHIVA_DESKTOP_SHIVA_ROOT && path.resolve(process.env.SHIVA_DESKTOP_SHIVA_ROOT)

  if (overrideRoot && isShivaSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Shiva source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `shiva` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isShivaSourceRoot.)
  if (!IS_PACKAGED && isShivaSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Shiva source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_SHIVA_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\shiva\\shiva-agent (Windows) or ~/.shiva/shiva-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Shiva runtime at ${ACTIVE_SHIVA_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `shiva` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    SHIVA_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.SHIVA_DESKTOP_IGNORE_EXISTING !== '1') {
    let shivaCommand = null
    const shivaOverride = process.env.SHIVA_DESKTOP_SHIVA

    if (shivaOverride) {
      const resolvedOverride = findOnPath(shivaOverride)

      if (resolvedOverride) {
        shivaCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(shivaOverride, { isWsl: IS_WSL })) {
        shivaCommand = shivaOverride
      } else {
        rememberLog(`Ignoring Windows Shiva override under WSL: ${shivaOverride}`)
      }
    } else {
      shivaCommand = findOnPath('shiva')
    }

    if (shivaCommand) {
      if (looksLikeDesktopAppBinary(shivaCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Shiva CLI: ${shivaCommand}`)
        shivaCommand = null
      }
    }

    if (shivaCommand) {
      const unwrapped = unwrapWindowsVenvShivaCommand(shivaCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `shiva` shim
      ...
      const shellForProbe = isCommandScript(shivaCommand)

      // SHIVA_DESKTOP_SHIVA is an explicit deployment override (used by
      ...
      if (shouldTrustShivaOverride(shivaOverride) || verifyShivaCli(shivaCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvShivaCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Shiva CLI at ${shivaCommand}`,
          command: shivaCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Shiva CLI at ${shivaCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed shiva_cli module via system Python.
  ...
  if (python) {
    ...
    // having shiva_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\shiva\shiva-agent\venv.
    if (canImportShivaCli(python)) {
      return {
        kind: 'python',
        label: `installed shiva_cli module via ${python}`,
        command: python,
        args: ['-m', 'shiva_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: shiva_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveShivaBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Shiva Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveShivaBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Shiva install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Shiva recovery was handed off to Shiva Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      shivaHome: SHIVA_HOME,
      logRoot: path.join(SHIVA_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Shiva install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Shiva bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(SHIVA_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startShiva() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // shiva:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveShivaBackend(backend.args))
  }
  ...
  if (!isShivaSourceRoot(ACTIVE_SHIVA_ROOT)) {
    throw new Error(
      `Shiva install at ${ACTIVE_SHIVA_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Shiva' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\shiva\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `shiva` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Shiva on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isShivaSourceRoot()
    // plus an importable shiva_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Shiva venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Shiva at ${ACTIVE_SHIVA_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Shiva runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----shiva-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Shiva backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Shiva backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Shiva backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Shiva-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Shiva-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('shiva:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveShivaCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('shiva:preview-file-changed', payload)
}
...
// waitForShivaReady how to read a 401 — rejected session vs gated route.
...
async function waitForShiva(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForShivaReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('shiva:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('shiva:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('shiva:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('shiva:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Shiva...".
function sendPowerResume() {
  ...
  webContents.send('shiva:power-resume')
}
...
// 'shiva:power-battery' push below.
ipcMain.handle('shiva:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('shiva:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('shiva:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('shiva:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('shiva:context-menu-spellcheck', {
...
//
// Hosted Shiva gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``shiva_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / shiva #37247).
//     Both are set as HttpOnly cookies (``shiva_session_at`` ~15 min,
//     ``shiva_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startShiva → the renderer's
// single-shot boot() with no retry) surfaced as the "Shiva couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Shiva
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Shiva Cloud agent…' : 'Sign in to Shiva gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Shiva backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in shiva_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Shiva Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:shiva-remote-oauth`:
...
// convention (shiva_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Shiva surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.SHIVA_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Shiva gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for shiva_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Shiva Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Shiva Cloud sign-in.'))
...
// Discover the hosted (Shiva Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Shiva Cloud. Open Settings → Gateway, choose Shiva Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Shiva Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Shiva Cloud session has expired. Sign in to Shiva Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Shiva Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every shiva:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteShivaPath`/`remoteProfile` —
...
// a valid stored value (pins the root SHIVA_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.SHIVA_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.SHIVA_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Shiva Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteShivaPath: (ssh || savedSsh)?.remoteShivaPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by SHIVA_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Shiva Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Shiva Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteShivaPath: input.sshRemoteShivaPath ?? existingBlock.remoteShivaPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (shiva #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Shiva gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Shiva gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.SHIVA_DESKTOP_REMOTE_TOKEN,
      url: process.env.SHIVA_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      shivaPath: result.shivaPath,
      shivaHome: result.shivaHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { shivaPath: result.shivaPath, shivaHome: result.shivaHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteShivaPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteShivaPath: sshConfig.remoteShivaPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForShiva: (baseUrl, token) => waitForShiva(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        shivaVersion: result.shivaVersion || '',
        ...
        // per-profile pools also use bare keys. Only startShiva' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.shivaVersion || 'shiva (version unknown)'} at ${result.shivaPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteShivaVersion: result.shivaVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (SHIVA_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.SHIVA_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startShiva() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Shiva
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteShivaPath: input.sshRemoteShivaPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteShivaPath || '')
          let shivaPath
          let shivaVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            shivaVersion = inspection.version
            supported = inspection.supported
          } else {
            shivaPath = await remoteLifecycle.locateShiva(ssh, sshConfig.remoteShivaPath || '')
            shivaVersion = await remoteLifecycle.probeShivaVersion(ssh, shivaPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, shivaPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Shiva on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteShivaPath: shivaPath,
            remoteShivaVersion: shivaVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startShiva())
  ...
  // connect to Shiva gateway". Mirror the renderer's connect here so the test
...
function resetShivaConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const shivaProcess = backendConnectionState.invalidate()
  stopBackendChild(shivaProcess)
...
// startShiva() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetShivaConnection() invalidates it.
  const shivaProcess = backendConnectionState.getProcess()
  const dying = shivaProcess && !shivaProcess.killed ? shivaProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('shiva:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('shiva:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startShiva()
  ...
  // spawning, so two children never share one profile's SHIVA_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Shiva backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteShivaPath: source.remoteShivaPath,
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Shiva backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. shiva:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: shivaLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForShiva(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.shivaPath || !state?.shivaHome) {
  ...
  return {
    ...
    shivaPath: state.shivaPath,
    shivaHome: state.shivaHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteShivaPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.shivaPath ? platform : await probeWindowsRemote(ssh, config.remoteShivaPath || '')
    ...
    const shivaPath = await remoteLifecycle.locateShiva(ssh, config.remoteShivaPath || '')
    const shivaHome = await remoteLifecycle.probeRemoteShivaHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, shivaPath, shivaHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { shivaPath: state.shivaPath, shivaHome: state.shivaHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForShiva(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startShiva() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForShiva(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited SHIVA_HOME env (see _apply_profile_override
  // step 3 in shiva_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveShivaBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const shivaCwd = resolveShivaCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(SHIVA_HOME, 'profiles', key))
  )

  rememberLog(`Starting Shiva backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: shivaCwd,
        SHIVA_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        SHIVA_WEB_DIST: webDist,
        ...(readyFile ? { SHIVA_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Shiva backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Shiva backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Shiva backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForShiva(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Shiva backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Shiva backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_shiva_home() recreates the deleted profile directory.
//
...
async function startShiva() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Shiva Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startShiva() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Shiva backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Shiva backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Shiva backend at ${remote.baseUrl}`, 24)
      await waitForShiva(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Shiva backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, shivaLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.shiva/active_profile file) and
    // resolves SHIVA_HOME the same way `shiva -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Shiva runtime', 28)

        return resolveShivaBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Shiva backend via ${backend.label}`, 84)
    rememberLog(`Starting Shiva backend via ${backend.label}`)
    ...
    const shivaProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin SHIVA_HOME for the child so Python's get_shiva_home()
          // resolves to the SAME location our resolveShivaHome() picked. Without
          // this pin, Python falls back to ~/.shiva on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\shiva, which differs from C:\Users\<u>\.shiva.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets SHIVA_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(shivaProcess)
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, shivaProcess)

    if (!processOwner) {
      stopBackendChild(shivaProcess)
      await waitForBackendExit(shivaProcess)
      releaseBackendChild(shivaProcess)
      throw new Error('Shiva backend start was superseded by a newer connection attempt.')
    }

    shivaProcess.stdout.on('data', rememberLog)
    shivaProcess.stderr.on('data', rememberLog)
    ...
    shivaProcess.once('error', error => {
      releaseBackendChild(shivaProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Shiva backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Shiva backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Shiva backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Shiva backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    shivaProcess.once('exit', (code, signal) => {
      releaseBackendChild(shivaProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Shiva backend exit (${signal || code})`)
      ...
      rememberLog(`Shiva backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Shiva backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Shiva backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentShivaLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Shiva backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(shivaProcess, {
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Shiva backend to become ready', 90)
    await Promise.race([waitForShiva(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => shivaProcess.exitCode === null && !shivaProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Shiva backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Shiva backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('shiva:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Shiva is
...
// pushes pet state over IPC (shiva:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via shiva:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Shiva app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Shiva icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see shiva:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Shiva in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('shiva:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Shiva can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('shiva:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('shiva:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[shiva] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('shiva:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `shiva:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, shiva:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('shiva:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('shiva:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('shiva:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'shiva desktop --force-build',
  ...
  startShiva().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('shiva:connection', async (_event, profile) => {
...
ipcMain.handle('shiva:connection:for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
  const registry = readDesktopConnectionsRegistry()
  const id = String(connectionId || '').trim() || registry.primary
  // Same single-owner claim as 'shiva:connection', keyed by the composite
...
ipcMain.on('shiva:connection:active-route', (event, route) => {
...
// "Starting Shiva…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('shiva:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetShivaConnection({ soft: true }),
...
ipcMain.handle('shiva:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('shiva:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('shiva:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('shiva:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('shiva:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('shiva:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveShivaBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Shiva is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `shiva-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, SHIVA_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('shiva:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('shiva:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('shiva:zoom:get', event => {
...
ipcMain.on('shiva:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('shiva:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startShiva() call restarts the
...
ipcMain.handle('shiva:bootstrap:repair', async () => {
  // Forceful repair: force the next startShiva() through the full installer
  ...
  // "bypass active runtime" path inside startShiva() and fall through
...
ipcMain.handle('shiva:boot-progress:get', async () => bootProgressState)
ipcMain.handle('shiva:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('shiva:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('shiva:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('shiva:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('shiva:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('shiva:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('shiva:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('shiva:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('shiva:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('shiva:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('shiva:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('shiva:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('shiva:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('shiva:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('shiva:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteShivaPath: entry.remoteShivaPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteShivaPath: connection.remoteShivaPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteShivaProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root SHIVA_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('shiva:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// shiva:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('shiva:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('shiva:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `shiva update` to every eligible registered connection at once.
...
ipcMain.handle('shiva:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/shiva/update', {}, { timeoutMs: 15_000 })
...
// analogue of the shiva:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('shiva:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('shiva:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startShiva() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('shiva:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Shiva Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('shiva:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('shiva:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('shiva:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('shiva:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of shiva:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('shiva:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('shiva:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new SHIVA_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('shiva:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('shiva:requestMicrophoneAccess', async () => {
...
ipcMain.handle('shiva:window:readBelow', async event => {
...
async function handleShivaApiRequest(request) {
  ...
  // backend calls ensure_shiva_home() which recreates the profile directory,
...
ipcMain.handle('shiva:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its SHIVA_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleShivaApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleShivaApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('shiva:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('shiva:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('shiva:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as shiva://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('shiva:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('shiva:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('shiva:data-url-read-max:get', () => ({
...
ipcMain.handle('shiva:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('shiva:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('shiva:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('shiva:readFileText', async (_event, filePath) => {
...
// `shiva:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('shiva:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('shiva:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('shiva:writeClipboard', (_event, text) => {
...
ipcMain.handle('shiva:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('shiva:readClipboard', () => clipboard.readText())

ipcMain.handle('shiva:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('shiva:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('shiva:context-menu:edit', (event, command) => {
...
ipcMain.handle('shiva:context-menu:copy-image', event => {
...
ipcMain.handle('shiva:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('shiva:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('shiva:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('shiva:saveClipboardImage', async () => {
...
ipcMain.handle('shiva:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('shiva:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('shiva:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('shiva:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('shiva:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('shiva:titlebar-theme', (_event, payload) => {
...
ipcMain.on('shiva:native-theme', (_event, mode) => {
...
ipcMain.on('shiva:translucency:support', event => {
  event.returnValue = { glass: GLASS_SUPPORTED, translucency: TRANSLUCENCY_SUPPORTED }
})

ipcMain.on('shiva:translucency', (_event, payload) => {
...
ipcMain.on('shiva:keep-awake', (_event, on) => {
...
ipcMain.handle('shiva:quick-entry:settings:get', async () => {
...
ipcMain.handle('shiva:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('shiva:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('shiva:quick-entry:submit', {
...
ipcMain.on('shiva:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('shiva:quick-entry:state', payload)
  }
})

ipcMain.on('shiva:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('shiva:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('shiva:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `shiva:window:openSession`).
...
ipcMain.handle('shiva:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('shiva:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('shiva:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('shiva:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveShivaCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('shiva:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveShivaCwd()
}))

ipcMain.handle('shiva:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('shiva:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('shiva:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('shiva:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('shiva:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('shiva:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('shiva:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: shivaLog.slice(-200) }))
...
ipcMain.on('shiva:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (shiva:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('shiva:updates:check', async () =>
...
ipcMain.handle('shiva:updates:apply', async (_event, payload) =>
...
ipcMain.handle('shiva:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('shiva:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Shiva version (the one `release.py` bumps in
// shiva_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Shiva version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveShivaVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'shiva_cli', '__init__.py')
...
// Renderer-bundle skew: `shiva update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Shiva version and push it into the native About panel
// just before showing it, so an in-place `shiva update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveShivaVersion()} — app build out of date, update the desktop app`
        : resolveShivaVersion(),
      copyright: 'Copyright © 2026 Nous Research'
    })
    app.showAboutPanel()
...
ipcMain.handle('shiva:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveShivaVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    shivaRoot: resolveUpdateRoot(),
...
// the actual removal via `shiva uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (shiva_cli/uninstall.py + shiva_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_SHIVA_ROOT
  ...
  const fallback = () => ({
    shiva_home: SHIVA_HOME,
    agent_installed: isShivaSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'shiva_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, SHIVA_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Shiva agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import shiva_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_SHIVA_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `shiva-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `shiva-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('shiva:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('shiva:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('shiva:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('shiva:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// shiva:// deep links (e.g. shiva://blueprint/morning-brief?time=08:00,
// shiva://mcp/install?name=NAME&config=B64 — the vendor "Add to Shiva"
// button, or shiva://plugin/install?repo=owner/repo). Dev
// (`SHIVA_DESKTOP_DEV_SERVER`) registers shiva-dev:// instead — bare
// Electron or a stale OS handler often owns shiva:// on dev machines.
...
// ---------------------------------------------------------------------------
const SHIVA_PROTOCOL = DEV_SERVER ? 'shiva-dev' : 'shiva'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['shiva-dev', 'shiva'] : ['shiva']
...
function handleDeepLink(url) {
  ...
  // shiva://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('shiva:deep-link', payload)
...
ipcMain.handle('shiva:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${SHIVA_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SHIVA_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(SHIVA_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${SHIVA_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `shiva://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startShiva), whose
...
app.whenReady().then(() => {
  ...
  // startShiva() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching shiva:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Shiva never keeps another app's chord hostage.
