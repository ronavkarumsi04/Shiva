...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(SHIVA_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Shiva-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.shiva/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'shiva-desktop-ssh')
  }

  return path.join(os.homedir(), '.shiva', 'desktop-ssh')
}
