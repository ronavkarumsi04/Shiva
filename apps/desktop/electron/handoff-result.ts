 ...
 * console. It writes SHIVA_HOME/.shiva-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Shiva within 30 minutes.
...
export function handoffResultPath(shivaHome: string): string {
  return path.join(shivaHome, '.shiva-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(shivaHome)
  let raw: string
