// Profile-delete routing logic for the `shiva:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_shiva_home() and
...
/** Parse a `shiva:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its SHIVA_HOME.
 *
...
/**
 * Route the next `shiva:api` request away from the primary/window backend
 ...
 * ensure_shiva_home() recreates the directory the delete just removed.
 */
