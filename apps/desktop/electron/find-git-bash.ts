 ...
 *   1. SHIVA_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\shiva\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect SHIVA_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.SHIVA_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'shiva', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'shiva', 'git', 'usr', 'bin', 'bash.exe'))
  }
