...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'shiva_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'shiva_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'shiva_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Shiva.app/Contents/MacOS/Shiva', 'darwin'),
    '/Applications/Shiva.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Shiva.app/Contents/MacOS/Shiva', 'darwin'),
    '/Users/x/Applications/Shiva.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Shiva\\Shiva.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Shiva'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\shiva-desktop\\Shiva.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\shiva-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Shiva.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_ShivaXXXX/shiva', 'linux', { APPIMAGE: '/home/x/Apps/Shiva.AppImage' }),
    '/home/x/Apps/Shiva.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/shiva/linux-unpacked/shiva', 'linux', {}), '/opt/shiva/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/shiva', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Shiva.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Shiva.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.shiva/shiva-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.shiva/shiva-agent',
    uninstallArgs: ['-m', 'shiva_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/shiva/linux-unpacked',
    shivaHome: '/home/x/.shiva'
  })
  ...
  assert.match(script, /'-m' 'shiva_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/shiva\/linux-unpacked'/)
  assert.match(script, /export SHIVA_HOME='\/home\/x\/\.shiva'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.shiva/shiva-agent',
    agentRoot: '/home/x/.shiva/shiva-agent',
    uninstallArgs: ['-m', 'shiva_cli.uninstall', '--mode', 'full'],
    appPath: null,
    shivaHome: '/home/x/.shiva'
  })

  // System python + source on PYTHONPATH so import shiva_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.shiva\/shiva-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'shiva_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'shiva_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    shivaHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'shiva_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\shiva',
    uninstallArgs: ['-m', 'shiva_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Shiva',
    shivaHome: 'C:\\Users\\x\\AppData\\Local\\shiva'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import shiva_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\shiva;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "shiva_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Shiva" >nul 2>&1/)
