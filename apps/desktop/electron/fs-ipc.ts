...
// hardening, SHIVA_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('shiva:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('shiva:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('shiva:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('shiva:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<SHIVA_HOME>/desktop-plugins`,
  // resolved from the main-process SHIVA_HOME (see resolveShivaHome) — NOT from
  // the connected backend. A remote backend reports its own `shiva_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped shiva_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(shivaHome, 'profiles', profile) : shivaHome
  ...
  ipcMain.handle('shiva:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<SHIVA_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where SHIVA_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('shiva:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<SHIVA_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('shiva:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('shiva:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('shiva:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('shiva:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('shiva:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('shiva:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
