...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'shiva-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/shiva-runner-test',
    sourceRepoRoot: null,
    shivaHome: '/tmp/shiva-runner-test',
    logRoot: '/tmp/shiva-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'shiva-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'shiva-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/shiva-agent',
      shivaHome: '/tmp/shiva'
    }),
    ['--dir', '/tmp/shiva-agent', '--shiva-home', '/tmp/shiva', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/shiva-agent', '--shiva-home', '/tmp/shiva', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/shiva',
      shivaHome: '/tmp/home'
    }),
    ['--dir', '/tmp/shiva', '--shiva-home', '/tmp/home', '--branch', 'main']
  )
})
