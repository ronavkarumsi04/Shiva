...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/shiva', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/shiva/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/shiva'), 'https://gw.example.com/shiva/auth/native/refresh')
})
