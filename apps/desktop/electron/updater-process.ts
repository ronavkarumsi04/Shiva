 ...
 *
 * The staged Tauri `shiva-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `shiva update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `shiva update`, swaps/relaunches
 * the app, and writes .shiva-update-result.json. With the app gone before
 * the update starts, the SHIVA_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `shiva update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into SHIVA_HOME on *every* platform
 * (`shiva-setup.exe` on Windows, `shiva-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_shiva_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `shiva update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(shivaHome, 'shiva-setup.exe')
 ...
 *
 * `copy_self_to_shiva_home` deliberately no-ops during `--update`
 ...
 * "Another Shiva update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `shiva update`, which
 ...
 * the installer is written to SHIVA_HOME at install/repair time, making mtime
