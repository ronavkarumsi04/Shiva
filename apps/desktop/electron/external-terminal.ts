// Launching the Shiva TUI in the user's OWN terminal emulator.
//
...
// `shiva --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Shiva runtime is often a venv Python invoked
//     as `python -m shiva_cli.main`, not a `shiva` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Shiva
