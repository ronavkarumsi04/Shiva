...
export function markerPath(shivaHome) {
  return path.join(shivaHome, '.shiva-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(shivaHome)
  let raw
 ...
 *
 * The Tauri-based shiva-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(shivaHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(shivaHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Shiva to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(shivaHome, opts)
