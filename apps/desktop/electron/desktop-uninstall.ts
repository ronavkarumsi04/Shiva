 ...
 * the three user-facing uninstall modes to the `shiva uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m shiva_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `shiva_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'shiva_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Shiva.app/Contents/MacOS/Shiva  → …/Shiva.app
 *   Windows: …\Shiva\Shiva.exe                 → …\Shiva  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Shiva.app/Contents/MacOS/Shiva → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Shiva.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Shiva.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Shiva$/i.test(dir) || /[\\/]shiva-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/shiva
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import shiva_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, shivaHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import shiva_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Shiva install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "SHIVA_HOME=${String(shivaHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
