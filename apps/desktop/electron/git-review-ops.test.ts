...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'shiva-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'shiva-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Shiva Test'], { cwd: dir })
