export type ParentWatchdogEnv = {
  SHIVA_PARENT_PID: string
  SHIVA_PARENT_START_MARKER?: string
  SHIVA_PARENT_NONCE?: string
  /** Spawn tag consumed by shiva_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { SHIVA_PARENT_PID: String(pid) }
  env.SHIVA_SPAWN = spawnTag(pid, startMarker)
  ...
  env.SHIVA_PARENT_START_MARKER = startMarker
  env.SHIVA_PARENT_NONCE = nonce

  return env
}

/** Build the `SHIVA_SPAWN` tag mirrored by shiva_cli/process_identity.py.
