...
const SCAN_MODULE = 'shiva_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Shiva processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'shiva_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Shiva process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Shiva installation is free.\n' +
    '\n' +
    'Close other Shiva windows and terminals, then retry.  If the problem\n' +
    'persists, run `shiva update` in a terminal for detailed diagnostics.'
  )
}
