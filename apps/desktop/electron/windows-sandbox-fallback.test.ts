...
test('sandbox marker round-trips through the userData file', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'shiva-sandbox-marker-'))
...
test('buildIcaclsGrantArgs targets ALL APPLICATION PACKAGES with inherited RX', () => {
  assert.deepEqual(buildIcaclsGrantArgs('C:\\Shiva\\win-unpacked'), [
    'C:\\Shiva\\win-unpacked',
    '/grant',
...
test('grantAllApplicationPackagesAcl is a no-op off Windows and reports exec failures', () => {
  ...
  const ok = grantAllApplicationPackagesAcl('C:\\Shiva', {
    platform: 'win32',
  ...
  assert.deepEqual(calls[0]?.args, buildIcaclsGrantArgs('C:\\Shiva'))

  const failed = grantAllApplicationPackagesAcl('C:\\Shiva', {
    platform: 'win32',
    execFileSync() {
...
test('buildNoSandboxRelaunchArgs appends a single --no-sandbox flag', () => {
  assert.deepEqual(buildNoSandboxRelaunchArgs(['--foo', '--no-sandbox', 'shiva://x']), [
    '--foo',
    'shiva://x',
    '--no-sandbox'
  ])
