// Backend subcommand routing for the desktop-managed Shiva process.
//
// The desktop app launches its own headless backend via `shiva serve` — it
...
// app hasn't updated yet, or an older `shiva` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Shiva profile to pin via `--profile`.
 */
 ...
 * `-m shiva_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `shiva_cli/subcommands/dashboard.py` source registers
