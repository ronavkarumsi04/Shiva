 ...
 *
 * Stamps the Shiva icon + identity onto the packed Windows Shiva.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `shiva desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Shiva')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Shiva'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Shiva.exe keeps the stock Electron icon`)
  }
}
