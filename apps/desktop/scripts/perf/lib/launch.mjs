...
//   · its own SHIVA_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · SHIVA_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated SHIVA_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate SHIVA_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = shivaHome ?? mkTemp('shiva-perf-home-')
  const userData = userDataDir ?? mkTemp('shiva-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !shivaHome) {
    seedConfigFrom(join(homedir(), '.shiva'), home)
  }
  ...
  try {
    ...
    // SHIVA_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set SHIVA_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...
      SHIVA_DESKTOP_CDP_PORT: String(port),
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.SHIVA_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'shiva-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'shiva-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.shiva'), home)
