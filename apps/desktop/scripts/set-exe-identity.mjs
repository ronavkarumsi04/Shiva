#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Shiva icon + version metadata onto the
// built Shiva.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Shiva.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `shiva desktop`, the installer's
...
// path (which rebuilds via `shiva desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Shiva.exe>
//
...
// Stamp the Shiva icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Shiva',
      FileDescription: 'Shiva',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — Shiva icon + identity stamped')
}
