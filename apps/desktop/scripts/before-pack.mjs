 ...
 * product name (`Shiva`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Shiva'
 *
 * This is a hard failure with no obvious cause for the user — `shiva desktop`
 ...
 * (the mac-unpacked Shiva.app bundle) and Windows (win-unpacked), so we
 ...
 * Shiva.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `shiva desktop --build-only` (shiva_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Shiva.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Shiva'}.exe`
