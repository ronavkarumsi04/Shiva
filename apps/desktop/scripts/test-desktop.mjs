...
// no longer bundles the Shiva Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Shiva.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Shiva'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Shiva.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Shiva'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default SHIVA_HOME for non-sandboxed runs -- matches main.ts's
// resolveShivaHome(). On Windows it's %LOCALAPPDATA%\shiva; elsewhere
// it's ~/.shiva. The fresh-install sandbox launchFresh() sets its own
// SHIVA_HOME and never touches this.
const DEFAULT_SHIVA_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'shiva')
  }
  return path.join(os.homedir(), '.shiva')
})()
const VENV_ROOT = path.join(DEFAULT_SHIVA_HOME, 'shiva-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'shiva-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.SHIVA_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Shiva-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Shiva-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Shiva-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Shiva-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.SHIVA_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.SHIVA_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const shivaHome = path.join(sandbox, 'shiva-home')
  ...
  env.SHIVA_DESKTOP_CWD = cwd
  env.SHIVA_DESKTOP_IGNORE_EXISTING = '1'
  env.SHIVA_DESKTOP_TEST_MODE = 'fresh-install'
  env.SHIVA_DESKTOP_USER_DATA_DIR = userDataDir
  env.SHIVA_HOME = shivaHome
  delete env.SHIVA_DESKTOP_SHIVA
  delete env.SHIVA_DESKTOP_SHIVA_ROOT
  ...
  console.log(`  SHIVA_HOME: ${shivaHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(shivaHome, 'shiva-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Shiva Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of shiva_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'shiva-agent', 'shiva_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Shiva
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + SHIVA_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  SHIVA_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
