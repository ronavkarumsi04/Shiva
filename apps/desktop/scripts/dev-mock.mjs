...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `shiva-dev-mock-${Date.now()}`))
  ...
  return { root, shivaHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(shivaHome, mockUrl) {
  fs.writeFileSync(
    path.join(shivaHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(shivaHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.shivaHome, mock.url)
  console.log(`  SHIVA_HOME: ${sandbox.shivaHome}`)
  ...
  const env = {
    ...
    SHIVA_DESKTOP_APP_NAME: `ShivaDevMock-${Date.now()}`,
  }
