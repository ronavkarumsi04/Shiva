...
<p align="center">
  <a href="https://github.com/NousResearch/shiva-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [Shiva Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Shiva surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Shiva and hear it back.</td></tr>
...
### Install with Shiva (recommended)

Already have the Shiva CLI? Just run:

```bash
shiva desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Shiva gateway or install Shiva locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Shiva Desktop website.](https://shiva-agent.nousresearch.com/).
...
```bash
# throwaway SHIVA_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Shiva Agent runtime into `SHIVA_HOME`
(`~/.shiva`, or `%LOCALAPPDATA%\shiva` on Windows), using the same layout as a
CLI install.
...
- **Shiva Agent** runs as a headless `shiva serve` process and exposes the
...
1. `SHIVA_DESKTOP_SHIVA_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `SHIVA_DESKTOP_SHIVA`, or `shiva` on `PATH`
5. a system Python that can import the Shiva runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Shiva
...
screen offers **Connect to existing Shiva** before starting the local installer.
...
terminal commands, and file operations run against the remote Shiva host, not
...
transport- or Shiva-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Shiva-Session-Token`.
...
Boot logs land in `SHIVA_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\shiva\shiva-agent\.shiva-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\shiva\shiva-agent\venv"
```

> The default Shiva home on Windows is `%LOCALAPPDATA%\shiva`. Set the `SHIVA_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://shiva-agent.nousresearch.com/docs/)
