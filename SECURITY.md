# Shiva Agent Security Policy

This document describes Shiva Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/shiva-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Shiva Agent does not operate a bug
bounty program.**
...
- Environment details (`shiva --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Shiva Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Shiva Agent's documentation
...
Shiva Agent supports two OS-level isolation postures. They address
...
Shiva Agent supports this in two ways:

- **Shiva Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Shiva Agent's in-process heuristics
...
Shiva Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Shiva Agent
itself. Bugs in Shiva Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Shiva Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Shiva Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Shiva Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  shiva-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Shiva Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Shiva Agent. Bugs in Shiva
...
- Shiva Agent includes supply-chain guards for MCP server
