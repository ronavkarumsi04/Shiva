#!/usr/bin/env python3
"""
SQLite State Store for Shiva Agent.
...
# transitive dependency via shiva_state_common). run_agent keeps its own
# predating copy — shiva_state cannot import run_agent (circular) — guarded
...
from shiva_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, TypeVar

from shiva_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from shiva_state_portability import SessionPortabilityMixin
from shiva_state_schema import SessionSchemaMixin
from shiva_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``shiva -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_shiva_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.shiva`` even when a test fixture later
    redirects ``SHIVA_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_shiva_home()`` fresh so a runtime
       ``SHIVA_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_shiva_home() / "state.db"
...
# ~/.shiva/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects SHIVA_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without SHIVA_HOME, a stale worktree
# without the re-pin, or a developer shell that exports SHIVA_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "SHIVA_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.shiva``).  The test conftest injects the pre-sandbox production
#: root here so custom-``SHIVA_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Shiva root for the guard.

    Deliberately avoids ``Path.home()`` / ``shiva_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``shiva_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "shiva"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "shiva"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".shiva"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``SHIVA_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Shiva test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``SHIVA_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "SHIVA_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``SHIVA_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``shiva`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Shiva home *root*.
    ...
    under ``~/.shiva/shiva-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``SHIVA_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``SHIVA_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Shiva root {root}). "
                "Tests must run against a temporary SHIVA_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "SHIVA_HOME. If this test genuinely needs the live "
...
# shiva_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``shiva sessions optimize`` (FTS merge + VACUUM)
    ...
    ``shiva_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``shiva_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``shiva update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Shiva-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Shiva"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Shiva-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `shiva doctor`. This warning fires once per "
...
def _log_journal_mode_upgrade_once(db_label: str, previous_mode: str) -> None:
    ...
    ``shiva_cli/managed_uv._default_live_venv``, which treats a database
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see shiva_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``synchronous`` — durability level: ``OFF``/``NORMAL``/``FULL``/``EXTRA``
    ...
    """
    try:
        # Local import avoids a circular import with shiva_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``shiva doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Shiva host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``shiva serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — Shiva's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``shiva_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        ...
        try:
            ...
            # Scaffold/embed installs ship shiva_state without shiva_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``shiva_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Shiva home tree (``get_shiva_home()``) — the safe repair scope:
      Shiva owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``shiva_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_shiva_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Shiva needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (shiva_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_shiva_state.py:600-632)
        ...
        probe_session_id = f"_shiva_fts_health_probe_{time.time_ns()}"
        try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, Shiva deliberately runs ``state.db`` in
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (shiva gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.shiva/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``shiva sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("SHIVA_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_shiva_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("SHIVA_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship shiva_state without shiva_cli). A
    ...
    try:
        ...
        logger.debug(
            "shiva_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch shiva_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``shiva_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `shiva snapshot list` / "
                "`shiva snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (shiva doctor, dashboards) ──────────
...
_SHIVA_CMDLINE_MARKERS = ("shiva_cli.main", "shiva_cli/main", "shiva serve",
                           "shiva-agent", "shiva gateway", "shiva chat")


def _looks_like_shiva(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a Shiva process?
    ...
    We only flag processes that look like Shiva, not every system daemon.
    """
    lower = cmdline.lower()
    return any(marker in lower for marker in _SHIVA_CMDLINE_MARKERS)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple shiva processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Shiva
    ...
    # `shiva update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `shiva snapshot list` / "
                    f"`shiva snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`shiva db optimize`),
            ...
            # ``shiva_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `shiva update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `shiva sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Shiva process held the "
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a Shiva process —
                        # only flag uninspectable holders that look like
                        # another Shiva instance, not every system daemon.
                        cmdline = _read_proc_cmdline(pid)
                        if cmdline is not None and _looks_like_shiva(cmdline):
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from shiva_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    #
    # `optimize_fts_storage()` (the `shiva sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`shiva sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``shiva sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``shiva --resume`` into a fresh
    ...
    def sweep_orphaned_sessions(
        ...
        *,
        ...
        ``shiva serve``, desktop WS sidecar), each backend registers a row
    ...
    # of truth: shiva_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: shiva_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``shiva -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``shiva sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``SHIVA_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Shiva, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Shiva session.

        A Shiva session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Shiva session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `shiva sessions vacuum` runs in a transient
