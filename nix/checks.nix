...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      shiva-agent = self'.packages.default;
      shivaVenv = shiva-agent.shivaVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "shiva-check";
                homeDirectory = "/home/shiva-check";
                stateVersion = "24.11";
              };
            }
            { services.shiva-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.shiva-agent = programs;
              services.shiva-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.shiva-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.shiva-agent);
      ...
      configKeys = pkgs.runCommand "shiva-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${shivaVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "shiva-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "shiva-build-package" { } ''
          echo "PASS: package built at ${shiva-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "shiva-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.SHIVA_TEST = "1";
                environmentFiles = [ "/run/secrets/shiva-env" ];
                shivaHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "shiva" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "shiva" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.shivaAgentSetup.data;

            failures =
              lib.optional (names != [
                "shiva-agent"
                "shiva-backend"
              ]) "expected shiva-agent + shiva-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/shiva gateway" (argvOf "shiva-agent")
              ) "gateway process does not run `shiva gateway`: ${argvOf "shiva-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/shiva serve" (argvOf "shiva-backend")
              ) "backend process does not run `shiva serve`: ${argvOf "shiva-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "shiva-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/shiva-check/.shiva" (envOf n)) names
              ) "gateway and backend must share one SHIVA_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.SHIVA_HOME or null != "/home/shiva-check/.shiva"
              ) "programs.shiva-agent.enable must export SHIVA_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "shiva-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/shiva-check/.shiva/SOUL.md" activation
              ) "shivaHomeFiles must install into SHIVA_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads SHIVA_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "SHIVA_MANAGED=home-manager" (envOf n)) names
              ) "processes must report SHIVA_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "shiva-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "shiva-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/shiva-check";
            ...
            cases = [
              ...
              {
                name = "shivaHomeFiles needs no directory";
                ok = accepts { shivaHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "shiva-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one SHIVA_HOME ───────────────
        # `programs.enable` exports SHIVA_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.shiva
        # while the services use the SHIVA_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/shiva-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                shivaHome = "/home/shiva-check/.shiva-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "shiva-desktop") cfg.home.packages;
            ...
            # second Shiva runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "shiva-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.shiva-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.shiva-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one shiva-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "SHIVA_HOME" != "/home/shiva-check/.shiva-work"
              ) "the launcher must carry SHIVA_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "SHIVA_HOME")})"
              ++ lib.optional (
                setValue "SHIVA_MANAGED" != "home-manager"
              ) "the launcher must report SHIVA_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "SHIVA_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "SHIVA_DESKTOP_SHIVA" != "${lib.head agentPackages}/bin/shiva"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "SHIVA_DESKTOP_SHIVA")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "SHIVA_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "SHIVA_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "SHIVA_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "SHIVA_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "SHIVA_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "shiva-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares SHIVA_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set SHIVA_HOME" wrapper
              ) "the launcher must carry SHIVA_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "SHIVA_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "SHIVA_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? shiva-backend || cfg.launchd.agents ? shiva-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "shiva-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `shiva`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.shiva-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.shiva-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "shiva-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "shiva-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "shiva" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."shiva-agent-setup".text;

            failures =
              lib.optional (names != [
                "shiva-agent"
                "shiva-backend"
              ]) "expected shiva-agent + shiva-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/shiva gateway" (execOf "shiva-agent")
              ) "gateway unit does not run `shiva gateway`: ${execOf "shiva-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/shiva dashboard" (execOf "shiva-backend")
              ) "backend unit does not run `shiva dashboard`: ${execOf "shiva-backend"}"
              ++ lib.optional (
                units.shiva-agent.environment.SHIVA_HOME != units.shiva-backend.environment.SHIVA_HOME
              ) "gateway and backend must share one SHIVA_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/shiva/.shiva/SOUL.md" activation
              ) "shivaHomeFiles must install into SHIVA_HOME";
          ...
          in
          pkgs.runCommand "shiva-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs shiva, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.shiva-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/shiva serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec shiva directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "shiva-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "shiva-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec shiva, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/shiva dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "shiva-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $SHIVA_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                SHIVA_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "shiva-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'SHIVA_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--shiva-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "shiva-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "shiva-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${shiva-agent}/bin/shiva || (echo "FAIL: shiva binary missing"; exit 1)
          test -x ${shiva-agent}/bin/shiva-agent || (echo "FAIL: shiva-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${shiva-agent}/bin/shiva --version 2>&1 | grep -qi "shiva" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "shiva-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in shiva shiva-agent shiva-acp; do
            test -x ${shiva-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "shiva-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking shiva --help ==="
          ${shiva-agent}/bin/shiva --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${shiva-agent}/bin/shiva --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "shiva-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${shiva-agent}/share/shiva-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${shiva-agent}/share/shiva-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "SHIVA_BUNDLED_SKILLS" ${shiva-agent}/bin/shiva || \
            (echo "FAIL: SHIVA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: SHIVA_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${shiva-agent}/share/shiva-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${shiva-agent}/share/shiva-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "SHIVA_OPTIONAL_SKILLS" ${shiva-agent}/bin/shiva || \
            (echo "FAIL: SHIVA_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, SHIVA_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "shiva-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${shiva-agent}/share/shiva-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${shiva-agent}/share/shiva-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "SHIVA_BUNDLED_PLUGINS" ${shiva-agent}/bin/shiva || \
            (echo "FAIL: SHIVA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: SHIVA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "shiva-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${shiva-agent}/share/shiva-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${shiva-agent}/share/shiva-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${shiva-agent}/share/shiva-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "SHIVA_BUNDLED_LOCALES" ${shiva-agent}/bin/shiva || \
            (echo "FAIL: SHIVA_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: SHIVA_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + SHIVA_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && SHIVA_BUNDLED_LOCALES=${shiva-agent}/share/shiva-agent/locales \
            ${shivaVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with SHIVA_BUNDLED_LOCALES set"; exit 1)
        ...
        # SHIVA_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "shiva-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${shiva-agent}/share/shiva-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${shiva-agent}/share/shiva-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "SHIVA_OPTIONAL_MCPS" ${shiva-agent}/bin/shiva || \
            (echo "FAIL: SHIVA_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: SHIVA_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${shiva-agent}/bin/shiva mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: shiva mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "shiva-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${shiva-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${shiva-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "SHIVA_TUI_DIR" ${shiva-agent}/bin/shiva || \
            (echo "FAIL: SHIVA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: SHIVA_TUI_DIR set in wrapper"
        ...
        # Verify SHIVA_NODE is set in wrapper and points to Node 26+
        # (Shiva pins its toolchain to Node 26 everywhere)
        shiva-node = pkgs.runCommand "shiva-node-version" { } ''
          set -e
          echo "=== Checking SHIVA_NODE in wrapper ==="
          grep -q "SHIVA_NODE" ${shiva-agent}/bin/shiva || \
            (echo "FAIL: SHIVA_NODE not set in wrapper"; exit 1)
          echo "PASS: SHIVA_NODE present in wrapper"

          SHIVA_NODE=$(sed -n "s/^export SHIVA_NODE='\(.*\)'/\1/p" ${shiva-agent}/bin/shiva)
          test -x "$SHIVA_NODE" || (echo "FAIL: SHIVA_NODE=$SHIVA_NODE not executable"; exit 1)
          echo "PASS: SHIVA_NODE executable at $SHIVA_NODE"

          NODE_MAJOR=$("$SHIVA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Shiva requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All SHIVA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify SHIVA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "shiva-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(SHIVA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking SHIVA_MANAGED guards ==="
          check_blocked "config set" ${shiva-agent}/bin/shiva config set model foo
          check_blocked "config edit" ${shiva-agent}/bin/shiva config edit
        ...
        extra-python-packages = let
          ...
          shivaWithExtra = shiva-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "shiva-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${shivaWithExtra}/bin/shiva || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${shivaWithExtra}/bin/shiva || \
          ...
          if grep -q "PYTHONPATH" ${shiva-agent}/bin/shiva; then
        ...
        extra-dependency-groups = let
          shivaWithGroups = shiva-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "shiva-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${shivaWithGroups}"
          echo "venv: ${shivaWithGroups.shivaVenv}"
        ...
        messaging-variant = pkgs.runCommand "shiva-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.shivaVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "shiva-config-roundtrip" {
          ...
          merge_and_load() {
            local shiva_home="$1"
            export SHIVA_HOME="$shiva_home"
            ${configMergeScript} ${nixSettings} "$shiva_home/config.yaml"
            ${shivaVenv}/bin/python3 -c '
import json, sys
