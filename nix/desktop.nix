# nix/desktop.nix — Shiva Desktop (Electron) app build + wrapper
#
# `shivaAgent` is the fully-built `.#default` package — it ships the
# `shiva` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `SHIVA_DESKTOP_SHIVA` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Shiva CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # SHIVA_HOME and SHIVA_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "shiva-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "shiva-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = shivaNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "shiva-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe shivaNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/shiva-desktop $out/bin
    cp -r ${renderer}/* $out/share/shiva-desktop/
    ...
    substituteInPlace $out/share/shiva-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/shiva-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # SHIVA_DESKTOP_SHIVA to the absolute path of the nix-built `shiva`
    # binary so the desktop's resolver step 4 ("existing Shiva CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/shiva-desktop \
      --add-flags "$out/share/shiva-desktop" \
      --set SHIVA_DESKTOP_SHIVA "${lib.getExe shivaAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/shiva.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../shiva_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/shiva-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/shiva.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/shiva.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Shiva Agent";
    homepage = "https://github.com/NousResearch/shiva-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "shiva-desktop";
  };
}
