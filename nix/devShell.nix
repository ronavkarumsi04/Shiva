...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      shivaNpmLib = self'.packages.default.passthru.shivaNpmLib;
      ...
      shivaAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "shiva" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../shiva} $out/bin/shiva
          '')
        ...
        shellHook = ''
          ${shivaAgentDevShellHook}
          ${shivaNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export SHIVA_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Shiva Agent dev shell in $SHIVA_PYTHON_SRC_ROOT"
          echo "Ready. Run 'shiva' or 'sandbox shiva' to start."
        '';
      };
    };
}
