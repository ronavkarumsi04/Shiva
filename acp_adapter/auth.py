"""ACP auth helpers — detect and advertise Shiva authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "shiva-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Shiva runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Shiva can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Shiva.
    ...
    not have Shiva provider credentials configured yet, so Shiva always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Shiva using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Shiva provider",
            description=(
                "Open Shiva' interactive model/provider setup in a terminal. "
                "Use this when Shiva has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
