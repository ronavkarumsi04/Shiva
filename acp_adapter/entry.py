"""CLI entry point for the shiva-agent ACP adapter.

Loads environment variables from ``~/.shiva/.env``, configures logging
...
try:
    ...
    # shadowing Shiva's own modules — ``shiva acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from SHIVA_HOME (default ``~/.shiva``)."""
    from shiva_cli.env_loader import load_shiva_dotenv

    shiva_home = get_shiva_home()
    loaded = load_shiva_dotenv(shiva_home=shiva_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", shiva_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="shiva-acp",
        description="Run Shiva Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Shiva version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Shiva provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.shiva/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import ShivaACPAgent  # noqa: F401

    print("Shiva ACP check OK")


def _run_setup() -> None:
    from shiva_cli.main import main as shiva_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "shiva", "model"]
        shiva_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting shiva-agent ACP adapter")
    ...
    import acp
    from .server import ShivaACPAgent
    ...
    if os.environ.get("SHIVA_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
