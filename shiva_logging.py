"""Centralized logging setup for Shiva Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.shiva/logs/`` (profile-aware via ``get_shiva_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``shiva_cli.web_server``,
    ``shiva_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Shiva (TUI, gateway, ``hy_memory`` server, MCP
...
from shiva_constants import get_config_path, get_shiva_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_shiva_session_injector", False):
    ...
    _session_record_factory._shiva_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``shiva logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``shiva logs --component gateway``.
    "gateway": ("gateway", "shiva_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("shiva_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "shiva_cli.web_server",
        "shiva_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Shiva logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    shiva_home
        Override for the Shiva home directory.  Falls back to
        ``get_shiva_home()`` (profile-aware).
    log_level
    ...
    home = shiva_home or get_shiva_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_shiva_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._shiva_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the shiva group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from shiva_cli.config import is_managed
...
# the module header): when several Shiva processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._shiva_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="shiva-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_shiva_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse shiva_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when shiva_cli.config isn't importable (bare
        # shiva_logging consumers).
        try:
            from shiva_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
