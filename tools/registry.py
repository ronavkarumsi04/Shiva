"""Central registry for all shiva-agent tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (shiva_constants itself is stdlib-only, so no cycle).
        from shiva_constants import get_shiva_home

        return Path(get_shiva_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``shiva tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Shiva-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("SHIVA_SESSION_ID", ""),
            get_session_env("SHIVA_BROWSER_CONTROL_PRINCIPAL", ""),
            get_session_env("SHIVA_BROWSER_CONTROL_TRANSPORT_FAMILY", ""),
        )
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``shiva tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved SHIVA_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("shiva_plugins."):
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``shiva_plugins.{name}``), not the exact module string.
                # A handler defined in ``shiva_plugins.pkg.handlers`` is
                # still owned by the ``shiva_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``shiva tools enable foo``)
