...
_PRIVATE_BROWSER_SENTINEL = "_SHIVA_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# shiva: pin this named session to its own tab (once per daemon process)
def _shiva_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "shiva-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_shiva_ensure_own_tab()
del _shiva_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Shiva's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Shiva's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_shiva_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `shiva tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Shiva' own bin dir ($SHIVA_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from shiva_constants import get_shiva_home

        return str(Path(get_shiva_home()) / "bin")
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Shiva' managed uv (bootstrapped on demand via
    ``shiva_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$SHIVA_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Shiva-managed copy, or resolution
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_shiva_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `shiva tools` → Browser Automation."
        )
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser Shiva launches on a
    SNAPSHOT of their real profile (see shiva_cli.browser_connect). Two ways
    in:
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a Shiva-managed copy of "
