...
Lets the LLM write a Python script that calls Shiva tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `shiva_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `shiva_tools.py` with file-based RPC stubs
...
# available on every platform Shiva itself runs on.
...
# match a safe prefix, the operational SHIVA_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Shiva
# code can lose the DB-layer Kanban mutation guard while still inheriting
# SHIVA_HOME.
#
# NB: the broad "SHIVA_" prefix was deliberately removed (#27303) — it leaked
# SHIVA_*-named config that lacks a secret substring (e.g. SHIVA_BASE_URL,
# SHIVA_KANBAN_DB, SHIVA_*_WEBHOOK).  The child only needs the few
# location/profile vars in _SHIVA_CHILD_ALLOWED below; SHIVA_RPC_SOCKET /
# SHIVA_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational SHIVA_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set shiva_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_SHIVA_CHILD_ALLOWED = frozenset({
    "SHIVA_HOME",
    "SHIVA_PROFILE",
    "SHIVA_CONFIG",
    "SHIVA_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational SHIVA_* vars (_SHIVA_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret SHIVA_* vars dropped by the tightened allowlist (#27303). The
    # broad "SHIVA_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like SHIVA_KANBAN_DB / SHIVA_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _SHIVA_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("SHIVA_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped SHIVA_* var.
            _dropped_shiva.append(k)
    if _dropped_shiva:
        logger.debug(
            "execute_code: dropped %d non-allowlisted SHIVA_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Shiva-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Shiva-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    shiva_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'shiva_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_shiva_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the shiva_tools.py stub module.
...
"""Auto-generated Shiva tools RPC stubs."""
...
def _connect():
    ...
    SHIVA_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["SHIVA_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("SHIVA_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (SHIVA_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("SHIVA_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated Shiva tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("SHIVA_RPC_DIR") or os.path.join(tempfile.gettempdir(), "shiva_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated shiva_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/shiva_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_shiva_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/shiva_tools.py", tools_src)
        ...
        env_prefix = (
            f"SHIVA_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"SHIVA_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("SHIVA_TIMEZONE", "").strip()
        if tz:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    child_env["SHIVA_RPC_SOCKET"] = rpc_endpoint
    child_env["SHIVA_RPC_TOKEN"] = rpc_token
    ...
    # SHIVA_TIMEZONE is an internal Shiva setting and must not leak
    # into child processes.
    _tz_name = os.getenv("SHIVA_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("SHIVA_TIMEZONE", None)

    apply_subprocess_home_env(child_env)
    # ``shiva_tools.py`` always lives in the staging directory, so that
    # directory must be importable even when project mode changes CWD.
    # Shiva's own package root is useful too, but only when the child
    # uses the same Python environment. Project mode can select an
    # external venv; exposing Shiva's site-packages to that interpreter
    # can mix incompatible compiled extensions (for example, Python 3.12
    # NumPy with a Python 3.9 project interpreter).
    #
    # Before re-injecting PYTHONPATH, strip Shiva-owned entries that
    # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
    # so it passes the scrub).  They are redundant for same-Shiva-
    ...
    _strip_shiva_owned_pythonpath(child_env)
    _shiva_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_shiva_python_environment(child_python):
        _pp_parts.append(_shiva_root)
    elif child_python not in _external_env_logged:
        # Import behavior changes silently otherwise — surface it (once
        # per interpreter path) so "import shiva_constants suddenly
        # fails" reports are diagnosable without log spam.
        _external_env_logged.add(child_python)
        logger.info(
            "execute_code: child interpreter %s is outside the Shiva "
            "environment; shiva root omitted from PYTHONPATH",
            child_python,
        )
    if _existing_pp:
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.shiva.gateway`, but the
    ...
    # --- Set up temp directory with shiva_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="shiva_sandbox_")
    ...
    # selector from SHIVA_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"shiva_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated shiva_tools module.
        ...
        tools_src = generate_shiva_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "shiva_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  SHIVA_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.shiva/.env')).
...
def _load_config() -> dict:
    ...
    ``shiva --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (shiva-agent's python). Reproducible and the
...
# Interpreter paths already reported as outside the Shiva environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the shiva root from every
...
def _uses_shiva_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Shiva's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``shiva tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and shiva-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.shiva/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls Shiva tools programmatically. "
        ...
        f"Available via `from shiva_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from shiva_tools import {import_str}` "
