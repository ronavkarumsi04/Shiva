...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``shiva tools`` repaint, tool-registration scans,
    ...
    2. ``~/.shiva/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``shiva auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_shiva_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.shiva/.env`` first, then ``os.environ``.

    Wraps :func:`shiva_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from shiva_cli.config import get_env_value as _shiva_get_env_value
    except ImportError:
        return os.environ.get(name, default)

    value = _shiva_get_env_value(name)
    return value if value is not None else default


def shiva_xai_user_agent() -> str:
    """Return a stable Shiva-specific User-Agent for xAI HTTP calls."""
    try:
        from shiva_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Shiva-Agent/{__version__}"


def shiva_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Shiva Agent,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": shiva_xai_user_agent()}


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Shiva config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Shiva home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_shiva_home() / "state"
...
def _resolve_explicit_xai_base_url(default: str = "https://api.x.ai/v1") -> str:
    ...
    Honors ``SHIVA_XAI_BASE_URL`` then ``XAI_BASE_URL`` (the same override
    pair the OAuth branch reads) and pins the origin with
    :func:`shiva_cli.auth._xai_validate_inference_base_url` so a tampered
    env override can't exfiltrate the bearer; on rejection it falls back to
    the default rather than raising.
    """
    override = str(
        get_env_value("SHIVA_XAI_BASE_URL")
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Shiva-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``shiva_cli.config.get_env_value`` so keys
    stored in ``~/.shiva/.env`` (the standard Shiva location) are honored —
    ...
    honors ``SHIVA_XAI_BASE_URL`` / ``XAI_BASE_URL`` behind the same
