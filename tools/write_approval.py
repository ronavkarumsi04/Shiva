...
(see ``shiva_cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<SHIVA_HOME>/pending/{memory,skills}/<id>.json``
...
def _pending_dir(subsystem: str) -> Path:
    return get_shiva_home() / "pending" / subsystem
