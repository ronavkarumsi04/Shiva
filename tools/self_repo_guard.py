...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Shiva's live source checkout "
        ...
        "checkout, stop Shiva, run the command externally, then restart "
        "Shiva."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(shiva_home).expanduser() if shiva_home else Path.home() / ".shiva"
