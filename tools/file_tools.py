...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``shiva_constants.get_subprocess_home()`` so that ``~`` resolves
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where shiva previously hard-rejected an oversized read
...
def _terminal_env_type_for_task(task_id: str = "default") -> str:
    ...
    try:
        ...
        if env is not None:
            ...
            if "daytona" in name:
                return "daytona"
            stamped = getattr(env, "_shiva_backend_name", None)
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Shiva.
...
_shiva_config_resolved: str | None = None
_shiva_config_resolved_loaded = False


def _get_shiva_config_resolved() -> str | None:
    """Return the resolved absolute path of the Shiva config file (cached)."""
    global _shiva_config_resolved, _shiva_config_resolved_loaded
    if _shiva_config_resolved_loaded:
        return _shiva_config_resolved
    _shiva_config_resolved_loaded = True
    try:
        from shiva_cli.config import get_config_path
        _shiva_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _shiva_config_resolved = str(Path(_expand_tilde("~/.shiva/config.yaml")).resolve())
        except Exception:
            _shiva_config_resolved = None
    return _shiva_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Shiva config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    shiva_config = _get_shiva_config_resolved()
    if shiva_config and (resolved == shiva_config or normalized == shiva_config):
        return (
            f"Refusing to write to Shiva config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.shiva/config.yaml directly or use 'shiva config' instead."
        )
    return None
...
# .cursorrules (or a project-local .shiva config tree) outlives the current
...
_real_shiva_home_cached: str | None = None
_real_shiva_home_loaded = False


def _get_real_shiva_home() -> str | None:
    """Return the realpath of the authoritative Shiva home (cached)."""
    global _real_shiva_home_cached, _real_shiva_home_loaded
    if _real_shiva_home_loaded:
        return _real_shiva_home_cached
    _real_shiva_home_loaded = True
    try:
        from shiva_constants import get_shiva_home
        _real_shiva_home_cached = os.path.realpath(str(get_shiva_home()))
    except Exception:
        try:
            _real_shiva_home_cached = os.path.realpath(_expand_tilde("~/.shiva"))
        except Exception:
            _real_shiva_home_cached = None
    return _real_shiva_home_cached
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.shiva home is governed by its own guards
    ...
    # ``.shiva`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_shiva_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .shiva config dirs (e.g. <repo>/.shiva/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.shiva`` — matching
        # any ancestor named .shiva would gate every write inside a
        # checkout that happens to live under ~/.shiva (e.g. the
        # shiva-agent repo itself at ~/.shiva/shiva-agent).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".shiva":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Shiva mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Shiva
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Shiva state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.shiva/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Shiva process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.shiva/…`` path.

    Returns ``None`` when the write is in-scope or outside Shiva scope.
    ...
    # in a session that cd'd into ``~/.shiva/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Shiva internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under SHIVA_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == SHIVA_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Shiva-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Shiva-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Shiva profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Shiva profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with shiva_tools.write_file() for very "
            "large files."
        )
