...
def auth_json_path():
    """Return the Shiva auth store path, respecting SHIVA_HOME overrides."""
    return get_shiva_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`shiva tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a Shiva release, and
