...
`mcp_tool.py`, and `shiva_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# ShivaMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_shiva_provider_class() -> Optional[type]:
    ...
    class ShivaMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._shiva_server_name = server_name
            self._shiva_home = ""
            ...
            self._shiva_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`ShivaTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, ShivaTokenStorage)
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not Shiva' — mcp 2.0 builds on httpx2,
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._shiva_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import ShivaTokenStorage
                        if isinstance(storage, ShivaTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._shiva_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import ShivaTokenStorage
            if not isinstance(storage, ShivaTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``shiva mcp reauth``.
            ...
            back to ``shiva mcp reauth``.
            """
            try:
                if self._shiva_preregistered:
                    return
                ...
                # walking back into the same refusal. `shiva mcp login`
                ...
                if cimd_url and rejected_id == cimd_url:
                    logger.warning(
                        ...
                        self._shiva_server_name, cimd_url,
                    )
    ...
    return ShivaMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_SHIVA_PROVIDER_CLS: Optional[type] = _make_shiva_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._shiva_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`ShivaMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _SHIVA_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `shiva mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``shiva mcp remove <name>`` and (indirectly) by
        ``shiva mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, shiva_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, shiva_home=shiva_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, shiva_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, shiva_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, shiva_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(shiva_home) / f"{_safe_filename(server_name)}.json"
            try:
