...
One ``CDPSupervisor`` runs per Shiva ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Shiva logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "shiva-dialog-bridge.invalid"
...
(() => {
  if (window.__shivaDialogBridgeInstalled) return;
  window.__shivaDialogBridgeInstalled = true;
  const ENDPOINT = "http://shiva-dialog-bridge.invalid/";
