...
provider** — turn on a backend by enabling a plugin (``shiva plugins
enable video_gen/<name>``) and selecting it in ``shiva tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/shiva-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `shiva tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `shiva tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `shiva tools` → "
...
# `shiva tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`shiva tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `shiva tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
