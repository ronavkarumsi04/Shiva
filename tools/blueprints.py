...
`shiva skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.shiva.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.shiva.blueprint`` is a mapping containing
    ...
    shiva = meta.get("shiva") if isinstance(meta, dict) else None
    blueprint = shiva.get("blueprint") if isinstance(shiva, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.shiva.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.shiva.blueprint`` block) they can hand
    to ``shiva skills publish`` to share. ``body`` is the plain-language
