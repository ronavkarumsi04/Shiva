...
it is automatically downloaded from GitHub releases to $SHIVA_HOME/bin/tirith.
...
def _get_shiva_home() -> str:
    """Return the Shiva home directory, respecting SHIVA_HOME env var."""
    return str(get_shiva_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_shiva_home(), ".tirith-install-failed")
...
def _shiva_bin_dir() -> str:
    """Return $SHIVA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_shiva_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SHIVA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_shiva_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SHIVA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SHIVA_HOME/bin/tirith
    ...
    shiva_bin = os.path.join(_shiva_bin_dir(), "tirith")
    if os.path.isfile(shiva_bin) and os.access(shiva_bin, os.X_OK):
        _resolved_path = shiva_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return shiva_bin
