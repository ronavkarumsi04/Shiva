...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   ``$SHIVA_HOME/cache/spillover/{tool_use_id}.txt`` — alongside the other
   Shiva-owned caches (images, audio, documents, ...) instead of littering
...
STORAGE_DIR = "/tmp/shiva-results"
SPILLOVER_SUBDIR = "cache/spillover"
SPILLOVER_MAX_AGE_HOURS = 24
HEREDOC_MARKER = "SHIVA_PERSIST_EOF"
...
def get_spillover_dir():
    """Return $SHIVA_HOME/cache/spillover as a Path (not created)."""
    from shiva_constants import get_shiva_home

    return get_shiva_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write content host-side to $SHIVA_HOME/cache/spillover.
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/shiva-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"SHIVA_PERSIST_{uuid.uuid4().hex[:8]}"
...
def maybe_persist_tool_result(
    ...
    # Always persist host-side first: $SHIVA_HOME/cache/spillover is the
    # single canonical home for spilled results (with the other Shiva-owned
