...
<url>``) is spawned as a direct child of the Shiva process. Shiva's own
...
Shiva process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Shiva startup's opt-in
