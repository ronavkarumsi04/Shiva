...
# Must match shiva_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``shiva-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``shiva-cli`` because those also contain
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. shiva-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (shiva-cli,
    # shiva-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child Shiva/Qwen subagent onto the parent's Claude Messages wire (or the
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.shiva/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_shiva_dir("cache/delegation", "delegation_cache")
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("SHIVA_UI_SESSION_ID", "") or None
            except Exception:
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from shiva_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # SHIVA_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Shiva-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("SHIVA_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Shiva-Session-Id once the batch completes.
            ...
            # SHIVA_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `shiva -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("SHIVA_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("SHIVA_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and SHIVA_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'shiva auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Shiva config.

    Prefer the shared persistent loader because it follows the active
    SHIVA_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``SHIVA_IGNORE_USER_CONFIG=1`` (``shiva chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("SHIVA_IGNORE_USER_CONFIG") == "1"
