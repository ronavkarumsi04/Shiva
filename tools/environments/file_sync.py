...
_FORCE_SYNC_ENV = "SHIVA_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.shiva") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.shiva to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``SHIVA_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, shiva_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.shiva/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(shiva_home=shiva_home)

    def _sync_back_transaction(self, shiva_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="shiva-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.shiva/skills/a.md`` →
        ``~/.shiva/skills/a.md``, a new remote file at
        ``/root/.shiva/skills/b.md`` maps to ``~/.shiva/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
