"""Base class for all Shiva execution environment backends.
...
# SHIVA_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("SHIVA_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {SHIVA_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_shiva_home() / "sandboxes"
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    recorded on ``proc._shiva_stdin_errors`` while stdin is still closed
    ...
    proc._shiva_stdin_errors = errors
    ...
    proc._shiva_stdin_thread = thread
    thread.start()
...
def _cwd_marker(session_id: str) -> str:
    return f"__SHIVA_CWD_{session_id}__"
...
# the FIRST session's SHIVA_SESSION_ID into the snapshot makes every LATER
...
# SHIVA_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not Shiva' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is SHIVA_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (SHIVA_SESSION_|SHIVA_UI_SESSION_ID|SHIVA_CRON_AUTO_DELIVER_|"
    "SHIVA_CRON_SESSION|SHIVA_BROWSER_CONTROL_)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``SHIVA_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!SHIVA_SESSION_*} ${!SHIVA_CRON_AUTO_DELIVER_*} "
        "${!SHIVA_BROWSER_CONTROL_*} "
        # AI_AGENT / SHIVA_AGENT are per-command attribution markers
        ...
        "AI_AGENT SHIVA_AGENT "
        f"SHIVA_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Shiva backends.
    ...
    # True only when commands execute on the SAME host as the Shiva process
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/shiva-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/shiva-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        _snap_tmp = '"$__shiva_snap_tmp"'
        snapshot_excluded = self._snapshot_excluded_passthrough_names()
        bootstrap = (
            f"umask 077\n"
            f"__shiva_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f"__shiva_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__shiva_fns\" ] && declare -f $__shiva_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_SHIVA_RUNTIME_PASSTHROUGH_{name}"
        ...
        # under Shiva via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the Shiva-specific
        # ``SHIVA_AGENT`` marker.  The value MUST equal our id in the public
        # agent-harness registry (``shiva-agent`` — see huggingface.js
        ...
        # inherited from the Shiva process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (Shiva running inside another agent's terminal).
        parts.append(
            'export AI_AGENT="${AI_AGENT:-shiva-agent}" '
            'SHIVA_AGENT="${SHIVA_AGENT:-true}"'
        )
        ...
        parts.append("__shiva_ec=$?")
        # Restrict Shiva metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__shiva_snap_tmp=$(mktemp {_snap_tmp_template}) && "
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"SHIVA_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_shiva_home() / "cache" / "terminal-output"
        ...
        # --- Debug tracing (opt-in via SHIVA_DEBUG_INTERRUPT=1) -------------
        ...
        stdin_thread = getattr(proc, "_shiva_stdin_thread", None)
        if stdin_thread is not None:
            stdin_thread.join(timeout=5)
        rendered = output.render()
        result = self._finalize_wait_result(output, rendered, proc.returncode)
        stdin_errors = getattr(proc, "_shiva_stdin_errors", None)
        if stdin_errors:
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __SHIVA_CWD_{session}__ marker from stdout output.
