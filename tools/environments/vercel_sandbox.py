...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Shiva'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``SHIVA_HOME`` and restores
...
def _ensure_vercel_sdk() -> None:
    ...
    # (vercel-internal-telemetry posts to telemetry.vercel.com). Shiva
...
def _snapshot_store_path() -> Path:
    return get_shiva_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.shiva"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_shiva = (
            "/.shiva"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.shiva"
        )
        archive_member = remote_shiva.lstrip("/")
        remote_tar = f"/tmp/.shiva_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
