#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Shiva Skills Hub.
...
Used by shiva_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _shiva_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            shiva_meta = metadata.get("shiva", {})
            if isinstance(shiva_meta, dict):
                tags = shiva_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.shiva/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "NousResearch/shiva-agent"
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``shiva update``. Rather than telling them to update first, resolve
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                shiva_meta = meta_block.get("shiva", {})
                if isinstance(shiva_meta, dict):
                    tags = shiva_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``shiva_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Shiva centralized index source
# ---------------------------------------------------------------------------

SHIVA_INDEX_URL = "https://shiva-agent.nousresearch.com/docs/api/skills-index.json"
SHIVA_INDEX_TTL = 6 * 3600  # 6 hours


def _shiva_index_cache_file() -> Path:
    return _index_cache_dir() / "shiva-index.json"


def _load_shiva_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for SHIVA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    shiva_index_cache_file = _shiva_index_cache_file()
    if shiva_index_cache_file.exists():
        try:
            age = time.time() - shiva_index_cache_file.stat().st_mtime
            if age < SHIVA_INDEX_TTL:
                return json.loads(shiva_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Shiva index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Shiva index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Shiva index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        shiva_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        shiva_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class ShivaIndexSource(SkillSource):
    """Skill source backed by the centralized Shiva Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "shiva-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "shiva-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ShivaIndexSource(auth=auth), # Centralized index (search + resolved install paths)
