import { buildShivaWebSocketUrl } from "@shiva/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/shiva/). The Python backend
// injects ``window.__SHIVA_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__SHIVA_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const SHIVA_BASE_PATH = readBasePath();
const BASE = SHIVA_BASE_PATH;
...
declare global {
  interface Window {
    __SHIVA_SESSION_TOKEN__?: string;
    __SHIVA_BASE_PATH__?: string;
    ...
    __SHIVA_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Shiva-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__SHIVA_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``shiva update``, ``shiva gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__SHIVA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__SHIVA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__SHIVA_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``shiva_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__SHIVA_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Shiva-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``shiva_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__SHIVA_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateShiva: () =>
    fetchJSON<ActionResponse>("/api/shiva/update", { method: "POST" }),
  checkShivaUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/shiva/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** shiva-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``shiva update``. */
  can_update_shiva?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the SHIVA_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the SHIVA_HOME volume — no staleness dimension, so no
 * sampled_at. */
