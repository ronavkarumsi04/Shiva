...
import { ShivaConsoleModal } from "@/components/ShivaConsoleModal";
...
export default function SystemPage() {
  ...
  // spawned `shiva import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_shiva === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkShivaUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_shiva],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_shiva === false) {
      showToast(
        "Shiva updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateShiva();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "shiva-update");
  ...
  const canUpdateShiva = status?.can_update_shiva !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateShiva && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Shiva?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'shiva update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'shiva update' (${updateInfo?.update_command ?? "shiva update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Shiva</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.shiva_version}</span>
                  {canUpdateShiva &&
                    updateInfo &&
          ...
          <CardContent className="flex flex-col gap-3 py-4">
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">shiva portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$SHIVA_HOME/backups/shiva-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Shiva backup?"
              description={`This will overwrite your current Shiva configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Shiva team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
