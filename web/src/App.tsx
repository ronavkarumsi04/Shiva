...
const SIDEBAR_COLLAPSED_KEY = "shiva-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="shiva-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="shiva-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateShiva = status?.can_update_shiva === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'shiva update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "shiva update";
    return (
      t.status.updateShivaConfirmMessage ??
      `This will run 'shiva update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateShivaConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateShiva) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateShiva,
      runningLabel: t.status.updatingShiva,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Shiva gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateShivaConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateShivaConfirmTitle ?? `${t.status.updateShiva}?`}
    />
    </>
  );
}
