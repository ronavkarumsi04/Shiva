...
interface ShivaConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function ShivaConsoleModal({ open, onClose }: ShivaConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("shiva> ");
  const inputPromptRef = useRef("shiva> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "shiva> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Shiva Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="shiva-console-title"
    >
      <div
        className={cn(
          themedBody,
