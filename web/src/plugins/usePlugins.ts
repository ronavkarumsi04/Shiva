...
import { api, SHIVA_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "shiva:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${SHIVA_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${SHIVA_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?shiva_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-shiva-plugin", manifest.name);
