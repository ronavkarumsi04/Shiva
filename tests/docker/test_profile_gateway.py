...
Phase 4 wires `shiva -p <profile> gateway start/stop` through the s6
...
def test_profile_create_then_gateway_start(
    ...
    r = _sh(container_name, f"shiva profile create {PROFILE}")
    ...
    r = _sh(container_name, f"shiva -p {PROFILE} gateway start", timeout=60)
    ...
    r = _sh(container_name, f"shiva -p {PROFILE} gateway stop", timeout=30)
    assert r.returncode == 0
