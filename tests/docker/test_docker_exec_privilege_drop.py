...
The shim (docker/shiva-exec-shim.sh, installed at /opt/shiva/bin/shiva)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> shiva login` would write /opt/data/auth.json as
...
failed: Shiva is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> shiva …`` (defaulting to root) gets dropped to the
   shiva user before the real binary runs.
2. ``docker exec --user shiva <c> shiva …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $SHIVA_HOME from a ``docker exec`` session land
   as shiva:shiva — the actual user-visible invariant.
4. The SHIVA_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-shiva-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown shiva:shiva`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to shiva:shiva,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$SHIVA_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (shiva_cli.container_boot), which s6 runs
    *strictly after* ``01-shiva-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_shiva_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `shiva` with a Python-style `-c` shim equivalent — there's no
    pure-shiva "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `shiva` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `shiva`
    do anything that writes to disk, then check the file's owner.

    Use `shiva config set` which writes config.yaml under SHIVA_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "shiva", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by shiva, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "shiva:shiva", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected shiva:shiva. "
        "The shim did not drop privileges before invoking shiva."
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/shiva/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid shiva shiva <args>` which resolves `shiva`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> shiva login`` (root) wrote
    ...
    "Shiva is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `shiva config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is shiva-readable.

    Specifically: pretend the operator ran `shiva login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the shiva UID. We use `shiva auth list` since that touches the
    ...
    # `shiva auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `shiva config set` first to
    # provoke a write into SHIVA_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "shiva", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # SHIVA_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "shiva", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the shiva user "
