...
Build the real image and verify the actual runtime behavior:
  ...
  1. PUID/PGID env vars remap the shiva user UID/GID at boot
  2. SHIVA_UID/SHIVA_GID take precedence over PUID/PGID aliases
...
def test_puid_pgid_remaps_shiva_user(
    built_image: str, container_name: str,
) -> None:
    """PUID=1000 PGID=1000 must remap the shiva user to UID 1000."""
    ...
    assert r.stdout.strip() == "1000", (
        f"expected shiva UID 1000 after PUID remap, got: {r.stdout.strip()}"
    )
    ...
    assert r.stdout.strip() == "1000", (
        f"expected shiva GID 1000 after PGID remap, got: {r.stdout.strip()}"
    )
...
def test_nas_low_uid_accepted(
    ...
    r = docker_exec_sh(container_name, "id -u shiva", timeout=10)
    assert r.stdout.strip() == "99", (
        f"expected shiva UID 99, got: {r.stdout.strip()}"
    )

    r = docker_exec_sh(container_name, "id -g shiva", timeout=10)
    assert r.stdout.strip() == "100", (
        f"expected shiva GID 100, got: {r.stdout.strip()}"
    )
...
def test_remap_enables_data_volume_writes(
    ...
    """After remap, the shiva user must be able to write to /opt/data."""
    start_container(built_image, container_name, "PUID=1000", "PGID=1000")
    ...
    assert "WRITE_OK" in r.stdout, (
        f"shiva user cannot write to /opt/data after remap: {r.stdout}"
    )