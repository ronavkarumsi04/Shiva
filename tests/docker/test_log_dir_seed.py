"""Runtime smoke test for Docker $SHIVA_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the shiva user after container boot.
...
registered later runs its log service as the dropped shiva user and
...
def test_logs_gateways_seeded_and_shiva_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by shiva after boot."""
    ...
    assert "logs=shiva" in r.stdout, (
        f"logs/ not owned by shiva: {r.stdout}"
    )
    assert "gateways=shiva" in r.stdout, (
        f"logs/gateways/ not owned by shiva: {r.stdout}"
    )
