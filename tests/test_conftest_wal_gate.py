"""The conftest WAL gate must agree with shiva_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``shiva_state``. That is deliberate: importing
``shiva_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.shiva`` before the per-test ``SHIVA_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_shiva_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and shiva_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_shiva_state_at_collection():
    """The gate must stay import-free of shiva_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.shiva, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``shiva_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "shiva_state" or name.startswith("shiva_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported shiva_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.shiva during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("shiva_state", None)
