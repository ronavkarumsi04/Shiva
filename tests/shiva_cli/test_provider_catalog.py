"""Tests for the unified provider catalog (shiva_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``shiva model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from shiva_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
