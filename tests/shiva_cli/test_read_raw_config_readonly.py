...
@pytest.fixture()
def isolated_shiva_home():
    """Per-test SHIVA_HOME dir (already redirected by the autouse conftest
...
def test_freshness_after_config_edit(isolated_shiva_home):
    from shiva_cli.config import read_raw_config_readonly

    cfg = _write_config(isolated_shiva_home, {"display": {"ephemeral_system_ttl": 1}})
    first = read_raw_config_readonly()
    assert first["display"]["ephemeral_system_ttl"] == 1

    _write_config(isolated_shiva_home, {"display": {"ephemeral_system_ttl": 7}})
...
def test_missing_config_returns_empty(isolated_shiva_home):
    from shiva_cli.config import read_raw_config_readonly

    cfg = isolated_shiva_home / "config.yaml"
    if cfg.exists():
        cfg.unlink()
