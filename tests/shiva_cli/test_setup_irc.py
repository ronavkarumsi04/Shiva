"""Tests for IRC gateway configuration via `shiva setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Shiva install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Shiva install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#shiva")
            monkeypatch.setenv("IRC_NICKNAME", "shiva-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `shiva setup gateway` experience with IRC present."""
