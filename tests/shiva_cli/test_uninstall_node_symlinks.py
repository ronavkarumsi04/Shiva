"""Tests for shiva_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $SHIVA_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Shiva-managed node dir.
"""
...
def _make_shiva_node(shiva_home: Path) -> Path:
    """Create a fake $SHIVA_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = shiva_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    shiva_home = fake_home / ".shiva"
    _make_shiva_node(shiva_home)
    ...
    removed = uninstall.remove_node_symlinks(shiva_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    shiva_home = fake_home / ".shiva"
    node_bin = _make_shiva_node(shiva_home)
