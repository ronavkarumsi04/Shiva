"""Tests for shiva_cli.update_receipt — Phase 1 of the fleet-update plan (#91277).
...
def receipt_home(tmp_path, monkeypatch):
    """Isolated SHIVA_HOME for receipt writes."""
    home = tmp_path / ".shiva"
    home.mkdir()
    monkeypatch.setattr(
        "shiva_cli.config.get_shiva_home", lambda: home, raising=False
    )
...
class TestReceiptLifecycle:
    def test_begin_record_finalize_roundtrip(self, receipt_home):
        ...
        ur.record_gateway_restart(
            restarted_services=["shiva-gateway"],
        ...
        assert gr["restarted_services"] == ["shiva-gateway"]
...
class TestCommandBoundaryFinalization:
    ...
    def test_pending_receipt_persisted_on_exit_2_refusal(self, receipt_home):
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "another shiva.exe running")
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        def _fake_impl(args, gateway_mode):
            ur.begin_update_receipt()
            ur.record_step("windows_preflight", False, "shiva.exe holds venv")
            sys.exit(2)

        monkeypatch.setattr(shiva_main, "_cmd_update_impl", _fake_impl)
        monkeypatch.setattr(
            shiva_main, "detect_install_method", lambda *a, **k: "git", raising=False
        )
        monkeypatch.setattr(
            shiva_main,
        ...
        monkeypatch.setattr(
            shiva_main, "_finalize_update_output", lambda state: None, raising=False
        )
        ...
        import shiva_cli.update_lock as update_lock_mod
...
class TestFleetClassification:
    def _fleet_with(self, monkeypatch, tmp_path, record, expected_sha="a" * 40):
        ...
        monkeypatch.setattr(
            "shiva_cli.profiles._get_profiles_root",
    ...
    def test_matrix_returns_true_only_on_stale(self, capsys):
        ...
        assert "STALE" in out
        assert "shiva -p <profile> gateway restart" in out
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("shiva_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
