"""Tests for ``shiva doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from shiva_cli import doctor_live
from shiva_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `shiva doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("shiva_cli.doctor.SHIVA_HOME", tmp_path / "home")
        monkeypatch.setattr("shiva_cli.doctor.PROJECT_ROOT", tmp_path / "root")
