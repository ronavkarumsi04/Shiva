"""Regression tests for #34107 — Docker UID/GID handling in ensure_shiva_home.

When Shiva runs in Docker with ``SHIVA_UID=1000`` / ``SHIVA_GID=911``,
the entrypoint chowns the top-level ``SHIVA_HOME`` once at startup. But
...
The fix is a ``_chown_to_shiva_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_shiva_uid_gid
# ---------------------------------------------------------------------------


class TestResolveShivaUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("SHIVA_UID", "1000")
        monkeypatch.setenv("SHIVA_GID", "911")
        from shiva_cli.config import _resolve_shiva_uid_gid
...
class TestChownToShivaUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_shiva_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SHIVA_UID", raising=False)
        monkeypatch.delenv("SHIVA_GID", raising=False)
        from shiva_cli import config as cfg
