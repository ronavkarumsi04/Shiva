"""``shiva update`` skips the editable reinstall when the pull can't affect it.
...
that rewrite is the only reason the running ``shiva.exe`` gets quarantined,
...
from shiva_cli.update_cmd import _editable_install_is_current
