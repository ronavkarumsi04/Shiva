"""Tests for the Linux XDG desktop entry installed by ``shiva desktop``."""
...
from shiva_cli import linux_desktop_entry as lde
...
def test_install_writes_entry_with_absolute_exec_and_icon(tmp_path, xdg_home, monkeypatch):
    ...
    shiva_bin = tmp_path / "bin" / "shiva"
    shiva_bin.parent.mkdir()
    shiva_bin.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "shiva_cli.relaunch.resolve_shiva_bin", lambda: str(shiva_bin)
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)

    assert entry == xdg_home / "applications" / "shiva.desktop"
    values = _parse(entry.read_text(encoding="utf-8"))

    # Exec must be the absolute path of the resolved binary. The launcher
    # runs with a minimal PATH, so a bare `shiva` would not resolve.
    assert values["Exec"] == f"{shiva_bin} desktop"
    ...
    assert values["Name"] == "Shiva"
    assert values["Terminal"] == "false"


def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("shiva_cli.relaunch.resolve_shiva_bin", lambda: "/usr/bin/shiva")
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_falls_back_to_interpreter_module(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("shiva_cli.relaunch.resolve_shiva_bin", lambda: None)
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)
    exec_line = _parse(entry.read_text(encoding="utf-8"))["Exec"]

    assert exec_line.endswith("-m shiva_cli.main desktop")
    assert Path(exec_line.split(" ")[0]).is_absolute()


# #90292: the shell installer's bash wrapper makes argv[0] the repo `shiva`
...
def test_exec_prefixes_interpreter_for_env_shebang_python_script(tmp_path, xdg_home, monkeypatch):
    ...
    shiva_bin = tmp_path / "bin" / "shiva"
    shiva_bin.parent.mkdir()
    shiva_bin.write_text("#!/usr/bin/env python3\nimport shiva_cli\n", encoding="utf-8")
    shiva_bin.chmod(0o755)
    monkeypatch.setattr("shiva_cli.relaunch.resolve_shiva_bin", lambda: str(shiva_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
    ...
    assert str(shiva_bin) in exec_line
...
def test_exec_leaves_shell_wrapper_launchers_alone(tmp_path, xdg_home, monkeypatch):
    ...
    shiva_bin = tmp_path / "bin" / "shiva"
    shiva_bin.parent.mkdir()
    shiva_bin.write_text('#!/bin/bash\nexec /opt/shiva/venv/bin/python "$@"\n', encoding="utf-8")
    shiva_bin.chmod(0o755)
    monkeypatch.setattr("shiva_cli.relaunch.resolve_shiva_bin", lambda: str(shiva_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
    ...
    assert exec_line == f"{shiva_bin} desktop"
...
def test_exec_leaves_venv_shebang_scripts_alone(tmp_path, xdg_home, monkeypatch):
    ...
    shiva_bin.parent.mkdir()
    interpreter = str(Path(sys.executable).resolve())
    shiva_bin.write_text(f"#!{interpreter}\nimport shiva_cli\n", encoding="utf-8")
    shiva_bin.chmod(0o755)
    monkeypatch.setattr("shiva_cli.relaunch.resolve_shiva_bin", lambda: str(shiva_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_install_without_source_icon_uses_themed_name(tmp_path, xdg_home, monkeypatch):
    ...
    # when Shiva is installed some other way.
    assert _parse(entry.read_text(encoding="utf-8"))["Icon"] == "shiva"
...
def test_exec_arg_quoting_handles_spaces(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    spaced = tmp_path / "my apps" / "shiva"
    spaced.parent.mkdir()
    spaced.write_text("", encoding="utf-8")
    monkeypatch.setattr("shiva_cli.relaunch.resolve_shiva_bin", lambda: str(spaced))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
