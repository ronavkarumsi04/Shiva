"""Regression for #41403 — ``shiva update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.shiva.gateway-<profile>`` services kept running pre-update
...
from shiva_cli.update_cmd import (
...
class TestLaunchdGatewayLabelsForInstall:
    def test_labels_derive_from_this_installs_profiles(self, monkeypatch):
        ...
        the shared per-user LaunchAgents dir. A sandboxed SHIVA_HOME (tests,
        ...
        assert launchd_gateway_labels_for_install() == [
            "ai.shiva.gateway",
            "ai.shiva.gateway-merit-ops",
            "ai.shiva.gateway-tfl-wiki",
        ]

    def test_no_profiles_means_no_fleet(self, monkeypatch):
        monkeypatch.setattr(shiva_cli.profiles, "list_profiles", lambda: [])
        assert launchd_gateway_labels_for_install() == []
...
class TestLocateLaunchdGatewayService:
    def test_domains_resolve_per_label_not_from_cache(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.shiva.gateway-a") == (
            f"gui/{UID}",
            4242,
        )
        assert _locate_launchd_gateway_service("ai.shiva.gateway-b") == (
            f"user/{UID}",
            4242,
        )
    ...
    def test_loaded_without_live_process(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.shiva.gateway-x") == (
            f"gui/{UID}",
            None,
        )
    ...
    def test_not_loaded_in_either_domain(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.shiva.gateway-x") == (None, None)
    ...
    def test_timeout_propagates_to_caller(self, monkeypatch):
        ...
        with pytest.raises(subprocess.TimeoutExpired):
            _locate_launchd_gateway_service("ai.shiva.gateway-x")
...
class TestProbeLaunchdDomainForLabel:
    def test_unloaded_label_falls_back_to_managername(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.shiva.gateway-x") == f"gui/{UID}"
    ...
    def test_unloaded_label_defaults_to_user_domain(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.shiva.gateway-x") == f"user/{UID}"
...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        monkeypatch.setattr(gw, "get_launchd_label", lambda: "ai.shiva.gateway")
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
            lambda: ["ai.shiva.gateway", "ai.shiva.gateway-a", "ai.shiva.gateway-b"],
        )
        located = {
            "ai.shiva.gateway": (f"gui/{UID}", 100),
            "ai.shiva.gateway-a": (f"gui/{UID}", 200),
            "ai.shiva.gateway-b": (None, None),  # not bootstrapped
        }
        monkeypatch.setattr(
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through shiva_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.shiva.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.shiva.gateway", current, "ai.shiva.gateway-user-scoped"],
            located={
                "ai.shiva.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.shiva.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.shiva.gateway",
            f"user/{UID}/ai.shiva.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.shiva.gateway",
            f"user/{UID}/ai.shiva.gateway-user-scoped",
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.shiva.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.shiva.gateway-a"],
            located={"ai.shiva.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.shiva.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.shiva.gateway",
            labels=["ai.shiva.gateway", "ai.shiva.gateway-idle"],
            located={
                "ai.shiva.gateway": (f"gui/{UID}", 100),
                "ai.shiva.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.shiva.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.shiva.gateway",
            labels=["ai.shiva.gateway", "ai.shiva.gateway-dormant"],
            located={
                "ai.shiva.gateway": (f"gui/{UID}", 100),
                "ai.shiva.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.shiva.gateway-dormant"]
        assert restarted == ["ai.shiva.gateway", "ai.shiva.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.shiva.gateway",
            labels=["ai.shiva.gateway", "ai.shiva.gateway-a"],
            located={
                "ai.shiva.gateway": (f"gui/{UID}", 100),
                "ai.shiva.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.shiva.gateway-a"]
        assert restarted == ["ai.shiva.gateway", "ai.shiva.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.shiva.gateway",
                "ai.shiva.gateway-bad",
                "ai.shiva.gateway-good",
            ],
            located={
                "ai.shiva.gateway": (f"gui/{UID}", 100),
                "ai.shiva.gateway-bad": (f"gui/{UID}", 200),
                "ai.shiva.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.shiva.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.shiva.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.shiva.gateway-good"]
        assert restarted == ["ai.shiva.gateway", "ai.shiva.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.shiva.gateway",
                "ai.shiva.gateway-wedged",
                "ai.shiva.gateway-after",
            ],
            located={
                "ai.shiva.gateway": (f"gui/{UID}", 100),
                "ai.shiva.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.shiva.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.shiva.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.shiva.gateway-after"]
        assert restarted == ["ai.shiva.gateway", "ai.shiva.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.shiva.gateway": (f"gui/{UID}", 100),
                "ai.shiva.gateway-wedged": (f"gui/{UID}", 200),
                "ai.shiva.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.shiva.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.shiva.gateway",
            labels=["ai.shiva.gateway", "ai.shiva.gateway-zombie"],
            located={
                "ai.shiva.gateway": (f"gui/{UID}", 100),
                "ai.shiva.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.shiva.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.shiva.gateway"]
        assert failed == ["ai.shiva.gateway-zombie"]
...
class TestWaitForLaunchdServicePid:
    def test_returns_true_once_pid_changes(self, monkeypatch):
        ...
        assert gw._wait_for_launchd_service_pid(
            "ai.shiva.gateway-x", old_pid=200, timeout=5.0, domain=f"gui/{UID}"
        )
