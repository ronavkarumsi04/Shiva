"""Regression test: `shiva dashboard --tui` must not hard-crash.

Older Shiva desktop app shells (<= 0.15.x) spawn the backend as::

    shiva dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Shiva couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real shiva_cli.main parser in a subprocess.
