...
def test_env_var_names_the_managing_system(shiva_home, monkeypatch, env_value, expected):
    if env_value is None:
        monkeypatch.delenv("SHIVA_MANAGED", raising=False)
    else:
        monkeypatch.setenv("SHIVA_MANAGED", env_value)
...
def test_marker_file_names_the_managing_system(
    shiva_home, monkeypatch, marker_text, expected
):
    """An interactive shell reads .managed, not the SHIVA_MANAGED of the service."""
    (shiva_home / ".managed").write_text(marker_text, encoding="utf-8")
    monkeypatch.delenv("SHIVA_MANAGED", raising=False)
...
def test_env_var_wins_over_the_marker(shiva_home, monkeypatch):
    (shiva_home / ".managed").write_text("nixos", encoding="utf-8")
    monkeypatch.setenv("SHIVA_MANAGED", "home-manager")
...
def test_managed_install_names_its_system_and_offers_an_update(
    shiva_home, monkeypatch, tmp_path, managed_value
):
    """The message names the system, so the user knows what owns the install."""
    monkeypatch.setenv("SHIVA_MANAGED", managed_value)
    ...
    # `shiva update` cannot run on a managed install, so the advice must not
    # name it.
    assert config_mod.recommended_update_command() != "shiva update"
...
def test_unmanaged_install_offers_no_update_command(shiva_home, monkeypatch):
    monkeypatch.delenv("SHIVA_MANAGED", raising=False)
    assert config_mod.get_managed_update_command() is None


def test_unreadable_marker_still_reports_managed(shiva_home, monkeypatch):
    """A marker we cannot read is still a marker. Fail closed, not open."""
    marker = shiva_home / ".managed"
