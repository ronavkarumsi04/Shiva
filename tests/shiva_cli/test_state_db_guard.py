...
from shiva_cli.backup import copy_db_and_verify, verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    SHIVA_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def shiva_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("shiva_cli.config") or mod == "shiva_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, shiva_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, shiva_home, capsys, monkeypatch):
