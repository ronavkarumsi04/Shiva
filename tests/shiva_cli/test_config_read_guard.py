...
Canonical owners:
  ...
  * ``shiva_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._shiva_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/shiva/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "shiva_cli/managed_scope.py",
