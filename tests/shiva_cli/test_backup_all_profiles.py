...
import shiva_cli.backup as backup
...
def profiles(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "shiva_cli.profiles._get_default_shiva_home", lambda: default_home
    )
    monkeypatch.setattr(
        "shiva_cli.profiles._get_profiles_root", lambda: tmp_path / "home" / "profiles"
    )
    monkeypatch.setattr(
        "shiva_cli.profiles._PROFILE_ID_RE",
...
class TestSiblingEnumeration:
    ...
    def test_never_raises(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("shiva_cli.profiles._get_default_shiva_home", _boom)
...
class TestAllProfileSnapshots:
    ...
    def test_one_failing_sibling_does_not_block_others(self, profiles, monkeypatch):
        ...
        def _flaky(label=None, shiva_home=None, keep=None, max_file_size=None):
            if shiva_home == profiles["work"]:
                raise OSError("disk full")
            return real(
                label=label, shiva_home=shiva_home, keep=keep,
