...
Covers:
 - Normal path (tokens already in Shiva auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Shiva
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `shiva auth openai-codex`. The old
...
@pytest.fixture()
def shiva_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Shiva auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(shiva_auth_only_env):
    """openai-codex appears when tokens are already in Shiva auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Shiva
    auth store."""
    shiva_home = tmp_path / ".shiva"
    shiva_home.mkdir()
