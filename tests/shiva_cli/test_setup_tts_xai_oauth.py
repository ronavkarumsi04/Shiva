...
def test_run_xai_oauth_login_from_setup_does_not_hijack_active_provider(
    ...
    `active_provider`), so `shiva setup tts` OAuth login hijacked the main
    chat provider.
    """
    ...
    monkeypatch.setattr("shiva_cli.auth._is_remote_session", lambda: True)

    from shiva_cli.auth import is_source_suppressed, suppress_credential_source
    from shiva_cli.setup import _run_xai_oauth_login_from_setup
