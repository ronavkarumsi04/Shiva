"""ensure_shiva_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".shiva"
    second = tmp_path / "b" / ".shiva"

    monkeypatch.setenv("SHIVA_HOME", str(first))
    cfg.ensure_shiva_home()

    # Profile switch: SHIVA_HOME moves → the new path is ensured too.
    monkeypatch.setenv("SHIVA_HOME", str(second))
    cfg.ensure_shiva_home()
