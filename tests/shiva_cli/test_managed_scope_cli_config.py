...
shiva_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `shiva config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_shiva_home = get_shiva_home()`` at import time (module
    singleton), so monkeypatching SHIVA_HOME after import doesn't move it.
    ...
    this only matters for tests that swap SHIVA_HOME.)
    """
    import cli

    cli._shiva_home = home
