"""Tests for the post-pull HEAD-movement gate in ``shiva update``.
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the shiva_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns shiva_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/shiva_cli/test_cmd_update.py).
    """
    ...
    monkeypatch.setattr(
        shiva_main, "_resolve_update_branch", lambda args: "main"
    )
    monkeypatch.setattr(shiva_main, "_is_windows", lambda: False)
    monkeypatch.setattr(
        shiva_main, "_get_origin_url",
    ...
    monkeypatch.setattr(
        shiva_main, "_run_pre_update_backup", lambda *a, **k: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(shiva_main, "_finish_dashboard_update_cleanup", lambda *a: None)
    ...
    monkeypatch.setattr(
        shiva_gateway, "supports_systemd_services", lambda: False
    )
    monkeypatch.setattr(
...
def test_update_success_when_head_moves(monkeypatch, tmp_path, capsys):
    ...
    shiva_main.cmd_update(args)  # completes normally (no SystemExit)
