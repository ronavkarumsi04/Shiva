"""Tests for shiva_cli configuration management."""
...
class TestGetShivaHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SHIVA_HOME", None)
            home = get_shiva_home()
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\shiva — see
                # shiva_constants._get_platform_default_shiva_home.
                ...
                assert home == base / "shiva"
            else:
                assert home == Path.home() / ".shiva"


class TestEnsureShivaHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from shiva_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``shiva logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from shiva_cli import config as cfg_mod
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from shiva_cli.config import _deep_merge
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SHIVA_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SHIVA_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SHIVA_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SHIVA_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("shiva_cli.config.sanitize_env_file", return_value=2),
            patch(
                "shiva_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("shiva_cli.config.read_raw_config", return_value={}),
            patch("shiva_cli.config.get_missing_env_vars", return_value=[]),
            patch("shiva_cli.config.get_missing_config_fields", return_value=[]),
            patch("shiva_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from shiva_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """SHIVA_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `shiva setup`) lets a
        ...
        via config.yaml; SHIVA_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from shiva_cli.config import OPTIONAL_ENV_VARS
        assert "SHIVA_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `shiva setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `shiva setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_shiva_home(self):
        from shiva_cli.config_migrations import support_floor_message
        from shiva_constants import display_shiva_home

        msg = support_floor_message()
        assert f"{display_shiva_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from shiva_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Shiva runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Shiva
    process on next startup via the dotenv → ``os.environ`` chain in
    ``shiva_cli/env_loader.py``.
    ...
    def _shiva_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "SHIVA_SPOTIFY_CLIENT_ID",
            "SHIVA_QWEN_BASE_URL",
            "SHIVA_MAX_ITERATIONS",
        ],
    )
    def test_shiva_integration_keys_still_writable(self, allowed_key):
        """``SHIVA_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "SHIVA_CRON_SESSION",
            "SHIVA_SINGLE_QUERY_SESSION",
            "SHIVA_SESSION_KEY",
            "SHIVA_SESSION_PLATFORM",
        ],
    )
    def test_shiva_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        from shiva_cli.config import invalidate_env_cache

        catalog = tmp_path / "custom-mcp-catalog"
        (tmp_path / ".env").write_text(
            f"SHIVA_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["SHIVA_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Shiva_Yolo_Mode", "SHIVA_YOLO_MODE"),
            ("Shiva_Optional_Mcps", "SHIVA_OPTIONAL_MCPS"),
            ("Shiva_Copilot_Acp_Command", "SHIVA_COPILOT_ACP_COMMAND"),
            ("Shiva_Copilot_Acp_Args", "SHIVA_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from shiva_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from shiva_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from shiva_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"SHIVA_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: shiva\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "shiva"
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import shiva_cli.config as cfg_mod
...
class TestConfigCommandFailClosedSurface:
    """`shiva config set/unset` must exit cleanly (no traceback) when the
