...
This rewrites files Shiva does not own (``~/.bashrc``, ``~/.zshrc``, ...) and
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point both ``Path.home()`` and ``SHIVA_HOME`` at a throwaway dir."""
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))
    monkeypatch.setenv("SHIVA_HOME", str(home / ".shiva"))
    return home


class TestHappyPath:
    def test_shiva_path_block_is_removed(self, fake_home: Path):
        rc = fake_home / ".zshrc"
        ...
        assert "# Shiva Agent" not in text
...
class TestCrashDurability:
    def test_shell_config_survives_an_interrupted_rewrite(self, fake_home: Path):
        ...
        # Path.home()/SHIVA_HOME patches the fake_home fixture installed.
    ...
    def test_symlinked_shell_config_stays_a_symlink(self, fake_home: Path):
        ...
        assert "# Shiva Agent" not in real.read_text(encoding="utf-8")
