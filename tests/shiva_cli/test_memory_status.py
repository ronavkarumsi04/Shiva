"""Tests for `shiva memory status` CLI command.
...
def _run_cmd_status(capfd, mem_config=None, memory_tools=None):
    ...
    """
    from shiva_cli.memory_setup import cmd_status

    config = {"memory": mem_config or {}}
    if memory_tools is None:
        memory_tools = {"memory"}

    with patch("shiva_cli.config.load_config", return_value=config):
        with patch("shiva_cli.memory_setup._get_available_providers", return_value=[]):
            with patch(
