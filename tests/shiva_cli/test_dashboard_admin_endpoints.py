...
def _client():
    ...
    # Keep the state DB under the isolated SHIVA_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_shiva_home):
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        ...
        env_text = (shiva_home / ".env").read_text()
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.shiva/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `shiva auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `shiva auth add`, which clears every suppression for the
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_shiva_home):
        ...
        (get_shiva_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_shiva_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_shiva_action", fake_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "shiva_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("shiva-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # shiva-index source advertises availability + featured search.
            idx = _Src("shiva-index")
        ...
        assert {"official", "github", "shiva-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "shiva_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/shiva/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/shiva/update`` runs ``shiva update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import shiva_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/shiva/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/shiva/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        ...
        monkeypatch.setattr("shiva_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `shiva tools` configurator."""
...
# ---------------------------------------------------------------------------
# _spawn_shiva_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_shiva_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _SHIVA_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    # Isolate the module-global proc registry: _spawn_shiva_action stores
    ...
    ws._spawn_shiva_action(["gateway", "restart"], "gateway-restart")

    assert "_SHIVA_GATEWAY" not in captured["env"]
    assert captured["env"]["SHIVA_NONINTERACTIVE"] == "1"
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    monkeypatch, _isolate_shiva_home
):
    ...
    startup under SHIVA_DESKTOP=1 so the stale orphan is cleared first.
    """
    import shiva_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from shiva_cli.gateway, so patch it
    # on that module.
    import shiva_cli.gateway as g
