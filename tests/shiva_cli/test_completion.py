"""Tests for shiva_cli/completion.py — shell completion script generation."""
...
from shiva_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real shiva structure."""
    p = argparse.ArgumentParser(prog="shiva")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_shiva_completion()" in out
        assert "complete -F _shiva_completion shiva" in out
...
class TestGenerateZsh:
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_shiva_profiles'" in out
...
class TestProfileCompletion:
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'shiva profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _shiva_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_shiva_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _shiva_profiles"
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__shiva_profiles" in out
        count = out.count("(__shiva_profiles)")
