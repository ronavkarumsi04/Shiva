"""``shiva`` must survive git operations on the checkout (launcher layout).

The Windows ``shiva`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``SHIVA_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``shiva-agent\\bin``) was swept by ``shiva update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``shiva`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``shiva update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``shiva_constants.venv_bin_dir``), so these
...
from shiva_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: SHIVA_HOME/shiva-agent/venv/Scripts + launchers."""
    home = tmp_path / "shiva"
    root = home / "shiva-agent"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under SHIVA_HOME gains nothing anywhere."""
    home = tmp_path / "shiva-home"
    home.mkdir()
    monkeypatch.setenv("SHIVA_HOME", str(home))
    root = tmp_path / "src" / "shiva-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``shiva -p <name>`` SHIVA_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("SHIVA_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `shiva update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "shiva.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "shiva.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\shiva\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/SHIVA/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``shiva update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/shiva.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/shiva.exe is not gitignored — shiva update's autostash "
