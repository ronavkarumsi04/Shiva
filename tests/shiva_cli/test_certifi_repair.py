...
1. The venv-repair import probes (early recovery + `shiva update`) must
...
2. ``shiva doctor`` must fail the certificate check in that state, and
   ``shiva doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `shiva update`'s venv repair must apply
    ...
    def _run_probe_script(self, monkeypatch, tmp_path, bundle_path):
        ...
        from shiva_cli import main as main_mod
...
# =========================================================================
# 2. shiva doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "shiva doctor --fix" in message
