"""Tests for the top-level `./shiva` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./shiva` should use `shiva_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "shiva"
    called = []

    fake_main_module = types.ModuleType("shiva_cli.main")

    def fake_main():
        called.append("shiva_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "shiva_cli.main", fake_main_module)
