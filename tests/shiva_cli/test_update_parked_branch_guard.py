"""Regression tests for the parked-branch guard in ``shiva update``.
...
memory-limit``, days behind main) left there by earlier tooling. ``shiva
...
def _no_config(monkeypatch):
    ...
    import shiva_cli.config as shiva_config

    monkeypatch.setattr(shiva_config, "load_config", lambda: {})
...
def test_skip_warning_names_branch_behind_count_and_commands(repo_pair, capsys):
    ...
    assert f"git -C {repo_pair} checkout main && shiva update" in out
...
def test_print_update_completion_carries_branch_and_sha(
    ...
    monkeypatch.setattr(shiva_main, "PROJECT_ROOT", repo_pair)
...
def _patch_update_flow(monkeypatch, repo, run_real_git=True):
    ...
    """
    monkeypatch.setattr(shiva_main, "PROJECT_ROOT", repo)
    ...
    monkeypatch.setattr(shiva_main, "_discard_lockfile_churn", lambda *a, **k: None)
    ...
    monkeypatch.setattr(shiva_main, "_record_bytecode_fingerprint", lambda *a, **k: None)
    monkeypatch.setattr(shiva_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(shiva_main, "_pause_windows_gateways_for_update", lambda: None)
    monkeypatch.setattr(
        shiva_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(shiva_main, "_capture_active_lazy_features", lambda: [])
    monkeypatch.setattr(shiva_main, "_capture_active_tool_dependencies", lambda: [])
