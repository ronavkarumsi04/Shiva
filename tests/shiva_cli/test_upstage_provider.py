...
Regression guard for the bug where `shiva model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`shiva_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in SHIVA_OVERLAYS
        overlay = SHIVA_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `shiva_cli/main.py` (cmd_model): read
