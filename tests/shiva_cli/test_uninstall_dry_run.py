...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_shiva_home", lambda: shiva_home)
    monkeypatch.setattr(uninstall, "_is_default_shiva_home", lambda home: False)
    ...
    assert str(shiva_home) in output
    assert project_root.exists()
    assert shiva_home.exists()
