...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "shiva_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # SHIVA_HOME.  For a standard install (SHIVA_HOME unset) that root is
        # the platform-native default (~/.shiva), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from shiva_constants import get_default_shiva_root
        assert env.get("SHIVA_HOME") == str(get_default_shiva_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (SHIVA_DESKTOP=1) must NOT
