...
def _write_ledger(entries: list[dict]) -> None:
    from shiva_cli import process_identity as pid_mod
...
def test_live_supervised_serve_suppresses_cold_start(sleeper, monkeypatch, tmp_path):
    from shiva_cli import gateway as shiva_gateway
    ...
    monkeypatch.setattr(shiva_gateway, "find_gateway_pids", lambda **_k: [])
...
def test_holder_scan_fallback_respects_token_classifier(sleeper, monkeypatch, tmp_path):
    ...
    serve_like = sleeper("-m", "shiva_cli.main", "serve")
    # Lookalike from the #90778 class — must NOT confer ownership.
    kanban_like = sleeper("-m", "shiva_cli.main", "kanban", "--preserve-cache")
    ...
    monkeypatch.setattr(
        "shiva_cli.main._detect_venv_python_processes", fake_holders
    )
    ...
    serve_like.wait()
    monkeypatch.setattr(
        "shiva_cli.main._detect_venv_python_processes",
