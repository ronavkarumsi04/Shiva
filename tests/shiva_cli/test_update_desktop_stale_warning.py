...
#88251: ``shiva update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "shiva_constants.with_shiva_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "shiva_constants.display_shiva_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_shiva_home", lambda: tmp_path)
