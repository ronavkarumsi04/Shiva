"""Regression for #74973 — `shiva update` must not leave the gateway down.
...
cron stayed dark until someone manually ran ``shiva gateway restart``.
...
def launchd(monkeypatch):
    """Stub shiva_cli.gateway so no real launchctl call is made."""
    calls: list[str] = []
    ...
    monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.shiva.gateway", raising=False)
...
class TestLaunchdRestartAfterUpdate:
    def test_plist_present_always_restarts_without_classifying(self, launchd, capsys):
        ...
        assert update_cmd._restart_launchd_gateway_after_update(supervision_verify=False) == (["ai.shiva.gateway"], [])
    ...
    def test_restart_failure_warns_that_gateway_is_down(self, launchd, capsys):
        ...
        assert "shiva gateway restart" in out
...
ai.shiva.gateway = {
\tactive count = 1
\tpath = /Users/u/Library/LaunchAgents/ai.shiva.gateway.plist
\tstate = running
\tpid = 59038
\tprogram = /Users/u/.shiva/bin/shiva
}
"""
...
class TestServicePidSweepExclusion:
    ...
    def macos_launchd(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.shiva.gateway")
...
class TestParseLaunchdPidFromPrintOutput:
    def test_running_service(self):
        from shiva_cli.gateway import _parse_launchd_pid_from_print_output
