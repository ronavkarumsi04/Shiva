...
SHIVA_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
@pytest.fixture
def shiva_home(monkeypatch, tmp_path):
    """Fresh SHIVA_HOME with .env + auth.json + config.yaml fixtures."""
...
def test_delete_clears_provider_models_cache(shiva_home):
    _write_env(shiva_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(shiva_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = shiva_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(shiva_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(shiva_home, OPENAI_API_KEY=old)
    _write_config(
        shiva_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = shiva_home.joinpath("config.yaml").read_text(encoding="utf-8")
