"""Tests for shiva_cli.mcp_catalog and shiva_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("SHIVA_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_shiva_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp SHIVA_HOME."""
    hh = tmp_path / "shiva-home"
    hh.mkdir()
    monkeypatch.setenv("SHIVA_HOME", str(hh))
    monkeypatch.setattr(
        "shiva_cli.config.get_shiva_home", lambda: hh
    )
    monkeypatch.setattr(
        "shiva_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "shiva_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_shiva_home() lazily through shiva_constants
    monkeypatch.setattr(
        "shiva_constants.get_shiva_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from shiva_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from shiva_cli.mcp_catalog import list_catalog
    ...
    def test_suggest_empty_block_rejected(self, catalog_dir):
        ...
        from shiva_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_http_api_key_builds_bearer_headers_template(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from shiva_cli.mcp_catalog import _build_server_config
    ...
    def test_http_api_key_requires_matching_env_declaration(self, catalog_dir):
        ...
        from shiva_cli.mcp_catalog import CatalogError, _parse_manifest
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from shiva_cli.mcp_catalog import install_entry
    ...
    def test_install_rejects_exfil_shaped_stdio_manifest(self, catalog_dir):
        ...
        from shiva_cli.config import load_config
        from shiva_cli.mcp_catalog import CatalogError, install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from shiva_cli.mcp_catalog import install_entry
        from shiva_cli.config import get_env_value, load_config
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from shiva_cli.mcp_catalog import install_entry, uninstall_entry
        from shiva_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from shiva_cli.mcp_catalog import uninstall_entry
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from shiva_cli import mcp_catalog
        from shiva_cli.mcp_catalog import _do_git_install
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "shiva_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no SHIVA_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("SHIVA_OPTIONAL_MCPS", raising=False)
        from shiva_cli.mcp_catalog import _catalog_root, _parse_manifest
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything Shiva fetches/launches is pinned
        to an exact version.
