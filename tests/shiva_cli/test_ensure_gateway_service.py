"""Tests for shiva_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `shiva setup` and `shiva import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("shiva_constants.is_container", lambda: container)
