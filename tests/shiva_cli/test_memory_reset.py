"""Tests for the `shiva memory reset` CLI command.
...
- Profile-scoped reset (uses SHIVA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SHIVA_HOME with memory files."""
    shiva_home = tmp_path / ".shiva"
    memories = shiva_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("SHIVA_HOME", str(shiva_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nShiva repo is at ~/.shiva/shiva-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `shiva memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        shiva_home = tmp_path / ".shiva"
        (shiva_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("SHIVA_HOME", str(shiva_home))
