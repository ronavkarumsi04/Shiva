...
def test_config_get_unset_subcommands_parse():
    """`shiva config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `shiva login` fails gracefully, not with argparse error ────
#
# `shiva login` is a removed command; its handler (`login_command` in
# `shiva_cli/auth.py`) prints a deprecation notice pointing at `shiva auth` /
# `shiva model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `shiva --help`.
