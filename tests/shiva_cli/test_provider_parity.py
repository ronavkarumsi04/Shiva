...
the SAME provider universe as ``shiva model`` (the CLI/TUI picker).
...
def test_every_shiva_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `shiva model` universe."""
    ...
    assert not missing, (
        "providers shown in `shiva model` but not configurable in the desktop "
