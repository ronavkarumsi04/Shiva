...
``shiva*.exe`` shims aside (``shiva.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``shiva`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("shiva", "shiva-agent", "shiva-acp", "shiva-gateway"):
...
# ---------------------------------------------------------------------------
# shiva_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "shiva.exe" in names, "shiva.exe must be restored after a no-op install"
    assert "shiva-acp.exe" in names
    assert "shiva-gateway.exe" in names
    assert (scripts / "shiva.exe").read_bytes() == b"MZ-old-shiva"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "shiva.exe").read_bytes() == b"MZ-new-shiva"
...
# ---------------------------------------------------------------------------
# shiva_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "shiva.exe" in names, "shiva.exe must be restored after a no-op recovery install"
    assert (scripts / "shiva.exe").read_bytes() == b"MZ-old-shiva"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "shiva.exe").write_bytes(b"MZ-new-shiva")
