...
When ``SHIVA_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.shiva/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.shiva            (has its own auth.json fixture)
    * Profile     -> tmp_path/.shiva/profiles/coder   (active, SHIVA_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Shiva is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (SHIVA_HOME != ~/.shiva, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    shiva_home = tmp_path / "classic"
    shiva_home.mkdir()
