"""Surfacing tests — managed scope shown in `config show` and `shiva doctor`."""
import pytest
...
def test_config_show_no_managed_scope_silent(tmp_path, monkeypatch, capsys):
    ...
    home.mkdir()
    monkeypatch.setenv("SHIVA_HOME", str(home))
    monkeypatch.setenv("SHIVA_MANAGED_DIR", str(tmp_path / "nope"))
    (home / "config.yaml").write_text("model:\n  default: user/model\n", encoding="utf-8")
    import shiva_cli.config as cfg
    from shiva_cli import managed_scope
...
def test_doctor_silent_with_no_managed_scope(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("SHIVA_MANAGED_DIR", str(tmp_path / "nope"))
    from shiva_cli import managed_scope, doctor
