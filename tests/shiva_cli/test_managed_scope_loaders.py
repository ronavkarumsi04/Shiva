...
shiva_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # shiva_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("SHIVA_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import shiva_time

    assert shiva_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (SHIVA_TIMEZONE, SHIVA_REDACT_SECRETS, SHIVA_MAX_ITERATIONS,
    ...
    # writing SHIVA_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
