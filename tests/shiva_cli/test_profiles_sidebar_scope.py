...
@pytest.fixture
def profiles_on_disk(tmp_path, monkeypatch, _isolate_shiva_home):
...
def client(monkeypatch, profiles_on_disk):
    ...
    import shiva_state
    from shiva_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
    from shiva_constants import get_shiva_home
...
def _seed_project(home, name, folder):
    from shiva_cli import projects_db
...
class TestCrossProfileProjectTree:
    ...
    def test_each_profile_contributes_its_own_projects_db(self, client, profiles_on_disk, tmp_path):
        ...
        filters through ``get_shiva_home()``. If the fan-out failed to rebind
