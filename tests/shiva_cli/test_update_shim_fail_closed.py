...
The bug class: on Windows, when `shiva.exe`/sibling shims could not be
...
import shiva_cli._install_repair as ir
import shiva_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("shiva", "shiva-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # shiva.exe cannot be renamed; shiva-gateway.exe can
    real_rename = Path.rename

    def deny_shiva(self, target):
        if self.name == "shiva.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_shiva)
    monkeypatch.setattr(cli_main, "_shiva_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "shiva.exe" in exc_info.value.failed_shims
    # The successful rename (shiva-gateway.exe) was rolled back.
    assert (scripts / "shiva-gateway.exe").exists()
    assert not list(scripts.glob("shiva-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("shiva",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("shiva",))
