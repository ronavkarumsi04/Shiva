...
Covers:
- Defense 1: gateway stop/restart refuse when _SHIVA_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "shiva gateway stop",
        "shiva gateway uninstall",
        "shiva  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "SHIVA GATEWAY RESTART",           # uppercase
    ])
    def test_shiva_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.shiva.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.shiva/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l shiva-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.shiva.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.shiva.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.shiva.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.shiva/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.shiva.gateway-apollo:/a.plist' "
            "'ai.shiva.gateway-cronus:/c.plist' 'ai.shiva.gateway-plutus:/p.plist' "
            "'ai.shiva.gateway:/Users/botuser/Library/LaunchAgents/ai.shiva.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated shiva note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.shiva.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.shiva.gateway',
        "launchctl kick'start' -k gui/501/ai.shiva.gateway",
        "launchctl kick\\start -k gui/501/ai.shiva.gateway",
        'launchctl "kickstart" -k gui/501/ai.shiva.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.shiva.gateway',
        "launchctl dis\\able gui/501/ai.shiva.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.shiva."gateway"',
        # Same class on the systemctl and shiva-CLI branches.
        'systemctl re"start" shiva-gateway',
        'shiva gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.shiva.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.shiva.update-checker',
        'systemctl re"start" shiva-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "shiva cron list",
        "shiva update",
        "shiva config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `shiva gateway start` is benign — starting a gateway from inside a
        ...
        "shiva gateway start",
        "shiva gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway shiva
        # services must not be falsely blocked (the old `.*shiva` matched any
        # shiva token).
        "launchctl unload ai.shiva.update-checker.plist",
        "launchctl restart ai.shiva.daemon",
        ...
        "systemctl restart shiva-meta.service",
        "systemctl restart shiva-cron-helper",
        ...
        "echo after the shiva gateway restarted cleanly",
        "the shiva gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "shiva skill view gateway-notes && echo shiva gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `shiva` is a path component
        # there, not a command.
        "cat '/docs/shiva gateway restart-notes.md'",
        "less /home/user/notes/shiva gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "shiva gateway restart",
        "shiva gateway restart; echo done",
        "shiva gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;shiva gateway restart",
        "true && shiva gateway stop",
        "echo $(shiva gateway restart)",
        "echo `shiva gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.shiva.gateway"])',
        'subprocess.run(["shiva", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.shiva.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["shiva", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: shiva gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `shiva -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry SHIVA_HOME/SHIVA_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("SHIVA_PROFILE", "zeus")
        monkeypatch.delenv("SHIVA_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "shiva -p zeus gateway stop",
        "shiva -p zeus gateway restart",
        "shiva --profile zeus gateway restart",
        "shiva --profile zeus gateway stop",
        "shiva --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "shiva -v -p zeus gateway restart",
        "shiva -p zeus -v gateway restart",
        "shiva --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "shiva -p venus gateway stop",
        "shiva -p venus gateway restart",
        "shiva --profile venus gateway restart",
        "shiva --profile=venus gateway stop",
        "shiva -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "shiva -p zeus gateway start",
        "shiva -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("shiva gateway restart")
        assert _contains_gateway_lifecycle_command("shiva gateway stop")

    def test_shiva_home_derived_profile(self, monkeypatch):
        # Without SHIVA_PROFILE the guard falls back to the SHIVA_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("SHIVA_PROFILE", raising=False)
        monkeypatch.delenv("SHIVA_PROFILE_NAME", raising=False)
        import shiva_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("shiva -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("shiva -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("shiva -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("shiva gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_shiva_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade shiva then run shiva gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_shiva_gateway_once.sh). The script must live under
        # SHIVA_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("SHIVA_HOME", str(tmp_path / ".shiva"))
        scripts_dir = tmp_path / ".shiva" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_SHIVA_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _SHIVA_GATEWAY=1.

    Issue #37453: systemctl --user restart shiva-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart shiva-gateway",
        "systemctl --user restart shiva-gateway",
        "systemctl stop shiva-gateway.service",
        "shiva gateway restart",
        "shiva gateway uninstall",
        "launchctl kickstart gui/501/ai.shiva.gateway",
        "launchctl bootout gui/501/ai.shiva.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.shiva.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.shiva/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.shiva.gateway.restart-once.plist",
        "pkill -f shiva.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart shiva-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.shiva.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-shiva label: label-independent detection is the point
        # (#62891 second reproduction used `ai.shiva.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.shiva.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _SHIVA_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _SHIVA_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_SHIVA_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="shiva gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["shiva gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.shiva.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.shiva.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-shiva systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run shiva gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("shiva gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeshiva gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under SHIVA_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("shiva gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/shiva\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 shiva gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_shiva_home()` falls back to Path.home(), which raises
        RuntimeError when neither SHIVA_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart shiva-gateway' .",
        "grep -rn 'shiva gateway restart' ./logs",
        "rg 'shiva gateway restart' ../archive",
        "grep -c 'systemctl stop shiva-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM shiva_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell shiva gateway restart"',
        'sqlite3 ./db ".system systemctl restart shiva-gateway"',
        'psql ./x -c "\\! systemctl restart shiva-gateway"',
        "grep -r 'shiva gateway restart' . | sh",
        "grep -r 'shiva gateway restart' ./logs | bash",
        "grep -r 'shiva gateway restart' . | sudo sh",
        "grep -r 'x' . ; shiva gateway restart",
        "grep -r 'x' . && systemctl restart shiva-gateway",
        'grep -r "$(shiva gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart shiva-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell shiva gateway restart"',
        'sqlite3 db ".system systemctl restart shiva-gateway"',
        'psql -c "\\! systemctl restart shiva-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    shiva_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run shiva gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHIVA_HOME", str(tmp_path / ".shiva"))
        (tmp_path / ".shiva").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run shiva gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop shiva-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart shiva-gateway' /var/log/syslog",
        "rg 'shiva gateway restart' /home/user/.shiva/logs/",
        "journalctl -u shiva-gateway --grep 'systemctl restart shiva-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'shiva_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell shiva gateway restart"',
        'psql -c "\\! systemctl restart shiva-gateway"',
        "grep 'systemctl restart shiva-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart shiva-gateway",
        'grep "$(systemctl restart shiva-gateway)" f',
        "grep 'restart' log; systemctl restart shiva-gateway",
        'sqlite3 db "SELECT 1"; shiva gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "shiva gateway restart",
        "sudo systemctl stop shiva-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart shiva-gateway%'\""
        )
