"""Tests for the auxiliary-model configuration UI in ``shiva model``.
...
from shiva_cli.config import DEFAULT_CONFIG, load_config
...
def test_reset_aux_clears_delegation_routing_preserves_settings(tmp_path, monkeypatch):
    ...
    from shiva_cli.config import load_config as _lc, save_config
...
def test_delegation_cfg_as_task_projection():
    ...
    shaped = _delegation_cfg_as_task(
        {"delegation": {"provider": "nous", "model": "Shiva-4.5"}}
    )
    assert _format_aux_current(shaped) == "nous · Shiva-4.5"
