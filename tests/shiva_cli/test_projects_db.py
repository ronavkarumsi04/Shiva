"""Tests for the per-profile Projects store (shiva_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Shiva Agent", folders=["/tmp/shiva"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "shiva-agent"
    assert proj.name == "Shiva Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/shiva"
    assert [f.path for f in proj.folders] == ["/tmp/shiva"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "shiva-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' SHIVA_HOME.
