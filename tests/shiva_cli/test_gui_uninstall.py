"""Tests for shiva_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import shiva_cli.gui_uninstall as gu


def _make_agent(shiva_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = shiva_home / "shiva-agent"
    (agent_root / "shiva_cli").mkdir(parents=True)
    (agent_root / "shiva_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(shiva_home: Path) -> None:
    """Create the source-built GUI artifacts a `shiva desktop` run produces."""
    desktop = shiva_home / "shiva-agent" / "apps" / "desktop"
    ...
    (shiva_home / "shiva-agent" / "node_modules").mkdir(parents=True)
    (shiva_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(shiva_home: Path) -> None:
    (shiva_home / "config.yaml").write_text("x: 1\n")
    (shiva_home / ".env").write_text("KEY=secret\n")
    (shiva_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(shiva_home)
    _make_gui_build(shiva_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(shiva_home)
    ...
    assert summary["shiva_home"] == str(shiva_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `shiva desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    shiva_home = tmp_path / ".shiva"
    _make_agent(shiva_home)
    icon = lde.icon_path(shiva_home / "shiva-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(shiva_home)
    ...
    assert lde.icon_path(shiva_home / "shiva-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (shiva_home / "shiva-agent" / "shiva_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".shiva")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import shiva_cli.uninstall as uninstall
