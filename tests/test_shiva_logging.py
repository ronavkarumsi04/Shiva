"""Tests for shiva_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import shiva_logging
# Use whatever RotatingFileHandler class shiva_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. shiva_logging
...
from shiva_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    shiva_logging._logging_initialized = False
    ...
    shiva_logging._reset_queued_handlers()
    ...
    shiva_logging._install_session_record_factory()
    yield
    ...
    shiva_logging._logging_initialized = False
    shiva_logging.clear_session_context()


@pytest.fixture
def shiva_home(tmp_path, monkeypatch):
    """Provide an isolated SHIVA_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_shiva_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["SHIVA_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, shiva_home):
        log_dir = shiva_logging.setup_logging(shiva_home=shiva_home)
        assert log_dir == shiva_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, shiva_home):
        shiva_logging.setup_logging(shiva_home=shiva_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in shiva_logging.rotating_file_handlers()
    ...
    def test_idempotent_no_duplicate_handlers(self, shiva_home):
        shiva_logging.setup_logging(shiva_home=shiva_home)
        shiva_logging.setup_logging(shiva_home=shiva_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, shiva_home):
        shiva_logging.setup_logging(shiva_home=shiva_home)

        test_logger = logging.getLogger("test_shiva_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        shiva_logging.flush_log_queue()

        agent_log = shiva_home / "logs" / "agent.log"
    ...
    def test_explicit_params_override_config(self, shiva_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (shiva_home / "config.yaml").write_text(yaml.dump(config))

        shiva_logging.setup_logging(shiva_home=shiva_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, shiva_home):
        shiva_logging.setup_logging(shiva_home=shiva_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, shiva_home):
        shiva_logging.setup_logging(shiva_home=shiva_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, shiva_home):
        ...
        gw_log = shiva_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, shiva_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, shiva_home):
        shiva_logging.setup_logging(shiva_home=shiva_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, shiva_home):
        shiva_logging.setup_logging(shiva_home=shiva_home, mode="gui")

        logging.getLogger("shiva_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        shiva_logging.flush_log_queue()

        gui_log = shiva_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, shiva_home):
        """When session context is set, log lines include [session_id]."""
        shiva_logging.setup_logging(shiva_home=shiva_home)
        shiva_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = shiva_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, shiva_home):
        shiva_logging.setup_logging(shiva_home=shiva_home)
        shiva_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_shiva_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        shiva_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in shiva_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
        ...
        shiva_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("shiva_cli.config.is_managed", return_value=True):
                shiva_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = shiva_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert shiva_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not shiva_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, shiva_home):
        level, max_size, backup = shiva_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, shiva_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> shiva_logging._ManagedRotatingFileHandler:
        handler = shiva_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that shiva_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        shiva_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        shiva_logging.setup_logging(shiva_home=shiva_home, mode="gateway")
        gw_path = shiva_home / "logs" / "gateway.log"
        rotated = shiva_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = shiva_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, shiva_home):
        shiva_logging.setup_logging(shiva_home=shiva_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_shiva_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in shiva_logging.rotating_file_handlers()
        )
