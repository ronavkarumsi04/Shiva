...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.shiva/shiva-agent/utils.py)
...
2. ``shiva update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``shiva_cli/main.py`` (the ``shiva update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``shiva_constants`` after the
