"""Tests for shiva_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Shiva entry point
(shiva, shiva-agent, shiva-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Shiva entry point imports shiva_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported shiva_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("shiva_bootstrap", None)
    import shiva_bootstrap  # noqa: WPS433
    return shiva_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Shiva CLI
...
class TestEntryPointsImportBootstrap:
    """Every Shiva entry point must import shiva_bootstrap as its
    ...
    # Entry points that invoke Shiva as a process.  Each one must
    # import shiva_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "shiva_cli/main.py",   # shiva CLI (console_script)
        "run_agent.py",          # shiva-agent (console_script)
        "acp_adapter/entry.py",  # shiva-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import shiva_bootstrap' and that
        ...
        half-finished ``shiva update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``shiva_bootstrap`` as a top-level module) leaves shiva
        ...
        """
        # Resolve relative to the shiva-agent repo root.  Tests live
        # at tests/test_shiva_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # shiva start even when shiva_bootstrap hasn't been
        ...
        assert first_import_name == "shiva_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'shiva_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import shiva_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Shiva's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("SHIVA_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["SHIVA_PYTHON_SRC_ROOT"] = env
            elif "SHIVA_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["SHIVA_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/shiva")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SHIVA_PYTHON_SRC_ROOT", None)
            else:
                os.environ["SHIVA_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/shiva", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Shiva root.  The guard must relocate Shiva to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/shiva"])
        assert result[0] == "/opt/shiva"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Shiva root.
        assert result.index("/opt/shiva") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["SHIVA_PYTHON_SRC_ROOT"] = "/env/shiva"
            hb.harden_import_path()
            assert sys.path[0] == "/env/shiva"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SHIVA_PYTHON_SRC_ROOT", None)
