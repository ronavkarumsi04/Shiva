...
def test_root_session_no_compression(db):
    ...
    assert prov["currentShivaSessionId"] == "root1"
    assert prov["rootShivaSessionId"] == "root1"
    assert prov["parentShivaSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_shiva_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentShivaSessionId"] == "old"
    assert prov["rootShivaSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousShivaSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootShivaSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"shiva"}
    assert "sessionProvenance" in meta["shiva"]
    assert meta["shiva"]["sessionProvenance"]["currentShivaSessionId"] == "root1"
