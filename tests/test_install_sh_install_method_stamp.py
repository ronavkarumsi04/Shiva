...
($INSTALL_DIR), not into the shared $SHIVA_HOME.

Background (shared-$SHIVA_HOME bug)
------------------------------------
$SHIVA_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.shiva:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$SHIVA_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``shiva update``).
...
``~/.shiva/shiva-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$SHIVA_HOME/\.install_method"', text), (
        "install.sh must not stamp $SHIVA_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `shiva update`"
    )
