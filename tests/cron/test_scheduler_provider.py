...
CronScheduler plan, .shiva/plans/cron-scheduler-provider-interface.md). They
...
No production code is exercised beyond the two ticker entry points:
  ...
  - shiva_cli/web_server.py::_start_desktop_cron_ticker  (desktop fallback)
...
def test_desktop_ticker_calls_tick_then_stops():
    ...
    no adapters/loop."""
    from shiva_cli.web_server import _start_desktop_cron_ticker
