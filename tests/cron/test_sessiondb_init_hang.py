...
which is already bounded by SHIVA_CRON_TIMEOUT — the worker thread submitted
...
timeout (SHIVA_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
class TestSessionDbInitTimeout:
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHIVA_CRON_SESSION_DB_TIMEOUT", "0.2")
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed SHIVA_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors SHIVA_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("SHIVA_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "SHIVA_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("SHIVA_CRON_SESSION_DB_TIMEOUT", raising=False)
    ...
    def test_late_session_db_result_is_closed(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("cron.scheduler._shiva_home", tmp_path), \
                 ...
                 patch("shiva_cli.env_loader.load_shiva_dotenv"), \
                 patch("shiva_cli.env_loader.reset_secret_source_cache"), \
                 patch("shiva_state.SessionDB", side_effect=_hanging_then_capture), \
                 patch(
                     "shiva_cli.runtime_provider.resolve_runtime_provider",
                     return_value={
                         "api_key": "test-key",
...
class TestSessionDbInitAfterEarlyReturns:
    ...
    def test_wake_gate_false_never_opens_session_db(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._shiva_home", tmp_path), \
             ...
             patch("shiva_cli.env_loader.load_shiva_dotenv"), \
             patch("shiva_cli.env_loader.reset_secret_source_cache"), \
             patch("shiva_state.SessionDB") as mock_db_cls, \
             patch(
