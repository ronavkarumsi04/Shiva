...
- Path resolution (absolute, relative to SHIVA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp SHIVA_HOME."""
    shiva_home = tmp_path / ".shiva"
    shiva_home.mkdir()
    (shiva_home / "cron").mkdir()
    (shiva_home / "cron" / "output").mkdir()
    (shiva_home / "scripts").mkdir()
    monkeypatch.setenv("SHIVA_HOME", str(shiva_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "SHIVA_DIR", shiva_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", shiva_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", shiva_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", shiva_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Shiva provider env (SECURITY.md §2.3)."""
        from tools.environments.local import _SHIVA_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_SHIVA_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.shiva/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("SHIVA_SESSION_PLATFORM") is None
        assert os.environ.get("SHIVA_SESSION_CHAT_ID") is None
        assert os.environ.get("SHIVA_SESSION_CHAT_NAME") is None
...
class TestScriptTimeoutTreeKill:
    ...
    def test_timeout_leaves_no_setsid_grandchild(self, cron_env, monkeypatch):
        ...
        monkeypatch.setenv("SHIVA_CRON_SCRIPT_TIMEOUT", "2")
