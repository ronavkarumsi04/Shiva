...
def test_run_job_releases_cwd_lock_when_body_raises(tmp_path):
    ...
    with patch("cron.scheduler._shiva_home", tmp_path), \
         ...
         patch("shiva_state.SessionDB", return_value=MagicMock()):
...
def test_cwd_lock_timeout_derivation(monkeypatch):
    """The bound tracks SHIVA_CRON_TIMEOUT (+margin) with a floor, and
