...
def cron_store(tmp_path, monkeypatch):
    ...
    (shiva_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("SHIVA_HOME", str(shiva_home))
    monkeypatch.setattr(J, "SHIVA_DIR", shiva_home)
    monkeypatch.setattr(J, "CRON_DIR", shiva_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", shiva_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", shiva_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_shiva_now", lambda: now):
            due = J.get_due_jobs()
