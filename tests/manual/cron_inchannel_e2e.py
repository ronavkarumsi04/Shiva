...
Uses a throwaway SHIVA_HOME so it never touches ~/.shiva. No real names.
"""
...
def _fresh_home():
    """Point SHIVA_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_shiva_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["SHIVA_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER SHIVA_HOME is set.
...
# a different get_shiva_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"SHIVA_HOME (throwaway): {HOME}")
