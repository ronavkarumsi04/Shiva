...
binary download are exercised manually by `shiva secrets bitwarden
...
def shiva_home(tmp_path, monkeypatch):
    """Point Shiva at an isolated home directory."""
    home = tmp_path / ".shiva"
    home.mkdir()
    monkeypatch.setenv("SHIVA_HOME", str(home))
    # Some modules cache get_shiva_home; clear if needed.
    import shiva_constants
    if hasattr(shiva_constants, "_SHIVA_HOME_CACHE"):
        shiva_constants._SHIVA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(shiva_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from shiva_cli.env_loader import _apply_external_secret_sources
