...
def server():
    ...
    # e.g. shiva_cli.active_sessions would bind the mocked get_shiva_home
    ...
    with patch.dict("sys.modules", {
        "shiva_constants": MagicMock(get_shiva_home=MagicMock(return_value="/tmp/shiva_test")),
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `SHIVA_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("SHIVA_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("SHIVA_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/shiva-agent-dev": {"name": "shiva-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "shiva-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient SHIVA_HOME
    ...
    # The gateway's own SHIVA_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `shiva config set` does)
    ...
    import shiva_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_shiva_home", lambda: tmp_path)
    ...
    # Activate midnight, as `shiva config set display.skin midnight` would.
