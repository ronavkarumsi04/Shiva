    ...
    with patch.dict(
        "sys.modules",
        {
            "shiva_constants": MagicMock(
                get_shiva_home=MagicMock(return_value="/tmp/shiva_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'shiva-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'shiva-release' patched"
    }
