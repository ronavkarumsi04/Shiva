...
Shiva's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
neutralize this before any non-stdlib import is resolved, by delegating to the
shared ``shiva_bootstrap.harden_import_path`` guard.
...
def test_entry_calls_shared_harden_guard_before_heavy_imports():
    """entry.py must call shiva_bootstrap.harden_import_path() before it
    ...
    assert harden_call_line is not None, (
        "entry.py must call shiva_bootstrap.harden_import_path()"
    )
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the Shiva root.  harden_import_path must relocate the
    Shiva root to the front so ``from utils import ...`` resolves to Shiva."""
    import sys

    original = sys.path[:]
    try:
        sys.path[:] = ["/home/user/tg-ws-proxy", "/opt/shiva", "/usr/lib"]
        shiva_bootstrap.harden_import_path(src_root="/opt/shiva")
        assert sys.path[0] == "/opt/shiva"
        assert sys.path.index("/opt/shiva") < sys.path.index(
