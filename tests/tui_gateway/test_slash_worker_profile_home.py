...
def test_slash_worker_accepts_profile_home():
    ...
    # shiva_state evaluates get_shiva_home() / "state.db" at import time, so
    # the mock must return a Path (a bare str raises TypeError under per-file
    # subprocess isolation).
    with patch.dict("sys.modules", {
        "shiva_constants": MagicMock(
            get_shiva_home=MagicMock(return_value=Path("/tmp/shiva_test")),
        ),
    }):
        with patch("subprocess.Popen") as mock_popen:
            ...
            worker = _SlashWorker(
                ...
                model="test-model",
                profile_home="/home/luke/.shiva/profiles/work"
            )

            # Verify Popen was called
            assert mock_popen.called

            # Check that SHIVA_HOME was set in the environment
            call_kwargs = mock_popen.call_args[1]
            assert "env" in call_kwargs
            assert call_kwargs["env"]["SHIVA_HOME"] == "/home/luke/.shiva/profiles/work"
