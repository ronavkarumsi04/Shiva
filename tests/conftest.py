"""Shared fixtures for the shiva-agent test suite.
...
2. **Isolated SHIVA_HOME.** SHIVA_HOME points to a per-test tempdir so
   code reading ``~/.shiva/*`` via ``get_shiva_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".shiva"`` instead of the canonical
   ``get_shiva_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SHIVA_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox SHIVA_HOME before ANY test module is imported ──────────────────
# `shiva_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_shiva_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.shiva/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes SHIVA_HOME, but fixtures run
...
# the REAL Shiva root — capture it BEFORE the sandbox rewires SHIVA_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.shiva (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("SHIVA_KANBAN_HOME", "").strip()
_PRE_SANDBOX_SHIVA_HOME = os.environ.get("SHIVA_HOME", "")


def _shiva_home_points_at_production(value: str) -> bool:
    """True when a pre-set SHIVA_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    SHIVA_HOME=~/.shiva``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``shiva_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.shiva`` — the escape vector that landed pytest
    ...
    SHIVA_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".shiva").resolve()
    except Exception:
        return True
...
if _shiva_home_points_at_production(os.environ.get("SHIVA_HOME", "")):
    _SESSION_SHIVA_HOME = tempfile.mkdtemp(prefix="shiva-test-home-")
    os.environ["SHIVA_HOME"] = _SESSION_SHIVA_HOME
    atexit.register(shutil.rmtree, _SESSION_SHIVA_HOME, True)
...
# like a real CLI") — which used to disarm shiva_state's live-DB guard in
# the child at the same moment the child lost the SHIVA_HOME redirect.
# SHIVA_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# shiva_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export SHIVA_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["SHIVA_TEST_ISOLATION"] = os.environ.get("SHIVA_HOME", "") or "1"

#: SHIVA_HOME as it stood when conftest was imported - i.e. before any test
...
SHIVA_HOME_AT_CONFTEST_IMPORT = os.environ.get("SHIVA_HOME", "")
...
# SHIVA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SHIVA_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``shiva_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Shiva agent session inherits them and
    # shiva_constants home-resolution helpers prefer them over monkeypatched
    ...
    "SHIVA_MAX_ITERATIONS",
    "SHIVA_SESSION_PLATFORM",
    "SHIVA_SESSION_CHAT_ID",
    "SHIVA_SESSION_CHAT_NAME",
    "SHIVA_SESSION_CHAT_TYPE",
    "SHIVA_SESSION_THREAD_ID",
    "SHIVA_SESSION_SOURCE",
    "SHIVA_SESSION_KEY",
    "SHIVA_GATEWAY_SESSION",
    "SHIVA_CRON_SESSION",
    "_SHIVA_GATEWAY",
    "SHIVA_PLATFORM",
    "SHIVA_MODEL",
    "SHIVA_INFERENCE_MODEL",
    "SHIVA_INFERENCE_PROVIDER",
    "SHIVA_TUI_PROVIDER",
    "SHIVA_MANAGED",
    "SHIVA_MANAGED_DIR",
    "SHIVA_DEV",
    ...
    "SHIVA_TIMEZONE",
    "SHIVA_REDACT_SECRETS",
    "SHIVA_BACKGROUND_NOTIFICATIONS",
    "SHIVA_EXEC_ASK",
    "SHIVA_HOME_MODE",
    "SHIVA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.shiva/kanban.db instead of the per-test SHIVA_HOME.
    "SHIVA_KANBAN_DB",
    "SHIVA_KANBAN_BOARD",
    "SHIVA_KANBAN_HOME",
    "SHIVA_KANBAN_WORKSPACES_ROOT",
    "SHIVA_KANBAN_LOGS_ROOT",
    "SHIVA_KANBAN_TASK",
    "SHIVA_KANBAN_WORKSPACE",
    "SHIVA_KANBAN_RUN_ID",
    "SHIVA_KANBAN_CLAIM_LOCK",
    "SHIVA_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "shiva" host.
    "SHIVA_HONCHO_HOST",
    ...
    "SHIVA_DASHBOARD_OAUTH_CLIENT_ID",
    "SHIVA_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SHIVA_HOME to per-test tempdirs so code that
    reads ``~/.shiva/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral SHIVA_* vars that could change test semantics.
    for name in _SHIVA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("SHIVA_HONCHO_HOST", "shiva")

    # 3. Redirect SHIVA_HOME to a per-test tempdir. Code that reads
    #    ``~/.shiva/*`` via ``get_shiva_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.shiva/*`` via
    #    ``Path.home() / ".shiva"`` instead of ``get_shiva_home()``
    #    is a bug to fix at the callsite.
    fake_shiva_home = tmp_path / "shiva_test"
    fake_shiva_home.mkdir()
    (fake_shiva_home / "sessions").mkdir()
    (fake_shiva_home / "cron").mkdir()
    (fake_shiva_home / "memories").mkdir()
    (fake_shiva_home / "skills").mkdir()
    monkeypatch.setenv("SHIVA_HOME", str(fake_shiva_home))
    ...
    # shiva_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("SHIVA_TEST_ISOLATION", str(fake_shiva_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("SHIVA_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. shiva_state computes ``DEFAULT_DB_PATH = get_shiva_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from shiva_state import ...``) that
    ...
    shiva_state_mod = sys.modules.get("shiva_state")
    if shiva_state_mod is not None and hasattr(shiva_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            shiva_state_mod, "DEFAULT_DB_PATH", fake_shiva_home / "state.db"
        )
    ...
    monkeypatch.setenv("SHIVA_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.shiva/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import shiva_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path SHIVA_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_shiva_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from shiva_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.shiva. This autouse
...
# allow-list because test-level fixtures legitimately move SHIVA_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session SHIVA_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``SHIVA_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Shiva root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_SHIVA_HOME and not _shiva_home_points_at_production(
        _PRE_SANDBOX_SHIVA_HOME
    ):
        # SHIVA_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_shiva_root().resolve()
    # No pre-existing SHIVA_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".shiva").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.shiva`` captured at import time. Hermetic tests that legitimately
    move SHIVA_HOME to sibling tempdirs are unaffected.

    Only patches when ``shiva_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("shiva_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import shiva_cli.kanban_db`
...
# ``shiva_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Shiva root. This fixture wires the
# test-side knobs:
...
#     installs where SHIVA_HOME is not ~/.shiva) into the guard's
...
# so subprocess children that import shiva_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("shiva_state")
    ...
    if _PRE_SANDBOX_SHIVA_HOME and not _shiva_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_SHIVA_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Shiva home override redirects every later
    # ``get_shiva_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from shiva_constants import get_shiva_home_override, set_shiva_home_override

        if get_shiva_home_override() is not None:
            set_shiva_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal shiva config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``shiva-gateway`` process
...
# correlated with a ``tests/shiva_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> shiva-gateway``
...
def _wal_is_usable() -> bool:
    """True when Shiva will actually put a database into WAL mode here.

    Shiva refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Shiva can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Shiva managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``shiva_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_shiva_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_shiva_home`` fixture
    redirects ``SHIVA_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.shiva/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``shiva_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["SHIVA_VOICE_TTS"] = "1"``. The
...
#      ``shiva_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``shiva_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``shiva_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``shiva_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Shiva runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.shiva``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Shiva falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Shiva uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart shiva-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting shiva/python patterns are also blocked.
    """
    ...
    def _matches_shiva_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SHIVA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_shiva_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch shiva-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m shiva_cli.main".
                if (
                    "shiva" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live shiva-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting shiva/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `shiva update` (or the
        # equivalent `python -m shiva_cli.main update`).  These commands
        ...
        if "update" in low and (
            # shiva update / shiva update --gateway / setsid bash -c ... shiva update
            ("shiva" in low and "update" in low.split())
            or
            # python -m shiva_cli.main update --gateway
            ("shiva_cli" in low and "update" in low.split())
            or
            # venv/bin/shiva update  (absolute path variant used in tests)
            (".venv/bin/shiva" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`shiva update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_SHIVA_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import shiva_cli.voice as _voice
    except Exception:
