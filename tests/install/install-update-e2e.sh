...
#
# Installs a real, earlier Shiva the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `shiva`.
#
...
# Keep sandbox state out of the default .shiva-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".shiva-sandbox-e2e-$ROUTE"
export SHIVA_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/shiva/.shiva/shiva-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/shiva-agent.git"
...
LOG_DIR="${SHIVA_E2E_LOG_DIR:-$(mktemp -d -t shiva-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED shiva accept FLAG on `shiva update`?
#
...
# that silently rots. `shiva update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "shiva update --help 2>&1" | grep -qF -- "$flag"
}
...
require_shiva_works() {
  local when="$1" out
  out="$(in_sandbox "shiva --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "shiva --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "shiva runs $when"
}

# ── install the earlier Shiva ─────────────────────────────────────────────
...
require_shiva_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: shiva update'
    ...
    # shiva which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="shiva update --yes"
    else
      update_cmd="shiva update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "shiva update failed ($update_cmd)"
    fi
    require_landed_on_target 'shiva update'
    require_shiva_works 'after shiva update'
    ;;
  installer)
    ...
    require_shiva_works 'after installer re-run'
    ;;
esac
