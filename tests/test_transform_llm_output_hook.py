...
from shiva_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(shiva_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <shiva_home>/plugins/<name> and opt it in."""
    plugin_dir = shiva_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SHIVA_HOME", str(tmp_path / "shiva_empty"))
