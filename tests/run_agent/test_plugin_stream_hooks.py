...
def test_stream_delta_plugin_hook_is_queued_off_token_path(monkeypatch):
    ...
    monkeypatch.setattr("shiva_cli.plugins.iter_hook_callbacks", _callbacks({"on_stream_delta": [on_stream_delta]}))
...
def test_stream_hook_queue_isolated_per_consumer(monkeypatch):
    ...
    monkeypatch.setattr(
        "shiva_cli.plugins.iter_hook_callbacks",
...
def test_reasoning_stream_delta_plugin_hook_is_opt_in(monkeypatch):
    ...
    with patch("shiva_cli.config.cfg_get", return_value=True):
...
def test_interim_message_plugin_hook_is_queued(monkeypatch):
    ...
    monkeypatch.setattr("shiva_cli.plugins.iter_hook_callbacks", _callbacks({"on_interim_message": [on_interim_message]}))
...
def test_stream_plugin_hook_counts_as_stream_consumer(monkeypatch):
    monkeypatch.setattr("shiva_cli.plugins.iter_hook_callbacks", _callbacks({"on_stream_delta": [lambda **_kwargs: None]}))
...
def test_interim_message_plugin_hook_does_not_count_as_stream_consumer(monkeypatch):
    monkeypatch.setattr("shiva_cli.plugins.iter_hook_callbacks", _callbacks({"on_interim_message": [lambda **_kwargs: None]}))
...
def test_bedrock_reasoning_delta_reaches_plugin_only_observer(monkeypatch):
    ...
    monkeypatch.setattr("shiva_cli.plugins.iter_hook_callbacks", _callbacks({"on_stream_delta": [on_stream_delta]}))
    monkeypatch.setattr("shiva_cli.config.cfg_get", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(
