...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    shiva_state = ModuleType("shiva_state")
    shiva_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "shiva_state", shiva_state)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "Shiva", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Shiva"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "Shiva", "detail": "full"}),
        ),
    )
