...
def _clear_env(monkeypatch):
    monkeypatch.delenv("SHIVA_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", raising=False)
    monkeypatch.delenv("SHIVA_GATEWAY_HTTPX_MAX_KEEPALIVE", raising=False)
...
def test_env_override_rejects_garbage(monkeypatch):
    ...
    monkeypatch.setenv("SHIVA_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", "not-a-number")
    monkeypatch.setenv("SHIVA_GATEWAY_HTTPX_MAX_KEEPALIVE", "-3")
