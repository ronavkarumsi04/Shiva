...
closure the PR changed, against a real temp ``SHIVA_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "shiva_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "shiva_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_shiva_home", shiva_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_shiva_home() / config.yaml`` — point it here.
    monkeypatch.setattr("shiva_constants.get_shiva_home", lambda: shiva_home)
    monkeypatch.setattr("shiva_cli.config.get_shiva_home", lambda: shiva_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_shiva_home", default_home)
    _stub_picker_dependencies(monkeypatch)
