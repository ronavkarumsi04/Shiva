...
(/opt/shiva/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable SHIVA_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_shiva_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable SHIVA_HOME."""
    ...
    monkeypatch.setattr(
        "shiva_constants.get_shiva_home", lambda: shiva_home
    )
    ...
    expected = shiva_home / "scripts" / "whatsapp-bridge"
