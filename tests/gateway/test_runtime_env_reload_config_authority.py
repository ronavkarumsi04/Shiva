...
SHIVA_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env SHIVA_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``shiva config get terminal.backend`` still says local.
    """
    shiva_home = tmp_path / ".shiva"
    shiva_home.mkdir()
    (shiva_home / "config.yaml").write_text(
    ...
    (shiva_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
