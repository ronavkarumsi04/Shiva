...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    shiva_home.mkdir()
    monkeypatch.setattr(base, "_SHIVA_HOME", shiva_home)
    monkeypatch.setattr(base, "_SHIVA_ROOT", shiva_home)
    path = shiva_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.shiva/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("SHIVA_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("SHIVA_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("SHIVA_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("SHIVA_KANBAN_HOME", str(tmp_path / "shiva"))
        monkeypatch.setenv("SHIVA_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "shiva" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("SHIVA_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("SHIVA_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("SHIVA_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.shiva/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        shiva_dir = fake_home / ".shiva"
        (shiva_dir / "mcp-tokens").mkdir(parents=True)
        secret = shiva_dir / rel
        secret.write_text('{"access_token": "live-bearer-abc123"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._SHIVA_HOME",
            shiva_dir,
        )
        monkeypatch.setattr(
            "gateway.platforms.base._SHIVA_ROOT",
            shiva_dir,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the SHIVA_HOME root (google_token.json)
        ...
        fake_home = tmp_path / "home"
        shiva_dir = fake_home / ".shiva"
        shiva_dir.mkdir(parents=True)
        token = shiva_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._SHIVA_HOME", shiva_dir)
        monkeypatch.setattr("gateway.platforms.base._SHIVA_ROOT", shiva_dir)

        assert BasePlatformAdapter.validate_media_delivery_path(str(token)) is None


    def test_denylist_blocks_non_cache_file_under_shiva_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.shiva
        ...
        shiva_dir = fake_home / ".shiva"
        shiva_dir.mkdir(parents=True)
        artifact = shiva_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting SHIVA_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose SHIVA_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.shiva) emits
        ...
        shiva_root = denied_root / ".shiva"
        prof_cache = shiva_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._SHIVA_ROOT", shiva_root
        )
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        """MEDIA:/root/.shiva/cache/images/... (the agent_visible_image path
        under docker) must translate to the HOST cache file, not the sandbox
        home copy."""
        shiva_home = tmp_path / ".shiva"
        cache = shiva_home / "cache" / "images"
    ...
    def test_container_credential_path_never_translates_through_home(self, tmp_path, monkeypatch):
        """/root/.shiva/* outside a cache mount (the sandbox's credential
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "SHIVA_HUMAN_DELAY_MODE": "natural",
            "SHIVA_HUMAN_DELAY_MIN_MS": "oops",
            "SHIVA_HUMAN_DELAY_MAX_MS": "still-bad",
        }
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"SHIVA_MEDIA_DELIVERY_STRICT": "1",
                                     "SHIVA_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHIVA_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Shiva
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../shiva/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.shiva/cache/media/sensitive_host_path_abc123.bin"
...
class TestDockerProfileSandboxMediaTranslation:
    ...
    def test_home_credential_surface_still_refused(self, monkeypatch):
        """The /root/.shiva exclusion survives profile scoping: translating
        ...
        for task in ("default", f"session:{self.SESSION_KEY}"):
            secrets = self._sandbox_dir(task) / "home" / ".shiva"
            secrets.mkdir(parents=True, exist_ok=True)
            (secrets / "auth.json").write_text("{}")

        assert (
            BasePlatformAdapter.validate_media_delivery_path(
                "/root/.shiva/auth.json", session_key=self.SESSION_KEY
            )
            is None
        )
