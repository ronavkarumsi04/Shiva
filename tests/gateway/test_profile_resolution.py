...
class TestResolutionOrder:
    ...
    def test_source_profile_wins_over_routing(self, mock_runner, discord_source):
        ...
        with patch("shiva_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("shiva_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("shiva_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/shiva/profiles/from-source")
                    result = mock_runner._resolve_profile_home_for_source(discord_source)
                    
                    assert result == Path("/shiva/profiles/from-source")
...
class TestMissingProfileWarning:
    ...
    def test_nonexistent_profile_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("shiva_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("shiva_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/shiva/profiles/nonexistent")
                with patch("shiva_cli.profiles.profile_exists", return_value=False):
                    with patch("shiva_constants.get_shiva_home", return_value=Path("/shiva")):
                        with caplog.at_level(logging.WARNING):
                            result = mock_runner._resolve_profile_home_for_source(discord_source)
                            
                            # Should fall back to global SHIVA_HOME
                            assert result == Path("/shiva")
...
class TestExceptionHandling:
    ...
    def test_get_profile_dir_exception_logs_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("shiva_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("shiva_cli.profiles.get_profile_dir", side_effect=ValueError("Invalid profile name")):
                with patch("shiva_constants.get_shiva_home", return_value=Path("/shiva")):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
...
class TestRoutingConsultation:
    ...
    def test_routing_consulted_when_source_profile_empty(self, mock_runner, discord_source):
        ...
        with patch("shiva_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("shiva_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/shiva/profiles/routed")
