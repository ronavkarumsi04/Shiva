...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("shiva.gateway.up"), _M("shiva.gateway.active_agents"),
        _M("shiva.gateway.busy"), _M("shiva.gateway.drainable"),
        _M("shiva.gateway.restart_requested"),
        _M("shiva.platform.up"), _M("shiva.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("shiva.cron.scheduler.heartbeat_age_seconds"),
        _M("shiva.cron.scheduler.last_success_age_seconds"),
        _M("shiva.cron.scheduler.catch_up_occurrences"),
        _M("shiva.cron.jobs.enabled"), _M("shiva.cron.jobs.running"),
        _M("shiva.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Shiva Agent-owned Relay transport health" in text
