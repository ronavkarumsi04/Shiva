...
The ``shiva update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m shiva_cli.main gateway run`` — image name
    ``pythonw``, not ``shiva.exe`` — so the ``taskkill /IM shiva.exe`` guard
    ...
    # The shiva.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM shiva.exe' in text
