...
def _inject_fake_shiva_cli(monkeypatch):
    """Inject fake shiva_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("shiva_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("shiva_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_shiva_cli = types.ModuleType("shiva_cli")
    fake_shiva_cli.config = fake_config_mod
    fake_shiva_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "shiva_cli", fake_shiva_cli)
    monkeypatch.setitem(sys.modules, "shiva_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "shiva_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["shiva", "--mode", "platform", "--api-key", "sk-test"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_shiva_home", lambda: tmp_path)
        _inject_fake_shiva_cli(monkeypatch)
        config = {"memory": {}}
    ...
    def test_selfhosted_flag_mode(self, tmp_path, monkeypatch):
        ...
        assert mem0_json["user_id"] == "shiva-user"
