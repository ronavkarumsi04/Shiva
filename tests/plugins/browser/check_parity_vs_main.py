...
Run from the PR worktree:
    ...
    cd ~/.shiva/shiva-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.shiva/shiva-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.shiva/shiva-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a shiva-agent checkout"
)
...
# Isolated SHIVA_HOME for the config write.
