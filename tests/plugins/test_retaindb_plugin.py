...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure SHIVA_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", shiva_home=str(tmp_path / ".shiva"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHIVA_HOME", str(shiva_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", shiva_home=str(shiva_home))
        return p
