...
Covers three layers:
  * ``shiva_cli.kanban_db`` accessors (add/list/get/delete + path helpers)
...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "shiva_dashboard_plugin_kanban_attach_test", plugin_file,
    )
...
def test_attachments_root_is_per_board(kanban_home, monkeypatch):
    ...
    monkeypatch.delenv("SHIVA_KANBAN_ATTACHMENTS_ROOT", raising=False)
...
# ---------------------------------------------------------------------------
# CLI — shiva kanban attach / attachments / attach-rm
# ---------------------------------------------------------------------------
