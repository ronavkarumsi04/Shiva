...
Under a gateway (Telegram, Discord, Slack, ...) Shiva passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHIVA_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Shiva via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Shiva profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `shiva -p partner` pinned to a different person's peer).
