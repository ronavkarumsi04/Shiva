"""
Tests for timezone support (shiva_time module + integration points).
...
def _reset_shiva_time_cache():
    """Reset the shiva_time module cache (replacement for removed reset_cache)."""
    shiva_time._cached_tz = None
    shiva_time._cached_tz_name = None
    shiva_time._cache_resolved = False


# =========================================================================
# shiva_time.now() — core helper
# =========================================================================

class TestShivaTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_shiva_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["SHIVA_TIMEZONE"] = "Asia/Kolkata"
        result = shiva_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["SHIVA_TIMEZONE"] = "Europe/London"
        tz = shiva_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When SHIVA_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) SHIVA_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("SHIVA_TIMEZONE=" + os.environ.get("SHIVA_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "SHIVA_TIMEZONE=NOT_SET" in result["output"], (
            "SHIVA_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When SHIVA_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("SHIVA_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=shiva_tz) which shifted
        absolute time when system-local tz != Shiva tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Shiva tz
        ...
        # Use a Shiva timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _shiva_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["SHIVA_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_shiva_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Shiva timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["SHIVA_TIMEZONE"] = "US/Eastern"
        _reset_shiva_time_cache()
