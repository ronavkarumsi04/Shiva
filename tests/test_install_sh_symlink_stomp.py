...
Older versions of ``install.sh`` created ``$command_link_dir/shiva`` as a
symlink to the pip-generated entry point at ``$SHIVA_BIN`` (i.e.
``venv/bin/shiva``). When ``setup_path()`` later switched to writing a bash
shim with ``cat > "$command_link_dir/shiva" <<EOF``, the redirect followed
the existing symlink and overwrote the pip entry point with the shim. The
shim's ``exec "$SHIVA_BIN" "$@"`` then self-recursed and ``shiva`` hung on
every invocation.

These tests pin the fix: ``setup_path()`` must remove ``$command_link_dir/shiva``
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/shiva\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/shiva"')
    cat_idx = block.find('cat > "$command_link_dir/shiva" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/shiva before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          venv/bin/shiva        <- pip entry point (the one we must preserve)
          local_bin/shiva       <- symlink → ../venv/bin/shiva  (old install)

    Then we run the exact shim-write block from setup_path() with
    ``SHIVA_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
    requires that, after the run:

      * ``venv/bin/shiva`` still contains its original pip-script body
      * ``local_bin/shiva`` is a regular file (not a symlink) holding the shim
    """
    venv_bin = tmp_path / "venv" / "bin"
    venv_bin.mkdir(parents=True)
    pip_entry = venv_bin / "shiva"
    ...
    shim_path = command_link_dir / "shiva"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/shiva was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/shiva must be replaced with a regular file, not "
