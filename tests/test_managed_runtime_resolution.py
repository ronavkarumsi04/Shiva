"""Guard: Shiva-owned subprocesses must not resolve managed runtimes by bare PATH.

Shiva installs runtimes for itself — ``uv`` at ``$SHIVA_HOME/bin/uv``, Node at
``$SHIVA_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Shiva's own
...
* when a system copy also exists, the one Shiva does not own wins — which is
...
``iter_shiva_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Shiva provisions into SHIVA_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Shiva-owned subprocess code: plugins ship their own
...
def _source_files() -> list[Path]:
    ...
    for path in REPO_ROOT.rglob("*.py"):
        ...
        # shiva_agent-0.20.5/, build/ and *.egg-info dirs). CI jobs that
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Shiva-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$SHIVA_HOME/bin (uv) and $SHIVA_HOME/node are not on an "
        ...
        "  node/npm -> shiva_constants.find_node_executable()\n"
        "  PATH env -> shiva_constants.iter_shiva_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(shiva_constants, helper))


def test_managed_uv_helpers_exist():
    from shiva_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
