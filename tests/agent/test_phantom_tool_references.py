...
Covers:
  * SHIVA_AGENT_HELP_GUIDANCE degrades to the docs-only variant when the
...
class TestShivaAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        from agent.prompt_builder import SHIVA_AGENT_HELP_GUIDANCE
        assert "skill_view(name='shiva-agent')" in SHIVA_AGENT_HELP_GUIDANCE

    def test_no_skills_variant_has_no_skill_view_reference(self):
        from agent.prompt_builder import SHIVA_AGENT_HELP_GUIDANCE_NO_SKILLS
        assert "skill_view" not in SHIVA_AGENT_HELP_GUIDANCE_NO_SKILLS
        assert "shiva-agent.nousresearch.com/docs" in SHIVA_AGENT_HELP_GUIDANCE_NO_SKILLS
...
class TestEssentialSkillsUndisableable:
    def test_agent_side_reader_strips_essential(self, monkeypatch, tmp_path):
        ...
        assert "shiva-agent" not in disabled
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the shiva-agent skill."""
        ...
        for cat, name in [
            ("autonomous-ai-agents", "shiva-agent"),
            ("media", "gif-search"),
        ]:
            d = bundled / cat / name
        ...
        monkeypatch.setattr(ss, "_shiva_home", lambda: home)
        ...
        assert result["copied"] == ["shiva-agent"]
        assert (home / "skills" / "autonomous-ai-agents" / "shiva-agent" / "SKILL.md").exists()
