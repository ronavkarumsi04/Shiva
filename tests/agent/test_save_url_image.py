...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SHIVA_HOME under tmp_path."""
    monkeypatch.setenv("SHIVA_HOME", str(tmp_path / ".shiva"))
    (tmp_path / ".shiva").mkdir()

    # Force the constants/image cache helpers to re-read SHIVA_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("shiva_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_shiva_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under SHIVA_HOME — gateway cleanup
