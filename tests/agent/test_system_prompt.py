...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_shiva_home``,
    ``get_default_shiva_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``SHIVA_HOME`` and no resolver mocks.
    """

    def test_real_shiva_home_under_profiles_renders_correct_paths(
        ...
        assert get_shiva_home() == profile_home
        assert get_default_shiva_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active Shiva profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """SHIVA_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".shiva"
        ...
        assert "Active Shiva profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "SHIVA_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "SHIVA_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_shiva_home", lambda: Path("/shiva"))

    expected_profile = (
        "Active Shiva profile: default. Other profiles (if any) live "
        "under /shiva/profiles/<name>/. Each profile has its own skills/, "
    ...
    with (
        ...
        patch("shiva_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("shiva_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under SHIVA_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from shiva_cli import config as _cfgmod
