...
def curator_env(tmp_path, monkeypatch):
    """Isolated SHIVA_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".shiva"
    ...
    # save_state() resolves the state path from SHIVA_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.shiva once monkeypatch
    ...
    # thread here, while SHIVA_HOME is still pinned to this test's tmp home
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from shiva_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `shiva model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from shiva_cli.main import _AUX_TASKS
    from shiva_cli.web_server import _AUX_TASK_SLOTS
    ...
    # 2. shiva_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. shiva_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
