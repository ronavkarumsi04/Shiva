"""Regression tests for the codex_app_server → Shiva UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_shiva_tools_mcp_server_emits_bare_tool_name(self):
        """The shiva-tools MCP server wraps Shiva' own tools for codex;
        ...
        mcp.shiva-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "shiva-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "shiva-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "shiva agent docs"
        assert calls[0].args[3] == {"query": "shiva agent docs"}
