"""Tests for the cross-Shiva-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Shiva profile's
...
Reference: May 2026 incident — a shiva-security profile session
accidentally edited skills under both ~/.shiva/profiles/shiva-security/skills/
AND ~/.shiva/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Shiva root with two profiles, monkeypatch the
# resolver helpers so the classifier sees the test layout.
# ---------------------------------------------------------------------------


@pytest.fixture
def fake_shiva(tmp_path, monkeypatch):
    """Build a fake Shiva layout:
    ...
    sec_home = root / "profiles" / "shiva-security"
    ...
    monkeypatch.setattr(shiva_constants, "get_default_shiva_root", lambda: root)

    # The reloads below ensure get_cross_profile_warning/classify see the patched root.
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_shiva_root_path", lambda: root)
...
def _set_active_home(monkeypatch, shiva_home: Path):
    """Point file_safety._shiva_home_path at a specific profile dir."""
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_shiva_home_path", lambda: shiva_home)
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_shiva, monkeypatch):
        _set_active_home(monkeypatch, fake_shiva["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"


    def test_falls_back_to_default_on_resolution_failure(self, fake_shiva, monkeypatch):
        """If SHIVA_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_shiva_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_shiva, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_shiva["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_shiva["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "shiva-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_shiva, monkeypatch):
        """Inverse direction — default-profile session reaching into a named profile."""
        _set_active_home(monkeypatch, fake_shiva["default_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_shiva["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "default"
        assert result["target_profile"] == "shiva-security"


    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_shiva, monkeypatch, area):
        _set_active_home(monkeypatch, fake_shiva["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_shiva["default_home"] / area / "foo.txt"
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_shiva, monkeypatch):
        _set_active_home(monkeypatch, fake_shiva["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_shiva, monkeypatch):
        _set_active_home(monkeypatch, fake_shiva["security_home"])
        from agent.file_safety import get_cross_profile_warning
        ...
        assert "shiva-security" in warn
        # Must name the bypass kwarg.
        assert "cross_profile=True" in warn
        # Must reference the area.
        assert "skills" in warn

    def test_warning_is_defense_in_depth_not_boundary(self, fake_shiva, monkeypatch):
        _set_active_home(monkeypatch, fake_shiva["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_shiva["default_home"] / "skills" / "foo" / "SKILL.md")
