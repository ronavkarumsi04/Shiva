...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated SHIVA_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="shiva_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".shiva"))
    prev_home = os.environ.get("SHIVA_HOME")
    os.environ["SHIVA_HOME"] = os.path.join(test_home, ".shiva")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("shiva_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["SHIVA_HOME"] = prev_home
