...
def backup_env(monkeypatch, tmp_path):
    """Isolate SHIVA_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".shiva"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_shiva_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd shiva_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under SHIVA_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level SHIVA_DIR picks up the tmp HOME."""
    import shiva_constants
    importlib.reload(shiva_constants)
