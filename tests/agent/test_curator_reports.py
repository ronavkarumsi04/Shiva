...
Reports live under ``~/.shiva/logs/curator/{YYYYMMDD-HHMMSS}/`` alongside
...
def curator_env(tmp_path, monkeypatch):
    """Isolated SHIVA_HOME with a skills/ dir + reset curator module state."""
    home = tmp_path / ".shiva"
    home.mkdir()
    (home / "skills").mkdir()
    (home / "logs").mkdir()
...
def curator_env_with_cron(curator_env, monkeypatch):
    ...
    monkeypatch.setattr(jobs_mod, "SHIVA_DIR", home)
