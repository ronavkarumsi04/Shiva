...
The official Codex endpoint must receive Shiva' own harness identity, rather
...
These tests pin:
- the required Shiva originator
- the versioned Shiva User-Agent
...
class TestCodexCloudflareHeaders:
    ...
    def test_user_agent_advertises_shiva_version(self):
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("run_agent.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "shiva-agent"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"ShivaAgent/{__version__}"
