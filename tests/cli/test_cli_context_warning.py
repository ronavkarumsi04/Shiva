...
def _isolate(tmp_path, monkeypatch):
    """Isolate SHIVA_HOME so tests don't touch real config."""
    home = tmp_path / ".shiva"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal ShivaCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ShivaCLI.__new__(ShivaCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["shiva-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Shiva' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Shiva' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Shiva' minimum context length."""
