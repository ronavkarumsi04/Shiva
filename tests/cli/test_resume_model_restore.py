...
SessionDB.session_gateway_runtime (shiva_state.py) — the round trip that
makes `shiva --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare ShivaCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.ShivaCLI)
