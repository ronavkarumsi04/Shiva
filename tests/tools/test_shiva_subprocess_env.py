"""Tests for shiva_subprocess_env() — the centralized credential-safe env
...
full credential environment. Two tiers:
  ...
  * Tier 2 (_SHIVA_PROVIDER_ENV_BLOCKLIST): LLM provider/tool keys — stripped
...
_TIER1_SAMPLE = {
    ...
    "SHIVA_DASHBOARD_SESSION_TOKEN": "dash-secret",
}
...
def _build(extra=None, *, inherit_credentials=False):
    ...
    with patch.dict(os.environ, env, clear=True):
        return shiva_subprocess_env(inherit_credentials=inherit_credentials)
...
class TestDelegatedChildMarker:
    def test_delegated_child_context_scrubs_parent_kanban_keys_and_sets_marker(self):
        ...
        with patch.dict(
            ...
            {
                **_SAFE_SAMPLE,
                "SHIVA_KANBAN_TASK": "t_parent",
                "SHIVA_KANBAN_RUN_ID": "123",
                "SHIVA_KANBAN_DB": "/tmp/parent-kanban.db",
                "SHIVA_KANBAN_WORKSPACE": "/tmp/parent-workspace",
            },
            clear=True,
        ):
