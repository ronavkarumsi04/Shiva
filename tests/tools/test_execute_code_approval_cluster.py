...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad SHIVA_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SHIVA_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_shiva_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SHIVA_HOME": "/h", "SHIVA_PROFILE": "p",
        "SHIVA_CONFIG": "/c.yaml", "SHIVA_ENV": "/e",
        "SHIVA_DELEGATED_CHILD_CONTEXT": "1",
        # other SHIVA_* → dropped (broad prefix removed)
        "SHIVA_BASE_URL": "https://x", "SHIVA_INTERACTIVE": "1",
        "SHIVA_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "SHIVA_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SHIVA_BASE_URL", "SHIVA_INTERACTIVE", "SHIVA_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped SHIVA_* vars."""
    import logging
