...
Older images made /opt/shiva/ui-tui and /opt/shiva/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV SHIVA_TUI_DIR=/opt/shiva/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R shiva:shiva /opt/shiva/ui-tui" not in text
    assert "chown -R shiva:shiva /opt/shiva/node_modules" not in text
