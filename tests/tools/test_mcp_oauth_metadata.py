...
Covers:
- :class:`ShivaTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.ShivaMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _SHIVA_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# ShivaTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = ShivaTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SHIVA_HOME", str(tmp_path))
        storage = ShivaTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (ShivaMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: ShivaTokenStorage, **context_attrs):
    ...
    provider = _SHIVA_PROVIDER_CLS.__new__(_SHIVA_PROVIDER_CLS)
    provider._shiva_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHIVA_HOME", str(tmp_path))
        storage = ShivaTokenStorage("mgr-srv")
        ...
        with patch.object(
            _SHIVA_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHIVA_HOME", str(tmp_path))
        storage = ShivaTokenStorage("flow-srv")
        ...
        with patch.object(
            _SHIVA_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
