...
UDS socket lifecycle, shiva_tools generation, timeout enforcement,
...
class TestShivaToolsGeneration(unittest.TestCase):
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_shiva_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"shiva_rpc\")", src)
        self.assertNotIn('os.environ.get("SHIVA_RPC_DIR", "/tmp/shiva_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_shiva_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/shiva_exec_", mkdir_cmd)
        self.assertIn("SHIVA_RPC_DIR=/data/data/com.termux/files/usr/tmp/shiva_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/shiva_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/shiva_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """SHIVA_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"SHIVA_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import shiva_constants; print(shiva_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("shiva_constants.py", result["output"])
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from shiva_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from shiva_tools import json_parse
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_shiva_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("SHIVA_RPC_SOCKET", child_env)


    def test_timezone_injected_when_set(self):
        env_backup = os.environ.copy()
        try:
            os.environ["SHIVA_TIMEZONE"] = "America/New_York"
            child_env = self._get_child_env()
    ...
    def test_timezone_not_set_when_empty(self):
        env_backup = os.environ.copy()
        try:
            os.environ.pop("SHIVA_TIMEZONE", None)
            child_env = self._get_child_env()
            if "TZ" in child_env:
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching SHIVA_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated shiva_tools module reads SHIVA_RPC_TOKEN and sends it."""
        src = generate_shiva_tools_module(["terminal"], transport="uds")
        self.assertIn("SHIVA_RPC_TOKEN", src)
