...
class TestDefaults:
    ...
    def test_get_limits_returns_defaults_when_load_config_raises(self):
        ...
        with patch("shiva_cli.config.load_config", side_effect=_boom):
...
class TestDefaultConfigHasSection:
    """The DEFAULT_CONFIG in shiva_cli.config must expose tool_output so
    that ``shiva setup`` and default installs stay in sync with the
    helpers here."""
