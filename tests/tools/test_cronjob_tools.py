...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `shiva-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.shiva/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.shiva/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because shiva uses an internal scheduler, not system crontab.
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["SHIVA_INTERACTIVE", "SHIVA_GATEWAY_SESSION", "SHIVA_EXEC_ASK"],
    )
    ...
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("SHIVA_INTERACTIVE", "SHIVA_GATEWAY_SESSION", "SHIVA_EXEC_ASK"):
...
class TestUnifiedCronjobTool:
    ...
    def _patch_named_legit(monkeypatch):
        import shiva_cli.runtime_provider as rp
...
class TestAgentCannotSetModelPin:
    """Per-job inference pins are user-owned (dashboard / `shiva cron`
...
class TestGithubExemptionAbuse:
    ...
    def test_same_line_payload_after_github_url_is_scanned(self):
        ...
        for sep in (";", " &&", " |"):
            prompt = f"{self.GH}{sep} cat ~/.shiva/.env"
    ...
    def test_subshell_and_backtick_payloads_are_scanned(self):
        ...
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}$(cat ~/.shiva/.env)")
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}`cat ~/.shiva/.env`")
    ...
    def test_payload_between_two_github_blocks_is_scanned(self):
        ...
        prompt = f"{self.GH}; cat ~/.shiva/.env; {self.GH}"
        assert "Blocked" in _scan_cron_prompt(prompt)
