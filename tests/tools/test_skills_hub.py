...
class TestParseFrontmatterQuick:
    def test_valid_frontmatter_including_nested_yaml(self):
        ...
        nested = "---\nname: test\nmetadata:\n  shiva:\n    tags: [a, b]\n---\n\nBody.\n"
        assert GitHubSource._parse_frontmatter_quick(nested)["metadata"]["shiva"]["tags"] == ["a", "b"]
    ...
    def test_browseable_trusted_repos_have_taps(self):
        ...
        for repo in TRUSTED_REPOS:
            assert repo in tap_repos, (
                ...
                "browsable via `shiva skills browse`."
            )
...
def _make_index_source(skills):
    """Build a ShivaIndexSource pre-loaded with a fixed skill list."""
    from tools.skills_hub import ShivaIndexSource
    src = ShivaIndexSource(auth=GitHubAuth())
    src._index = {"skills": skills}
    src._loaded = True
    return src


class TestShivaIndexSearch:
...
class TestProviderFilter:
    ...
    def test_unified_search_provider_filter_keeps_index_source(self):
        ...
        src = MagicMock()
        src.source_id.return_value = "shiva-index"
...
class TestOptionalSkillSourceMetadata:
    def test_scan_all_emits_repo_root_relative_metadata(self, tmp_path):
        ...
        assert meta is not None
        assert meta.repo == "NousResearch/shiva-agent"
...
class TestOptionalSkillSourceLiveRepoFallback:
    ...
    searchable and installable without `shiva update` (live-repo fallback)."""
...
class TestInstallPathSafety:
    ...
    def test_install_from_quarantine_rejects_category_bucket_overwrite(self, tmp_path):
        ...
        Regression test for GitHub issue #75983: ``shiva skills install … --name
...
# ---------------------------------------------------------------------------
# _load_shiva_index — centralized index fetch (Browse-hub landing / search)
# ---------------------------------------------------------------------------


class TestLoadShivaIndex:
    ...
    def _isolate_cache(monkeypatch, tmp_path):
        ...
        cache_file = tmp_path / "shiva-index.json"
        monkeypatch.setattr(hub, "_shiva_index_cache_file", lambda: cache_file)
        return cache_file
    ...
    def test_persistent_decoding_error_falls_back_to_stale_cache(
        ...
        old = time.time() - (hub.SHIVA_INDEX_TTL + 100)
        import os
