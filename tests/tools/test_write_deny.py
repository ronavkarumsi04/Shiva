...
class TestWriteDenyExactPaths:
    ...
    def test_shiva_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.shiva/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "shiva_root"
        ...
        # Sanity check: SHIVA_HOME does point to the profile dir, not the root.
        from shiva_constants import get_shiva_home, get_default_shiva_root
...
class TestWriteAllowed:
    ...
    def test_shiva_control_files_requested_writable(self):
