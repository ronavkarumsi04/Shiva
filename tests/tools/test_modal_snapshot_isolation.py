...
def _restore_tool_modules():
    original_shiva_home = os.environ.get("SHIVA_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_shiva_home is None:
            os.environ.pop("SHIVA_HOME", None)
        else:
            os.environ["SHIVA_HOME"] = original_shiva_home
        _reset_modules(("tools", "shiva_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    shiva_cli = types.ModuleType("shiva_cli")
    shiva_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["shiva_cli"] = shiva_cli
    shiva_home = tmp_path / "shiva-home"
    os.environ["SHIVA_HOME"] = str(shiva_home)
    sys.modules["shiva_cli.config"] = types.SimpleNamespace(
        get_shiva_home=lambda: shiva_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="shiva-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": shiva_home / "modal_snapshots.json",
