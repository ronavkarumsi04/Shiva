...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("shiva_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="shiva-spill-test-")
    ...
    def test_default_directory_uses_shiva_home(self):
        """When no directory override, spill under SHIVA_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="shiva-home-")
        try:
            with patch.dict(os.environ, {"SHIVA_HOME": test_home}):
                # Also patch get_shiva_home to the env var to mirror production.
            ...
            # ~/.shiva/hook_outputs depending on get_shiva_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.shiva/hook_outputs/sess")),
            ]
