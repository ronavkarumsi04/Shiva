...
1. ``ShivaTokenStorage.set_tokens`` persisted only relative ``expires_in``,
...
- ``ShivaMCPOAuthProvider._initialize`` seeds ``context.token_expiry_time``
...
# ---------------------------------------------------------------------------
# ShivaTokenStorage — absolute expiry persistence
# ---------------------------------------------------------------------------
...
class TestGetTokensReconstructsExpiresIn:
    ...
    def test_get_tokens_returns_zero_ttl_for_expired_token(
        ...
        monkeypatch.setenv("SHIVA_HOME", str(tmp_path))
        from tools.mcp_oauth import ShivaTokenStorage, _get_token_dir
...
# ---------------------------------------------------------------------------
# ShivaMCPOAuthProvider._initialize — seed token_expiry_time
# ---------------------------------------------------------------------------
...
async def test_initialize_seeds_token_expiry_time_from_stored_tokens(
    ...
    from tools.mcp_oauth import ShivaTokenStorage
    from tools.mcp_oauth_manager import _SHIVA_PROVIDER_CLS, reset_manager_for_tests

    assert _SHIVA_PROVIDER_CLS is not None
    ...
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Shiva Agent",
    )
    provider = _SHIVA_PROVIDER_CLS(
        server_name="srv",
...
async def test_initialize_skips_prefetch_when_no_tokens(tmp_path, monkeypatch):
    ...
    storage = ShivaTokenStorage("srv")  # empty — no tokens on disk
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Shiva Agent",
    )
