...
and the shared slash-command handlers (shiva_cli/write_approval_commands.py).
...
@pytest.fixture
def shiva_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="shiva_wa_test_")
    home = os.path.join(d, ".shiva")
    os.makedirs(home)
    monkeypatch.setenv("SHIVA_HOME", home)
    yield home
...
def test_default_gate_is_off(shiva_home):
...
def test_invalid_subsystem_is_off(shiva_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(shiva_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(shiva_home, capsys):
...
def test_load_on_disk_store_honors_configured_limits_and_permissions(shiva_home, monkeypatch):
    ...
    monkeypatch.setattr("shiva_cli.config.load_config", _boom)
...
def test_handle_approve_all(shiva_home):
...
def test_handle_approval_on(shiva_home):
    from shiva_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(shiva_home):
    from shiva_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(shiva_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(shiva_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(shiva_home):
