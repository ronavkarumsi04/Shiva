...
def fake_shiva(tmp_path, monkeypatch):
    """Build a two-profile Shiva layout and point SHIVA_HOME at
    the shiva-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_shiva):
        from tools.file_tools import write_file_tool
        target = fake_shiva["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_shiva):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_shiva["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "shiva-security" in result["error"]
        # File untouched.
        assert target.read_text() == original


    def test_non_shiva_path_unaffected(self, fake_shiva, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_shiva):
    ...
    def test_cross_profile_patch_bypass(self, fake_shiva):
        from tools.file_tools import patch_tool
        target = fake_shiva["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_shiva):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_shiva, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_shiva["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up SHIVA_HOME (set in
        ...
        assert "not found in active profile 'shiva-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.shiva/profiles/<name>/."""
        # Don't set SHIVA_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_shiva_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_shiva_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_shiva):
        """When active profile is 'shiva-security', the prompt warns
        ...
        assert "Active Shiva profile" in src
        assert "cross_profile=True" in src
        assert "~/.shiva/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Shiva profile: default" in src
        assert "Active Shiva profile: {active_profile}" in src
