...
class TestModalBulkUpload:
    ...
    def test_tar_archive_contains_all_files(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src_a), "/root/.shiva/credentials/a.json"),
            (str(src_b), "/root/.shiva/skills/b.py"),
        ]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            ...
            assert "root/.shiva/credentials/a.json" in names
            assert "root/.shiva/skills/b.py" in names

            # Verify content
            a_content = tar.extractfile("root/.shiva/credentials/a.json").read()
            assert a_content == b"cred_content"
            b_content = tar.extractfile("root/.shiva/skills/b.py").read()
    ...
    def test_stdin_chunked_for_large_payloads(self, monkeypatch, tmp_path):
        ...
        files = [(str(src), "/root/.shiva/large.bin")]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            names = tar.getnames()
            assert "root/.shiva/large.bin" in names
