...
def test_output_path_rejects_shiva_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    shiva_home = tmp_path / "shiva-home"
    shiva_home.mkdir()
    monkeypatch.setattr(file_safety, "_shiva_home_path", lambda: shiva_home)
    monkeypatch.setattr(file_safety, "_shiva_root_path", lambda: shiva_home)

    target = shiva_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    shiva_home = tmp_path / "shiva-home"
    token_dir = shiva_home / "mcp-tokens"
