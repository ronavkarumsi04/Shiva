"""
Shiva MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Shiva-specific extra)

Usage:
    shiva mcp serve
    shiva mcp serve --verbose
...
logger = logging.getLogger("shiva.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SHIVA_HOME."""
    try:
        from shiva_constants import get_shiva_home
        return get_shiva_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SHIVA_HOME", Path.home() / ".shiva")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from shiva_constants import get_shiva_home
        directory_file = get_shiva_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SHIVA_HOME", Path.home() / ".shiva")
...
class EventBridge:
    ...
    This is the Shiva equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from shiva_constants import get_shiva_home
            db_file = get_shiva_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SHIVA_HOME", Path.home() / ".shiva")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Shiva MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "shiva",
        instructions=(
            "Shiva Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Shiva MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
