"""Windows UTF-8 bootstrap for Shiva entry points.
...
should be imported at the very top of every Shiva entry point
(``shiva``, ``shiva-agent``, ``shiva-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``shiva_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``shiva_bootstrap``, never ``shiva_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Shiva modules.

    Shiva ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Shiva source root
    ...
    """
    root = src_root or os.environ.get("SHIVA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``SHIVA_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("SHIVA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import shiva_bootstrap``
# (or ``from shiva_bootstrap import apply_windows_utf8_bootstrap``) at
